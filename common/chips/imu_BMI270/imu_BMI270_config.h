#pragma once
#include <stdint.h>

/*
 * BMI270 maximum FIFO configuration file from Bosch BMI270_SensorAPI.
 *
 * Source: https://github.com/boschsensortec/BMI270_SensorAPI
 * File: bmi270_maximum_fifo.c (array bmi270_maximum_fifo_config_file)
 * Size: 328 bytes
 * License: BSD-3-Clause
 *
 * This is the simpler config variant — enables basic accel/gyro
 * with maximum FIFO.  The base 8192-byte config from bmi270.c was
 * found incompatible with some BMI270 revisions (INT_STAT=0x02).
 */
static const uint8_t bmi270_config_file[] = {
    ,
};
