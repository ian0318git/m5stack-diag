/* $Id: uart.c,v 1.2 2017/07/28 07:58:36 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/dsp_src/src/oakenshield_dsp/dss/src/uart.c,v $
 *------------------------------------------------------------------
 * uart.c
 *      Oakenshield - DSS uart 
 *
 * Mar 2012, Smita Rane
 *
...
uint32_t uartputs (const char *str)
{
        uint32_t num_char = 0;
    char ch;
...
void uartputlong (uint32_t val, uint32_t base)
{
    char bufUltostr[33];
        char *str;
        int c;
...
void uart_put_long (int core, uint32_t val, uint32_t base)
{
    char bufUltostr[33];
	char *str;
	int c;
