/* $Id: libeth.c,v 1.3 2021/04/15 00:52:57 achiu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/dsp_src/src/oakenshield_dsp/ppb/lib/libeth.c,v $
 *------------------------------------------------------------------
 * libeth.c
 *
 * Mar 2012, Smita Rane
                        ...
                        uint8_t *rx_buf)
{
    ...
    while (bdstat & RX_BQUE_STAT_OWN_BM) {
        if (oak_diag_flag == 1) {
            bsp_debug_printf(" \r\nStart RX PCE#%d activity on Queue %d, bdstat = 0x%x, cur buf index = %d",
                            eth_port, rxq, bdstat, ii);
        }
...
/* copy the data, then release the buffer */
        ...
        if (oak_diag_flag == 1) {
            bsp_debug_printf(" \r\n copied to local Buffer Rx Packet @addr: 0x%x,size = 0x%x[%d]",
                                    tp, cpy_sz, cpy_sz);
        }
            ...
            if (bdstat & RX_BQUE_STAT_EOF_BM) {
                ...
                if (bdstat & RX_BQUE_STAT_COR_BM) {
                    ...
                    if (bdstat & RX_BQUE_STAT_CKSM_ERR_BM) {
                        ...
                        status = EMAC_ERR_FMT_ERROR;
                    } else if (((bdstat & RX_BQUE_STAT_TYPE_BM) >> RX_BQUE_STAT_TYPE_BO) == 
                                              PCE_BD_TYPE_ETH_IPV4_UDP) {
...
/******** History ********
$Log: libeth.c,v $
Revision 1.3  2021/04/15 00:52:57  achiu2
...
Revision 1.3.84.1  2017/03/30 10:25:49  harrchan
