...
int jam_jtag_io(int tms, int tdi, int read_tdo)
{
	...
	else
	{
		...
		if(tdi)
			gpio_set_tdi(cpld_target);
		else
			gpio_clear_tdi(cpld_target);

		if(tms)
			gpio_set_tms(cpld_target);
		else
			gpio_clear_tms(cpld_target);

		if (read_tdo)
		{
			// Read TDO
			tdo = gpio_get_tdo(cpld_target);
		}

		// Pulse TCK
		gpio_set_tck(cpld_target);
		gpio_clear_tck(cpld_target);
#else
...
void jam_export_integer(char *key, long value)
{
	if (verbose)
	{
		bsp_debug_printf("\r Export: key = \"%s\", value = %ld\n", key, value);
		fflush(stdout);
	}
}
...
void jam_export_boolean_array(char *key, unsigned char *data, long count)
{
	...
	if (verbose)
	{
		...
		{
			bsp_debug_printf("Export: key = \"%s\", %ld bits, value = HEX\n", key, count);
		...
		else
		{
			...
			bsp_debug_printf("Export: key = \"%s\", %ld bits, value = HEX %s\n",
				key, count, string);
			fflush(stdout);
		}
	}
...
{
#if PORT == LINUX_RPI
	bsp_debug_printf("\r BUG : vector IO called\n");
	exit(-1);
#endif
...
int jam_set_frequency(long hertz)
{
	if (verbose)
	{
		bsp_debug_printf("Frequency: %ld Hz\n", hertz);
		fflush(stdout);
	}
...
//remove main function and porting to Phoenix code base
int jam_upgrade_cpld (char *firmware)
{
	long offset = 0L;
	long error_line = 0L;
	...
	char action[10] = "program";
	char *init_list[10];     /* Optional */
	...
	verbose = TRUE;
    file_buffer = firmware;
    for (i=0; i<PHOENIX_CPLD_UPGRADE_SIZE; i++)
        if(file_buffer[i] == 0) break;
    file_length = i+1;
    bsp_debug_printf("\r %s(): file_length=%d\n", __FUNCTION__, file_length);
	...
	if (verbose || (crc_result == JAMC_CRC_ERROR))
	{
		...
		{
		    case JAMC_SUCCESS:
				bsp_debug_printf("\r CRC matched: CRC value = %04X\n", actual_crc);
				break;

			case JAMC_CRC_ERROR:
				bsp_debug_printf("\r CRC mismatch: expected %04X, actual %04X\n",
					expected_crc, actual_crc);
				break;

			case JAMC_UNEXPECTED_END:
				bsp_debug_printf("\r Expected CRC not found, actual CRC value = %04X\n",
					actual_crc);
				break;

			default:
				bsp_debug_printf("\r CRC function returned error code %d\n", crc_result);
				break;
		}
	}
    ...
    if (verbose)
    {
	    ...
	    {
	        bsp_debug_printf("\r NOTE \"%s\" = \"%s\"\n", key, value);
	    }
    }

    for (i=CPLD_MB; i<=CPLD_DB3; i++)
    {
        cpld_target = i;
        if( cpld_target == CPLD_DB2 && !phoenix_has_dbx(BOARD_DB2_TEST))
            continue;
        if( cpld_target == CPLD_DB3 && !phoenix_has_dbx(BOARD_DB3_TEST))
            continue;

    	/*	Execute the JAM program */
    	exec_result = jam_execute(
    		file_buffer, file_length,
	    	NULL, 0, action, init_list,
    	...
    	if (exec_result == JAMC_SUCCESS)
	    {
	    	...
	    	bsp_debug_printf("\r Exit code = %d... %s\n", exit_code, exit_string);
    	}
    	    ...
    	    (exec_result == JAMC_ACTION_NOT_FOUND))
	    {
	    	...
	    	{
		    	bsp_debug_printf("\r Error: no action specified for Jam file.\nProgram terminated.\n");
    		}
	    	else
    		{
	    		bsp_debug_printf("\r Error: action \"%s\" is not supported for this Jam file.\nProgram terminated.\n", action);
    		}
    	}
    	else if (exec_result < MAX_ERROR_CODE)
    	{
	    	bsp_debug_printf("\r Error on line %ld: %s.\nProgram terminated.\n",
		    	error_line, error_text[exec_result]);
    	}
    	else
    	{
	    	bsp_debug_printf("\r Unknown error code %d\n", exec_result);
    	}
