/* $Id: diag_env.c,v 1.2 2017/07/28 07:58:51 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/dsp_src/src/oakenshield_dsp/ppb/src/diag_env.c,v $
 *------------------------------------------------------------------
 * diag_env.c
...
boot_environment_t boot_environment;
#ifdef ENET_UBL
packed_boot_environment_t packed_boot_environment;
#else
 ...
 */    
env_entry_t *find_entry (char *name, int create) 
{
    int i;
 ...
 *
 */
int format_digit (char *buf, uint32_t num, int base, uint32_t *nchars) 
{
 ...
 */
uint32_t compute_sum (env_entry_t *env_db) 
{
    int i;
    uint32_t sum = 0;
...
void env_eeprom_read_write(int is_read)
{
...
Revision 1.1.2.3  2017/04/11 07:43:50  olin2
Fix hang on issue. (CSCvd70723)
