/* $Id: ssp.c,v 1.2 2017/07/28 07:58:51 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/dsp_src/src/oakenshield_dsp/ppb/src/ssp.c,v $
 *------------------------------------------------------------------
 * ssp.c
 *      SSP Port functions
 *
...
int spi_write_fpga(uint16_t fpga_addr, int alen, uint8_t *pDataIn, int size);
int spi_read_fpga(uint16_t fpga_addr, int alen, uint8_t *pDataIn, int size);
...
/******************************************************************************
LSI_SP27XX_MANFUNCTION: LSI_SP27XX_SSP_WaitRX
...
/******************************************************************************
 * FUNCTION: spi_write_fpga
 *
 * DESCRIPTION:
 *	Generic FPGA write function
 *
 * INPUT:
 *	fpga_addr		- FPGA address
...
int spi_write_fpga ( uint16_t fpga_addr, int alen, uint8_t *pDataIn, int size)
{
    ...
    //bsp_debug_printf("\r\n spi_read(0x%x, size 0x%x) translated addr = 0x%x ", fpga_addr, size, addr);
    spi_init_fpga();
	/* make sure thee SSP port is not busy */
	sr = readl(LSI_SP27XX_SSP_SSPSR_RA);
	while ((sr & LSI_SP27XX_SSPSR_BSY_BM) == LSI_SP27XX_SSPSR_BSY_BM) {
	...
	max_write_size = LSI_SP27XX_SSPFIFO_SIZE - 3;
	...
	while (size > 0) {
		...
		/* write FPGA read command */
		writel(LSI_SP27XX_FPGAWRIT, LSI_SP27XX_SSP_SSPDR_RA);

        /* write FPGA data address */
        for (ix = alen - 1; ix >= 0 ; ix--) {
            writel(((fpga_addr >> (ix*8)) & 0xFF), LSI_SP27XX_SSP_SSPDR_RA);
        }
        ...
        for (ix = 0; ix < write_size; ix++) {
            writel(*pDataIn, LSI_SP27XX_SSP_SSPDR_RA); 
            pDataIn += 1;
        }
...
/******************************************************************************
 * FUNCTION: spi_read_fpga
 *
 * DESCRIPTION:
 *    Generic FPGA read function
 *
 * INPUT:
 *   fpga_addr      - FPGA address
 *   alen           - address size (will always be 2 bytes)
...
int spi_read_fpga(uint16_t fpga_addr, int alen, uint8_t *pDataIn, int size)
{
    ...
    /* Opcode + Address = 3 bytes */
	max_read_size = LSI_SP27XX_SSPFIFO_SIZE - 3;
	...
	while (size > 0) {
		...
		/* write FPGA read command */
		writel(LSI_SP27XX_FPGAREAD, LSI_SP27XX_SSP_SSPDR_RA) ;
...
HISTORY:
	none
*******************************************************************************/
static uint8_t spi_get_rdsr(void)
{
...
HISTORY:
	none
*******************************************************************************/
static uint8_t spi_put_wrsr(uint8_t sr_val)
{
	...
	int	size)
{
    ...
    //bsp_debug_printf("\r\n spi_write(0x%x), translated addr = 0x%x,
    //sector 0x%x, eeprom_write_cache 0x%x", eeaddr, trans_addr, sector, eeprom_write_cache);
...
HISTORY:
	none
*******************************************************************************/
static int spi_write_enable (unsigned dev_addr, int state)
{
	volatile uint32_t	sspsr;
...
int eeprom_read (unsigned offset, uint8_t *buffer, unsigned cnt)
{
...
Revision 1.3.78.2  2017/01/05 06:06:34  olin2
Support FXS Ring and Calibration

Revision 1.3.78.1  2016/12/14 04:57:38  olin2
Initial commit code for Oakenshield
