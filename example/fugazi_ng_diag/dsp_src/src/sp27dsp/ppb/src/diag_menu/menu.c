/* $Id: menu.c,v 1.4 2012/06/28 21:31:37 srane Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/dsp_src/src/sp27dsp/ppb/src/diag_menu/menu.c,v $
 *------------------------------------------------------------------
...
//    loopmask = D_LOOPONERR,
//    warning_mask = D_WARNING, 
...
//#if 0
	    ...
	    //longjmp(menujmp, 1);  /* Back to previous point */
        }
//#endif
    }
...
menu (menuinfo_t *menuptr, mitem_t *second_miptr, char menucmd)
{
	...
	if(menucmd) {
		...
		dismen = 0;
            ...
            bsp_debug_printf("\r\n");
	    if(menuptr->mprompt) uart_puts(menuptr->mprompt);
	    else {
	...
	if(c == EXIT_CHAR) {  /* EXIT_CHAR entered, return to caller */
            /* Send the exit command for picocom */
            //uart_puts("\r\n character is escape send exit to picocom");
            //linebuf[0] = 'A';
            //sp_SerialPutC(linebuf[0]);
            //linebuf[0] = 'A';
            //sp_SerialPutC(linebuf[0]);
            //linebuf[0] = '\050';
            //sp_SerialPutC(linebuf[0]);
...
displaymenu (struct menuinfo *menuptr, int mflag)
{
	...
	if (item_is_called_for(miptr, mflag)) {
            ...
            if(miptr->mline) {
                ...
                //uart_puts(miptr->mlparam);
            }
            if(miptr->mlfunc) (*miptr->mlfunc)((int)miptr->mlparam);
            bsp_debug_printf("\r\n");
        }
	if (mletter == 'z') {
	    mletter = '`';   /* mletter++ == 'a' */
