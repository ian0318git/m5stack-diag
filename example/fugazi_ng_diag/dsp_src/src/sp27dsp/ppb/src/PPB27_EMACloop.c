/* $Id: PPB27_EMACloop.c,v 1.6 2012/08/15 15:02:56 srane Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/dsp_src/src/sp27dsp/ppb/src/PPB27_EMACloop.c,v $
 *------------------------------------------------------------------
 * PPB27_EMACloop.c
 *     Ethernet functions 
 *
...
#endif
//#define SP_PPB_EMAC0_ETH_ADDR		(0x00FACE270401LL)
...
#ifdef PCE_DEBUG
    sp_SerialPutS("\r\n Graffham MAC is ");
    bsp_debug_printf("0x%x:0x%x:0x%x:0x%x:0x%x:0x%x", print_mac[0],print_mac[1], print_mac[2], print_mac[3], print_mac[4], print_mac[5]);
    ...
    for (ii = which_emac; ii < which_emac+1; ii++) {
        for (jj = 0; jj <= DLT_DEST_MAX; jj++) {
            while (1) {
                ...
                if (status == EMAC_SUCCESS){
bsp_debug_printf("\r\n Status = EMAC_SUCCESS for DLT Q %d, size of pkt = %d", jj, (int)*size);
                    rcv_cnt++;
                    ...
                    uint32_t size, uint32_t total_size)
{
    ...
    if (lpbk == EXTERNAL) {
        ...
        //StarProPPB_UTILS_memcpy8((uint8_t*) &(pkt__p->pkt_data) - 2), hwAddr, size);
    ...
    if (lpbk == INTERNAL) {
        ...
        if (loopSelf) {
            /* poke the 'serial number' into the outgoing frames & update the UDP checksums */
             //* (uint32_t *) (&out_frame[0][44]) = serial << 4;
             * (uint32_t *) (&out_frame[1][44]) = serial;
             //* (uint32_t *) (&out_frame[0][44]) = serial << 4;
             //* (uint32_t *) (&out[44]) = serial;
...
//#ifdef PCE_DEBUG
    bsp_debug_printf(" \r\n sp_EthTxStart() Success on EMAC%d", which_emac);
//#endif
            result = SUCCESS;
        }
    } else {
