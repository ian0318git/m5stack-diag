...
)
{
    ...
    for (addrIndex = 0; addrIndex < addrArrayLength; addrIndex++)
    {
        ...
        bytesWritten = sprintf(&dumpBuffer[*bufferOffset],"%02d %02d.%04x=%04x /* %s */\n", (int)port,
                               (int)addrArray[addrIndex].mmd, (int)addrArray[addrIndex].reg, (int)valueRead, addrArray[addrIndex].desc);
