/* $Id: nim_arkenstone.h,v 1.5 2019/12/11 10:10:22 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/include/nim_arkenstone.h,v $
 *******************************************************************************
 *
 * nim_arkenstone.h - Arkenstone Header file, leverage from F35
 *
...
typedef struct __attribute__((__packed__)) arkenstone_test_menu_ {
    char  *desc;
    uint  msg_id;
    uint  cmd_id;
    uint  msg_len;
} arkenstone_test_menu_t;

/* msg_id defines */
#define NO_MSG                         0
#define KW_MSG                         1
#define Arkenstone_CMD                        2
#define CON_CMD                        3
/* KW has decoded the command and has ACK it - step 2 */
#define KW_CMD_RESP                    4   
/* KW has decoded the command and has NACK it - step 2 */
#define Arkenstone_CMD_ACK                    5   
...
typedef enum kw_diag_tests_ {
    /*  
       ...
       KW   : -"- , command line
    */
    Arkenstone_INT_REG_TEST=1,
    DRMW_FPGA_REG_TEST,
    ALTERA_PCI_REG_TEST,
    
    Arkenstone_MSI_INTR_TEST,
    DRMW_MSI_INT_TEST,
    DRMW_GDF_IPC_ERR_INT_TEST,
    
    Arkenstone_USB_TEST,
    Arkenstone_SPI_FLASH_TEST,

    Arkenstone_MODEM_ENUM_TEST,
    AT_COMMAND_TEST,
    Arkenstone_PHY_ADDR_INFO,
    EXIT_Arkenstone_DIAG,
    START_Arkenstone_DIAG,
    ANTENNAE_TEST,
    SIM_CARD_TEST,
    ...
    /* 
       ...
       KW   : command line
    */
    Arkenstone_INT_REG_ALT,
    Arkenstone_INT_REG_DISP,
    Arkenstone_MEM_ALT,
    Arkenstone_MEM_DISP,
    ...
    */
    ...
    ALTERA_PCI_REG_ALT,
    ALTERA_PCI_REG_DISP,
    Arkenstone_CPU_REG_DISP,
    USER_GDF_TEST,
    GDF_TEST,
    ALTERA_PCI_LINK_TEST,
    MODEM_ENUM_TEST,
    MODEM_AT_TEST,
    END_Arkenstone_COMMAND_LINE,

    /* 
       These tests cannot be invoked on command line 
       Host : IPC command (intr/poll)
    */
    DRMW_GDF_EGR_TEST = END_Arkenstone_COMMAND_LINE,
    DRMW_GDF_IGR_TEST,
...
typedef struct arkenstone_ds_t_ {
...
typedef struct {
    cmd_hdr_t cmd_hdr;
    cmd_info_t cmd_info;
} arkenstone_cmd_pkt_t;

typedef struct arkenstone_priv_t_ {
    arkenstone_cmd_pkt_t *cmd_packet;
} arkenstone_priv_t;
...
/* need to increase the buffer size to match NIM Arkenstone on Tachi */    
...
static int arkenstone_test(int);
...
static int arkenstone_uart_test(void);
...
#define READY_TIME_OUT      200
#define RETRY_TIME          5

#define AUTOBOOT_STOP_TIME  16000
...
/******** History ********
$Log: nim_arkenstone.h,v $
Revision 1.5  2019/12/11 10:10:22  lucywang
Merged Nanook to main trunk

Revision 1.4  2017/08/10 10:10:30  iachang
CSCvf44161: Merge Goldbeach into USD platform as one image

Revision 1.3  2017/03/30 08:17:09  hondwang
Tachi-L brach merge

Revision 1.2.2.2  2017/01/03 01:27:52  haohsu
Modify NIM for Tachi-l

Revision 1.2.2.1  2016/12/22 08:49:40  haohsu
Add Arkenstoen to Tachi-l

Revision 1.2  2016/10/16 12:28:09  iachang
Supported Goldbeach Platform.

Revision 1.1  2016/04/18 04:14:58  xiaoyizh
Initail check-in for Arkenstone.
