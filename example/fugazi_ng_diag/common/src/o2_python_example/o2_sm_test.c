/* $Id: o2_sm_test.c,v 1.2 2014/06/03 10:53:31 erwu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/o2_python_example/o2_sm_test.c,v $
 *------------------------------------------------------------------
 * Description: hard-coded printing for simulating sm test
 *
...
main(int argc,char *argv[])
{
    ...
    fru_table_offset = MB;
    strcpy(mb_get_pid, "CISCO1200");
    strcpy(mb_get_loc, "SM0");
    ...
    if (argc == ARGS_3) {
        /* if sm_test without any args in o2.pcfg, at least 3 args here */
        /* first arg       : executable */
        /* second-last arg : testpass */
        /* last arg        : diag flag value */
        /* last and second-last which came from python script. */

        /* if sm_test has no arg as slot number in o2.pcfg, */
        /* remove slot_id.tmp if it existed */
        rm_slot_tmp();

        cterr('f',0,"FATAL error when getting slot id from SM slot\n");
        return FAILED;

    } else if (argc > ARGS_3){
        /* if sm_test has args in o2.pcfg, take first one as slot number, */
        /* and save it to slot_id.tmp */
        save_slot_num(argc, argv);

        /* pass first arg of sm_test in o2.pcfg as slot number */
        ret_val = sm_test_fn(atoi(argv[1]));
 ...
 *
 * Outputs     : sm module cookie id
 *
 *****************************************************************************/
int read_sm_cookie(int slot_num)
{
    printf("reading sm slot %d cookie\n",slot_num);
    printf("Write cookie id to tmp file for python to compare against\n");
    return LEBOWSKI_MOD_ID;
}
 ...
 *****************************************************************************/
int
sm_test_fn(int slot_num)
{
    int sm_cookie_val = 0;

    printf("init system to discovery sm slot %d\n",slot_num);

    /* flag usage example */
    if (trace_mode == 1) {
        printf("\n Trace mode flag ON \n");
        printf("\n trace_mode in o2_sm_test \n");
    } else {
        printf("\n Trace mode flag OFF \n");
        printf("\n non trace_mode in o2_sm_test \n");
    }

    sm_cookie_val = read_sm_cookie(slot_num);

    if (sm_cookie_val != FAILED) {
        save_mod_id(sm_cookie_val);
    }
    return PASSED;
}
