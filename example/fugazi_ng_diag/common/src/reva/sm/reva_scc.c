/* $Id: reva_scc.c,v 1.3 2017/03/20 09:44:02 umlin Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/reva/sm/reva_scc.c,v $
 *------------------------------------------------------------------
 *
 * reva_scc.c - Reva-SM Serial Channel Controller main function/menu.
 *
 *
...
static int reva_scc_bisync_tx_ctrl_reg_test(channel)
{
    ...
    if(channel >= 24) {
        scc_bisync_tx_ctrl_base += 0x28000;
    }
...
static int reva_scc_bisync_rx_ctrl_reg_test(channel)
{
    ...
    if(channel >= 24) {
        scc_bisync_rx_ctrl_base += 0x28000;
    }
...
static int reva_scc_ppp_tx_ctrl_reg_test(channel)
{
    ...
    if(channel >= 24) {
        as_scc_ppp_tx_ctrl_base += 0x28000;
    }
...
static int reva_scc_ppp_rx_ctrl_reg_test(channel)
{
    ...
    if(channel >= 24) {
        as_scc_ppp_rx_ctrl_base += 0x28000;
    }
...
//    if ((reva_scc_ctrl_reg_test(channel) != PASSED) ||
    if ((reva_scc_rx_dma_ctrl_reg_test(channel) != PASSED) ||
...
int scc_reg_rd ()
{
    ...
    offset = gethex_answer("\nEnter register offset[0x1000 to 0x67fff]:",
               0x1000, 0x1000, 0x67fff);
...
static int mgmt_intr_test_one (int chan, ushort type)
{
    ...
    if(chan >= 24) {
        base_addr = (prince_scc_regs_t *)(get_scc_base() + ((chan / 4) * 0x4000) + 0x28000);
    } else {
...
int reva_scc_intr_test(int dummy)
{
    ...
    case BOARD_ID_SM_64A_4G:
        as_num_chan = 64;
        sprintf(as_scc_menu_header.title, "SM-64A");
        break;
    default:
...
Revision 1.1.2.2  2016/10/19 22:40:23  umlin
Reva-SM: Removed BISYNC related logic registers test from diag code.
