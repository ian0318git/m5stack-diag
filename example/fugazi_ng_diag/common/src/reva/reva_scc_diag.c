 /* $Id: reva_scc_diag.c,v 1.2 2016/05/06 03:43:53 umlin Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/reva/reva_scc_diag.c,v $
 *------------------------------------------------------------------
 *
 * reva_scc_diag.c - Reva Serial Channel Controller diag function.
 *
 *
 ...
 *                    extern functions                               *
 *===================================================================*/
extern void reva_set_async_timer(prince_serial_ds_t *s_ds, ushort value);
...
prince_serial_ds_t reva_serial_ds[REVA_MAX_CH_NUM];

/*
 * Sync Clock Baud rate tables 
 */
prince_baud_t reva_sync_baud[] = {
 ...
 * Output: None
 ******************************************************************************/
void prince_display_cntrl_1_regs (ulong scc_base_addr)
{
 ...
 * Output: None 
 ******************************************************************************/
void prince_display_proto_regs (ulong scc_base_addr, uchar disp_opt,
                         uchar chan)
{
    ...
    for (i = start_chan; i < end_chan; i++) {  
        proto_regs = &base_addr-> proto_regs[i];
        printf("\nREVA PROTOCOL REGISTERS CHAN %d:................\n",i);
 ...
 * Output: None
 ******************************************************************************/
void prince_display_dma_regs (ulong scc_base_addr, uchar option, uchar 
    ...
    for (i = start_chan; i < end_chan; i++) {  
        if (option == PRINCE_DMA_TX) {
            if (priority == PRINCE_SCC_HIGH_PRIORITY) {
                dma_regs = &base_addr-> dma_tx_regs_hp[i];
                printf("\nREVA DMA-TX REGISTERS CHAN %d HP @%#x:................\n", dma_regs, i);
            } else {
                dma_regs = &base_addr-> dma_tx_regs_lp[i];
                printf("\nREVA DMA-TX REGISTERS CHAN %d LP @%#x:................\n", dma_regs, i);
            }
        } else {
            dma_regs = &base_addr-> dma_rx_regs[i];
            printf("\nREVA DMA-RX REGISTERS CHAN %d:................\n",i);
        }
 ...
 *
 * Description: This function will display bcc registers. 
 *
 * Input:  scc_base_addr - FPGA SCC register base addr
 *         disp_opt - Display mode
 *         chan - Channel number
 * Output: None
 ******************************************************************************/
static void prince_display_bcc_regs (ulong scc_base_addr, uchar disp_opt,
                       uchar chan)
{
    prince_scc_regs_t * base_addr;
    int i,  num_chan, start_chan, end_chan;
 ...
 * Output: None
 ******************************************************************************/
static void prince_display_ppp_regs (ulong scc_base_addr, uchar option, 
                       uchar disp_opt, uchar chan)
{
    ...
    for (i = start_chan; i < end_chan; i++) {  
        if (option == PRINCE_PPP_TX) {
            ...
            printf("\nREVA PPP-TX REGISTERS CHAN %d:................\n",i);
        } else {
            ppp_regs = &base_addr-> ppp_rx_regs[i];
            printf("\nREVA PPP-RX REGISTERS CHAN %d:................\n",i);
        }
 ...
 * Output: None
 ******************************************************************************/
void prince_display_serial_itf_regs (ulong scc_base_addr, 
                              uchar disp_opt, uchar chan)
{
 ...
 * Output: None
 ******************************************************************************/
void prince_display_cntrl_2_regs (ulong scc_base_addr)
{
...
/******************************************************************************
 * Function: ngwic_reva_chan_lpbk_test 
 *
 ...
 ******************************************************************************/
int ngwic_reva_chan_lpbk_test (int channel)
{
    ulong scc_base_addr;
    ...
    if (reva_async_lpbk_test(scc_base_addr, s_ds)) {
        return (FAILED);
    }
 ...
 * Output: Nonne 
 ******************************************************************************/
void reva_trap (prince_serial_ds_t *s_ds, int priority)
{
    ...
    *s_ptr = 0;
    ...
    reva_form_name_buf(tmp_name_buf, s_ds, priority);
    if (s_ds->clk_src == PRINCE_CLK_SRC_SYNC )
        des_baud = reva_sync_baud[s_ds->speed_idx].desired_baud;
    else if (s_ds->clk_src == PRINCE_CLK_SRC_ASYNC) 
        des_baud = reva_async_baud[s_ds->speed_idx].desired_baud;
...
int reva_chan_lpbk_test (uchar run_mode, ulong scc_base_addr, ushort speed_idx,
                         ...
                         prince_serial_ds_t *s_ds, int priority)
{
    ...
    case PRINCE_UART_ASYNC:
        ...
        s_ds->proto_regs_addr->flow_cntrl_config = 0x5200;
    ...
    if (diagflag_xram & D_TRACE) {
        printf(" Step 10: Setup Channel Flag & Flow Control \n");
    ...
    case PRINCE_UART_ASYNC:
        ...
        reva_set_async_timer(s_ds, PRINCE_ASYNC_TIMER_COUNT);
        break;
...
int reva_async_lpbk_test (ulong scc_base_addr, prince_serial_ds_t *s_ds)
{
    ...
    if (!((NVRAM)->diagflag & D_EXT_LOOPBACK)) {
        ...
        if( reva_dcd_dtr_test(scc_base_addr, s_ds) != PASSED) {
            cterr('f',0," Channel [%d], DCD <-> DTR loopback test fail,", s_ds->org_port_num);
            return (FAILED);
        }
    }
    ...
    if (((NVRAM)->diagflag & D_EXT_LOOPBACK)) {
        for (index = REVA_AS_BPS_300; index <= REVA_AS_BPS_256K;
            ...
            if (reva_chan_lpbk_test(run_mode, scc_base_addr, index, clk_src,
                                    lpbk_mode, protocol, s_ds,
                                    PRINCE_SCC_HIGH_PRIORITY)) {
                return (FAILED);
            }
            if (reva_chan_lpbk_test(run_mode, scc_base_addr, index, clk_src,
                                    lpbk_mode, protocol, s_ds,
...
/******************************************************************************
 * Function: prince_config_serial_ds
 *
 ...
 * Output: PASSED / FAILED
 ******************************************************************************/
void prince_config_serial_ds (uchar run_mode, ulong scc_base_addr, ushort speed_idx,
                              uchar clk_src, uchar lpbk_mode, uchar protocol,
                              prince_serial_ds_t *s_ds, int priority,
                              ulong *des_baud, ushort *clk_src_bits)
{
    ...
    *des_baud = 0;
    ...
    case PRINCE_CLK_SRC_SYNC:
        divider = reva_sync_baud[speed_idx].divider;
        baudrate = reva_sync_baud[speed_idx].baudrate;
        *des_baud = reva_sync_baud[speed_idx].desired_baud;
        *clk_src_bits = reva_sync_baud[speed_idx].clk_src;
        break;

    case PRINCE_CLK_SRC_ASYNC:
        divider = reva_async_baud[speed_idx].divider;
        baudrate = reva_async_baud[speed_idx].baudrate;
        *des_baud = reva_async_baud[speed_idx].desired_baud;
        *clk_src_bits = reva_async_baud[speed_idx].clk_src;
        break;
 ...
 * Output: None
 ******************************************************************************/
void prince_setup_dma_reg (prince_serial_ds_t *s_ds)
{
    prince_dma_bd_t *tx_desc;
    prince_dma_bd_t *rx_desc;
    ushort i;
 ...
 ******************************************************************************/
ushort prince_start_dma_tx (prince_serial_ds_t *s_ds, int priority)
{
    prince_dma_bd_t *tx_desc;
    prince_dma_bd_t *rx_desc;
     ...
     */
    if (priority == PRINCE_SCC_HIGH_PRIORITY) {
        s_ds->proto_regs_addr->command_reg = PRINCE_CMD_START_TX;
    } else {
        s_ds->proto_regs_addr->command_reg = PRINCE_CMD_LP_START_TX;
    }

    if (diagflag_xram & D_TRACE) {
        printf("\ns_ds->speed_idx = %d REVA_LOW_SPEED_INDEX = %d\n", 
 ...
 * Output: PASSED or FAILED.
 ******************************************************************************/
ushort prince_check_dma_received_data (prince_serial_ds_t *s_ds)
{
    ushort size, i;
 ...
 * Output: None
 ******************************************************************************/
void prince_init_tx_dma_buffers (prince_serial_ds_t *s_ds, uchar data, uchar pattern) 
{
    ushort i, size;
    uchar *data_ptr;
 ...
 * Output: None
 ******************************************************************************/
void prince_init_rx_dma_buffers (prince_serial_ds_t *s_ds) 
{
    ushort i, size;
    uchar *data_ptr;
 ...
 * Output: PASSED / FAILED 
 ******************************************************************************/
int prince_init_tx_dma_ring (uchar run_mode, ulong scc_base_addr,
                       prince_serial_ds_t *s_ds)
{
    ushort i;
    prince_dma_bd_t *tx_desc;
 ...
 * Output: PASSED / FAILED 
 ******************************************************************************/
int prince_init_rx_dma_ring (uchar run_mode, ulong scc_base_addr,
                             prince_serial_ds_t *s_ds)
{
    ushort i;
    prince_dma_bd_t *rx_desc;
 ...
 * Output: None
 ******************************************************************************/
void prince_cleanup_dma_ring (ulong scc_base_addr, prince_serial_ds_t *s_ds)
{
 ...
 * Output: None
 ******************************************************************************/
void prince_cleanup_serial (prince_serial_ds_t *s_ds, int priority)
{
    int i;
 ...
 * Output: None
 ******************************************************************************/
void prince_init_serial_ds (prince_serial_ds_t *s_ds)
{
 ...
 * Output: PASSED / FAILED
 ******************************************************************************/
int reva_form_name_buf (char *tmp_name_buf, prince_serial_ds_t *s_ds, int priority)
{
 ...
 * Output: None
 ******************************************************************************/
void reva_set_async_timer (prince_serial_ds_t *s_ds, ushort value)
{
 ...
 * Output: PASSED / FAILED
 ******************************************************************************/
int reva_dcd_dtr_test (ulong scc_base_addr, prince_serial_ds_t *s_ds)
{
    prince_scc_regs_t * base_addr;
    int rc;
    ...
    s_ds->serial_itf_addr->modem_cntrl |= 0x100;
    msleep(500);

    if ((s_ds->serial_itf_addr->flow_cntrl & 0x40) == 0) {
        rc = FAILED;
    } else {
        rc = PASSED;
    }
