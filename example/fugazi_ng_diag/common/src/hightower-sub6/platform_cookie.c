/* $Id: platform_cookie.c,v 1.2 2021/06/02 02:56:23 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/hightower-sub6/platform_cookie.c,v $
 *------------------------------------------------------------------
 * 
 * platform_cookie.c
 *
...
extern int highrise_toggle_act2_i2c_reset(void);
                                    ...
                                    uchar slot, uchar *cookie_p)
{
    ...
    *i2c_err = '\0';
    ...
    case MOTHER_BOARD:
        ...
        con_p->quack_reset = (PFT)highrise_toggle_act2_i2c_reset;
 ...
 *   PASSED/FAILED
 */
static int alter_cookie(int board_type, int dump_contents)
{
    void *tam_handle_ptr;
    ...
    if (dump_contents) { 
        ...
        cli_cmd.type = CLI_COOKIE_DISPLAY_FMT;

        ret_val = cookie_4_processor_x(cookie_contents_buf, board_type, 0,
                                   EEPROM_RD_WR_LENGTH, &cli_cmd); 
        return (ret_val); 
    }
...
int alter_mb_cookie (int dump)
{
    ...
    if (alter_cookie(MOTHER_BOARD, dump) != PASSED ) {
        return (FAILED);
    }
...
int smartchip(int submenu_flag)
{
    ...
    if (highrise_toggle_act2_i2c_reset() != PASSED) {
        return (FAILED);
    }
 ...
 *
 * Revision 1.1.4.4  2020/12/09 01:52:02  alpeng
 * use C comment
 *
 * Revision 1.1.4.3  2020/11/26 03:38:00  alpeng
 * support temperature interrupt test for all version
 *
 * Revision 1.1.4.2  2020/11/25 02:38:06  alpeng
 * update cvs id field
 *
 *
 *********************************************************************
