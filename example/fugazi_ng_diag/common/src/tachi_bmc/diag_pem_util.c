/* $Id: diag_pem_util.c,v 1.2 2016/04/20 11:25:31 benchen2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tachi_bmc/diag_pem_util.c,v $
 *------------------------------------------------------------------
 *
 * diag_pem_util.c  
...
static void build_pem_fan_menu_wr(void);
...
static submenu_xtable_t psu_menu_table[] = {
    ...
    {"PEM Reg Read",                        (PFT)pem_monitor_dump_wr,    0,
        0,                  (PFT)0, 0, (PFT)0, 0},
#if 0  /* Not support on Tachi PSU, need to confirm PSU firmware */
 ...
 *              and it is supporting POE.
 * Inputs     : No. of PSU that will be checked
 * Outputs    : TRUE/FALSE
 *
 *******************************************************************************
 */
unsigned int check_poe_psu_present (int psu_no, int dummy)
{
 ...
 *******************************************************************************
 */
unsigned int check_psu_present (uint32_t psu_no)
{
    unsigned int result = FALSE, psu_present_mask = 0, reg;

    psu_present_mask = PSU1_PRESENT_MSK;

    diag_fpga_reg_read(FPGA_PSU_STS_REG, &reg);  
    if (reg & psu_present_mask) {
       result = TRUE;
    } else {
 ...
 *******************************************************************************
 */
unsigned int check_psu_stat (unsigned int psu_no)
{
    unsigned int result = FALSE, psu_present_mask = 0;
    unsigned int psu_ac_in_mask = 0, psu_12v_out_mask = 0, reg;
    ...
    /* Check AC Input stat */
    if (reg & psu_ac_in_mask) {
        if (reg & psu_12v_out_mask) {
            result = TRUE;
...
{
    /* it is menu mode only */
    return(pem_show_cookie_x(MENU_MODE, NULL));
}
...
/*******************************************************************************
 *
 * function   : pem_rd_cookie_pid
 * Description: read psu pid cookie 
 * Inputs     : pid - return pid string
 * Outputs    : PASS/FAIL
 *
 *******************************************************************************
 */
int pem_rd_cookie_pid (uchar *pid) 
{
    int ps_num, ia;
    uchar num_byte, *data_ptr, *eeprom_data;
...
{
    int field_length, field_type, ia, rv;
...
//    uint32 size = 0;
    ...
    rv = idprom_get_field_data(&rdbuf[IDPROM_TLV_OFFSET],
