/* $Id: diag_pem_lib.c,v 1.2 2016/04/20 11:25:30 benchen2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tachi_bmc/diag_pem_lib.c,v $
 *------------------------------------------------------------------
 *
 * diag_pem_lib.c: 
...
struct rp1ruve_pem_register_t rp1ruve_pem_reg[] = {
...
int get_psu_i2c_struct (n2g_i2c_if_t *psu_i2c, uint32_t dev_addr)
{
    n2g_i2c_if_t *tmp;

    /* init i2c_if for I2C */
    tmp = (n2g_i2c_if_t *)platform_fpga_get_n2g_i2c_if(I2C_CTRL_FOUR, I2C_MUX_ZERO,
                                                       dev_addr);

    if (tmp == NULL) {
        printf("%s: Failed to get PSU I2C interface structure.\n",
               __FUNCTION__);
        return (FAILED);
    }

    memcpy(psu_i2c, tmp, sizeof(n2g_i2c_if_t));
 ...
 **********************************************************************
 */
int rp1ruve_pem_check_offset (int offset, uint32_t *transaction_type) {
...
int rp1ruve_pem_read (int ps, uint32_t command_code, uint16_t* data) {
    ...
    switch (ps){
    case 1:
        rc = get_psu_i2c_struct(&i2c_if, MB_I2C_ADDR_PEM0_MCNTRL);
        break;
    case 2:
        rc = get_psu_i2c_struct(&i2c_if, MB_I2C_ADDR_PEM1_MCNTRL);
        break;
    default:
 ...
 **********************************************************************
 */
int rp1ruve_pem_write_eeprom (uint8_t *idprom, int tot_wr_size, uint32_t unused, int ps) {
    ...
    switch (ps){
    case 1:
        rc = get_psu_i2c_struct(&i2c_if, MB_I2C_ADDR_PEM0_EEPROM);
        break;
    case 2:
        rc = get_psu_i2c_struct(&i2c_if, MB_I2C_ADDR_PEM1_EEPROM);
        break;
    default:
        printf("*** %s:%d Got Unknown PSU no.(%d)\n",
               __FUNCTION__, __LINE__, ps);
 ...
 **********************************************************************
 */
int rp1ruve_pem_read_eeprom (uint8_t *idprom, int numBytes, uint32_t addr_input, int unused, int ps) {
