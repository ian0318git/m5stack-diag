/* $Id: testcard_fpga.c,v 1.2 2016/04/20 11:25:32 benchen2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tachi_bmc/testcard_fpga.c,v $
 *------------------------------------------------------------------
...
#define TC_SIG_TEST_HOST_FPGA_THRES_TACHI   0x000000
...
int tc_fpga_reg_test (void)
{
    ...
    cterr_add_component("FPGA register", "Testcard NIM FPGA");
    cterr_add_debug("FPGA register rd/wr test. ",
                    "Check platform FPGA I2C to NIM",
                    "Check NIM FPGA");
...
int tc_sync_sig_test (void)
{
    ...
    host_fpga_rev = get_diag_fpga_ver(); 
    thres = TC_SIG_TEST_HOST_FPGA_THRES_TACHI; 
    
    if ((host_fpga_rev & HOST_FPGA_REV_MSK) < thres) {
        ...
        printf("Bypass FPGA ver check, assume Tachi FPGA is supporting ");
        printf("signal test \n"); 
    }
    ...
    diag_fpga_reg_read(sync_reg_off, &ngio_sync_ctrl);
    diag_fpga_reg_read(FPGA_SYNC_DEBUG_REG_OFF, &sync_debug);

    ngio_sync_ctrl = TC_SYNC_SIG_TEST_PATTERN1;
    diag_fpga_reg_write(sync_reg_off, ngio_sync_ctrl);
    diag_fpga_reg_read(sync_reg_off, &ngio_sync_ctrl);

    if ((NVRAM)->diagflag & D_VERBOSE) {
        printf("%s:%d Host platform FPGA SYNC_CONTROL register: 0x%04X.\n",
    ...
    if ((NVRAM)->diagflag & D_VERBOSE) {
        printf("%s:%d Host platform FPGA SYNC_DEBUG register: 0x%04X.\n",
...
Revision 1.1.2.3  2015/11/24 06:14:19  alpeng
update testcard fpga menu

Revision 1.1.2.2  2015/08/28 03:43:55  alpeng
fix testcard i2c read/write

Revision 1.1.2.1  2015/07/31 10:40:02  alpeng
