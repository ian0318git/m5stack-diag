/* $Id: jbimain.c,v 1.2 2015/02/14 12:48:42 kodko Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/timingcard/jbimain.c,v $
 */
/*******************************************************************************
...
{
	return (((unsigned int) buf[0]) | (((unsigned int) buf[1]) << 8L) |
	        (((unsigned int) buf[2]) << 16L) | (((unsigned int) buf[3]) << 24L));
}
...
/****************************************************************************/
{
	...
	unsigned int action_table = 0;
	unsigned int proc_table = 0;
	unsigned int string_table = 0;
	unsigned int symbol_table = 0;
	unsigned int data_section = 0;
	unsigned int code_section = 0;
	unsigned int debug_section = 0;
	...
	*/
	...
	if ((status == JBIC_SUCCESS) && (symbol_count > 0)) {
        ...
        variables = (unsigned long *) jbi_malloc(
		...
		if (status == JBIC_SUCCESS) {
		    attributes = (char *) jbi_malloc((unsigned long) symbol_count);

			if (attributes == NULL) status = JBIC_OUT_OF_MEMORY;
		}

		if ((status == JBIC_SUCCESS) && (version > 0)) {
		    proc_attributes = (unsigned char *) jbi_malloc((unsigned long) proc_count);
			...
			for (i = 0; i < (unsigned int) symbol_count; ++i) {
				...
				*/
				...
				if ((attributes[i] & 0x0c) == 0x04) {
					...
					uncompressed_size =
						(((unsigned int) GET_BYTE(long_index)) |
						(((unsigned int) GET_BYTE(long_index + 1)) << 8L) |
						(((unsigned int) GET_BYTE(long_index + 2L)) << 16L) |
						(((unsigned int) GET_BYTE(long_index + 3L)) << 24L));
					variable_size[i] = uncompressed_size;
#else
					uncompressed_size = jbi_get_dword(&program[data_section + value]);

					/* allocate a buffer for the uncompressed data */
					variables[i] = (unsigned long) jbi_malloc(uncompressed_size);
					...
					if (variables[i] == 0) {
						...
						/* uncompress the data */
						if (jbi_uncompress(&program[data_section + value],
						                   variable_size[i],
						                   (unsigned char *) variables[i],
						                   uncompressed_size, version)
						                   != uncompressed_size) {
							/* decompression failed */
							status = JBIC_IO_ERROR;
						} else {
						    variable_size[i] = uncompressed_size * 8;
						}
					}
#endif
					...
					if (variable_size[i] > 0)
					{
						...
						variables[i] = (int) jbi_malloc(size);
					...
					if (variable_size[i] > 0) {
						...
						if (attributes[i] & 0x10) {
							/* integer array */
						    size = (unsigned int) (variable_size[i] * sizeof(long));
						} else {
							/* Boolean array */
							size = (unsigned int)
								((variable_size[i] + 7) / 8);
		...
		case 0x12: /* CMPS */
			...
			*/
			IF_CHECK_STACK(4) {
				...
				{
					long_temp &= ((-1) >> (32 - count));

					stack[stack_ptr - 1] =
						((a & long_temp) == (b & long_temp)) ? 1 : 0;
				}
			}
			break;
				...
				*/
				if ((version > 0) &&
					...
					*/
					...
					charptr_temp = jbi_malloc((unsigned long) long_temp);
				...
				if ((attributes[variable_id] & 0x1c) != 0x08) {
					...
					if (long_count < 1) {
						...
						for (i = 0; i < (unsigned int) long_count; ++i) {
							if (long_temp & (1 << (int) i)) {
								charptr_temp[long_index >> 3L] |=
									(1 << (long_index & 7L));
							} else {
								charptr_temp[long_index >> 3L] &=
									~ (unsigned int) (1 << (long_index & 7L));
							}
							++long_index;
						}
					}
			...
			IF_CHECK_STACK(2) {
				...
				else if ((opcode == 0x51) && !reverse)
				{
					status = jbi_do_drscan_multi_page(
						...
						(unsigned int) long_index, version);
				}
				else
				{
				...
				if (long_temp > variable_size[variable_id]) {
					...
					*/
                    variables[variable_id] = (long)
                        jbi_malloc((unsigned long) long_temp);
		...
		case 0x5C: /* EXPV */
			...
			*/
			IF_CHECK_STACK(3) {
				...
				if ((long_index & 7) != 0) {
                    charptr_temp2 = jbi_malloc((unsigned long)
                        ((long_count + 7) / 8));
					if (charptr_temp2 == 0) {
						status = JBIC_OUT_OF_MEMORY;
						break;
			...
			IF_CHECK_STACK(3)
			{
				...
				*/
				...
				if ((version > 0) && ((attributes[variable_id] & 0x9c) == 0x0c)) {
					...
					*	Allocate a writable buffer for this array
					*/
					long_temp = (variable_size[variable_id] + 7) >> 3;
					charptr_temp2 = (unsigned char *) variables[variable_id];
					charptr_temp = jbi_malloc((unsigned long) long_temp);
					variables[variable_id] = (long) charptr_temp;
		...
		case 0x84: /* WAIT */
			...
			*/
			IF_CHECK_STACK(2) {
				...
				if ((status == JBIC_SUCCESS) && (long_temp != 0)) {
...
/****************************************************************************/
{
	...
	unsigned int note_strings = 0;
	unsigned int note_table = 0;
...
)
{
	...
	unsigned int action_count = 0;
	unsigned int proc_count = 0;
	unsigned int act_name_id = 0;
	unsigned int act_desc_id = 0;
	unsigned int act_proc_id = 0;
	unsigned int act_proc_name = 0;
