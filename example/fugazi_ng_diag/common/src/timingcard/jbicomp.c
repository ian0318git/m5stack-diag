/* $Id: jbicomp.c,v 1.2 2015/02/14 12:48:41 kodko Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/timingcard/jbicomp.c,v $
 */
/*******************************************************************************
...
unsigned int jbi_in_length = 0;
unsigned int jbi_in_index = 0;	/* byte index into compressed array */
...
/****************************************************************************/
{
	...
	else
	{
		...
		long_end = jbi_out_length -
			(() jbi_current_page * () MATCH_DATA_LENGTH);
...
/****************************************************************************/
{
	...
	if (variable_id != jbi_current_variable_id)
	{
		...
		jbi_out_length =
			(((unsigned int) GET_BYTE(data_section + value)) |
			(((unsigned int) GET_BYTE(data_section + value + 1)) << 8) |
			(((unsigned int) GET_BYTE(data_section + value + 2)) << 16) |
			(((unsigned int) GET_BYTE(data_section + value + 3)) << 24));
		jbi_in_index = 4;	/* skip over length field */
		jbi_out_index = 0;
...
/*------------------------------------------------------------------------------
 * $Log: jbicomp.c,v $
 * Revision 1.2  2015/02/14 12:48:41  kodko
