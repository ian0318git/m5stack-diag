/* $Id: diag_spi_flash_util.c,v 1.2 2019/10/17 02:16:23 kehuang2 Exp $
 *------------------------------------------------------------------
 * 
...
int tabei_spi_flash_utils(int opt);
int tabei_spi_flash_get_prod_id(int, uint*);
int tabei_spi_flash_get_prod_name(int, char*);
int tabei_spi_flash_get_rdsr(int, uchar*);
int tabei_spi_flash_set_wrsr(int, uchar); 
int tabei_spi_flash_show_info(int); 
int tabei_spi_flash_write_protect_enable(int);
...
static submenu_xtable_t spi_flash_utils_tbl[] = {
    {"Show Rommon Golden SPI Flash Info", (type_t(*)())tabei_spi_flash_show_info, GOLDEN_ROMMON_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Show Rommon Upgrade SPI Flash Info", (type_t(*)())tabei_spi_flash_show_info, UPGRADE_ROMMON_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Show FPGA SPI Flash Info", (type_t(*)())tabei_spi_flash_show_info, FPGA_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
#ifdef SPI_FLASH_DEBUG
     /* for Debug*/
     {"RDSR - Golden", (type_t(*)())denverton_spi_read_status_reg, GOLDEN_ROMMON_SPI_FLASH, 0,
     (type_t(*)())this_is_tabei_j, 0, (type_t(*)())0,   0},
...
int denverton_spi_ctrl_mm_read32 (uint offset, uint *buf)
{
    ...
    target = (off_t)(TABEI_DENVERTON_SPI_CTRL_MM_ADDR + offset);
...
int denverton_spi_write_status_reg (int select_flash)
{
    ...
    if (select_flash == GOLDEN_ROMMON_SPI_FLASH) {
        ...
        /* tabei uses fpga to mux the bootflash */
...
/*******************************************************************************
 *
 * Function    : tabei_spi_flash_utils
...
/*******************************************************************************
 *
 * Function    : tabei_spi_flash_get_prod_id
 ...
 *******************************************************************************
 */
int tabei_spi_flash_get_prod_id (int select_flash, uint *spi_id) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
...
/*******************************************************************************
 *
 * Function    : tabei_spi_flash_get_prod_name
 ...
 *******************************************************************************
 */
int tabei_spi_flash_get_prod_name (int select_flash, char *buf) 
{
 
    uint spi_prod_id = 0;
    int ret = 0;

    ret = tabei_spi_flash_get_prod_id(select_flash, &spi_prod_id);
    if (ret != PASSED) {
        return (FAILED);
    } 
...
/*******************************************************************************
 *
 * Function    : tabei_spi_flash_get_rdsr
 ...
 *******************************************************************************
 */
int tabei_spi_flash_get_rdsr (int select_flash, uchar * rdsr_val) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
...
/*******************************************************************************
 *
 * Function    : tabei_spi_flash_set_wrsr
 ...
 *******************************************************************************
 */
int tabei_spi_flash_set_wrsr (int select_flash, uchar wrsr_val) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
...
/*******************************************************************************
 *
 * Function    : tabei_spi_flash_show_info
 ...
 *******************************************************************************
 */
int tabei_spi_flash_show_info (int select_flash) 
{
    int ret = 0;
    ...
    ret = tabei_spi_flash_get_prod_name(select_flash, prod_name);
    if (ret != PASSED) {
        printf("%s: Failed to get SPI Flash product name.\n", __FUNCTION__);
        return (FAILED);
    } 

    ret = tabei_spi_flash_get_rdsr(select_flash, &rdsr);
    if (ret != PASSED) {
...
/*******************************************************************************
 *
 * Function    : tabei_spi_flash_write_protect_enable
 ...
 *******************************************************************************
 */
int tabei_spi_flash_write_protect_enable (int select_flash) 
{
    uint spi_prod_id = 0;
    int ret = 0;
    uchar wrsr = 0;
    ...
    ret = tabei_spi_flash_set_wrsr(select_flash, wrsr);    
    if (ret != PASSED) {
...
int fpga_spi_flash_write_protect_test (void)
{
    ...
    ret = tabei_spi_flash_get_prod_id(FPGA_SPI_FLASH, &spi_prod_id);
    if (ret != PASSED) {
        return (FAILED);
    } 	
