 /* $Id: diag_usb_util.c,v 1.2 2019/10/17 02:16:24 kehuang2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tabei-l/diag_usb_util.c,v $
 *------------------------------------------------------------------
 *
...
int usb_test_mode (int option)
{
    ...
    if(ret > 0) {
        ...
        if (tabei_mem_write32(reg_int, 0x80) != PASSED) {
    ...
    sprintf(reg, "%s%s", prefix, USB_PORT_CTRL_TEST);
    reg_int = strtoul(reg, NULL, 16); 
    if (tabei_mem_write32(reg_int, reg_val) != PASSED) {
 ...
 *
 * Revision 1.1.2.5  2019/04/30 05:57:32  kodko
 * Remove needless enumerate USB utility
 *
 * Revision 1.1.2.4  2019/01/18 02:30:16  olin2
 * Clean up code
 *
 * Revision 1.1.2.3  2018/11/16 13:42:30  kodko
 * Support front USB hub and PIM USB hub connect with USB3.0 and USB2.0 storage read/write test.
 *
 * Revision 1.1.2.2  2018/10/03 06:51:01  kodko
