 /* $Id: dnv_gpio_lib.c,v 1.2 2019/10/17 02:16:24 kehuang2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tabei-l/dnv_gpio_lib.c,v $
 *------------------------------------------------------------------
 * 
 * dnv_gpio_lib.c
 *
...
int     dnv_ethernet_sdp_pin_read(ulong, uint *);
...
/*******************************************************************************
 *
 * Function    : dnv_ethernet_sdp_pin_read
 * Description : Function to read the RX value of Software Define 
 *               Pin in Ethernet Controller X550 Series 
 * Inputs      : which_port - 0/1
 *               *val   - data buffer 
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
 */
int dnv_ethernet_sdp_pin_read (ulong which_port, uint *val) 
{
    int      fd = -1;
    ...
    if (which_port == LAN0_PORT0_SPD0) {
        sprintf(ethaddr, "%s%s", DNV_PCIE_X553_LAN0, DNV_PCIE_X553_LAN0_PORT0);
    } else {
        sprintf(ethaddr, "%s%s", DNV_PCIE_X553_LAN0, DNV_PCIE_X553_LAN0_PORT1);
    } 
    ...
    virt_addr = map_base + DNV_EXTENDED_SDP_CONTROL;

    if ((NVRAM)->diagflag & D_VERBOSE) {
        printf("Data: %x\n", *(volatile uint32_t*)virt_addr);
    }
    *val = *(uint*)virt_addr & DNV_ESDP_SDP0_DATA_MASK;
...
int dnv_set_pcie_1f_1 (void)
{
    ...
    offset = CONFIG_TABEI_PCIE_ADDR + tmp_bus_dev_fun;
 ...
 *
 * Revision 1.1.2.6  2019/08/27 07:45:06  kehuang2
 * Clean up code
 *
 * Revision 1.1.2.5  2019/08/26 07:55:00  kehuang2
 * Clean up code by the comment of code review
 *
 * Revision 1.1.2.4  2019/07/02 09:51:35  olin2
 * Enhance read SDP0 function
 *
 * Revision 1.1.2.3  2019/06/28 04:00:54  kehuang2
 * Change GEPHY Inperttupt pin from GPIO to Ethernet Controller SDPdiag_gephy_test.c
 *
