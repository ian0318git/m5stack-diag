 /* $Id: diag_emmc_test.c,v 1.2 2019/10/17 02:16:20 kehuang2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tabei-l/diag_emmc_test.c,v $
 *------------------------------------------------------------------
 *
 ...
 *******************************************************************************
 */
static int get_emmc_size (char* emmc_size, int bufsize, char* dev_name)
{
    char cmd[MAX_COMMAND_LENGTH]={0};

    sprintf(cmd, "fdisk -l 2>/dev/null | grep Disk | grep -i -w %s | awk '{print $5}'", dev_name);
    
    if( (ExecuteCmdbyPopen (cmd, emmc_size, bufsize)) == 0 ) {
        return (FAILED);
    }
...
int diag_emmc_rw_test (int opt)
{
    ...
    cterr_add_component("Intel Denverton SOC C3758", "SDIO/MMC", 
                        "eMMC Flash");
 ...
 *
 * Revision 1.1.2.6  2019/09/10 07:41:09  olin2
 * Code clean up
 *
 * Revision 1.1.2.5  2019/06/20 06:21:13  kehuang2
 *
 * 1. Support linux_block_test function
 * 2. Update Diag menu item base on currently project information
 *
 * Revision 1.1.2.4  2019/05/22 09:01:57  kehuang2
 * Clean up code
 *
 * Revision 1.1.2.3  2019/04/29 08:14:26  kehuang2
 * Clean up code
 *
 * Revision 1.1.2.2  2018/12/12 08:49:38  olin2
 * Remove eMMC GPP test
 *
