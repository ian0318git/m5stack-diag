// $Id: fpga_smartfan.c,v 1.3 2020/08/06 07:54:55 kehuang2 Exp $
// $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tabei-l/fpga_smartfan.c,v $
//------------------------------------------------------------------------------
...
void fpga_smartfan_display (unsigned fan)
{
    ...
    if (ret == DATA_IS_VALID) {
        ...
        printf("mfg sn : %s\n", parsed_ptr->mfg_sn);
#if 0
    for (ix=0; ix < sizeof(raw_ptr->raw_data); ix++) {
        printf("%x ", raw_ptr->raw_data[ix]);
    }
    printf("\n");
#endif
    } else {
...
//--------------------------------------------------------------------------------
//  Description:
//    Display smartfan info with retry mechanism
//  Parameters:
//    fan     : Which fan, fan1 or fan2
//  Returns:
//    PASSED / FAILED
//--------------------------------------------------------------------------------

int fpga_smartfan_display_workaround (unsigned fan)
{
    smartfan_rawdata_t *raw_ptr = malloc(sizeof(smartfan_rawdata_t));
    smartfan_parsedata_t *parsed_ptr = malloc(sizeof(smartfan_parsedata_t));
    unsigned int timeout=10, ret = 0, retry_count = 1;
    if (raw_ptr == NULL) {
        cterr('f', 0, "Allocate raw_ptr failed.\n");
        return (FAILED);
    }

    if (parsed_ptr == NULL) {
        cterr('f', 0, "Allocate parsed_ptr failed.\n");
        return (FAILED);
    }

/*    FPGA/HW have bug, HW engineer suggest to add retry for display smartfan info */
__retry:
    ret = fpga_smartfan__read_fsm(fan, raw_ptr, parsed_ptr, timeout);
    if (ret == DATA_IS_VALID) {
        printf("\nSmartfan %d : Valid\n", fan+1);
        ...
        if (retry_count <= MAX_RETRY) {
            printf("\nSmartfan %d : Data unable to read, retry count = %d\n", fan+1, retry_count);
            retry_count++;
            msleep(RETRY_DELAY);
            goto __retry;
        } else {
            free(raw_ptr);
            free(parsed_ptr);
            cterr('f', 0, "\nSmartfan %d : Data unable to read, we tried.\n", fan+1);
            return (FAILED);
       }
    }
    free(raw_ptr);
