/* $Id: diag_emmc_test.c,v 1.2 2019/01/10 06:36:22 wilbhuan Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/betelgeuse/diag_emmc_test.c,v $
 *------------------------------------------------------------------
 *
 * diag_emmc_test.c
 *
...
static int access_emmc_test (int full_test)
{
    ...
    if (devfd < 0) {
        cterr('f', 0, "%s:%d:open file:%s fail", buf, __FUNCTION__, __LINE__);
        return (FAILED);
    }
