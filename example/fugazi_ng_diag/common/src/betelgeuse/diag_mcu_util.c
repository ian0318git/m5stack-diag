/* $Id: diag_mcu_util.c,v 1.2 2019/01/10 06:36:23 wilbhuan Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/betelgeuse/diag_mcu_util.c,v $
 *------------------------------------------------------------------
 * 
 * diag_mcu_util.c
 *
...
int show_reg (void)
{
    ...
    while (reg_ptr->offset != 0xFFFFFFFF) {
        ...
        if ((result = plat_mcu_reg_rd(offset, &buffer)) != RC_I2C_OP_OK) {
            printf("%s:%d:Unable to read Regiser %#x.\n", 
