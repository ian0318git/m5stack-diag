/* $Id: diag_wifi_util.c,v 1.2 2019/01/10 06:36:24 wilbhuan Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/betelgeuse/diag_wifi_util.c,v $
 *------------------------------------------------------------------
 *
 * diag_wifi_util.c
 *
...
int wifi_console_redirect (void)
{
    ...
    if (plat_release_wifi_from_reset() != PASSED) {
        return (FAILED);
    }
#endif
...
int wifi_led_control_util (void)
{
    ...
    printf("3: Turn WiFi LED Amber\n");
    cmd = (int)gethex_answer("\nEnter your choice(0 ~ 3)",
                             cmd, WIFI_LED_OFF, WIFI_LED_AMBER);
