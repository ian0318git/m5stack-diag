/* $Id: diag_rbcp_lib.c,v 1.2 2019/01/10 06:36:23 wilbhuan Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/betelgeuse/diag_rbcp_lib.c,v $
 *------------------------------------------------------------------
 * 
 * diag_rbcp_lib.c
 *
...
int platform_cleanup_rbcp_ge_env (void)
{
    ...
    if (status) {
        printf("%s:%d:Failed to cleanup, status = 0x%x\n", 
               __FUNCTION__, __LINE__, status);
        return (FAILED);
    }
    close(socket_gl);
