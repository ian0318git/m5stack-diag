/* $Id: diag_wifi_lib.c,v 1.2 2019/01/10 06:36:24 wilbhuan Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/betelgeuse/diag_wifi_lib.c,v $
 *------------------------------------------------------------------
 *
 * diag_wifi_lib.c
 *
 ...
 *******************************************************************************
 */
int wifi_enable_platform_wifi_eth(void);
int wifi_set_uboot_tftp(char *, char *, char *);
int wifi_confirm_gpio(char *);
...
static wifi_gpio_t plat_wifi_gpio_tbl[] = {
 ...
 * Description: Function to confirm WiFi GPIO configuration.
 * Inputs     : tty_dev
 * Outputs    : PASSED/FAILED
 *
 *******************************************************************************
 */
int wifi_confirm_gpio (char *tty_dev)
{
    char cmd_str[PLAT_WIFI_PARMS_LENGTH];
    char chk_cmd[PLAT_WIFI_PARMS_LENGTH];
    char cmp_str[PLAT_WIFI_PARMS_LENGTH];
    uint32_t addr = 0;
    uint16_t value = 0;
    ...
    total_num = (int)(sizeof(plat_wifi_gpio_tbl) / sizeof(wifi_gpio_t));
    if ((NVRAM)->diagflag & D_VERBOSE) {
    ...
    for (set_ctr = 0; set_ctr < total_num; set_ctr++) {
        ...
        gpio_p = &plat_wifi_gpio_tbl[set_ctr];
        ...
        if (result != PASSED) {
            printf("%s: Failed to configure WiFi %s GPIO.\n",
