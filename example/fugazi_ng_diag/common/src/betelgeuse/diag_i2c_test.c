/* $Id: diag_i2c_test.c,v 1.2 2019/01/10 06:36:23 wilbhuan Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/betelgeuse/diag_i2c_test.c,v $
 *------------------------------------------------------------------
 *
 * diag_i2c_test.c
 *
...
int diag_i2c_scan_test (int option)
{
    ...
    for (now_test = 0; now_test < test_end; now_test++) {
        ...
        /* Skipped to check WiFi if platform has no WiFi */
        if ((i2c_if.i2c_dev == WIFI_I2C_PLAT_ADDR_TEMP) &&
            platform_has_wifi() == FALSE) {
            printf("Skipped WiFi temperature sensor because platform is not WiFi SKU.\n");
            continue;
        }
