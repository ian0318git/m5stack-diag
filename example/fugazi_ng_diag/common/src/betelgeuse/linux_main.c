/* $Id: linux_main.c,v 1.2 2019/01/10 06:36:24 wilbhuan Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/betelgeuse/linux_main.c,v $
 *------------------------------------------------------------------
 * 
 * linux_main.c
 *
...
int quiet_launch = 0;
static void init_eth2(void);
static int xdsl_eth2_linkup(void);
...
static void init_eth2 (void)
{
    if (platform_has_xdsl() || platform_has_gshdsl()) {
        if (xdsl_eth2_linkup() != PASSED) {
            printf("\n%s:%d:xdsl eth2 linkup fail\n",__FUNCTION__,__LINE__);
            cterr('f', 0, "Failed to config GE Force link up.");
        } else {
            printf("xDSL eth2 linkup successfully\n");
        }
    }
    return;
}
...
#define PLAT_G2_TX_SSC_AMP    0x23
    ...
    reg_val |= (PLAT_G2_TX_SSC_AMP << G2_TX_SSC_AMP_OFFSET);
...
static void platform_init (void)
{
    ...
    /* A shor-term solution for 4P ESW issue 
     * Issue description: diag cannot run ext. lpbk test
     * in 88E6176 PHY after booted up. */
    if (platform_esw_type() == ESW_MRVL88E6176) {
        system("devmem 0xf2132e0c 32 0x9260");
    }
    ...
    /* init xDSL and G.SHDSL */
    init_eth2();

    /* Initialize USB register */
    if (plat_usb_init() != PASSED) {
        printf("%s:%d:Failed to init USB\n", __FUNCTION__, __LINE__);
    }
}
