/* $Id: nim_dm_cpss_extserv.c,v 1.10 2020/05/22 02:28:26 qingcwan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/dreamliner/nim_dm_cpss_extserv.c,v $
 *------------------------------------------------------------------
...
pcie_mapping_array_t * nim_dm_get_pcie_mapping(void)
{
    ...
    */
...
#ifdef TABEIL
    ents[0].sub_bus = ents[0].sec_bus; /* Tabei-L platform didn't have PCIe SW */
#else
    if (is_goldbeach() || is_curie_1ru() || is_curie_2ru()) {
        ents[0].sub_bus = ents[0].sec_bus; /* GB and curie 1RU/2RU platform didn't have PCIe SW */
    } else {
...
#ifdef TABEIL
    ents[1].sub_bus = ents[1].sec_bus; /* Tabie-L platform didn't have PCIe SW */
#else
    if (is_goldbeach() || is_curie_1ru() || is_curie_2ru()) {
        ents[1].sub_bus = ents[1].sec_bus; /* GB and curie 1RU/2RU platform didn't have PCIe SW */
    } else {
    ...
    if (is_dagger() || is_sword() || is_goldbeach() ||
        is_curie_1ru() || is_curie_2ru()) {
	ents[6].sec_bus = get_bus_num(DAUGHTER_CARD, NGWIC1_SLOT);
    } else
	ents[6].sec_bus = 0;
#else
    ents[6].sec_bus = 0;
#endif
    ents[6].sub_bus = ents[6].sec_bus + 8;
    ...
    if (is_dagger() || is_sword() || is_goldbeach() ||
        is_curie_1ru() || is_curie_2ru()) {
	ents[7].sec_bus = get_bus_num(DAUGHTER_CARD, NGWIC2_SLOT);
    } else
	ents[7].sec_bus = 0;
#else
    ents[7].sec_bus = 0;
#endif
    ents[7].sub_bus = ents[6].sec_bus + 8;
 ...
 * Revision 1.10  2020/05/22 02:28:26  qingcwan
 * Merge switzer-carrier code into main chunk.
 *
 * Revision 1.9  2020/01/09 01:02:10  jiajliu
 * Merge Curie 2RU to main trunk
 *
 * Revision 1.8  2019/10/17 02:16:15  kehuang2
 * Collapse Tabei-L into main trunk
 *
 * Revision 1.7  2019/08/06 06:56:07  alpeng
 * merge curie, switzer and nightwatch to trunk
 *
 * Revision 1.6.2.2  2018/10/01 06:00:25  leschen
 * Modify NGSM PCIe bus number and support Dreamliner module
 *
 * Revision 1.6.2.1  2018/07/16 09:28:04  alpeng
 ...
 *
 * Revision 1.3.2.3  2018/05/17 10:50:21  alpeng
 *  sync with trunk <trunk-051618>
 *
 * Revision 1.3.2.2  2017/04/05 06:32:47  leschen
 * Sync with <ng_diag-tag-032917>
 *
 * Revision 1.3.2.1  2016/12/07 05:59:00  alpeng
 ...
 *
 * Revision 1.2.4.2  2015/09/14 08:17:17  alpeng
 * update dreamliner init seq to support Tachi-Entry
 *
