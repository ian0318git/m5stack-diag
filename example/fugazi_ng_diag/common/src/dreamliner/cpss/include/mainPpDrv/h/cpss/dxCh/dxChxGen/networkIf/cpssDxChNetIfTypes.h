/* $Id: cpssDxChNetIfTypes.h,v 1.1 2015/02/13 11:32:14 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/dreamliner/cpss/include/mainPpDrv/h/cpss/dxCh/dxChxGen/networkIf/cpssDxChNetIfTypes.h,v $
 *------------------------------------------------------------------
 *
 * Ian Chang - Nov. 2014
 *
 ...
 *  vid - the value of vlan id (APPLICABLE RANGES: 0..4095)
 ...
 *
 *      srcIsPe - Used for split horizon filtering in L2MLL:
 *                GT_FALSE: Source ePort is not PE
 *                GT_TRUE:  Source ePort is PE
 *                (APPLICABLE DEVICES: Lion3)
 *
 *      isTrgPortValid - Indicates whether the target physical port field
 ...
 *
 *
 *      analyzerIsTrgPortValid      - The target Analyzer interface indication that the
 *                      target analyzer port field <Reserved Analyzer_TrgPort>
 *                      has a valid value
 *                      NOTE:This field is relevant only when <Reserved Analyzer_UseVIDX> = 0
 *      analyzerTrgDev      - The target Analyzer interface device number
 *      analyzerTrgPort     - The target Analyzer interface physical port number
 *      analyzerTrgEport    - The target analyzer interface ePort
 *      analyzerUseVidx     - The target Analyzer interface Use_VIDX
 *      analyzerEvidx       - The target analyzer interface eVIDX
 *
 *
 */
typedef struct{
    ...
    GT_HW_DEV_NUM                   analyzerTrgDev;
    GT_PHYSICAL_PORT_NUM            analyzerTrgPort;
    GT_PORT_NUM                     analyzerTrgEport;
    GT_BOOL                         analyzerUseVidx;
    GT_U32                          analyzerEvidx;
 ...
 *             ePort - When isEgressPipe == GT_FALSE:
 *                  This tag contains source information, and this field
 *                  indicates the source ePort number assigned by the ingress device.
 *                  When isEgressPipe == GT_TRUE:
 *                  This tag contains target information, and this field
 *                  indicates the target ePort number assigned by the ingress device.
 *
 *
...
/*
 *------------------------------------------------------------------
 * $Log: cpssDxChNetIfTypes.h,v $
 * Revision 1.1  2015/02/13 11:32:14  iachang
