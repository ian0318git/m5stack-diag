/* $Id: cpssDxChCfgInit.h,v 1.1 2015/02/13 11:31:37 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/dreamliner/cpss/include/mainPpDrv/h/cpss/dxCh/dxChxGen/config/cpssDxChCfgInit.h,v $
 *------------------------------------------------------------------
 *
 * Ian Chang - Nov. 2014
 *
...
/*******************************************************************************
* cpssDxChCfgSrcPortPeSet
*
* DESCRIPTION:
*       Set indication that this port is Provider Edge (PE). Used for filtering in
*       L2MLL table entry and passed in DSA tag.
*
* APPLICABLE DEVICES:
*        Lion3.
*
...
*       devNum        - device number
*       portNum       - port number
*       enable        - GT_TRUE:  Source port is PE
*                       GT_FALSE: Source port is not PE
* OUTPUTS:
*       None.
*
* RETURNS:
...
*
*******************************************************************************/
GT_STATUS cpssDxChCfgSrcPortPeSet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    IN  GT_BOOL                             enable
...
/*******************************************************************************
* cpssDxChCfgSrcPortPeGet
*
* DESCRIPTION:
*       Get indication if this port is Provider Edge (PE). Used for filtering in
*       L2MLL table entry and passed in DSA tag.
*
* APPLICABLE DEVICES:
*        Lion3.
...
*
*******************************************************************************/
GT_STATUS cpssDxChCfgSrcPortPeGet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    OUT GT_BOOL                             *enablePtr
...
*       Global ePorts are used to identify:
*       1. ECMP Primary ePorts - all ePorts that share the same ECMP path are
*          assigned with a single global primary ECMP ePort# in the global ePort
*          range.
*       2. ePorts that are mapped over a physical trunk.
*
* APPLICABLE DEVICES:
*        Lion3.
*
...
* INPUTS:
*       devNum      - device number
*       ecmpPtr     - (pointer to) L2 ECMP Primary ePorts.
*       trunkPtr    - (pointer to) Trunk ePorts.
*
* OUTPUTS:
*       globalPtr    - (pointer to) Global ePorts.
*
* RETURNS:
*       GT_OK                    - on success.
*       GT_BAD_PARAM             - on wrong input parameters
...
*
* COMMENTS:
*       L2 ECMP and trunk configured ePort values cannot be configured
*       independently, but must obey the following rules\constraints:
*       Rule 1: The same ePort cannot be a member of both the ECMP Primary
*               ePorts and the trunk ePorts.
*       Rule 2: Since the ePort range is presented with a pattern and mask
*               pair, there must be a way to present the union of the ECMP
*               and trunk ranges also as a pattern and a mask pair.
*
*       In order for the listed above rules to be fulfilled the following
*       bitwise relations must be obeyed (in case both ECMP and trunk ranges
*       are configured, CPSS_DXCH_CFG_GLOBAL_EPORT_STC.enable == GT_TRUE):
*       1. ECMP_ePort.mask == Trunk_ePort.mask
*       2. The value of:
*           (ECMP_ePort.mask AND ECMP_ePort.pattern) XOR
*                                   Trunk_ePort.mask AND Trunk_ePort.pattern)
*          must contain one and only one bit set to 1.
*
*******************************************************************************/
GT_STATUS cpssDxChCfgGlobalEportSet
(
    IN  GT_U8                           devNum,
    IN  CPSS_DXCH_CFG_GLOBAL_EPORT_STC  *ecmpPtr,
    IN  CPSS_DXCH_CFG_GLOBAL_EPORT_STC  *trunkPtr,
...
*
* COMMENTS:
*       Please refer to the cpssDxChCfgGlobalEportSet COMMENTS section for
*       details regarding the constraints imposed on the requested configured
*       ePort ranges.
*       In this API - cpssDxChCfgGlobalEportGet - a check that the constraints
*       are fulfilled is performed as well.
*
*******************************************************************************/
GT_STATUS cpssDxChCfgGlobalEportGet
(
    IN  GT_U8                           devNum,
    OUT CPSS_DXCH_CFG_GLOBAL_EPORT_STC  *ecmpPtr,
    OUT CPSS_DXCH_CFG_GLOBAL_EPORT_STC  *trunkPtr,
    OUT CPSS_DXCH_CFG_GLOBAL_EPORT_STC  *globalPtr
);
...
/*
 *------------------------------------------------------------------
 * $Log: cpssDxChCfgInit.h,v $
 * Revision 1.1  2015/02/13 11:31:37  iachang
