/* $Id: prvCpssDxChPcl.h,v 1.1 2015/02/13 11:32:24 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/dreamliner/cpss/include/mainPpDrv/h/cpss/dxCh/dxChxGen/pcl/private/prvCpssDxChPcl.h,v $
 *------------------------------------------------------------------
 *
 * Ian Chang - Nov. 2014
 *
 ...
 *   hasExtension - 0 - generate kew without extension
 *                  1 - generate kew with extension
...
#define PRV_CPSS_DXCH_PCL_LOOKUP_CFG_HW_FIELDS_AMOUNT_CNS 8
 ...
 *    hasExtensionOff  - Offset of "hasExtension" bit
 *                       (APPLICABLE DEVICES: Lion3)
 *    hasExtensionLen  - Length of "hasExtension" bit - 1
 *                       (APPLICABLE DEVICES: Lion3)
 *
 * Comments:
...
typedef struct
{
    ...
    GT_U8    pclIdL01Off;
    GT_U8    pclIdL01Len;
    GT_U8    hasExtensionOff;
    GT_U8    hasExtensionLen;
 ...
 *
 * Fields:
 *    srcPortPtr        - (pointer to) srcPort field.
 *    portListBmpPtr    - (pointer to) portListBmp field mask.
 *
 * Comments:
 */
typedef struct
...
*       withExtension  - GT_FALSE - no extension,
*                        GT_TRUE - with extension
...
*
* NOT APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; xCat2; Lion2; Lion3.
*
* INPUTS:
*       devNum         - device number
*       portGroupId         - The port group Id. relevant only to 'multi-port-groups' devices.
...
*                          For Lion3 the additional size
*                          caused by extension ignored.
*
* RETURNS:
*       GT_OK                    - on success
*       GT_BAD_PARAM             - on wrong parameters
...
*   For Lion3 they are source port and port list bitmap.
...
*
*******************************************************************************/
GT_VOID prvCpssDxChPclRuleDataLoadPortGroupModifiedFields
(
    IN     GT_U8                                                    devNum,
    IN     GT_U32                                                   hwMaskArr[],
...
*                               Convert rule with extension (APPLICABLE DEVICES: Lion3).
...
/*******************************************************************************
* prvCpssDxChPclDefaultMapPortsToBitIndexesSet
*
* DESCRIPTION:
*   The function sets the default map of local ports numbers
*   to indexes of bits in port list bitmap.
*
* APPLICABLE DEVICES:
*        Lion3.
*
...
*
* COMMENTS:
*       Not treats port list bitmap and source port.
*
*******************************************************************************/
GT_STATUS prvCpssDxChPclDefaultMapPortsToBitIndexesSet
(
    IN GT_U8 devNum
);
...
/*
 *------------------------------------------------------------------
 * $Log: prvCpssDxChPcl.h,v $
 * Revision 1.1  2015/02/13 11:32:24  iachang
