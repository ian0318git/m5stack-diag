/* $Id: nim_arkenstone.c,v 1.11 2020/01/09 01:01:51 jiajliu Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nim_arkenstone.c,v $
 *******************************************************************************
 *
 * nim_arkenstone.c - Arkenstone code, leverage from F35
 *
 * Oct 2015, Xiaoying Zhang
 *
...
//static arkenstone_ds_t arkenstone_ds[3];
static struct ngio_intf_t *arkenstone_iface;
static void (*arkenstone_saved_diag_exec)(void) = NULL;
 ...
 */
submenu_xtable_t arkenstone_mod_utils_submenu_table[] = {
    {"Console Redirect",              (PFT)arkenstone_console_switch,0,   0,
     (type_t(*)())0, 0,    (type_t(*)())0,          0},
    {"Power off Arkenstone NGWIC",     (PFT)arkenstone_power_off,   0,   0,
     (type_t(*)())0, 0,    (type_t(*)())0,          0},
};
 ...
 * Primary & secondary submenu items (filled in from xtable)
 */
submenu_xtable_t arkenstone_utils_submenu_table[] = {
    {"Console Redirect",              (PFT)arkenstone_console_switch, 0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"Power off Arkenstone NGWIC",     (PFT)arkenstone_power_off,   0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"Power on Arkenstone NGWIC",      (PFT)arkenstone_pwr_on,      0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"Power cycle Arkenstone NGWIC",   (PFT)arkenstone_pwr_cycle,   0,   MM_2,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"Enable Backplane GE loopback",  (PFT)enable_bp_ge_lpbk,     0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"Disable Backplane GE loopback", (PFT)disable_bp_ge_lpbk,    0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"UART Test",          (PFT)arkenstone_uart_test,   0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    ...
    {"LTC4215 Register Write",        (PFT)ltc4215_reg_write,     0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"Arkenstone Power Stat",        (PFT)arkenstone_power_stat,     0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"PCA9557 Register Read",         (PFT)pca9557_reg_read,      0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},
    {"PCA9557 Register Write",        (PFT)pca9557_reg_write,     0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},    
    {"Escape to Shell (debugging only)", (PFT)arkenstone_o2_shell,0,   0,
     (type_t(*)())0, 0,	   (type_t(*)())0,          0},    
    {"Execute a Shell command (debugging only)",(PFT)arkenstone_o2_command,0,0,
      (type_t(*)())0, 0,   (type_t(*)())0,          0},   
};

#define Arkenstone_UTILS_SUBMENU_TABLE_SZ (sizeof(arkenstone_utils_submenu_table) / sizeof(submenu_xtable_t))

static mitem_t arkenstone_utils_primary_items[Arkenstone_UTILS_SUBMENU_TABLE_SZ+MAX_BASE_ITEMS];
static mitem_t arkenstone_utils_secondary_items[Arkenstone_UTILS_SUBMENU_TABLE_SZ+MAX_BASE_ITEMS];

char arkenstoneutiltitle[50];
menuinfo_t arkenstone_util_submenu = {
    arkenstoneutiltitle,
    0,                                /* mtparam added by init_empty_menu */
    (PFT)menu_show_dflags,            /* notes missing WICs in combos */
    0,                                /* use generic prompt */
    0,                                /* size (bumped by add_menu_item() */
    arkenstone_utils_primary_items,
};

menuinfo_t *arkenstone_util_submenup = &arkenstone_util_submenu;

/*=========================================
 * Main menu items
 *=========================================
 */
static submenu_xtable_t arkenstone_mainmenu_tbl[] = {
    {"Arkenstone Interface Utilities",  (PFT)arkenstone_utils,          0,  0,
     (type_t(*)())0, 0, (type_t(*)())arkenstone_utils, 0},
    {"GPIO Expander/LTC4215 test",      (PFT)gpio_ltc_test,      0,  MM_2,
     (type_t(*)())0, 0, (type_t(*)())0,         0},
    {"Arkenstone NIM test",             (PFT)arkenstone_console_switch, 0,  MM_2,
     (type_t(*)())0, 0,	(PFT)arkenstone_console_switch,          1},
};

#define Arkenstone_MAINMENU_TBL_SIZE (sizeof(arkenstone_mainmenu_tbl) / sizeof(submenu_xtable_t))

/*
 * Primary & secondary submenu items (filled in from xtable)
 */
static mitem_t main_menu_primary_items[Arkenstone_MAINMENU_TBL_SIZE + MAX_BASE_ITEMS];
static mitem_t main_menu_secondary_items[Arkenstone_MAINMENU_TBL_SIZE + MAX_BASE_ITEMS];
...
int arkenstone_tests_use_enet = 1; /* continuos mode, run all tests on module */

/***********************************************************************
 * Name: arkenstone_cleanup_ge_env (common)
 *
 * Description:
 ...
 ***********************************************************************
 */
int arkenstone_cleanup_ge_env (int slot)
{
    ngio_priv_t *ep;
    int status = PASSED;
    int sgmii_port = 0;
...
#elif TABEIL
        sprintf(if_name, TABEI_ETH_BP);
#elif NANOOK
        sprintf(if_name, inface_lan0p0);
#else
 ...
 **********************************************************************
 */
static int wait_for_ge_packet (int slot, int count, int disp)
{
    ngio_priv_t *ep;
 ...
 //   printf("\b");
    ...
    if ((chk_macaddr(recv_buffer, (uchar *)&(ep->eth_hdr.src_addr[0])) != 0) ||
        ...
        //if (disp_err == 1) /* This will always be a non-zero value */ {
            ...
            printf("\r\n HOST: Expected MAC : %02x:%02x:%02x:%02x:%02x:%02x",
                   (uchar)ep->eth_hdr.dest_addr[0], (uchar)ep->eth_hdr.dest_addr[1],
                   (uchar)ep->eth_hdr.dest_addr[2], (uchar)ep->eth_hdr.dest_addr[3],
                   (uchar)ep->eth_hdr.dest_addr[4], (uchar)ep->eth_hdr.dest_addr[5]);
            printf("\r\n HOST: Received MAC : ");
            printf("%02x:%02x:%02x:%02x:%02x:%02x\n",
                   recv_buffer[6], recv_buffer[7],
                   recv_buffer[8], recv_buffer[9],
                   recv_buffer[10], recv_buffer[11]);
        //}
        return (FAILED);
    }
 ...
 **********************************************************************
 */
static int wait_result_packet (int slot, uint16_t retry, int disp)
{
    int ret_val = PASSED;
    int count = retry;
 ...
 **********************************************************************
 */
static int send_tx_pkt (int slot, int size)
{
    ngio_priv_t *ep;
    eth_tx_pkt_t tx_pkt;
    ...
    assert(arkenstone_iface);

    ep = (ngio_priv_t *)arkenstone_iface->priv;
     ...
     */
    printf("\n Arkenstone module MAC : %x:%x:%x:%x:%x:%x ",
           (uchar)ep->eth_hdr.dest_addr[0], (uchar)ep->eth_hdr.dest_addr[1],
           (uchar)ep->eth_hdr.dest_addr[2], (uchar)ep->eth_hdr.dest_addr[3],
           (uchar)ep->eth_hdr.dest_addr[4], (uchar)ep->eth_hdr.dest_addr[5]);
 ...
 **********************************************************************
 *  
 *  Function: arkenstone_check_kernel_up
 *
 *  Description: Unreset Arkenstone, dnld firmware and check if ready pin set.
 *
 *  Input: None
 *  
 *  Returns: PASSED if successful; 
 ...
 **********************************************************************
 */ 
static int arkenstone_check_kernel_up (void)
{
    n2g_i2c_if_t  *pca;
    arkenstone_ds_t *ep;
    int i2c_dev, i;
    ...
    if (arkenstone_tests_use_enet == 0) {  /* for uart mode */
        printf("\n Make sure GE Loopback mode is disabled.");
        disable_bp_ge_lpbk(0);
        disable_bp_ge_lpbk(1);
    }

    ep = (arkenstone_ds_t *)arkenstone_iface->priv;
    assert(ep);
    ...
    pca = (n2g_i2c_if_t *)arkenstone_iface->pca;
    i2c_dev = pca->i2c_dev;
    ...
    if (arkenstone_tests_use_enet == 0) {
 ...
 **********************************************************************
 *
 *  Function: arkenstone_build_pkt
 *
 *  Description: Prepare the ethernet pkt with test data
 *
 *  Input: pkt_type - 
 *         test_cmd - which test to run on arkenstone module 
 *
 *  Returns: PASSED if successful; 
 *           FAILED, otherwise
 *
 **********************************************************************
 */
static int arkenstone_build_cmd_pkt (int test_cmd, int indx)
{
    ipc_msg_t ipc_msg;

    ipc_msg->msg_id = arkenstone_tests_tbl[indx].msg_id;
    ipc_msg->cmd_id = arkenstone_tests_tbl[indx].cmd_id;
    ipc_msg->msg_len = arkenstone_tests_tbl[indx].msg_len;
    ipc_msg->cmd_rslt = NO_RSLT;

    switch (test_cmd) {
    case arkenstone_INT_REG_TEST:
    case arkenstone_USB_TEST:
    case Arkenstone_SPI_FLASH_TEST:
    case Arkenstone_PHY_ADDR_INFO:         /* Done cmdline */
        break;
    case Arkenstone_MODEM_ENUM_TEST:
 ...
 **********************************************************************
 *
 *  Function: arkenstone_select_test
 *
 ...
 **********************************************************************
 */
static int arkenstone_run_test (uint test_cmd, int wait_time)
{
    int      indx, ret_val = PASSED;
    uchar    print_str[IPC_MSG_STR], *cmd;

    indx = find_index_arkenstone_test_tbl(test_cmd);
    if (indx == -1) {
        printf("\n%s() : Cannot find test cmd %d in arkenstone_tests_tbl",
               __FUNCTION__, ipc_cmd);
        return (FAILED);
    }

    sprintf(print_str, "%s", arkenstone_tests_tbl[indx].desc);
    testname_disp(print_str);

    /* Check if Arkenstone Kernel code is up and running */
    if (arkenstone_check_kernel_up() == FALSE) {
        printf("\n %s() : Arkenstone Kernel is not up", __FUNCTION__);
        cterr('f', 0, "%s(): \n Arkenstone READY PIN not set. Test : Please check "
              "if \"/firmware/arkenstone_fw_diag.img\" exists.\n", __FUNCTION__);
        return (FAILED);
    }
    ...
    ret_val = arkenstone_build_cmd_pkt(test_cmd, indx);

    if ((retval = arkenstone_send_tx_pkt(slot, size)) != PASSED) {
            cterr('f', 0,"%s(): arkenstone_send_command_packet() returned tx "
...
//#endif
    ...
    arkenstone_cleanup_ge_env(slot);
 ...
 **********************************************************************
 *
 *  Function: arkenstone_utils
 *
 *  Description: Arkenstone Utitlities menu
 *
 *  Input: None 
 *
 *  Returns: PASSED
 ...
 **********************************************************************
 */
static int arkenstone_utils (void)
{

    assert(arkenstone_iface);
    
    sprintf(arkenstoneutiltitle, "Arkenstone NIM%d Host Interface Utilities Menu", 
            arkenstone_iface->slot);
    build_primary_submenu(arkenstone_utils_submenu_table,
                          Arkenstone_UTILS_SUBMENU_TABLE_SZ,
                          arkenstoneutiltitle, &arkenstone_util_submenup);

    build_secondary_submenu(arkenstone_utils_submenu_table,
                            Arkenstone_UTILS_SUBMENU_TABLE_SZ,
                            arkenstone_utils_secondary_items);

    menu(arkenstone_util_submenup, arkenstone_utils_secondary_items, '\0');
 ...
 **********************************************************************
 *
 *  Function: arkenstone_mod_utils
 *
 *  Description: Arkenstone Module Utitlities menu
 *
 *  Input: None 
 *
 *  Returns: PASSED
 ...
 **********************************************************************
 */ 
static int arkenstone_mod_utils (void)
{   
    assert(arkenstone_iface);

    sprintf(arkenstonemodutiltitle, "Arkenstone Slot %d Module Utilities Menu",
            arkenstone_iface->slot);
    build_primary_submenu(arkenstone_mod_utils_submenu_table,
                          Arkenstone_MOD_UTILS_SUBMENU_TABLE_SZ,
                          arkenstonemodutiltitle, &arkenstone_mod_util_submenup);

    build_secondary_submenu(arkenstone_mod_utils_submenu_table,
                            Arkenstone_MOD_UTILS_SUBMENU_TABLE_SZ,
                            arkenstone_mod_utils_secondary_items);

    menu(arkenstone_mod_util_submenup, arkenstone_mod_utils_secondary_items, '\0');
 ...
 **********************************************************************
 */
static void arkenstone_cleanup (void)
{
    assert(arkenstone_iface);

    if (arkenstone_saved_diag_exec) {
        pre_diag_exec = arkenstone_saved_diag_exec;
        arkenstone_saved_diag_exec = NULL;
    }
}

/*
 *************************************************************************
 * Function: arkenstone_uart_test
 *
 * Test the UART connection from the host to Arkenstone.
 ...
 *************************************************************************
 */
int arkenstone_uart_test (void)
{
    const int maxlen = 28;
    char test_if[maxlen];;
    int rv; 

    /* 'n\n' for trigger Arkenstone side diag sub-item,
     * which will invoke 'uname -a'.
     */
    assert(arkenstone_iface);

#ifdef TACHI
    diag_uart_to_nim_cnnt(arkenstone_iface->slot); 
    snprintf(test_if, maxlen-1, UART_TTYS2_DEV);
#else
    snprintf(test_if, maxlen-1, "/dev/ttyDASH%d",
             arkenstone_iface->uart_ctrl);
#endif

    prpass(testpass, "Arkenstone UART ");

    rv = uart_msg_exh_test(test_if, "\n", "Submenuitem>", TRIG_DIAG_M); 
    if (rv == FAILED) {
        cterr('f',0,"Arkenstone UART test failed\n");
    }
...
/*
 *************************************************************************
 * Function: arkenstone_iface_test
 *
 * Test entry for Arkenstone interface test.
 ...
 *************************************************************************
 */
int arkenstone_iface_test (void)
{
    int wait_time = 3000;
    int i;
    uchar data;
    ...
    printf("\nWait for Arkenstone module to boot up.");
    fflush(stdout);
    arkenstone_iface->unreset(arkenstone_iface);
    /* poll for Primary Interface Ready pin (GPIO pin 3) which is set 
       by Arkenstone module side when the diag menu is up. */
    for (i = 0; i < wait_time; i++) {

        if (io_port_8bit_i2c_read(arkenstone_iface->pca, 0x0, &data, TRUE) == FAILED) {
    ...
    printf("Interface ready bit set by Arkenstone\n");
    fflush(stdout);
    printf("Modem needs 10-15 sec to come up\n");
    fflush(stdout);
    msleep(15000); /* Arkenstone module waits for 10-15 sec for modem to come up */
    printf("Arkenstone Menu is up ... Ready for Uart test\n");
    fflush(stdout);
//    msleep(5000); /* Arkenstone module waits for 10-15 sec for modem to come up */
    printf("Arkenstone Menu is up ... Ready for Uart test\n");
    fflush(stdout);

    /* Testing UART and GE0 interfaces */
    if (arkenstone_uart_test()) {
        return (FAILED);
    }
 ...
 *
 * Arkenstone data structure store the host and arkenstone module MAC addr
 *
 * Input : None
 *
 * Output: PASSED or FAILED
...
static int get_host_module_mac (void)
{
    ...
    get_ngio_mac_addr(arkenstone_iface->slot, arkenstone_iface->mod_type,  &print_mac[0]);

    for (i=0; i<6; i++) {
        ep->eth_hdr.dest_addr[i] = (uint8_t) print_mac[i];
    }
    printf("\r\n HOST: Arkenstone module MAC : %02x:%02x:%02x:%02x:%02x:%02x ",
           print_mac[0], print_mac[1],
           print_mac[2], print_mac[3],
           print_mac[4], print_mac[5]);

#ifdef TACHI
    system_mac_addr_get(ETH1_MAC1, (mac_addr_t *)print_mac);
#elif TABEIL
    system_mac_addr_get(TABEI_ETH_BP, (mac_addr_t *)print_mac);
#elif NANOOK
    system_mac_addr_get(inface_lan0p0, (mac_addr_t *)print_mac);
#else
    get_host_mac_addr(arkenstone_iface->slot, &print_mac[0]);
#endif

    for (i=0; i<6; i++) {
        ep->eth_hdr.src_addr[i] = (uint8_t) print_mac[i];
    }
    printf("\r\n HOST: Host MAC : %02x:%02x:%02x:%02x:%02x:%02x ",
           print_mac[0], print_mac[1],
           print_mac[2], print_mac[3],
           print_mac[4], print_mac[5]);
    fflush(stdout);
 ...
 *
 * Arkenstone data structure init
 *
 * Input : None
 *
 * Output: PASSED or FAILED
...
{
    ngio_priv_t *ep;
    //arkenstone_priv_ds *arkenstone_ds;
    int slot;

    slot  = arkenstone_iface->slot;
    arkenstone_iface->priv = (void *) &ngio_priv_ds[slot];
    ep = (ngio_priv_t *) arkenstone_iface->priv;
    ep->ngiop = (struct ngio_intf_t *) arkenstone_iface;

    get_pid(arkenstone_iface->cookie, ep->pid);
    ...
    //arkenstone_ds = ep->dev_priv;

    //arkenstone_ds->modem_id[0] = 0;
    //arkenstone_ds->uboot_info = 0;
    //arkenstone_ds->modem_info[0] = 0;
    return (PASSED);
}

#if 0
static int arkenstone_modem_test (void)
{
    return (PASSED);

}

static int arkenstone_emmcflash_test (void)
{
    return (PASSED);

}

static int arkenstone_bootflash_test (void)
{
    return (PASSED);

}

static int arkenstone_soc_test (void)
{
    return (PASSED);

}

static int arkenstone_mem_short_test (void)
{
    return (PASSED);

}

static int arkenstone_led_test (void)
{
    return (PASSED);

}

static int arkenstone_cpld_test (void)
{
...
#endif
#if 0
static void * start_picocom (void *dummy)
{
...
/**********************************************************************
 *
 * Function: arkenstone_test
 *  
 * This function invokes the Arkenstone module specific tests
 *
 * Input : None
 *
 * Output: PASSED or FAILED
 *  
 ***********************************************************************/
static int arkenstone_test (int menu_display)
{
    pid_t pid;
    const int maxlen = 128;
    char cmd[maxlen];
    ...
    if (menu_display == 0)
        arkenstone_tests_use_enet = 1;
    else
        arkenstone_tests_use_enet = 0;

#if DEBUG_HOST_MOD_COMM
    printf("\n arkenstone_tests_use_enet = %d", arkenstone_tests_use_enet);
    fflush(stdout);
#endif
...
#else
    snprintf(cmd, maxlen-1, "/dev/ttyDASH%d", arkenstone_iface->uart_ctrl);
#endif  
    if (is_goldbeach()) { 
        for (ix = 0; ix < RETRY_TIME; ix++) {
            ...
            printf("Aarkenstone power cycle");
            if (arkenstone_pwr_off()) {
                cterr('f', 0, "Failed to Power Off the Aarkenstone NGWIC");
                return(FAILED);
            }
            ...
            do {
                status = utah_port_is_linkup(arkenstone_iface->slot + 1);
                if (diagflag_xram & D_DEBUG_OPTIONS) {
                    printf("\neth%d status  = %x", (arkenstone_iface->slot) +1, 
        		            status);
                    fflush(stdout);
                }
            ...
            msleep(DELAY_1_SEC);
            if (arkenstone_pwr_on()) {
                cterr('f', 0, "Failed to Power On the Aarkenstone NGWIC");
                return(FAILED);
            }
            ...
            prpass(testpass, "\r HOST: Waiting for Arkenstone Interface ready ... ");
            fflush(stdout);
            if (intf_ready(arkenstone_iface) == PASSED) {
                break;
            } else {
                printf("\narkenstone_iface Primary Interface Not Ready\n");
            }
        }
    }
    ...
    pid = fork();
    if (pid < 0) {
        perror("Cannot fork in arkenstone_test()");
        return (FAILED);
    } else if(pid == 0) { /* child */
    ...
    prpass(testpass, "\r HOST: Waiting for Arkenstone Interface ready ... ");
    if (intf_ready(arkenstone_iface) == FAILED) {
        cterr('f', 0, "HOST: %s(): Arkenstone Primary Interface Not Ready ", __FUNCTION__);
        retval = FAILED;
    } 
    printf("\r\n HOST: Arkenstone Primary Interface ready in expander GPIO");

    sleep(3); /* module will wait 15 secs for host packet */

    if (arkenstone_iface->test_type == IFACE_TEST) {
        kill(pid, SIGTERM);
        wait(NULL);
        return(retval);
    }
    ...
    /* Indicate to Arkenstone module that host is ready and sends information
       for the test environment - continuous run, menu display etc */

    if (arkenstone_ready_test(arkenstone_iface->slot) == FAILED) {
        printf("\r\n HOST:%s(): FAILED show Arkenstone submenu ", __FUNCTION__);
        retval = 3; /* kill the shell and start picocom */
    } else {
        if ((retval = arkenstone_rslt_test(arkenstone_iface->slot)) == FAILED) {
            printf("\r\n HOST: Did not receive Arkenstone tests result pkt");
            kill(pid, SIGTERM);
            wait(NULL);
            return (retval);
        }
    ...
    if ((retval == 2) || (retval == 3)) { /* the tests results failed */
        //cterr('f', 0, "Arkenstone Tests failed");
 ...
 *
 ***********************************************************************/
static int arkenstone_main_test (void)
{
    int  retval = PASSED;
    ...
    if (retval == PASSED) {
        retval = arkenstone_test(0);  /* 0:menu_display is off run all tests on Arkenstone */
    }
 ...
 **********************************************************************
 *
 *  Function: arkenstone_build_command_packet
 *  
 *  Description: arkenstone_build test command 
 ...
 *       field
 *    3. size is sizeof(arkenstone_cmd_pkt_t)
 *  
 ...
 **********************************************************************
 */ 
void arkenstone_build_command_packet (int slot, uint16_t select_test, uint32_t param1,
                               uint32_t param2, uint8_t mod_type, int *size)
{   
    ngio_priv_t *ep;
    arkenstone_cmd_pkt_t *cmd_packet_p;
    int i;
    ...
    *size = sizeof(arkenstone_cmd_pkt_t);
    cmd_packet_p = (arkenstone_cmd_pkt_t *)&(ep->tx_pak[0]);
    ...
    cmd_packet_p->cmd_hdr.data_len = (sizeof(arkenstone_cmd_pkt_t));
...
unsigned long warncount = 0;
    ...
    if (select_test == SELECT_READY) {
        cmd_packet_p->cmd_info.location = arkenstone_tests_use_enet;
        cmd_packet_p->cmd_info.errorcount = errcount;
        cmd_packet_p->cmd_info.testcounter = testpass;
        cmd_packet_p->cmd_info.faults = warncount;
        cmd_packet_p->cmd_info.actual = err_accum;
...
/*
 ***********************************************************************
 * Name: arkenstone_rslt_test (common)
 *
 * Description:
 ...
 ***********************************************************************
 */
static int arkenstone_rslt_test (int slot) 
{
    struct ngio_intf_t *iface;
    ngio_priv_t *ep;
    fe_packet_t   *recv_packet_p;
    arkenstone_cmd_pkt_t *ready_msg_p;
    arkenstone_cmd_pkt_t ready_msg;
    int retval = PASSED;

    printf("\r\n HOST: Wait for Result Pkt From Arkenstone ");

    ep = &ngio_priv_ds[slot];
    iface = ep->ngiop;
    assert(iface);
    
    ep = arkenstone_iface->priv;
    printf("\r\n HOST: Arkenstone module MAC : %02x:%02x:%02x:%02x:%02x:%02x ",
           (uchar)ep->eth_hdr.dest_addr[0], (uchar)ep->eth_hdr.dest_addr[1],
           (uchar)ep->eth_hdr.dest_addr[2], (uchar)ep->eth_hdr.dest_addr[3],
           (uchar)ep->eth_hdr.dest_addr[4], (uchar)ep->eth_hdr.dest_addr[5]);
...
//dismem((uchar *)ready_msg_p, 0x50, (ulong) ready_msg_p, 0x4);
    ...
    if (arkenstone_cleanup_ge_env(slot) == FAILED) {
        cterr('f', 0, "HOST: %s(): arkenstone_cleanup_ge_env() failed", __FUNCTION__);
        return (FAILED);
    }

    if (/*(ready_msg_p->cmd_hdr.src_id !=
        ((arkenstone_iface->slot << 24) | (arkenstone_iface->id) << 16)) || */
        (ready_msg_p->cmd_info.param3 != arkenstone_iface->mod_type) ||
        ...
        cterr('f', 0, "HOST: %s(): Did not receive test result from correct NIM "
              ...
              " 0x%x", __FUNCTION__, 
              ((arkenstone_iface->slot << 24) | (arkenstone_iface->id) << 16),
              ready_msg_p->cmd_hdr.src_id, arkenstone_iface->mod_type, 
              ready_msg_p->cmd_info.param3, ready_msg_p->cmd_hdr.dest_id, HOST_ID);
        return (FAILED);
    } else {
        printf("\r\n HOST: Received Test Result from Arkenstone Armada ");
    }
/*    errcount = ready_msg_p->cmd_info.actual;
    err_accum += errcount;*/
    if (ready_msg_p->cmd_info.result == RESULT_FAILED) {
        cterr('f', 0, "Module tests failed");
        return (2);
    }
    return retval;
...
/* Building packet to be sent to Arkenstone module restrictions :-

 cmd_hdr.src_id : should be equal to HOST_ID (0xArkenstone)
 cmd_info.select: tells what type of packet is this :- SELECT_READY etc
                  should be equal to the type expected by Arkenstone module
   ...
   For non READY packets following is checked by the Arkenstone module 
        cmd_pkt.cmd_hdr.dest_id != nim_slot || cmd_pkt.cmd_info.param3 != mod_type
   Arkenstone module sends the packets to host with the following information:
 ...
 **********************************************************************
 *
 * Function: arkenstone_ready_test.
 *
 ...
 **********************************************************************
 */
static int arkenstone_ready_test (int slot) 
{
    struct ngio_intf_t *iface;
    ngio_priv_t *ep;
    fe_packet_t   *recv_packet_p;
    ...
    printf("\r\n HOST: Send Ready Info Packet to Arkenstone ");
    ...
    ep = arkenstone_iface->priv;
#ifdef DEBUG
    printf("\r\n HOST: Arkenstone module MAC : 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x ",
    ...
    arkenstone_build_command_packet(slot, SELECT_READY, (NVRAM)->diagflag, 
                             arkenstone_iface->id, arkenstone_iface->mod_type, &size);
    ...
    if (ready_msg_p->cmd_hdr.src_id == 
        ((arkenstone_iface->slot << 24) | (arkenstone_iface->id) << 16))
        printf("\r\n HOST: Received Ready Response from Arkenstone Armada ");
    return retval;
 ...
 **********************************************************************
 *
 * Function: arkenstone_nim_test().
 *
 * Description: This function is the entry point for Arkenstone NGWIC test .
 *
 * Input:  nim - pointer to ngio_intf_t struct
 *
 * Output: PASSED/FAILED.
 ...
 **********************************************************************
 */
int arkenstone_nim_test (void *nim)
{ 
    struct ngio_intf_t *iface;
    int slot;
    int ret_val = PASSED;
    ...
    if (arkenstone_iface->mod_type == SM_DAUGHTER_CARD) {
        printf("\nArkenstone test, board_id %#x, SM slot %d\n", board_id, slot);
        sprintf(nimsubmenutitle, "Arkenstone SM%d DC Host Main Menu", slot);
        testname("SM DC Slot%d Arkenstone", slot);
    } else {
        printf("\nArkenstone test, board_id %#x, NGWIC slot %d\n", board_id, slot);
        sprintf(nimsubmenutitle, "Arkenstone NIM%d Host Main Menu", slot);
        testname("Slot%d Arkenstone NGWIC ", slot);

        /* Curie 2RU: Force eth port to 1Gb/s for 10G MAC BCM57412 which has
         * not the ability of auto-negotiation between 1G and 10G */
        module_type = arkenstone_iface->mod_type;
    ...
    if (tftp_get(0, "nim_cwan_aks_fw.img", 0, "/firmware/nim_cwan_aks_fw.img", 1) < 0) {
    ...
    data = 0;
    if (io_port_8bit_i2c_write(arkenstone_iface->pca, POLARITY_INV_REG, &data) 
        == FAILED) {
    ...
    if (io_port_8bit_i2c_write(arkenstone_iface->pca, CONFIGURATION_REG, &data) 
        == FAILED) {
        cterr('f', 0, "%s(): Unable to write PCA9557 config register \n", 
              __FUNCTION__);
        return (FAILED);
    ...
    if (io_port_8bit_i2c_write(arkenstone_iface->pca, OUTPUT_PORT_REG, &data) 
        == FAILED) {
        cterr('f', 0, "%s(): Unable to write PCA9557 Output register \n", 
              __FUNCTION__);
        return (FAILED);
...
//    }
    ...
    /* The firmware name of both F35 and Arkenstone are nim_cwan_fw.img. 
       Please refer the $FILE_NAME env in uboot.
       Therefore we need to rename Arkenstone firmware to nim_cwan_fw.img.
       In case that F35 and Arkenstone in the same platform, 
       we save nim_cwan_fw.img first, it will be restored after the test. */
    if (file_exist("/firmware/nim_cwan_fw.img", &size)) {
        system("cp /firmware/nim_cwan_fw.img /firmware/nim_cwan_f35_fw.img");
        system("cp /firmware/nim_cwan_aks_fw.img /firmware/nim_cwan_fw.img");
    } else {
        system("mv /firmware/nim_cwan_aks_fw.img /firmware/nim_cwan_fw.img");
    }

    if (iface->test_type == IFACE_TEST) {
        ret_val = arkenstone_iface_test();

        /* Restore F35 firmware image if necessary. */
        if (file_exist("/firmware/nim_cwan_f35_fw.img", &size)) {
            system("mv /firmware/nim_cwan_f35_fw.img /firmware/nim_cwan_fw.img");
        } else {
            system("mv /firmware/nim_cwan_fw.img /firmware/nim_cwan_aks_fw.img");
        }
     ...
     */
    arkenstone_saved_diag_exec = pre_diag_exec;
    pre_diag_exec = NULL;

    build_primary_submenu(arkenstone_mainmenu_tbl, Arkenstone_MAINMENU_TBL_SIZE, "Arkenstone Menu",
			  &maindiagp);
    build_secondary_submenu(arkenstone_mainmenu_tbl, Arkenstone_MAINMENU_TBL_SIZE,
    ...
    if (iface->menu_display) {
        ...
        arkenstone_tests_use_enet = 1;
        arkenstone_main_test();
    }

    arkenstone_cleanup();

    if (module_type != SM_DAUGHTER_CARD) {
...
static int ltc4215_register_test (void)
{
    ...
    ret = oir_ltc4215_register_test(arkenstone_iface->oir);
    if (ret == FAILED)
...
static int ltc4215_reg_write(void)
{
    ...
    oir_if = (void *)(arkenstone_iface->oir);
    assert(oir_if);
 ...
 **********************************************************************
 *  
 * Function: arkenstone_power_stat
 *
 ...
 **********************************************************************
 */
static int arkenstone_power_stat (void)
{
    void *oir_if;
    ...
    /* The sense resistor on Arkenstone is .015 ohms */
    i = (vdd / 15);  /* mv/mohms = A */
 ...
 **********************************************************************
 *
 * Function: arkenstone_pwr_off
 *
 * Description: This function power off Arkenstone NGWIC.
 *
 * Input :  None
 *
 * Output: PASSED/FAILED
 ...
 **********************************************************************
 */
static int arkenstone_pwr_off (void)
{
    void *oir_if;
    uint8_t data = 0;
    ...
    printf("\nPower Off the Arkenstone NGWIC.\n");
    if (is_goldbeach()) {
        /* disable power interrupt avoid system alert */
        ngiowic_disable_intr(arkenstone_iface->slot, NGIO_FLT_INTR);
    }
 ...
 **********************************************************************
 *
 * Function: arkenstone_power_off
 *
 * Description: This function is a wrapper to power off Arkenstone NGWIC.
 *
 * Input :  None
 *
 * Output: PASSED/FAILED
 ...
 **********************************************************************
 */
static int arkenstone_power_off (void)
{
    uint8_t ans;
	...
	printf("\n\n");
    if (ans != 'y' && ans != 'Y') {
        printf("\nPower Off ABORT! Arkenstone NGWIC Still Power On.\n\n");
        return (PASSED);
    }

    return (arkenstone_pwr_off());
}
 ...
 **********************************************************************
 *
 * Function: arkenstone_pwr_on
 *
 * Description: This function power on Arkenstone NGWIC.
 *
 * Input :	None
 *
 * Output: PASSED/FAILED
 ...
 **********************************************************************
 */
static int arkenstone_pwr_on (void)
{
    void *oir_if;
    uint8_t data = 0;
    ...
    printf("\nPower On the Arkenstone NGWIC.\n");

    /* turn on board power and take I2C out of reset */
    slot_i2c_unreset(arkenstone_iface, arkenstone_iface->slot, "WIC");
    ...
    printf("Waiting for Arkenstone NGWIC to Power-Up.\n");
    msleep(2000);
    ...
    arkenstone_iface->uart_on(arkenstone_iface);    

    /* take Arkenstone NGWIC out of reset */
    arkenstone_iface->unreset(arkenstone_iface);

    printf("Arkenstone NGWIC is powered up.\n");
 ...
 **********************************************************************
 */
static int arkenstone_pwr_cycle (void)
{
    uint8_t i;
//, ans;

    printf("\n");
    printf("Power Cycle the Arkenstone NGWIC");
    ...
    if (ans != 'y' && ans != 'Y') {
        printf("\nPower Cycle ABORT! "
                "Arkenstone is not Power Cycled.\n\n");
        return (PASSED);
    }
#endif

    if (arkenstone_pwr_off()) {
        cterr('f', 0, "Failed to Power Off the Arkenstone NGWIC");
        return(FAILED);
    }
    ...
    if (arkenstone_pwr_on()) {
        cterr('f', 0, "Failed to Power On the Arkenstone NGWIC");
        return(FAILED);
    }
 ...
 **********************************************************************
 *
 * Function: arkenstone_console_switch
 *
 * Description: use picocom linux app to display the Arkenstone console
 *
 * Input : None.
 *
 * Output: PASSED
 ...
 **********************************************************************
 */
static int arkenstone_console_switch (void)
{
    const int maxlen = 128;
    char cmd[maxlen];
 ...
 **********************************************************************
 */
static int arkenstone_o2_shell (void)
{
    int slot;
 ...
 **********************************************************************
 *
 * Function: arkenstone_o2_command
 *
 ...
 **********************************************************************
 */
static int arkenstone_o2_command (void)
{
    const int maxlen = 128;
    char cmd[maxlen];
...
static int enable_bp_ge_lpbk (void)
{
    ...
    if (is_goldbeach() || is_curie_1ru() || is_curie_2ru()) {
        ...
        printf("\nGoldbeach and Curie Didn't Support GESW\n");
        return (PASSED);
    }
#ifdef TACHI
    ge_port = tachi_get_ge_sw_port_num(arkenstone_iface->slot, TGT_DEV_NGWIC, 0);
#else
    ge_port = ovld_get_ge_sw_port_num(arkenstone_iface->slot, TGT_DEV_NGWIC, 0);    
#endif
...
/******** History ********
$Log: nim_arkenstone.c,v $
Revision 1.11  2020/01/09 01:01:51  jiajliu
Merge Curie 2RU to main trunk

Revision 1.10  2019/12/11 10:10:22  lucywang
Merged Nanook to main trunk

Revision 1.9  2019/10/17 02:16:15  kehuang2
Collapse Tabei-L into main trunk

Revision 1.8  2019/08/06 06:56:06  alpeng
merge curie, switzer and nightwatch to trunk

Revision 1.7.2.2  2018/12/27 08:17:48  alpeng
...
Revision 1.3  2017/03/30 08:21:05  hondwang
Tachi-L brach merge

Revision 1.2.2.3  2017/01/04 03:02:58  haohsu
Modify NIM on Tachi-l

Revision 1.2.2.2  2016/12/27 10:03:00  haohsu
Modify Arkenstone on Tachi-l

Revision 1.2.2.1  2016/12/22 01:00:02  haohsu
Add Arkenstone on Tachi-l

Revision 1.2  2016/10/16 12:28:12  iachang
Supported Goldbeach Platform.

Revision 1.1.4.4  2018/05/17 10:50:20  alpeng
 sync with trunk <trunk-051618>

Revision 1.1.4.3  2017/06/13 09:25:57  alpeng
fix edvt issue for racing condition

Revision 1.1.4.2  2017/04/05 06:40:23  leschen
Sync with <ng_diag-tag-032917>

Revision 1.1.4.1  2016/12/05 06:36:59  alpeng
...
Revision 1.1  2016/04/15 10:19:24  xiaoyizh
