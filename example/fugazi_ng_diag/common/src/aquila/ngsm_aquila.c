/* $Id: ngsm_aquila.c,v 1.4 2019/01/25 06:46:28 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/aquila/ngsm_aquila.c,v $
 *------------------------------------------------------------------------------
 *
 * ngsm_aquila.c: This file contains functions for NGSM Aquila
 *
...
static int aquila_power_off(void);
static int aquila_pwr_off(void);
static int aquila_pwr_on(void);
static int aquila_pwr_cycle(void);
static void aquila_check_bmc_ready(void);
static void (*wait_aquila_bmc_ready_func)(void) = aquila_check_bmc_ready;
static int aquila_utils(void);
static int aquila_ioe_reg_test(void);
static int aquila_o2_shell(void);
static int aquila_o2_command(void);
...
static void (*aquila_saved_diag_exec)(void) = NULL;
static char pca_buff0[256];
static void *oir_if;
static int aquila_set_rbcp_mac_add(void);

int aquila_rbcp_bmc_con_switch(void);
int aquila_rbcp_intel_con_switch(void);
int aquila_rbcp_lsi_con_switch(void);
void set_aquila_bp_loopback(int, int, int);
int aquila_ngsm_test(void *);

static void display_i2c_regs_cter(void);
static void display_i2c_regs(void);

long aquila_rbcp_picocom_switch (void);
long aquila_show_ioe_regs(char *);
long aquila_gesw_lpbk_set(void);

static speed_t slot1_uart = AQUILA_B9600;
static speed_t slot2_uart = AQUILA_B9600;
static speed_t slot3_uart = AQUILA_B9600;

int aquila_test_slot = 1;
ushort aquila_board_id;
boolean pca9557;
 ...
 *******************************************************************************
 */
aquila_ds_t aquila_iface[MAX_SM+1];
static aquila_ds_t *aquila_iface_p;
...
static struct ngio_intf_t *aquila_ngsm_iface;
 ...
 *=========================================
 */
static submenu_xtable_t aquila_utils_submenu_table[] = {
    {"Power off aquila SM",      (PFT)aquila_power_off,    0,   0,
     (type_t(*)())0, 0,         (type_t(*)())0,          0},
    {"Power on aquila SM",       (PFT)aquila_pwr_on,       0,   0,
     (type_t(*)())0, 0,         (type_t(*)())0,          0},
    {"Power cycle aquila SM",    (PFT)aquila_pwr_cycle,    0,   0,
     (type_t(*)())0, 0,         (type_t(*)())0,          0},
    ...
    {"IO Expander Reg. Write", (PFT)pca9557_reg_write_util,    0,   0,
     (type_t(*)())0, 0,         (type_t(*)())0,          0},  
    {"Escape to Shell (debugging only)", (PFT)aquila_o2_shell,    0,   0,
     (type_t(*)())0, 0,         (type_t(*)())0,          0},    
    {"Execute a Shell command (debugging only)",(PFT)aquila_o2_command,0,0,
      (type_t(*)())0, 0,        (type_t(*)())0,          0},
    {"Set Power Fault Debug Flag", (PFT)util_debug_o2_power_fault,0,0,
      (type_t(*)())0, 0,   (type_t(*)())0,          0},
    {"Picocom Console redirect Link", (PFT)aquila_rbcp_picocom_switch,0,0,
      (type_t(*)())0, 0,   (type_t(*)())0,          0},
    {"Platform GE Switch Line Loopback Setup", (PFT)aquila_gesw_lpbk_set,0,0,
      (type_t(*)())0, 0,   (type_t(*)())0,          0},
    { "LSI Console redirect Test", (PFT)aquila_rbcp_lsi_con_switch, 0, 0,
     (type_t(*)())0, 0,		 	(type_t(*)())0, 0 },
};

#define AQUILA_UTILS_SUBMENU_TABLE_SZ (sizeof(aquila_utils_submenu_table) / sizeof(submenu_xtable_t))

static mitem_t aquila_utils_primary_items[AQUILA_UTILS_SUBMENU_TABLE_SZ + 
                                        MAX_BASE_ITEMS];
static mitem_t aquila_utils_secondary_items[AQUILA_UTILS_SUBMENU_TABLE_SZ + 
                                          MAX_BASE_ITEMS];

char aquilautiltitle[50];

menuinfo_t aquila_util_submenu = {
    aquilautiltitle,
    0,                              /* mtparam added by init_empty_menu */
    (PFT)menu_show_dflags,          /* notes missing WICs in combos */
    0,                              /* use generic prompt */
    0,                              /* size (bumped by add_menu_item() */
    aquila_utils_primary_items,
};

menuinfo_t *aquila_util_submenup = &aquila_util_submenu;
...
static submenu_xtable_t main_menu_table[] = {
    {"Aquila Utilities",      (PFT)aquila_utils,        0,   0,
     (type_t(*)())0, 0,         (type_t(*)())aquila_utils,    0},
    {"LTC4215 OIR Test",            (PFT)oir_ltc4215_tests,         0,  MM_3,
     (type_t(*)())0, 0,         (PFT)oir_ltc4215_tests,             TRUE},
    {"I2C IO Expander Register Test",       (PFT)aquila_ioe_reg_test, 0,  MM_3,
     (type_t(*)())0, 0,         (type_t(*)())0,                     0}, 
    {"RBCP Tests",            (PFT)build_aquila_rbcp_menu, 0, 	MM_3,
     (type_t(*)())0, 0, 	(PFT)build_aquila_rbcp_menu, TRUE},
    { "BMC Console redirect Test",  (PFT)aquila_rbcp_bmc_con_switch, 0, 0,
     (type_t(*)())0, 0, 		(type_t(*)())0, 0 },
    { "INTEL Console redirect Test", (PFT)aquila_rbcp_intel_con_switch, 0, 0,
     (type_t(*)())0, 0,		 	(type_t(*)())0, 0 },
};
 ...
 *******************************************************************************
 */
static void aquila_cleanup (void)
{
    assert(aquila_ngsm_iface);

    if (aquila_saved_diag_exec) {
        pre_diag_exec = aquila_saved_diag_exec;
        aquila_saved_diag_exec = NULL;
    }
}
...
static void setup_uart (void)
{
    ...
    assert(aquila_ngsm_iface);
    slot = aquila_ngsm_iface->slot;

    snprintf(tty, maxlen-1, "/dev/ttyDASH%d", aquila_ngsm_iface->uart_ctrl);
    ...
    if (slot == AQUILA_NGSM_SLOT1) {
        new_baud = slot1_uart;
    } else if (slot == AQUILA_NGSM_SLOT2) {
        new_baud = slot2_uart;
    } else if (slot == AQUILA_NGSM_SLOT3) {
        new_baud = slot3_uart;
    } else {
    ...
    if (new_baud == AQUILA_B115200) {
        newtio.c_cflag |= B115200;
    } else if (new_baud == AQUILA_B9600) {
...
/**********************************************************************
 *
 *  Function: aquila_gesw_lpbk_set
 *
 *  Description: This function is the wrapper to enables or disable the line
 *		 loopback of the GE switch port connected to the aquila.
 *
 *  Input  : none
 *
...
long aquila_gesw_lpbk_set (void)
{
    ...
    set_aquila_bp_loopback(slot, port, enable);
...
/*******************************************************************************
 *
 * Function: aquila_iface_test
 *
 * Description: Test entry for aquila interface test.
...
/*******************************************************************************
 *
 * Function: aquila_ngsm_test().
 *
 * Description: This function is the main entrance for aquila NGSM test .
 *
 ...
 *******************************************************************************
 */
int aquila_ngsm_test (void *ngsm)
{ 
    int slot;
    ushort board_id = 0;
    int ret_val = PASSED;
    ...
    aquila_ngsm_iface = (struct ngio_intf_t *)ngsm;

    slot = aquila_ngsm_iface->slot;
    aquila_test_slot = aquila_ngsm_iface->slot;
    board_id = aquila_ngsm_iface->id;
    aquila_board_id=aquila_ngsm_iface->id;

    slot_get_bd_pid(aquila_ngsm_iface->cookie, board_pid, &num_byte);
    num_byte = 9; /* P1A2 MFG pads PID with 9 white spaces. */

    aquila_ngsm_iface->uart_on(ngsm); 
    
    printf ("\n%s, board_id %#x, slot %d.\n", board_pid, board_id, slot);

    testname ("Slot%d aquila NGSM ", slot);

    pca_init_i2c((void *)&pca_i2c[0]);
    pca_i2c[0].i2c_ctrl = aquila_ngsm_iface->i2c_ctrl;
    pca_i2c[0].i2c_dev = SM_I2C_ADDR_IO_PORT1;
    pca_i2c[0].buf   = pca_buff0;

    oir_if = (void *)(aquila_ngsm_iface->oir);

    /*
     * Initialize an instance of aquila data structure
     */
    aquila_iface_p = (aquila_ds_t *) &aquila_iface[slot];
    aquila_iface_p->board_id = board_id;
    aquila_iface_p->slot = slot;
    aquila_iface_p->uart = aquila_ngsm_iface->uart_ctrl;
    aquila_iface_p->aquila_ngsm_iface = (struct ngio_intf_t *)ngsm;

    /*
     * Initialize an instance of aquila data structure
     */
     
    aquila_ngsm_iface->unreset(ngsm);

    /* Set aquila boot up parameters */
    setup_uart();
     ...
     */
    aquila_saved_diag_exec = pre_diag_exec;
    pre_diag_exec = NULL;
    ...
    aquila_set_rbcp_mac_add();


    if (!(diagflag_xram & D_SET_OPTIONS)) {
        (*wait_aquila_bmc_ready_func)();
    }
    ...
    clear_aquila_regis_done_flag(aquila_test_slot);
    ...
    ret_val = PASS;
    if (aquila_ngsm_iface->menu_display == TRUE) {
    	menu(maindiagp, main_menu_secondary_items, '\0');
    } else {
    	if (aquila_ngsm_iface->test_type == IFACE_TEST) {
        	ret_val = aquila_iface_test();
    	} else {
            menu (maindiagp, main_menu_secondary_items, '\0');
    	}
    }
...
/*******************************************************************************
 *
 *  Function: aquila_utils
 *
 *  Description: Aquila Utilities menu
 *
 *  Input: None 
 *
 *  Returns: PASSED
...
static int aquila_utils (void)
{
    ...
    sprintf(aquilautiltitle, "Aquila Slot %d Utilities Menu", 
            aquila_ngsm_iface->slot);
    build_primary_submenu(aquila_utils_submenu_table,
                          AQUILA_UTILS_SUBMENU_TABLE_SZ,
                          aquilautiltitle, &aquila_util_submenup);

    build_secondary_submenu(aquila_utils_submenu_table,
                            AQUILA_UTILS_SUBMENU_TABLE_SZ,
                            aquila_utils_secondary_items);

    menu(aquila_util_submenup, aquila_utils_secondary_items, '\0');
...
static void display_i2c_regs (void)
{
  aquila_show_ioe_regs("\nI2C IO Expander config");
}

/*******************************************************************************
 *
 * Function   : aquila_ioe_reg_test
 *
 * Description: Wrapped function to do Aquila IO Expander register test.
 *
 * Inputs     : None
 *
...
static int aquila_ioe_reg_test (void)
{
    ...
    memcpy(ngsm_get_pid,(char*)&aquila_board_id,2);
    ...
    cterr_add_debug("Go to the Aquila Utilities menu",
 ...
 **********************************************************************
 */
long aquila_show_ioe_regs (char *title)
{
    uint8_t data;
    n2g_i2c_if_t *pca = &pca_i2c[0];
 ...
 *
 * Description: Entry function of Aquila OIR(LTC4215)
...
/*******************************************************************************
 *
 * Function: aquila_pwr_off
 *
 * Description: This function power off aquila NIM.
 *
 * Input :  None
 *
 * Output: PASSED/FAILED
...
/*******************************************************************************
 *
 * Function: aquila_power_off
 *
 * Description: This function is a wrapper to power off aquila NIM.
 *
 * Input :  None
 *
 * Output: PASSED/FAILED
...
static int aquila_power_off (void)
{
    ...
    if (ans != 'y' && ans != 'Y') {
        printf("\nPower Off ABORT! aquila NIM Still Power On.\n\n");
        return (PASSED);
    }

    return (aquila_pwr_off());
}

/*******************************************************************************
 *
 * Function: aquila_pwr_on
 *
 * Description: This function power on aquila NIM.
 *
 * Input :    None
 *
 * Output: PASSED/FAILED
...
static int aquila_pwr_on (void)
{
    ...
    printf("\nPower On the aquila NGSM.\n");

    assert(aquila_ngsm_iface);
    assert(oir_if);

    /* turn on board power and take I2C out of reset */
    slot_i2c_unreset(aquila_ngsm_iface, aquila_ngsm_iface->slot, "WIC");
    ...
    printf ("Waiting for aquila NGSM to Power-Up.\n");
    msleep (2000);
    ...
    aquila_ngsm_iface->uart_on(aquila_ngsm_iface);    

    /* take aquila NGSM out of reset */
    aquila_ngsm_iface->unreset(aquila_ngsm_iface);

    printf("Aquila NGSM is powered up.\n");
...
static int aquila_pwr_cycle (void)
{
    ...
    printf("\n");
    printf("Power Cycle the aquila NGSM");
    ...
    if (ans != 'y' && ans != 'Y') {
        printf("\nPower Cycle ABORT! "
                "aquila is not Power Cycled.\n\n");
        return (PASSED);
    }

    if (aquila_pwr_off()) {
        cterr('f', 0, "Failed to Power Off the aquila NGSM");
        return (FAILED);
    }
    ...
    if (aquila_pwr_on()) {
        cterr('f', 0, "Failed to Power On the Aquila NIM");
        return (FAILED);
    }
...
/*******************************************************************************
 *
 * Function   : aquila_rbcp_bmc_con_switch
 *
...
int aquila_rbcp_bmc_con_switch (void)
{
    ...
    aquila_rbcp_bmc_console_switch();
    ...
    snprintf(cmd, maxlen-1, "picocom -%s -d8 -pn -fn /dev/ttyDASH%d",
             new_baud ? "b9600" : "b115200", aquila_ngsm_iface->uart_ctrl);
#if DEBUG_UARTCOM
    printf("cmd=%s\n", cmd);
#endif
    system(cmd);
...
/*******************************************************************************
 *
 * Function   : aquila_rbcp_intel_con_switch
 *
...
int aquila_rbcp_intel_con_switch (void)
{
    ...
    aquila_rbcp_intel_console_switch();
...
/*******************************************************************************
 *
 * Function   : aquila_rbcp_lsi_con_switch
 *
 * Description: A utility to Switch uart console to LSI with RBCP.
 *
 * Inputs     : NONE
 *
 * Outputs    : PASSED/FAILED
...
int aquila_rbcp_lsi_con_switch (void)
{
    ...
    aquila_rbcp_lsi_console_switch();
...
static int pca9557_reg_dump_util (void)
{
    ...
    printf("\nAquila IO Expander(PCA9557) registers dump:\n");
    for (ctr = 0; ctr < total_reg_num; ctr++) {
...
/*************************************************************************
 * Function: aquila_rbcp_picocom_switch
 *
...
/**********************************************************************
 *
 *  Function: aquila_set_rbcp_mac_add
 *
 *  Get ge mac address
 *
 *  Input: none
...
/**********************************************************************
 *
 * Function: set_aquila_bp_loopback
 *
 * This function is used by Aquila diag to set the line loopback of the
...
void set_aquila_bp_loopback (int slot, int local_port, int mode)
{
    int port, onoff;
 ...
 **********************************************************************
 */
void aquila_check_bmc_ready (void)
{
    char j, rc = 0, recv_retry = 50;
    char buf[RBCP_MSG_BUF_SIZE];
    int pkt_len = 0;

    printf("Waiting for Aquila BMC to boot up\n");
    
    for (j = 0; j < recv_retry; j++) {
        printf(".");
        fflush(stdout);
    ...
    if (j >= recv_retry) {
        printf("Warning: Aquila BMC not ready, don't receive the RBCP packet\n");
    }
...
/*------------------------------------------------------------------------------
 * $Log: ngsm_aquila.c,v $
 * Revision 1.4  2019/01/25 06:46:28  alpeng
 ...
 *
 * Revision 1.1.2.3  2016/10/27 08:04:04  olin2
 * Remove set loopback
 *
 * Revision 1.1.2.2  2016/08/04 03:03:14  olin2
 * Init board PID value
 *
