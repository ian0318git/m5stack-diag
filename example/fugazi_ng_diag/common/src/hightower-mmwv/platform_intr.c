/* $Id: platform_intr.c,v 1.2 2021/06/02 02:56:21 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/hightower-mmwv/platform_intr.c,v $
 *********************************************************************
 *
 * platform_intr.c -
 *
...
struct hr_intr {
    char   name[64]              ;   //Uniq name of intr tree
...
{
    //Remove from hierachy, free self and all siblings
    //TODO
    return 0;
}
...
#if 0
static int _intr_test_get(struct hr_intr *intr)
{
    return (!intr) ? -(__LINE__) : intr->status(intr->pdata);
}

static int _intr_test_trg(struct hr_intr *intr)
{
    return (!intr) ? -(__LINE__) : intr->trigger(intr->pdata);
}

static int _intr_test_enb(struct hr_intr *intr)
{
    return (!intr) ? -(__LINE__) : intr->enable(intr->pdata);
}
#endif
