/* $Id: platform_emmc.c,v 1.1 2020/08/19 09:50:05 markzha Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/hightower-mmwv/platform_emmc.c,v $
 *-----------------------------------------------------------------------------
 * 
...
int emmc_pslc_fully_enable (int opt)
{
...

    {
        ...
        char buf[64];
        snprintf(buf, sizeof(buf) - 1, "/bin/bash /opt/emmc_init"); 
        system(buf);
    }
...
static int get_emmc_cid_info (highrise_emmc_info_t *emmc_info)
{
    ...
    //TODO: Not sure for Highrise eMMC vendor
