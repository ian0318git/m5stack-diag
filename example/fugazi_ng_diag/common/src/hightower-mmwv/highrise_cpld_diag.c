/* $Id: highrise_cpld_diag.c,v 1.2 2021/06/02 02:56:21 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/hightower-mmwv/highrise_cpld_diag.c,v $
 *------------------------------------------------------------------
...
long cpld_reg_read(void);
long cpld_margin_ctrl(int);
...
static mitem_t hr_cpld_util_submenu_table[] = {
    ...
    { "Read CPLD Register", 0, 0, cpld_reg_read,
      (long *)&zero, 0, (type_t(*)())0, 0 },
    ...
    { "WatchDog System Reset", 0, 0, hr_cpld_wdog_sys_reset,
      (long *)&zero, 0, (type_t(*)())0, 0 },
    { "Display voltage margin", 0, 0, cpld_margin_ctrl,
      (long *)&zero, 0, (type_t(*)())0, 0 },
    { "High voltage margin", 0, 0, cpld_margin_ctrl,
      (long *)&one, 0, (type_t(*)())0, 0 },
    { "Low voltage margin", 0, 0, cpld_margin_ctrl,
      (long *)&two, 0, (type_t(*)())0, 0 },
    { "Clear voltage margin", 0, 0, cpld_margin_ctrl,
      (long *)&three, 0, (type_t(*)())0, 0 },
#ifdef HR_CPLD_UPGRADE
...
long cpld_reg_read(void)
{
    ...
    addr = gethex_answer("Reg offset to read", 0, 0, 0xff);

    rc = hr_cpld_reg_read_32(addr, &value);

    if (rc == PASSED) {
        printf("CPLD register %lx = %lx\n", addr, value);
    } else {
        printf("Failed to read out the value of CPLD reg %lx\n", addr);
    }
...
/**********************************************************************
 *
 * Function: cpld_margin_ctrl
 *
 * To set and display CPLD voltage margin utility.
 *
 * Input : option - To display or set CPLD voltage margin 
 *
 * Output: PASSED/FAILED
 *
 **********************************************************************
 */
long cpld_margin_ctrl(int option)
{
    unsigned long value;
    unsigned long addr = 0x48; /* Currently margin ctrl register is 0x48 */
    int rc;

    /* To read out the margin value */
    rc = hr_cpld_reg_read_32(addr, &value);
    if (rc != PASSED) {
        printf("Failed to read out the value of CPLD reg %lx\n", addr);
        goto exit;
    ...
    /* 
     * CPLD offset 0x48 bit 8 and 9 are software-controlled voltage margin 
     * 2'b11 : no voltage margin
     * 2'b10 : high voltage margin
     * 2'b01 : low voltage margin
     * 2'b00 : no voltage margin
     */
    if (option == zero) {
        /* Show margin status */
        value &= 0x300;
        ...
        if (value == 0x0) {
            printf("<No> voltage margin\n");
        } else if (value == 0x300) {
            printf("<No> voltage margin\n");
        } else if (value == 0x200) {
            printf("<High> voltage margin\n");
        } else if (value == 0x100) {
            printf("<Low> voltage margin\n");
        } else {
            printf("Unsupported voltage margin status\n");
        }
    } else if (option == one) {
        /* Margin high */
        printf("To set <High> voltage margin\n");
        value &= 0xfffffcff;
        value |= 0x200;
        rc = hr_cpld_reg_write_32(addr, value);
    } else if (option == two) {
        /* Margin low */
        printf("To set <Low> voltage margin\n");
        value &= 0xfffffcff;
        value |= 0x100;
        rc = hr_cpld_reg_write_32(addr, value);
    } else if (option == three) {
        /* No margin */
        printf("To clear voltage margin\n");
        value &= 0xfffffcff;
        rc = hr_cpld_reg_write_32(addr, value);
    } else {
        printf("Not support this CPLD voltage option %x\n", option);
    }

    if (option != zero) {
        if (rc != PASSED) {
            printf("Failed to write the value to CPLD reg %lx\n", addr);
        }
    }
...
static long hr_cpld_fw_prog (void)
{
    ...
    char *arg[64] = {[0 ... sizeof(arg)/sizeof(char *) - 1] = NULL};
 ...
 *
 * Revision 1.1.4.4  2021/05/10 07:05:46  leschen
 * Support CPLD voltage margin utilities.
 *
 * Revision 1.1.4.3  2021/05/05 01:39:27  leschen
 * Support CPLD read reg utility
 *
 * Revision 1.1.4.2  2020/12/03 08:42:56  alpeng
 * apply -ddo_real_time_isp=1 flag while program cpld
 *
 * Revision 1.1.4.1  2020/09/10 05:53:52  alpeng
