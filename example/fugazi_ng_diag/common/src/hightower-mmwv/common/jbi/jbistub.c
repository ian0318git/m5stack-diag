...
#include <fcntl.h>
    ...
    #ifdef MODE_64_BIT
        #define POINTER_ALIGNMENT sizeof(QWORD)
    #else
...
#ifdef MODE_64_BIT
    QWORD peak_memory_usage = 0;
    QWORD peak_allocations = 0;
    QWORD n_allocations = 0;
#else
...
#if defined(USE_STATIC_MEMORY)
#ifdef MODE_64_BIT
    QWORD n_bytes_not_recovered = 0;
#else
    unsigned int n_bytes_not_recovered = 0;
#endif
#endif /* USE_STATIC_MEMORY */
#ifdef MODE_64_BIT
    const QWORD BEGIN_GUARD = 0x0123456789ABCDEF;
    const QWORD END_GUARD = 0xFEDCBA9876543210;
#else
...
int jbi_jtag_io(int tms, int tdi, int read_tdo)
{
    ...
    else
    {
     ...
     //-set_gpio_out(fd_tms, tms);
     //-set_gpio_out(fd_tdi, tdi);
     //-set_gpio_out(fd_tck, 0);
...
#if defined(USE_STATIC_MEMORY)
#ifdef MODE_64_BIT
        if ((((QWORD) ptr - (QWORD) static_memory_heap) + n_bytes_to_free) == (QWORD) n_bytes_allocated)
#else
        if ((((unsigned int) ptr - (unsigned int) static_memory_heap) + n_bytes_to_free) == (unsigned int) n_bytes_allocated)
#endif
        {
...
/************************************************************************/
#ifndef JBI_STANDALONE
int jbi_main(int argc, char **argv)
#else
    ...
    for (arg = 0; arg < argc; arg++)
    {
...
#endif
        {
            ...
            case 'M':                /* set memory size */
                if (sscanf(&argv[arg][2], "%d", &workspace_size) != 1)
                    error = TRUE;
...
#else
        printf("Initialize GPIO hardware\n");
        if (gpio_init_jtag() < 0)
            exit(1);
#endif
    }
            ...
            if (returned_length == sizeof(ULONG))
            {
                ...
                else
                {
                    fprintf(stderr,
                        ...
                        (unsigned int) PGDC_HDLC_NTDRIVER_VERSION);
                }
            }
            else
            {
