/* $Id: highrise_cpld_api.c,v 1.2 2021/06/02 02:56:21 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/hightower-mmwv/highrise_cpld_api.c,v $
 *******************************************************************************
...
long hr_cpld_intr_enable(unsigned long enb_msk, int enable)
{
    if (enable)
        return hr_cpld_reg_bit_set(HR_CPLD_INT_ENABLE, enb_msk, enb_msk); 
    else
        return hr_cpld_reg_bit_set(HR_CPLD_INT_ENABLE, enb_msk, 0);
}

long hr_cpld_intr_clear(unsigned long enb_msk)
{
    ...
    status |= enb_msk;
    ERR_RET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_INT_STATUS, status), 
        FAILED, "Failed to read HR_CPLD_INT_ENABLE\n");
    return 0;
}
...
int ht_cpld_show_antenna_info (void)
{
    unsigned long val = 0;
    ERR_RET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_MODEM_STATUS, &val), FAILED, "Read cpld failed.\n");
    ...
    if ((val & HR_CPLD_MODEM_STA_ANTE_NAME) && (val & HR_CPLD_MODEM_STA_ANTE_REST)) {
        printf("Both bit10 and bit11 are set, cannot detect Antenna ID\n");
    } else {
        printf( "%-18sAntenna is-%s \n",
                " ", val & HR_CPLD_MODEM_STA_ANTE_NAME  ? "North America" : "Rest of the World");
    }
