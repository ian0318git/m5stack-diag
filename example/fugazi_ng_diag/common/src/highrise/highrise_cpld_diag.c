/* $Id: highrise_cpld_diag.c,v 1.1 2020/08/19 09:49:35 markzha Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/highrise/highrise_cpld_diag.c,v $
 *------------------------------------------------------------------
...
static long hr_cpld_led_test(void);
...
static submenu_xtable_t hr_cpld_submenu_tbl[] = {
    ...
    { "CPLD LED Test", (type_t(*)())hr_cpld_led_test, FALSE,
...
#ifdef HR_CPLD_UPGRADE
    { "Speed Up CPLD Upgrade From CPLD Utility", 0, 0,
        speedup_cpld_upgrade_from_cpld,
      (long *)&zero, MF_HIDDEN_EXE, (type_t(*)())0, 0 },
#ifdef SLOW_UPGRADE
    { "CPLD Upgrade From CPLD Utility", 0, 0,
        cpld_upgrade_from_cpld,
      (long *)&zero, MF_HIDDEN_EXE, (type_t(*)())0, 0 },
#endif
    { "CPLD Upgrade From IO Expander Utility", 0, 0,
        cpld_upgrade_from_io_expander,
      (long *)&zero, MF_HIDDEN_EXE, (type_t(*)())0, 0 },
#endif
};
...
static long hr_cpld_reg_test (void)
{
    ...
    //TODO: handle special 'diag' reg in cpld
...
static long hr_cpld_led_test (void)
{
    ...
    switch(board_id) {
    case HR_CPLD_BOARD_HIGHTOWER     :
    case HR_CPLD_BOARD_HIGHRISE_5G   :
    case HR_CPLD_BOARD_HIGHRISE_CAT18:
        break;

    default:
        cterr('f', 0, "%s failed, unidentified board.", tname);
        return FAILED;
    }
...
#define _PWR_INTR_REG_DUMP(REG) do {                 \
    unsigned long v___ = 0;                          \
    if (PASSED == hr_cpld_reg_read_32(REG, &v___)) { \
        printf("%s val:%#lx\n", #REG, v___);         \
    }                                                \
} while(0)
...
#define _PWR_INTR_CLR_ALL(TAG) do {                                          \
    ...
    printf("%s\n", TAG);                                                     \
    v___ = 0;  /* wzclr */                                                   \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_ERROR_TEST, v___), \
        FAILED, "Write cpld failed.\n");                                     \
    v___ = ~0; /* woclr */                                                   \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_PWR_STATUS, v___), \
        FAILED, "Read cpld failed.\n");                                      \
    v___ = 0;  /* wzclr */                                                   \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_INT_ENABLE, v___), \
        FAILED, "Write cpld failed.\n");                                     \
    v___ = ~0; /* woclr */                                                   \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_INT_STATUS, v___), \
        FAILED, "Write cpld failed.\n");                                     \
}while(0)
...
#define _PWR_INTR_ST_SHOW() do {                                                 \
    ...
    char *q___ = NULL;                                                           \
    unsigned long v___ = 0;                                                      \
    TERR_URET_COND(PASSED != hr_cpld_get_sys_info(buf, sizeof(buf) - 1),         \
        FAILED, "Get current powr status failed.\n");                            \
    if ((p___ = strstr(buf, "Power Status")) && (q___ = strstr(p___, "\n\n"))) { \
        *q___ = 0;                                                               \
        printf("Current %s\n", p___);                                            \
    }                                                                            \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_ERROR_TEST, &v___),     \
        FAILED, "Read cpld failed.\n");                                          \
...
#define _PWR_INTR_ENB(ENB, REG, MSK) do {                                       \
    unsigned long v___ = 0;                                                     \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_##REG, &v___), FAILED, \
        "Read  Reg-%s failed.\n", "HR_CPLD_"#REG);                              \
    v___ = ENB ? (v___ | HR_CPLD_##MSK) : (v___ & ~HR_CPLD_##MSK);              \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_##REG, v___), FAILED, \
        "Write Reg-%s failed, val:%lx.\n", "HR_CPLD_"#REG, v___);               \
}while(0)

/* Enable only the bit by MSK, disbale all others */
#define _PWR_INTR_ENB_ONLY(ENB, REG, MSK) do {                                  \
    unsigned long v___ = ENB ? HR_CPLD_##MSK : 0;                               \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_##REG, v___), FAILED, \
        "write Reg-%s failed, val:%lx.\n", "HR_CPLD_"#REG, v___);               \
}while(0)
...
#define _PWR_INTR_CTC(ENB, IMSK, TREG, TMSK, SREG, SMSK, DREG, DMSK) do {          \
    ...
    printf("Trigger %s ...\n", "HR_CPLD_"#TMSK);                                   \
/* a, clear trig      */                                                           \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_##TREG, &v___), FAILED,   \
        "Read  Reg-%s failed.\n", "HR_CPLD_"#TREG);                                \
    v___ = (v___ & ~_PWR_INTR_TRIG_MSK_ALL); /* wzclr */                           \
    printf("Write reg %s val %lx\n", "HR_CPLD_"#TREG, v___);                       \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_##TREG, v___), FAILED,   \
        "write Reg-%s failed, val:%lx.\n", "HR_CPLD_"#TREG, v___);                 \
                                                                                   \
/* b, clear st sum    */                                                           \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_##SREG, &v___), FAILED,   \
        "Read  Reg-%s failed.\n", "HR_CPLD_"#SREG);                                \
    v___ = (~v___ | _PWR_INTR_SST_MSK_ALL); /* woclr */                            \
    printf("Write reg %s val %lx\n", "HR_CPLD_"#SREG, v___);                       \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_##SREG, v___), FAILED,   \
        "write Reg-%s failed, val:%lx.\n", "HR_CPLD_"#SREG, v___);                 \
                                                                                   \
/* c, clear st detail */                                                           \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_##DREG, &v___), FAILED,   \
        "Read  Reg-%s failed.\n", "HR_CPLD_"#DREG);                                \
    v___ = (~v___ | _PWR_INTR_DST_MSK_ALL); /* woclr */                            \
    printf("Write reg %s val %lx\n", "HR_CPLD_"#DREG, v___);                       \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_##DREG, v___), FAILED,   \
        "write Reg-%s failed, val:%lx.\n", "HR_CPLD_"#DREG, v___);                 \
                                                                                   \
/* d, trig */                                                                      \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_##TREG, &v___), FAILED,   \
        "Read  Reg-%s failed.\n", "HR_CPLD_"#TREG);                                \
    v___ = (v___ & ~_PWR_INTR_TRIG_MSK_ALL) | HR_CPLD_##TMSK; /* wzclr */          \
    printf("Write reg %s val %lx\n", "HR_CPLD_"#TREG, v___);                       \
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_##TREG, v___), FAILED,   \
        "write Reg-%s failed, val:%lx.\n", "HR_CPLD_"#TREG, v___);                 \
                                                                                   \
/* e, check st summary*/                                                           \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_##SREG, &v___), FAILED,   \
        "Read  Reg-%s failed.\n", "HR_CPLD_"#SREG);                                \
    TERR_URET_COND(!(v___ & (HR_CPLD_##SMSK)), FAILED,                             \
        "Intr %s failed to be triggered, reg val:%#lx\n",                          \
        "HR_CPLD_"#SMSK, v___);                                                    \
                                                                                   \
    TERR_URET_COND(((v___ & _PWR_INTR_SST_MSK_ALL) & (~(HR_CPLD_##SMSK))), FAILED, \
        "Intr other than %s occured when it/they should not, reg val:%#lx\n",      \
        "HR_CPLD_"#SMSK, v___);                                                    \
/* f, check st detail */                                                           \
    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_##DREG, &v___), FAILED,   \
        "Read  Reg-%s failed.\n", "HR_CPLD_"#DREG);                                \
    TERR_URET_COND(!(v___ & (HR_CPLD_##DMSK)), FAILED,                             \
        "Intr %s failed to be triggered, reg val:%#lx\n",                          \
        "HR_CPLD_"#DMSK, v___);                                                    \
                                                                                   \
    TERR_URET_COND(((v___ & _PWR_INTR_DST_MSK_ALL) & (~(HR_CPLD_##DMSK))), FAILED, \
        "Intr other than %s occured when it/they should not, reg val:%#lx\n",      \
        "HR_CPLD_"#DMSK, v___);                                                    \
                                                                                   \
    _PWR_INTR_ST_SHOW();                                                           \
...
/* g, check CPU side intr line status(MPP0) */                                     \
    TERR_URET_COND(0 != gpio_read(CPLD_CPU_INT_L, (int *)&v___), FAILED,           \
        "Failed to read CPLD_CPU_INT_L(MPP0).\n");                                 \
    printf("CPLD_CPU_INT_L(MPP0):%ld\n", v___);                                    \
                                                                                   \
    v___ = !v___; /*NOTE:Low-active??, need to confirm with HW */                  \
    TERR_URET_COND(!!(ENB) != !!v___, FAILED,                                      \
        "CPLD_CPU_INT_L(MPP0) is %s while it should %sbe.\n",                      \
        v___ ? "set" : "NOT set", ENB ? "" : "NOT ");                              \
                                                                                   \
...
int hr_cpld_pwr_st_chk_tst(void)
{
    char *tname     = "Power Status Check Test";
    char  buf[4096] = {[0 ... sizeof(buf) - 1] = 0};
    int   uret      = PASSED;
    unsigned int long val = 0;
    const int wtsec = 5;
    ...
    _PWR_INTR_ST_SHOW();
    _PWR_INTR_CLR_ALL("Clear Current Power Status ...\n");

    printf("Wait %d seconds before check ...\n", wtsec);
    sleep(5);

    TERR_URET_COND(PASSED != hr_cpld_reg_read_32(HR_CPLD_PWR_STATUS, &val),
        FAILED, "Read  Reg-%s failed.\n", "HR_CPLD_PWR_STATUS");

    _PWR_INTR_ST_SHOW();

    if ((val & (HR_CPLD_PWR_INT_3300S |
               HR_CPLD_PWR_INT_1800S |
               HR_CPLD_PWR_INT_3300  |
               HR_CPLD_PWR_INT_2500  |
               HR_CPLD_PWR_INT_1800  |
               ...
               HR_CPLD_PWR_INT_0900  |
               HR_CPLD_PWR_INT_0800)) ||
               ((val & HR_CPLD_PWR_POE_DET) ? !(val & HR_CPLD_PWR_POE_GOOD) : 0)) {
        TERR_URET_COND(1, FAILED, "%s failed, power fault occured.\n", tname);
    }
...
int hr_cpld_pwr_st_chk_util(void)
{
    char  buf[4096] = {[0 ... sizeof(buf) - 1] = 0};
    int   uret      = PASSED;
...
int hr_cpld_pwr_fault_clr_util(void)
{
    char  buf[4096] = {[0 ... sizeof(buf) - 1] = 0};
    int   uret      = PASSED;
    unsigned int long val = 0;

    printf("Power fault status before clearing:\n");
    _PWR_INTR_ST_SHOW();

    val = 0;  /* wzclr */
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_ERROR_TEST, val),
        FAILED, "Write cpld failed.\n");
    val = ~0; /* woclr */
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_PWR_STATUS, val),
        FAILED, "Read cpld failed.\n");
    val = ~0; /* woclr */
    TERR_URET_COND(PASSED != hr_cpld_reg_write_32(HR_CPLD_INT_STATUS, val),
        FAILED, "Write cpld failed.\n");

    printf("\nPower fault status after clearing:\n");
    _PWR_INTR_ST_SHOW();
...
static long hr_cpld_fw_prog (void)
{
    ...
    printf("Power cycle the board now?(Y/n):");
    fflush(stdout);
    chr = getchar();
    _DRAIN_STDIN();
    if (chr != 'n' && chr != 'N' ) {
        hr_cpld_wdog_sys_reset();
        /* should not be here if success */
        printf("Power cycle by watchdog failed, you should do it mannuely.\n");
    }
    return 0;
}
...
static long cpld_upgrade_firmware (int interface)
{
    ...
    if (hr_cpld_reg_read_lib(HR_CPLD_VERSION, &cpld_buf) == FAILED) {
    ...
    if (getc_answer(" Do you want to start CPLD upgrade (Y/N)",
        ...
        if (interface != SPEED_UP_UPGRADE_FROM_CPLD) {
            if (interface == UPGRADE_FROM_CPLD) {
                 ...
                 * JTAG_ON bit. */
                if (hr_cpld_jtag_ctrl(TRUE) == FAILED) {
                    return (FAILED);
                }
            }
            ...
            if (interface == UPGRADE_FROM_CPLD) {
                 ...
                 * JTAG_ON bit. */
                if (hr_cpld_jtag_ctrl(FALSE) == FAILED) {
                    return (FAILED);
                }
            }
        } else {
            ...
            if (hr_simply_program_cpld(pof_cpld_fw_array) == FAILED) {
                return (FAILED);
            }
