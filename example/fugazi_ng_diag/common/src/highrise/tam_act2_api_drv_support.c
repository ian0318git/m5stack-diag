/* $Id: tam_act2_api_drv_support.c,v 1.1 2020/08/19 09:49:36 markzha Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/highrise/tam_act2_api_drv_support.c,v $
 *------------------------------------------------------------------
 * 
...
int set_i2c_slave_addr(int fp, int addr)
{
    if (ioctl(fp, I2C_SLAVE, addr) < 0) {
        printf("Error: %s ioctl(I2C_SLAVE, 0x%x) failed, errno(%d)", __FUNCTION__, addr, errno);
        return (-1);
    }
                       ...
                       uint8_t * send_buffer, uint32_t length)
{
    ...
    if (NULL == n2g_i2c_act) {
        printf("Error: [%s]:%d, NULL pointer Error!\n", __FUNCTION__, __LINE__);
        return (-1);
    }

    fp = get_i2c_fd(n2g_i2c_act->i2c_bus_type);
    if (fp == -1) {
        printf("Error: [%s]:%d, failed to get fp for I2C_BUS[%d]",
