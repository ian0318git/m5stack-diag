/* $Id: platform_cpu.c,v 1.2 2021/01/25 09:21:15 markzha Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/highrise/platform_cpu.c,v $
 *------------------------------------------------------------------
 *
 * platform_cpu.c
...
int highrise_get_cpu_ondie_temp(const char *ap_cp, float *temp_c)
{
    ...
    uint32_t     regst = 0;
    uint32_t       wait_idx  = 0;
    const uint32_t wait_max  = 200000; //us
    const uint32_t wait_unit = 1500;   //us

    if (strcasecmp(ap_cp, "AP") == 0) {
        rega = CPU_AP_ONDIE_TEMP_REG_CTL0;
    }
    else if (strcasecmp(ap_cp, "CP") == 0) {
    ...
    set_bits32(&regc0, 0, CPU_ONDIE_TEMP_TSN_ENB_OFF  , CPU_ONDIE_TEMP_TSN_ENB_WIDTH  );     //disable & reset the sensor
    set_bits32(&regc0, 1, CPU_ONDIE_TEMP_TSN_RST_OFF  , CPU_ONDIE_TEMP_TSN_RST_WIDTH  );
    set_bits32(&regc0, 0, CPU_ONDIE_TEMP_TSN_START_OFF, CPU_ONDIE_TEMP_TSN_START_WIDTH);
    ERR_RET_COND(PASSED != highrise_mem_write32(rega, regc0), -(__LINE__), "Failed write reg:0x%08x\n", rega);
    usleep(10000);

    ERR_RET_COND(PASSED != highrise_mem_read32(rega, &regc0), -(__LINE__), "Failed read reg:0x%08x\n", rega);
    set_bits32(&regc0, 2, CPU_ONDIE_TEMP_TSN_OSR_OFF  , CPU_ONDIE_TEMP_TSN_OSR_WIDTH  );
    ...
    set_bits32(&regc0, 0, CPU_ONDIE_TEMP_TSN_START_OFF, CPU_ONDIE_TEMP_TSN_START_WIDTH);     //enable & release but stop it
    ERR_RET_COND(PASSED != highrise_mem_write32(rega, regc0), -(__LINE__), "Failed write reg:0x%08x\n", rega);
    usleep(10000);
    ...
    ERR_RET_COND(PASSED != highrise_mem_write32(rega + 4, regc1), -(__LINE__), "Failed write reg:0x%08x\n", rega + 4);

    ERR_RET_COND(PASSED != highrise_mem_read32(rega, &regc0), -(__LINE__), "Failed read reg:0x%08x\n", rega);
    set_bits32(&regc0, 1, CPU_ONDIE_TEMP_TSN_START_OFF, CPU_ONDIE_TEMP_TSN_START_WIDTH);     //start it
    ERR_RET_COND(PASSED != highrise_mem_write32(rega, regc0), -(__LINE__), "Failed write reg:0x%08x\n", rega);

    for(wait_idx = 0; wait_idx < wait_max; wait_idx += wait_unit) {
        ERR_RET_COND(PASSED != highrise_mem_read32(rega + 8, &regst), -(__LINE__), "Failed read reg:0x%08x\n", rega + 8);
        if (!((regst >> CPU_ONDIE_TEMP_TSN_VALID_OFF) & ((1 << CPU_ONDIE_TEMP_TSN_VALID_WIDTH) - 1))) {
            usleep(wait_unit);
            continue;
        }
    ...
    ERR_RET_COND(1, -(__LINE__), "Timeout to wait for CPU-%s Temperature\n", ap_cp);
}
...
int highrise_cpu_ondie_temp (int opt)
{
    ...
    #if 0
    ERR_RET_COND(highrise_get_cpu_ondie_temp("CP", &temp) < 0, FAILED, "FAiled.\n");
    ERR_RET_COND(PASSED != show_thermal(NULL, &temp, "CPU SB Temp"), FAILED, "Failed.\n");
    #endif
    return PASSED;
}
