/* $Id: i2c_api.c
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/highrise/i2c_api.c,v $
 *------------------------------------------------------------------
 * Filename: i2c_api.c
 *
...
//TODO: double check CPLD register TBL
...
/* CPU I2C controller 2 devices */
//static n2g_i2c_dev_t n2g_i2c2_m2 = { CPU_I2C2, MB_I2C_ADDR_M2, 1, 1, 0 };     /* M2 connector */
...
i2c_dev_read(n2g_i2c_dev_t * dev_p, ulong offset, uint8_t size, char *buf)
{
    ...
    if ((fd_i2c0 < 0) || (fd_i2c1 < 0) || (fd_i2c2 < 0)) {
        cterr('f', 0, "[%s]:%d: error fd_i2c0:%d, fd_i2c1:%d, fd_i2c2:%d \n",
                __FUNCTION__, __LINE__, fd_i2c0, fd_i2c1, fd_i2c2);
    }
