/* $Id: platform_rtc.c,v 1.1 2020/08/19 09:49:35 markzha Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/highrise/platform_rtc.c,v $
 *------------------------------------------------------------------
...
static submenu_xtable_t rtc_main_menu_table[] = {
    ...
    // OSC test is same to RTC init, don't need to run again
    { "Oscillator Stop Check",  (PFT)ds1337_osc_stop_wrapper,       0,
        MF_SHOW_ERRCOUNT | MF_CONTINUOUS | MF_HIDDEN_EXE,
        (type_t(*)())0,     0,
        (type_t(*)())0, 0 },
};
 ...
 **********************************************************************
 */
int build_rtc_menu (boolean show_menu)
{
 ...
 *************************************************************************
 */
int build_rtc_utils_menu (boolean option)
{
 ...
 * rtc_init is logic same to ds1337_osc_stop_wrapper()
 *
...
int rtc_init(int option)
{
    ...
    TERR_URET_COND(rc != PASSED, FAILED, "Device attach failed\n");
     ...
     */
    rc = clear_rtc_status_reg(&i2c_if);
    TERR_URET_COND(rc != PASSED, FAILED, "Write clear OSF bit failed\n");
    
    /* Call the common device driver init */
    rc = ds1337.base.dev_object_fvt->dev_init(dev);
    TERR_URET_COND(rc != PASSED, FAILED, "RTC initialization dev_init() failed!\n");

    rc = utility_display_rtc(TRUE);
    TERR_URET_COND(rc != PASSED, FAILED, "Display rtc failed\n");
...
ds1337_register_test_wrapper (int menu_item)
{
    ...
    TERR_URET_COND(rc != PASS, FAILED, "Registers test failed\n");
...
ds1337_time_validity_test_wrapper (int menu_item)
{
    ...
    TERR_URET_COND(rc != PASSED, FAILED, "RTC time_validity test failed\n");
 ...
 * It will first clear OSC bit,
 * then read it back to check whether it has been cleared.
 *
 * Input:       menu_item - not used
 *
 * Output:      PASSED/FAILED
...
ds1337_osc_stop_wrapper (int menu_item)
{
     ...
     */
    rc = clear_rtc_status_reg(&i2c_if);
    TERR_URET_COND(rc != PASSED, FAILED, "failed to write clear OSF bit\n");

    // dev_init will check OSF bit
    rc = ds1337.base.dev_object_fvt->dev_init(dev);
    TERR_URET_COND(rc != PASSED, FAILED, "RTC Oscillator OSF Check failed\n");
