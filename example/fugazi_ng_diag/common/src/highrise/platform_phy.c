...
int phy_check_t_link_status(MTD_DEV *pMtdDev, MTD_U16 port, MTD_U16 *speed_p, MTD_U16 *linkup_p);
int phy_check_h_link_status(MTD_DEV *pMtdDev, MTD_U16 port, MTD_U16 *linkup_p);
int phy_check_cpu_mac_port_status(uint32_t *linkup_p);
int phy_get_t_link_speed(MTD_DEV *pMtdDev, MTD_U16 port, MTD_U16 *t_speed);
void dump_cpu_eth_statistic(int port);

static long _phy_send_pkt_utils(void);
...
static long _phy_intr_trig(void);
static long _phy_config_mode_utils(void);
...
static submenu_xtable_t phy_test_tbl[] = {
    ...
    { "PHY Intr Test", (type_t(*)())_phy_intr_test, 0,
     MF_HIDDEN_EXE, (type_t(*)())0, 0, (type_t(*)())0, 0 },
};
...
static submenu_xtable_t phy_utils_table[] = {
    ...
    { "PHY Send Packet Util",
     (PFT)_phy_send_pkt_utils,           0,
     0,
     (type_t(*)())0,                     0,
     (type_t(*)())0,                     0},
};
...
int marvell_mtd_smi_create(MTD_DEV *pMtdDev)
{
    ...
    if (rc < 0) {
        printf("ERR: [%s]%d: Create smi dev failed! rc=%d\n",
...
int marvell_mtd_xsmi_create(MTD_DEV *pMtdDev)
{
    ...
    if (rc < 0) {
        printf("ERR: [%s]%d: Create xsmi dev failed! rc=%d\n",
                __FUNCTION__, __LINE__, rc);
        return(FAILED);
    }
...
int marvell_mtd_init_drv(MTD_DEV *pMtdDev)
{
    ...
    rc = mtdLoadDriver((FMTD_READ_MDIO)marvell_mtd_smi_read,
            ...
            pMtdDev);
    if(rc != 0 ) {
        printf("ERR: [%s]%d: Load driver failed! rc=%d\n",
                __FUNCTION__, __LINE__, rc);
        return(FAILED);
    }
...
static int marvell_mtd_ram_firmware_download(MTD_DEV *pMtdDev, MTD_U16 port)
{
    ...
    if (MTD_OK == rc) {
        ...
        printf("ERR: [%s]%d: mtd update RamImage failed! rc=%d\n",
...
int marvell_mtd_revision_check(MTD_DEV *pMtdDev, MTD_U16 port)
{
    ...
    rc = mtdGetPhyRevision(pMtdDev, port,
            ...
            &(pMtdDev->thisPort));
    if (rc != MTD_OK) {
        printf("ERR: [%s]%d: mtd failed to get PhyRevison! rc=%d\n",
                __FUNCTION__, __LINE__, rc);
        return(FAILED);
    }
    ...
    MTD_U16 mode)
{
    ...
    if(mode > MARVELL_MTD_MODE_1G) {
        printf("ERR: [%s]%d: unsupported mode:%d\n",
    ...
    MTD_U16 mode)
{
    ...
    switch (mode) {
        ...
        default:
            printf("ERR: [%s]%d: Error: Unsupported mode:%d !\n",
                    __FUNCTION__, __LINE__, mode);
            return(FAILED);
    }
    return MTD_OK;
...
int marvell_mtd_host_link_get(MTD_DEV *pMtdDev, uint32_t port)
{
    ...
    if (port != 0) {
        printf("ERR: [%s]%d: Error port number :%d !\n",
...
int marvell_config_media_select(MTD_DEV *pMtdDev, MTD_U16 port, int media_select)
{
    ...
    if (media_select > MTD_AUTO_MEDIA) {
        printf("ERR: [%s]%d: Error Media Select value:%d\n",
...
#endif
    if (PASSED != rc) {
        printf("ERR: [%s]%d: Error! return code:%d \n",
                __FUNCTION__, __LINE__, rc);
        return(FAILED);
    }
...
static int _phy_bin_select(const struct phy_bin *bins, unsigned char **bin, size_t *len)
{
    ...
    _DRAIN_STDIN();
    if (!(ch >= '0' && ch < '0' + num)) {
        printf("ERR: [%s]%d:Invalid choice '%c'\n", __FUNCTION__, __LINE__, ch);
        goto _EXIT_POINT;
    }
...
static int phy_firmware_flash_program_utils(void)
{
    ...
    if (ch == 'y' || ch == 'Y') {
        ...
        if (retStatus == MTD_FAIL) {
            printf("ERR: [%s]%d:Program flash image failed, error code:0x%x\n", __FUNCTION__, __LINE__, error);
            goto _EXIT_POINT;
        }
...
static long _phy_config_mode_utils(void)
 {
     ...
     * Auto media support is only available for 10G/1G/100M/10M.
     * Auto-media is not supported for 2.5G and 5G speeds*/
    int mode = MARVELL_MTD_MODE_1G;
    int input = 3;
    printf(" Only used for Diag debugging, otherwise, pls use 'ethtool' instead.\n");
    printf(" 0  100M mode \n");
    printf(" 1  1G SGMII mode\n");
    printf(" 2  2500-BASEX mode\n");
    printf(" 3  quit.. \n");
    fflush(stdout);
 
   _DRAIN_STDIN();
    input = getdec_answer("\nEnter mode [0-3]", input, 0, 3);
    _DRAIN_STDIN();

    switch(input) {
        case 0: mode = MARVELL_MTD_MODE_100M; break;
        case 1: mode = MARVELL_MTD_MODE_1G; break;
        case 2: mode = MARVELL_MTD_MODE_2P5G; break;
        case 3: printf("quit..\n"); return (PASSED);
        default : printf("Invalid input\n"); return(PASSED);
     }
    ERR_RET_COND(marvell_mtd_config_mode(pMtd3310Dev, MTD_PORT_0, mode) != MTD_OK,
            FAILED, "failed to config mode\n");
...
int marvell_mtd_enable_deep_mac_loopback(
        ...
        int enable)
{
    MTD_STATUS rc = MTD_OK;
    rc = mtdSetTunitDeepMacLoopback(pMtdDev, port, speed, enable);
    ERR_RET_COND(rc != MTD_OK, FAILED,
            "[Speed:%s] %s DeepMacLoopback failed!\n", 
            mtd_speed_desc[speed], (enable ? "Enable" : "Disable"));
    return(PASSED);
}

int marvell_mtd_get_deep_mac_loopback(
        MTD_DEV *pMtdDev,
        MTD_U16 port,
        MTD_U16 speed,
        MTD_U16 *en)
{
    MTD_STATUS rc = MTD_OK;
    rc = mtdHwGetPhyRegField(pMtdDev,port,MTD_TUNIT_IEEE_PCS_CTRL1,14,1,(MTD_U16 *)en);
    ERR_RET_COND(rc != MTD_OK, FAILED, "get_deep_mac_loopback failed!\n");
    return(PASSED);
}
        ...
        int enable)
{
    ...
    rc = mtdSetHXunitMacLoopback(pMtdDev, port, MTD_H_UNIT, port, enable);
    ERR_RET_COND(rc != MTD_OK, FAILED,
            "%s ShallowMacLoopback failed!\n", (enable ? "Enable" : "Disable"));
    return(PASSED);
}

int marvell_mtd_get_shallow_mac_loopback(
        MTD_DEV *pMtdDev,
        MTD_U16 port,
        MTD_U16 *en)
{
    MTD_STATUS rc = MTD_OK;
    rc = mtdHwGetPhyRegField(pMtdDev,port,MTD_H_UNIT,MTD_SERDES_CONTROL1,12,1,(MTD_U16 *)en);
    ERR_RET_COND(rc != MTD_OK, FAILED, "get_shallow_mac_loopback failed!\n");
    return(PASSED);
}
...
int phy_config_loopback_utils(void)
{
    ...
    switch (level) {
        case 0:
            //Shallow Mac loopback
            ERR_URET_COND(PASSED != marvell_mtd_enable_shallow_mac_loopback(pMtdDev, port, enable),
                    FAILED, "shallow mac loopback configure failed!\n");
            break;

        case 1:
            //Deep Mac loopback
            ERR_URET_COND(PASSED != phy_get_t_link_speed(pMtdDev, port, &speed),
                    FAILED, "failed to get T-Unit speed before deep mac loopback!\n");
            ERR_URET_COND(PASSED != marvell_mtd_enable_deep_mac_loopback(pMtdDev, port, speed, enable),
                    FAILED, "deep mac loopback configure failed!\n");
            break;

        default:
            printf("ERR: [%s]:%d, Invalid loopback input!\n", __FUNCTION__, __LINE__);
            return(FALSE);
    }
...
static long _phy_reg_test(void)
{
    ...
    char *tname = "PHY Register";
    testname("%s", tname);
    prpass(testpass, "\nStart ");

    rc = register_tests(0, &mv88x3310_test_regs[0]);
    TERR_URET_COND(PASS != rc, FAILED, "Marvel 88X3310 Register Test Failed\n");
...
static long _phy_reg_dump(void)
{
    ...
    for(idx = 0; to_dump[idx].arr && to_dump[idx].size; idx++) {
        ...
        if (ret_mtd == MTD_OK) {
            ...
            printf("ERR: [%s]%d:Error when dump phy registers.\n",
                __FUNCTION__, __LINE__);
            return(FAILED);
        }
    }
...
static long _phy_reg_rdwr(void)
{
    ...
    if (MTD_OK != mtdHwXmdioRead(pMtd3310Dev, MTD_PORT_0, dev, reg, (MTD_U16 *)&val)) {
        printf("ERR: [%s]%d: Read dev-%u, reg-0x%04x failed\n",
    ...
    if (opr == 1) {
        ...
        if (MTD_OK != mtdHwXmdioWrite(pMtd3310Dev, MTD_PORT_0, dev, reg, val)) {
            printf("ERR: [%s]%d: Write dev-%u, reg-0x%04x failed\n",
                __func__, __LINE__, dev, reg);
            return (FAILED);
        }
    } else {
...
int phy_show_temperature(const char *tag)
{
    ...
    ERR_RET_COND(PASSED != phy_enable_temperature(), FAILED, "Enable phy temperature failed\n");
...
static long _phy_shallow_mac_lpbk_test(void)
{
    ...
    /* 0.1 Get speed and check T-Unit link status before LPBK*/
    TERR_URET_COND(PASSED != phy_check_t_link_status(pMtdDev, port, &speed, &t_linkup),
            FAILED, "Check T-Unit status failed before LPBK\n");
    if (t_linkup) {
        printf("\nInterfaces T-Unit link up before LPBK\n");
    } else {
        cterr('f', 0, "Error! T-Unit link down before LPBK!\n");
        goto _EXIT_POINT;
    }
    /* 0.2 check CPU MAC and H-Unit link status before LPBK*/
    TERR_URET_COND(PASSED != phy_check_cpu_mac_port_status(&mac_linkup),
            FAILED, "Check MAC status failed before LPBK\n");
    TERR_URET_COND(PASSED != phy_check_h_link_status(pMtdDev, port, &h_linkup),
            FAILED, "Check H-Unit failed before LPBK\n");
    if (mac_linkup && h_linkup) {
        printf("\nInterfaces MAC and H-Unit link up before LPBK \n");
    } else {
        cterr('f', 0, "Error! MAC or H-Unit link down before LPBK!\n");
        goto _EXIT_POINT;
    }

    /* Name the Test*/
    sprintf(tname, "PHY %s Shallow LPBK", mtd_speed_desc[speed]);
    testname("%s", tname);
    prpass(testpass, "\nStart ");

    /* 1. Set loopback */
    printf("\n1. Set Shallow MAC loopback\n");
    TERR_URET_COND(PASSED != marvell_mtd_enable_shallow_mac_loopback(pMtdDev, port, 1),
            FAILED, "Enable shallow LPBK failed\n");
    sleep(5);

    TERR_URET_COND(PASSED != marvell_mtd_get_shallow_mac_loopback(pMtdDev, port, &lpbk_en),
            FAILED, "Get shallow LPBK failed\n");
    if (lpbk_en != 1) {
        cterr('f', 0, "Error! The shallow LPBK is not enabled!\n");
        goto _EXIT_POINT;
    }

    /* 2.0 Check linkstatus */
    printf("\n2. Check link status\n");
    TERR_URET_COND(PASSED != phy_check_cpu_mac_port_status(&mac_linkup),
            FAILED, "Check MAC status failed after LPBK\n");
    TERR_URET_COND(PASSED != phy_check_h_link_status(pMtdDev, port, &h_linkup),
            FAILED, "Check H-Unit failed after LPBK\n");
    if (mac_linkup && h_linkup) {
        printf("Interfaces MAC and H-Unit link up after LPBK\n");
    } else {
        cterr('f', 0, "Error! MAC or H-Unit link down after LPBK!\n");
        goto _EXIT_POINT;
    }

    /* 2.1 Dump CPU counters*/
    dump_cpu_eth_statistic(0);

    /* 3. Send packet*/
    printf("\n3. Send packets\n");
    if (PASSED != highrise_phy_socket_test()) {
        cterr('f', 0, "%s test, pls clear Shallow LPBK manually in PHY utility\n", tname);
        dump_cpu_eth_statistic(0);
        goto _EXIT_POINT;
    }
    ...
    /* 4. Clear loopback*/
    printf("\n4. Clear loopback\n");
    TERR_URET_COND(PASSED != marvell_mtd_enable_shallow_mac_loopback(pMtdDev, port, 0), 
				     FAILED, "Clear Shallow MAC LPBK failed\n");
    sleep(2);

    TERR_URET_COND(PASSED != marvell_mtd_get_shallow_mac_loopback(pMtdDev, port, &lpbk_en),
            FAILED, "Get shallow LPBK failed\n");
    if (lpbk_en != 0) {
        cterr('f', 0, "Error! The shallow LPBK is not clear!\n");
        goto _EXIT_POINT;
    }
...
_EXIT_POINT:
    if (FAILED == uret) {
        marvell_dump_link_status(pMtdDev, port);
        marvell_dump_loopback_status(pMtdDev, port);
    }
    prpass(testpass, "%s ", uret==PASSED ? "Passed" : "Failed");
    prcomplete(testpass, errcount, (char *)0);
    return(uret);
...
static long _phy_deep_mac_lpbk_test(void)
{
    ...
    /* 0. Get speed and T-Unit link status*/
    TERR_URET_COND(PASSED != phy_check_t_link_status(pMtdDev, port, &speed, &t_linkup),
            FAILED, "Check T-Unit status failed before LPBK\n");
    if (t_linkup) {
        printf("\nInterfaces T-Unit link up before LPBK\n");
    ...
    /* Name the Test*/
    sprintf(tname, "PHY %s Deep LPBK", mtd_speed_desc[speed]);
    testname(tname);
    prpass(testpass, "\nStart ");

    /* 1. Set loopback */
    printf("\n1. Enable deep MAC loopback\n");
    TERR_URET_COND(PASSED != marvell_mtd_enable_deep_mac_loopback(pMtdDev, port, speed, 1),
            FAILED, "Enable deep LPBK failed\n");
    sleep(5);
    TERR_URET_COND(PASSED != marvell_mtd_get_deep_mac_loopback(pMtdDev, port, speed, &lpbk_en),
            FAILED, "Get deep LPBK failed\n");
    if (lpbk_en != 1) {
        cterr('f', 0, "Error! The deep LPBK is not enabled!\n");
        goto _EXIT_POINT;
    }
    ...
    printf("\n4. Send packets\n");
    if (PASSED != highrise_phy_socket_test()) {
        cterr('f', 0, "%s test, pls clear Deep LPBK manually in PHY utility\n", tname);
        dump_cpu_eth_statistic(0);
        goto _EXIT_POINT;
    }
    ...
    /* 4. Clear loopback */
    printf("\n5. Clear loopback\n");
    TERR_URET_COND(PASSED != marvell_mtd_enable_deep_mac_loopback(pMtdDev, port, speed, 0),
            FAILED, "Clear deep LPBK failed\n");
    sleep(2);
    TERR_URET_COND(PASSED != marvell_mtd_get_deep_mac_loopback(pMtdDev, port, speed, &lpbk_en),
            FAILED, "Get deep LPBK failed\n");
    if (lpbk_en != 0) {
        cterr('f', 0, "Error! The deep LPBK is not clear!\n");
        goto _EXIT_POINT;
    }
...
static long _phy_send_pkt_utils(void)
{
    printf("Send and check packets util\n");
    // Send packets
    if (highrise_phy_socket_test()) {
        printf("\nError: send and check packets failed\n");
    } else {
        printf("\nPass: send and check packets passed\n");
    }
    return (PASSED);
}
...
static long _phy_100m_ext_lpbk_test(void)
{
    ...
    char *tname = "PHY 100M Ext-lpbk";
    prpass(testpass, "\nStart ");
    testname(tname);
    printf("\nWarning: %s test requires self-lpbk cable inserted\n", tname);

    /* 0. Config Mac & PHY Mode to 100M */
    printf("\n0. Config mode to 100M via ethtool\n");
    ...
    printf("\n1. Check Link status\n");
    TERR_URET_COND(PASSED != phy_check_t_link_status(pMtdDev, port, &speed, &t_linkup),
            FAILED, "Check T-Unit failed\n");
    if (t_linkup) {
        printf("\nInterfaces T-Unit link up\n");
    } else {
        cterr('f', 0, "Error! T-Unit link down!\n");
        goto _EXIT_POINT;
    }

    TERR_URET_COND(PASSED != phy_check_cpu_mac_port_status(&mac_linkup),
            FAILED, "Check MAC status failed\n");
    TERR_URET_COND(PASSED != phy_check_h_link_status(pMtdDev, port, &h_linkup),
            FAILED, "Check H-Unit failed\n");
    if (mac_linkup && h_linkup) {
        printf("\nInterfaces MAC and H-Unit link up\n");
    } else {
        cterr('f', 0, "Error! MAC or H-Unit link down!\n");
        goto _EXIT_POINT;
    }
    ...
    // Send packets
    printf("\n2. Send packets\n");
    if (FAILED != highrise_phy_socket_test()) {
        cterr('f', 0, "%s test, PHY is left 100M mode!\n", tname);
        dump_cpu_eth_statistic(0);
        goto _EXIT_POINT;
    }

    // Restore back to SGMII
    printf("\n3. Restore Mac & PHY mode via ethtool\n");
...
_EXIT_POINT:
    if (FAILED == uret) {
        marvell_dump_link_status(pMtdDev, port);
        printf("\nPHY speed is 100M now, to restore PHY speed, run below cmd under Linux shell#\n");
        printf("'ethtool -s eth0 autoneg on; ethtool -r eth0'\n");
    }
    prpass(testpass, "%s ", uret==PASSED ? "Passed" : "Failed");
    prcomplete(testpass, errcount, (char *)0);
    return(uret);
        ...
        MTD_U16 *linkup_p)
{
    ...
    *linkup_p = 0;
    ...
    for (i = 0; i < LINK_UP_MAX_POLL; i++) {
        rc = mtdGet1000BXSGMIIStatus(pMtdDev, port, MTD_H_UNIT,
                    &an_complete, &remote_fault, &link_status_latched,
                    &link_status_current);
        ERR_RET_COND(rc != MTD_OK, FAILED, "failed to check H-Unit link status!\n");
    ...
    if ((i == LINK_UP_MAX_POLL) && !(*linkup_p)) {
        printf("Warning: H-unit link error after polling [%d]s\n", LINK_UP_MAX_POLL);
    }
    return(PASSED);
}
        ...
        MTD_U16 *linkup_p)
{
    ...
    *linkup_p = 0;
    ...
    for (i = 0; i < LINK_UP_MAX_POLL; i++) {
        rc = mtdIsBaseTUp(pMtdDev, port, &t_speed, &t_linkup);
        ERR_RET_COND(rc != MTD_OK, FAILED, "failed to check T-Unit link status!\n");
        if (t_linkup) {
            *linkup_p = 1;
            *speed_p = t_speed;
            return(PASSED);
    ...
    if ((i == LINK_UP_MAX_POLL) && !(*linkup_p)) {
        printf("Warning: T-unit link error after polling [%d]s\n", LINK_UP_MAX_POLL);
    }
    return(PASSED);
}
...
{
    MTD_BOOL t_linkup;
    ERR_RET_COND(MTD_OK != mtdIsBaseTUp(pMtdDev, port, t_speed, &t_linkup),
            FAILED, "Get T-Unit speed failed!\n");
    return(PASSED);
}
...
int phy_check_cpu_mac_port_status(uint32_t *linkup_p)
{
    ...
    uint32_t mac_port_status_reg = 0xF2130E10;
    uint32_t mac_port_status = 0;
    *linkup_p = 0;

    for (i = 0; i < LINK_UP_MAX_POLL; i++) {
        rc = highrise_mem_read32(mac_port_status_reg, &mac_port_status);
        ERR_RET_COND(rc != PASSED, FAILED, "failed to mem_read mac_status @0x%x\n", mac_port_status_reg);
    ...
    if ((LINK_UP_MAX_POLL == i) && !(*linkup_p)) {
        printf("Warning: MAC link failed after polling [%d]s, [0x%x]:0x%hx\n",
                LINK_UP_MAX_POLL, mac_port_status_reg, mac_port_status);
    }
...
static int _phy_intr_gpio_dump(const char *tag)
{
    uint32_t regv = 0;
    int      idx  = 0;

    for(idx = 0; phy_intr_mpp[idx].name; idx++) {
        ERR_RET_COND(PASSED != highrise_mem_read32(phy_intr_mpp[idx].rego, &regv),
            -(__LINE__), "Failed to read ts intr gpio reg-%s.\n", phy_intr_mpp[idx].name);

        printf("%-16s:@%08x:%08x:boff-%-2u:blen-%u:bval-%x\n",
            phy_intr_mpp[idx].name,
            phy_intr_mpp[idx].rego, regv,
            phy_intr_mpp[idx].boff,
            phy_intr_mpp[idx].blen,
            (regv >> phy_intr_mpp[idx].boff) & ((1 << phy_intr_mpp[idx].blen) - 1)
        );
    }
    return(PASSED);
}
...
static const struct {
    ...
    /*0*/{"Unit Intr Status", 31, 0xF040},
    /*1*/{"Unit Intr Mask"  , 31, 0xF043},
    /*2*/{"Port Intr Status", 31, 0xF042},
    /*3*/{"Intr Polarity"   , 31, 0xF041},
    /*4*/{"Temp Ctrl-0"     , 31, 0xF089},
    /*5*/{"Temp Ctrl-1"     , 31, 0xF08a},
    /*6*/{"Temp Ctrl-2"     , 31, 0xF08c},
         {NULL, ~0, ~0},
};

#define _EXTD_VAR(V) #V, V
#define _DUMP_INTR(TAG, VAL, ENB, ...) \
    __intr_dump(TAG, VAL, ENB, \
                sizeof(VAL) == sizeof(MTD_U16) ? 2 : ( \
                    sizeof(VAL) == sizeof(MTD_U32) ? 4 :  \
                        (assert(NULL == "Invalid size."), 111)), \
                ##__VA_ARGS__)

#define _STR_VAR(...) #__VA_ARGS__

#define _FOLD(A, B) A,B
#define _DUMP_INTR_REG(TAG, NAME, DEV_REG) do { \
    mtdHwXmdioRead(pMtd3310Dev, MTD_PORT_0, DEV_REG, &val16); \
    if (TAG) printf("%s\n", TAG ? TAG : ""); \
    printf("%s:@%-2u:0x%04x:0x%04x\n", NAME, DEV_REG, val16); \
}while(0)

#define END_OF_VAR_LIST "END_OF_VAR_LIST"
static long __intr_dump(const char *tag, uint32_t val, uint32_t enb_msk, int val_len, ...)
{
    va_list ap;
    uint32_t mask;
    char *name;
    ...
    default: //invalid length
        printf("Invalid value len:%d\n", val_len); break;
    }

    printf(buf, " ", val);

    printf("%9soff val enb name\n", " ");
    va_start(ap, val_len);
    while(1) {
        name = va_arg(ap, char *);
        if (strncmp(name, END_OF_VAR_LIST, sizeof(END_OF_VAR_LIST) - 1) == 0)
            break;
        mask = va_arg(ap, int);
        printf("%9s%-3u %-3u %-3u %s\n", " ", BIT_START(mask,0),
                mask & val ? 1 : 0, enb_msk & (1 << BIT_START(mask,0)) ? 1 : 0, name);
    }
...
static long _phy_intr_dump(void)
{
    MTD_BOOL        trig = 0;
    MTD_U16         val16 = 0;
    MTD_U32         val32 = 0;
    MTD_INTR_STATUS intr_st;

    // intr status may be cleared on read
    // make sure we have the correct status when read
    usleep(300000);
    ERR_RET_COND(MTD_OK != mtdGetPortIntrStatus(pMtd3310Dev, MTD_PORT_0, &trig, &intr_st),
        FAILED, "Get phy intr status failed.\n");
    // time may be needed for intr bit set again for reg dump
    usleep(300000);

    printf("Triggerred status:%d\n", trig);

    //MTD_CUNIT_INTERRUPT_STATUS
    _DUMP_INTR_REG("\nC-unit interrupt status",
        _STR_VAR(MTD_CUNIT_INTERRUPT_STATUS),
        MTD_CUNIT_INTERRUPT_STATUS);

    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_CUNIT_INTERRUPT_CTRL),
        MTD_CUNIT_INTERRUPT_CTRL);
    mtdGetTopInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, &val16);
    _DUMP_INTR(NULL,
                intr_st.cunitIntr,
                val16,
                _EXTD_VAR(MTD_WOL_INTR),
                _EXTD_VAR(MTD_TEMP_SENSOR_INTR),
                _EXTD_VAR(MTD_F2R_MEM_INTR),
                _EXTD_VAR(MTD_MACSEC_INTR),
                _EXTD_VAR(MTD_PROC_INTR),
                _EXTD_VAR(MTD_GPIO_INTR),
                _EXTD_VAR(MTD_M_UNIT_INTR),
                _EXTD_VAR(MTD_X_UNIT_INTR),
                _EXTD_VAR(MTD_T_UNIT_INTR),
                END_OF_VAR_LIST);

    //MTD_TUNIT_INTERRUPT_STATUS1
    //MTD_TUNIT_INTERRUPT_STATUS2
    _DUMP_INTR_REG("\nT-unit interrupt status",
        _STR_VAR(MTD_TUNIT_INTERRUPT_STATUS1),
        MTD_TUNIT_INTERRUPT_STATUS1);
    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_TUNIT_INTERRUPT_STATUS2),
        MTD_TUNIT_INTERRUPT_STATUS2);

    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_TUNIT_INTERRUPT_CTRL1),
        MTD_TUNIT_INTERRUPT_CTRL1);
    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_TUNIT_INTERRUPT_CTRL2),
        MTD_TUNIT_INTERRUPT_CTRL2);

    mtdGetTunitInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, &val32);
    _DUMP_INTR(NULL,
                intr_st.tunitIntr,
                val32,
                _EXTD_VAR(MTD_SYMBOL_ERR_INTR),
                _EXTD_VAR(MTD_FALSE_CARR_INTR),
                _EXTD_VAR(MTD_TX_FIFO_OF_UF_INTR),
                _EXTD_VAR(MTD_JABBER_INTR),
                _EXTD_VAR(MTD_AN_ERR_INTR),
                _EXTD_VAR(MTD_SPEED_CHG_INTR),
                _EXTD_VAR(MTD_DX_CHG_INTR),
                _EXTD_VAR(MTD_PAGE_RX_INTR),
                _EXTD_VAR(MTD_AN_COMPLETED_INTR),
                _EXTD_VAR(MTD_LINK_STATUS_CHG_INTR),
                _EXTD_VAR(MTD_AN_MDI_CROS_CHG_INTR),
                _EXTD_VAR(MTD_DOWNSHIFT_INTR),
                _EXTD_VAR(MTD_ENG_DETECT_INTR),
                _EXTD_VAR(MTD_FLP_XCHG_NO_LINK_INTR),
                _EXTD_VAR(MTD_POLARITY_CHG_INTR),
                END_OF_VAR_LIST);

    //MTD_H_UNIT, MTD_X2_X4_INTERRUPT_STATUS1
    //MTD_H_UNIT, MTD_X2_X4_INTERRUPT_STATUS2
    _DUMP_INTR_REG("\nH-unit interrupt status - X2/X4 Interrupts",
        _STR_VAR(MTD_X2_X4_INTERRUPT_STATUS1),
        MTD_X2_X4_INTERRUPT_STATUS1);
    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_X2_X4_INTERRUPT_STATUS2),
        MTD_X2_X4_INTERRUPT_STATUS2);

    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_X2_X4_INTERRUPT_CTRL1),
        MTD_X2_X4_INTERRUPT_CTRL1);
    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_X2_X4_INTERRUPT_CTRL2),
        MTD_X2_X4_INTERRUPT_CTRL2);

    mtdGetX2X4InterruptsEnabled(pMtd3310Dev, MTD_PORT_0, &val32);
    _DUMP_INTR(NULL,
                intr_st.hunitIntr.x2x4Intr,
                val32,
                _EXTD_VAR(MTD_LINK_UP_TO_DN_INTR),
                _EXTD_VAR(MTD_LINK_DN_TO_UP_INTR),
                _EXTD_VAR(MTD_FAULT_LINE_TO_CORE_INTR),
                _EXTD_VAR(MTD_FAULT_CORE_TO_LINE_INTR),
                _EXTD_VAR(MTD_LN1_ENG_DETECT_CHG_INTR),
                _EXTD_VAR(MTD_LN0_ENG_DETECT_CHG_INTR),
                _EXTD_VAR(MTD_LN1_SYNC_CHG_INTR),
                _EXTD_VAR(MTD_LN0_SYNC_CHG_INTR),
                END_OF_VAR_LIST);


    //MTD_H_UNIT,MTD_10GBR_INTERRUPT_STATUS
    _DUMP_INTR_REG("\nH-unit interrupt status - 5G/10G BaseR Interrupts",
        _STR_VAR(MTD_H_UNIT,MTD_10GBR_INTERRUPT_STATUS),
        _FOLD(MTD_H_UNIT,MTD_10GBR_INTERRUPT_STATUS));
    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_H_UNIT,MTD_10GBR_INTERRUPT_CTRL),
        _FOLD(MTD_H_UNIT,MTD_10GBR_INTERRUPT_CTRL));

    mtdGetBaseRInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, MTD_H_UNIT, &val16);
    _DUMP_INTR(NULL,
                intr_st.hunitIntr.baseRIntr,
                val16,
                _EXTD_VAR(MTD_BASER_LOCAL_FAULT_TX_INTR),
                _EXTD_VAR(MTD_BASER_LOCAL_FAULT_RX_INTR),
                _EXTD_VAR(MTD_BASER_LINK_STATUS_CHG_INTR),
                _EXTD_VAR(MTD_BASER_HIGH_BER_CHG_INTR),
                _EXTD_VAR(MTD_BASER_BLK_LOCK_CHG_INTR),
                END_OF_VAR_LIST);

    //MTD_H_UNIT,MTD_1GBX_INTERRUPT_STATUS
    _DUMP_INTR_REG("\nH-unit interrupt status - 1G/2.5GBASE-X Interrupts",
        _STR_VAR(MTD_H_UNIT,MTD_1GBX_INTERRUPT_STATUS),
        _FOLD(MTD_H_UNIT,MTD_1GBX_INTERRUPT_STATUS));
    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_H_UNIT,MTD_1GBX_INTERRUPT_CTRL),
        _FOLD(MTD_H_UNIT,MTD_1GBX_INTERRUPT_CTRL));

    mtdGetBaseXInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, MTD_H_UNIT, &val16);
    _DUMP_INTR(NULL,
                intr_st.hunitIntr.baseXIntr,
                val16,
                _EXTD_VAR(MTD_BASEX_SPEED_CHG_INTR),
                _EXTD_VAR(MTD_BASEX_DX_CHG_INTR),
                _EXTD_VAR(MTD_BASEX_PAGE_RX_INTR),
                _EXTD_VAR(MTD_BASEX_AN_COMPLETED_INTR),
                _EXTD_VAR(MTD_BASEX_LINK_UP_TO_DN_INTR),
                _EXTD_VAR(MTD_BASEX_LINK_DN_TO_UP_INTR),
                _EXTD_VAR(MTD_BASEX_SYMBOL_ERR_INTR),
                _EXTD_VAR(MTD_BASEX_FALSE_CARR_INTR),
                END_OF_VAR_LIST);

    //MTD_H_UNIT,MTD_FIFO_CRC_INTR_STATUS
    _DUMP_INTR_REG("\nH-unit interrupt status - misc interrupts",
        _STR_VAR(MTD_H_UNIT,MTD_FIFO_CRC_INTR_STATUS),
        _FOLD(MTD_H_UNIT,MTD_FIFO_CRC_INTR_STATUS));

    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_H_UNIT,MTD_FIFO_CRC_INTR_ENABLE),
        _FOLD(MTD_H_UNIT,MTD_FIFO_CRC_INTR_ENABLE));
    mtdGetMiscInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, MTD_X_UNIT, &val16);
    _DUMP_INTR(NULL,
                intr_st.hunitIntr.miscIntr,
                val16,
                _EXTD_VAR(MTD_CRC_INTR),
                _EXTD_VAR(MTD_FIFO_OVERFLOW_INTR),
                _EXTD_VAR(MTD_FIFO_UNDERFLOW_INTR),
                END_OF_VAR_LIST);

    //MTD_X_UNIT,MTD_10GBR_INTERRUPT_STATUS
    _DUMP_INTR_REG("\nX-unit interrupt status - 5G/10G BaseR Interrupts",
        _STR_VAR(MTD_X_UNIT,MTD_10GBR_INTERRUPT_STATUS),
        _FOLD(MTD_X_UNIT,MTD_10GBR_INTERRUPT_STATUS));

    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_X_UNIT,MTD_10GBR_INTERRUPT_CTRL),
        _FOLD(MTD_X_UNIT,MTD_10GBR_INTERRUPT_CTRL));
    mtdGetBaseRInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, MTD_X_UNIT, &val16);
    _DUMP_INTR(NULL,
                intr_st.xunitIntr.baseRIntr,
                val16,
                _EXTD_VAR(MTD_BASER_LOCAL_FAULT_TX_INTR),
                _EXTD_VAR(MTD_BASER_LOCAL_FAULT_RX_INTR),
                _EXTD_VAR(MTD_BASER_LINK_STATUS_CHG_INTR),
    ...
    //MTD_X_UNIT,MTD_1GBX_INTERRUPT_STATUS
    _DUMP_INTR_REG("\nX-unit interrupt status - 1G/2.5GBASE-X Interrupts",
        _STR_VAR(MTD_X_UNIT,MTD_1GBX_INTERRUPT_STATUS),
        _FOLD(MTD_X_UNIT,MTD_1GBX_INTERRUPT_STATUS));

    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_X_UNIT,MTD_1GBX_INTERRUPT_CTRL),
        _FOLD(MTD_X_UNIT,MTD_1GBX_INTERRUPT_CTRL));
    mtdGetBaseXInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, MTD_X_UNIT, &val16);
    _DUMP_INTR(NULL,
                intr_st.xunitIntr.baseXIntr,
                val16,
                _EXTD_VAR(MTD_BASEX_SPEED_CHG_INTR),
                _EXTD_VAR(MTD_BASEX_DX_CHG_INTR),
                _EXTD_VAR(MTD_BASEX_PAGE_RX_INTR),
    ...
    //MTD_X_UNIT,MTD_FIFO_CRC_INTR_STATUS
    _DUMP_INTR_REG("\nX-unit interrupt status - misc interrupts",
        _STR_VAR(MTD_X_UNIT,MTD_FIFO_CRC_INTR_STATUS),
        _FOLD(MTD_X_UNIT,MTD_FIFO_CRC_INTR_STATUS));

    _DUMP_INTR_REG(NULL,
        _STR_VAR(MTD_X_UNIT,MTD_FIFO_CRC_INTR_ENABLE),
        _FOLD(MTD_X_UNIT,MTD_FIFO_CRC_INTR_ENABLE));
    mtdGetMiscInterruptsEnabled(pMtd3310Dev, MTD_PORT_0, MTD_X_UNIT, &val16);
    _DUMP_INTR(NULL,
                intr_st.xunitIntr.miscIntr,
                val16,
                _EXTD_VAR(MTD_CRC_INTR),
                _EXTD_VAR(MTD_FIFO_OVERFLOW_INTR),
                _EXTD_VAR(MTD_FIFO_UNDERFLOW_INTR),
...
static long _phy_intr_regs_dump(char *tag)
{
    int      idx = 0;
    uint16_t dev = 0;
    uint16_t reg = 0;
    ...
    for(idx = 0; phy_intr_regs[idx].name; idx++) {
        dev = phy_intr_regs[idx].dev;
        reg = phy_intr_regs[idx].reg;
        if (MTD_OK != mtdHwXmdioRead(pMtd3310Dev, MTD_PORT_0, dev, reg, (MTD_U16 *)&val))
            return(FAILED);
        printf("%-16s:0x%02x:%02u:0x%04x:0x%04x\n",
            phy_intr_regs[idx].name, MTD_PORT_0, dev, reg, (MTD_U16)val);
    }

    return(PASSED);
}

static long _phy_intr_enable_all(int enb)
{
    MTD_BOOL enb_dis = enb ? MTD_TRUE : MTD_FALSE;

    mtdEnableDisableBaseRInterrupts(
        pMtd3310Dev, MTD_PORT_0, MTD_H_UNIT,
        MTD_ALL_BASER_INTR,
        enb_dis);

    mtdEnableDisableBaseRInterrupts(
        pMtd3310Dev, MTD_PORT_0, MTD_X_UNIT,
        MTD_ALL_BASER_INTR,
        enb_dis);
...
static long _phy_intr_pin_polarity(void)
{
    MTD_U16 pol = 0;
    MTD_U16 frc = 0;
    char    chr = 0;

    printf(" 0  Active Low , Inactive High\n");
    printf(" 1  Active High, Inactive Low\n");
    printf(" 2  Active Low , Inactive Tri\n");
    printf(" 3  Active High, Inactive Tri\n");
    printf("Select INT pin polarity:");
    fflush(stdout);
    ...
    switch(chr) {
    case '0': pol = MTD_ACTIVE_LOW_INACTIVE_HIGH; break;
    case '1': pol = MTD_ACTIVE_HIGH_INACTIVE_LOW; break;
    case '2': pol = MTD_ACTIVE_LOW_INACTIVE_TRI ; break;
    case '3': pol = MTD_ACTIVE_HIGH_INACTIVE_TRI; break;
    default :
        printf("Invalid choice.\n");
        return(PASSED);
    }

    printf("Force interrupt(Y/n):");
    fflush(stdout);
    chr = getchar();
    _DRAIN_STDIN();
    frc = (chr == 'Y' || chr == 'y') ? 1 : 0;

    mtdInterruptPinControl(pMtd3310Dev, MTD_PORT_0, pol, frc);
    return(PASSED);
}
...
static long _phy_intr_trig(void)
{
    ...
    _phy_intr_pin_polarity();

    printf("trigger/clear(t/c):");
    _DRAIN_STDIN();
    chr = getchar();
    _DRAIN_STDIN();
    ...
    case 't': case 'T':
        ...
        _phy_intr_enable_all(1);

        //enable TSensor intr
        ERR_RET_COND(MTD_OK != mtdHwXmdioRead(pMtd3310Dev, MTD_PORT_0,
            phy_intr_regs[4].dev, phy_intr_regs[4].reg, &val), FAILED, "Read failed.\n");

        val |= 1 << 7;
        ERR_RET_COND(MTD_OK != mtdHwXmdioWrite(pMtd3310Dev, MTD_PORT_0,
            phy_intr_regs[4].dev, phy_intr_regs[4].reg, val), FAILED, "Write failed.\n");

        //set threshold to lower than current
        ERR_RET_COND(MTD_OK != mtdHwXmdioRead(pMtd3310Dev, MTD_PORT_0,
            phy_intr_regs[6].dev, phy_intr_regs[6].reg, &val), FAILED, "Read failed.\n");

        val = (((val & 0xff) - 5) << 8) | (val & 0xff); /* set thresh to less 5 than current */
        ERR_RET_COND(MTD_OK != mtdHwXmdioWrite(pMtd3310Dev, MTD_PORT_0,
            phy_intr_regs[6].dev, phy_intr_regs[6].reg, val), FAILED, "Write failed.\n");
        break;
    case 'c': case 'C':
        _phy_intr_enable_all(0);

        //disable TSensor intr
        ERR_RET_COND(MTD_OK != mtdHwXmdioRead(pMtd3310Dev, MTD_PORT_0,
            phy_intr_regs[4].dev, phy_intr_regs[4].reg, &val), FAILED, "Read failed.\n");
...
static long _phy_intr_test(void)
{
    ...
    _phy_intr_regs_dump(NULL);
    _phy_intr_gpio_dump(NULL);

    hr_cpld_intr_status(&status, &enable);
    printf("CPLD intr status:0x%08x\n", (uint32_t)status);
    printf("CPLD intr enable:0x%08x\n", (uint32_t)enable);

    printf("PHY INTR is not used currently, placeholder for later.\n");

    return(PASSED);
}

int phy_show_phy_dev_info(const char *tag)
{
    MTD_DEV *pMtdDev = pMtd3310Dev;
    MTD_U16 port     = MTD_PORT_0;
    MTD_U16 device_identifier;
    ...
    if (MTD_OK != mtdHwGetPhyRegField(pMtdDev, port, 1, 0x0003, 0, 16, &device_identifier)) {
        fprintf(stderr, "%s:%d:Failed to get phy device info.\n", __FILE__, __LINE__);
        return(FAILED);
    }

    if (MTD_OK != mtdGetFirmwareVersion(pMtdDev, port, &major, &minor, &inc, &test)) {
        fprintf(stderr, "%s:%d:Failed to get phy firmware version.\n", __FILE__, __LINE__);
        return(FAILED);
    }
    ...
    printf("%-16s: 0x%hx, 0x%hx, 0x%hx, %d.%d.%d.%d\n", tag, revision, model_num, uniq_id, major, minor, inc, test);
