/* $Id: ngio_testcard.c,v 1.2 2021/04/15 00:52:26 achiu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/phoenix/ngio_testcard.c,v $
 *------------------------------------------------------------------
...
#define PHOENIX_NGWIC1_PCIE_P 10
#define PHOENIX_NGWIC2_PCIE_P 8
...
static int init_tc_pci_i2c_struct (n2g_i2c_if_t *i2c_if, int if_type, int slot)
{
    ...
    switch (slot) {
        case NGWIC1_SLOT:
            tc_real_pcie_port = PHOENIX_NGWIC1_PCIE_P;
            break;

        case NGWIC2_SLOT:
            tc_real_pcie_port = PHOENIX_NGWIC2_PCIE_P;
            break;

        default:
            printf("%s:%d Invalid %s slot number (%d).",
                   __func__, __LINE__, i2c_if->dev_name, slot);
            return (FAILED);
    }
...
boolean eth_is_linkup (int port)
{
    ...
    if (port == 1) {
        sprintf(eth, "%s", PHOENIX_ETH0_BP); 
    } else if (port == 2) {
        sprintf(eth, "%s", PHOENIX_ETH1_BP); 
    } else {
        printf("Error: unsupported slot=%d\n", port);       
        return (FALSE);
    }

    sprintf(file_name, "/sys/class/net/%s/operstate", eth); 
