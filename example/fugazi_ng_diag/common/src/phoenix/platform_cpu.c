/* $Id: platform_cpu.c,v 1.2 2021/04/15 00:52:27 achiu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/phoenix/platform_cpu.c,v $
 *------------------------------------------------------------------
 *
 * platform_cpu.c
 *
...
int cpu_core_test (int do_more_test)
{
    int rc = FAILED;
    char *tname = "CPU core";
    int phoenix_cpu_num = C3758_CPU_NUM;

    testname("%s", tname);
    prpass(testpass, "%s, ", tname);
    
    /* check CPU SKU */
    if(system("cat /proc/cpuinfo | grep -e C3558 > /dev/null") == 0) {
        phoenix_cpu_num = C3558_CPU_NUM;
    }

    if (linux_cpu_core_test(phoenix_cpu_num) != PASSED) {
