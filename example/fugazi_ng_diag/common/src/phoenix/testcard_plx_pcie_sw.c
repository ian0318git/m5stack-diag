/* $Id: testcard_plx_pcie_sw.c,v 1.2 2021/04/15 00:52:27 achiu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/phoenix/testcard_plx_pcie_sw.c,v $
 *------------------------------------------------------------------
...
int phoenix_plx_pcie_link_up_test(void);
...
int testcard_pcie_linkup_test (void)
{
    ...
    return (phoenix_plx_pcie_link_up_test());
}
...
int testcard_pcie_linkup_test_wrapper (int slot)
{
    ...
    sprintf(cmd, "/opt/script/reset_pcie_lane.sh %x", bus);
    ...
    if (testcard_pcie_linkup_test() == FAILED) {
        error = 1;
            ...
            for (retry = 0; retry < 3 ; retry++) {
                ...
                /* the function-level reset should be no less than 100 ms */
                printf("\nreset PCIe for linkup recovery ...\n");
                system(cmd);
...
/*******************************************************************************
 *
 * Function   : phoenix_plx_pcie_link_up_test
...
int phoenix_plx_pcie_link_up_test (void) 
{
    ...
    if (reg_val == FAILED) {
        printf("\nError: Can't get PCI cap pointer\n");
        return (FAILED);
    }
