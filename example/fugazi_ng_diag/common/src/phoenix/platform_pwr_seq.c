/* $Id: platform_pwr_seq.c,v 1.2 2021/04/15 00:52:27 achiu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/phoenix/platform_pwr_seq.c,v $
 *------------------------------------------------------------------
...
int pwr_seq_pwr_led_green(void);
int pwr_seq_pwr_led_amber(void);
int pwr_seq_pwr_led_off(void);
int pwr_seq_pwr_led_reg_read(uint16_t *);
int pwr_seq_pwr_led_reg_write(uint16_t);

/* Tabei-L - scratchpad register table */
...
static reg_info_t pwr_seq_table[]=
{
	...
	READ_ONLY | SAVE_RESTORE | REG_ACCESS, {(uint)REG_EXT},
	0x0000, 0x0000},
    {"VP1P6_VP1P95_CPU Voltage : Latest Reading",    PWR_SEQ_1_95_CPU_LR,
	READ_ONLY | SAVE_RESTORE | REG_ACCESS, {(uint)REG_EXT},
	0x0000, 0x0000},
    {"VP1P6_VP1P95_CPU Voltage: Maximum Reading",    PWR_SEQ_1_95_CPU_MAX,
	READ_ONLY | SAVE_RESTORE | REG_ACCESS, {(uint)REG_EXT},
	0x0000, 0x0000},
    {"VP1P6_VP1P95_CPU Voltage: Minimum Reading",    PWR_SEQ_1_95_CPU_MIN, 
	READ_ONLY | SAVE_RESTORE | REG_ACCESS, {(uint)REG_EXT},
	0x0000, 0x0000},
    {"VP1P6_VP1P95_CPU Voltage: Ramp Time Maximum and Reading",  PWR_SEQ_1_95_CPU_RPT,
	READ_ONLY | SAVE_RESTORE | REG_ACCESS, {(uint)REG_EXT},
	0x0000, 0x0000},
...
static pwr_seq_v_t voltage_test_table[] =
{
    ...
    {" VP1P6_VP1P95_CPU Voltage ",  PWR_SEQ_1_95_CPU_MAX_OP_THRE, PWR_SEQ_1_95_CPU_MIN_OP_THRE,
    PWR_SEQ_1_95_CPU_LR,  PWR_SEQ_1_95_CPU_MAX,  PWR_SEQ_1_95_CPU_MIN},
    ...
    {" VP12P0 Current from PSU2 ",  PWR_SEQ_12_0_C_PSU2_MAX_OP_THRE, PWR_SEQ_12_0_C_PSU2_MIN_OP_THRE,
...
/*
 * Power Sequencer Test Menu
 */
static submenu_xtable_t pwr_testmenu_table[] = {
    {"Power Sequencer Utilities",       (PFT)build_pwr_seq_menu, TRUE,
      0, (type_t(*)())0, 0, (PFT)0, 0 },
...
#define PWR_TESTMENU_TABLE_SIZE (sizeof(pwr_testmenu_table) / sizeof(submenu_xtable_t))

/*
 * Primary & secondary submenu items (filled in from xtable)
 */
static mitem_t pwr_testmenu_primary_items[PWR_TESTMENU_TABLE_SIZE + MAX_BASE_ITEMS];
static mitem_t pwr_testmenu_secondary_items[PWR_TESTMENU_TABLE_SIZE + MAX_BASE_ITEMS];

static struct menuinfo pwrdiagtest = {
    "Power Sequencer Test Menu",	/* title */
    0,				/* title string added by init_empty_menu */
    (PFT)menu_show_dflags,	/* shows major flags */
    0,				/* generic prompt */
    0,				/* size -- bumped by add_menu_item() */
    pwr_testmenu_primary_items,
};

static struct menuinfo *pwrdiagtestp = &pwrdiagtest;
 ...
 * Description: Build Power Sequencer utilities menu.
 * Inputs     : None
 * Outputs    : None
 *
 *******************************************************************************
...
void build_pwr_seq_menu (int menu_opt)
{
    build_primary_submenu(pwr_menu_table, PWR_MENU_TABLE_SIZE,
			  "Power Sequencer Utilities Menu", &pwrdiagp);
...
/*******************************************************************************
 *
 * function   : build_pwr_seq_tst_menu
 * Description: Build Power Sequencer test menu.
 * Inputs     : None
 * Outputs    : None
 *
 *******************************************************************************
 */
void build_pwr_seq_tst_menu (int menu_opt)
{
    build_primary_submenu(pwr_testmenu_table, PWR_TESTMENU_TABLE_SIZE,
			  "Power Sequencer Test Menu", &pwrdiagtestp);
    build_secondary_submenu(pwr_testmenu_table, PWR_TESTMENU_TABLE_SIZE,
			    pwr_testmenu_secondary_items);

    if (menu_opt) {
        /* Entered with submenu */
        menu(&pwrdiagtest, pwr_testmenu_secondary_items, 0);
    } else {
        do_all_menu_items(pwrdiagtestp);
    }
}
...
static int pwr_reg_test (int submenu)
{
    ...
    */    
    ...
    /* Ported from Utah. to test scratched register is enough to verify 
...
static int pwr_vtg_check(int submenu)
{
	...
	while (vtg_p->volt_p) {
        ...
        /* Got all voltages. Check Real Time is within Max */
	    if (rt > max) {
...
int CyBtldr_TransferData(unsigned char* inBuf, int inSize, unsigned char* outBuf, int outSize)
{
    ...
    /* I2c Read */
    i2c_if.size = outSize;
    memset(outBuf, 0, sizeof(unsigned char));
...
/* Phoenix PWR LED control */
int pwr_seq_pwr_led_green(void)
{
    return pwr_seq_write_reg(PWR_SEQ_PWR_LED_REG, PWR_SEQ_PWR_LED_GREEN);
}

int pwr_seq_pwr_led_amber(void)
{
    return pwr_seq_write_reg(PWR_SEQ_PWR_LED_REG, PWR_SEQ_PWR_LED_AMBER);
}

int pwr_seq_pwr_led_off(void)
{
    return pwr_seq_write_reg(PWR_SEQ_PWR_LED_REG, PWR_SEQ_PWR_LED_OFF);
}

int pwr_seq_pwr_led_reg_read(uint16_t *data)
{
    return pwr_seq_read_reg(PWR_SEQ_PWR_LED_REG, data);
}

int pwr_seq_pwr_led_reg_write(uint16_t data)
{
    return pwr_seq_write_reg(PWR_SEQ_PWR_LED_REG, data);
}
