/* $Id: diag_eth_pkt_txrx_api.c,v 1.2 2021/04/15 00:52:24 achiu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/phoenix/diag_eth_pkt_txrx_api.c,v $
 *------------------------------------------------------------------
...
/************************************************************************
 * Function: set_host_eth_bridge
 *           Setup host ethernet bridge for DSP firmware download
 * Input: None 
 *
 ************************************************************************
 */
void set_host_eth_bridge (void)
{
    uint8_t src_mac[6] = {0}; 
    char mac[128];
    ...
    /* ifconfig eth-N up */
    /* brctl addbr br0 */
    sprintf (mac, "brctl addbr br0");
    system(mac);    
    sprintf (mac, "brctl addif br0 eth%d", DSP_SLOT0_ETH_INTF);
    system(mac);    
    sprintf (mac, "brctl addif br0 eth%d", DSP_SLOT1_ETH_INTF);
    system(mac);    
    ...
    sprintf(mac,"ifconfig br0 hw ether %x:%x:%x:%x:%x:%x",(uint8_t)src_mac[0],
           (uint8_t)src_mac[1],(uint8_t)src_mac[2],(uint8_t)src_mac[3],
    ...
    system(mac);
    /* ifconfig br0 up */ 
    sprintf (mac, "ifconfig br0 192.123.123.1 up");
    system(mac);    
    sprintf (mac, "ifconfig eth%d up",DSP_SLOT0_ETH_INTF);
    system(mac);    
    sprintf (mac, "ifconfig eth%d up",DSP_SLOT1_ETH_INTF);
    system(mac);    
}
