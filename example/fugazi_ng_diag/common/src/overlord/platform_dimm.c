/* $Id: platform_dimm.c,v 1.10 2018/05/18 09:24:51 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/platform_dimm.c,v $
 *------------------------------------------------------------------
...
extern uint32_t init_dimm_i2c_struct(n2g_i2c_dev_t *, uint32_t);
...
/******************************************************************************
 *
 * Function   :	get_cp_memsize
 * Description:	Dump CP memsize
 * Inputs     :	Dimm number
 * Outputs    :	PASSED/FAILED
 *
 ******************************************************************************/
uint32_t get_cp_memsize (int sel_dimm)
{
    uint32_t rc = FAILED, dimmsize;
    char strbuf[8];
    n2g_i2c_dev_t i2c_dev;

    /* Init I2C device structure */
    rc = init_dimm_i2c_struct(&i2c_dev, sel_dimm);
    if (rc != PASSED) {
    ...
    dimmsize = dump_dimm_info(&i2c_dev, DIMM_DUMP_MEMSIZE);

    /* Only 2, 4 and 8 GB are supported on O2*/
    switch(dimmsize) {
    case DIMM_2GB:
        sprintf(strbuf, "2GB");
    break;
    ...
    break;
    default:
        cterr('f', 0, "Not supported this dimm size"
                      "on DIMM%d", sel_dimm);
        return (FAIL);
    break;
    }

    printf("Intel memory size on DIMM%d is %s\n", sel_dimm, strbuf);
...
static uint32_t dump_dimm_info (n2g_i2c_dev_t *dev, uint32_t cmd)
{
    ...
    /* CSCuz22902 : Fixed Goldbeach DIMM SPD display issue*/
...
static uint32_t dump_dimm_info_wrap (int opt)
{
    ...
    /* Init I2C device structure */
    rc = init_dimm_i2c_struct(&i2c_dev, dimm_no);
    if (rc != PASSED) {
        cterr('f', 0, "Init DIMM i2c_dev struct failed"
                      "(return code = %#x).", rc);
        return (rc);
...
static uint32_t alter_dimm_data_wrap (int opt)
{
    ...
    if (rc != PASSED) {
        cterr('f', 0, "Alter DIMM data failed(return code = %#x).", rc);
    }
    return (rc);
}
#endif
...
Revision 1.9  2017/08/10 10:10:41  iachang
CSCvf44161: Merge Goldbeach into USD platform as one image

Revision 1.8  2017/07/10 02:51:58  leschen
Remove unused variable

Revision 1.7  2016/10/16 12:28:18  iachang
Supported Goldbeach Platform.

Revision 1.6.64.2  2018/05/17 10:50:22  alpeng
 sync with trunk <trunk-051618>


Revision 1.6.64.1  2017/04/05 09:15:16  leschen
...
Revision 1.6  2013/07/03 03:32:23  alpeng
fixed i2c to byte access, both O2 and Utah is working

Revision 1.5  2012/11/12 03:44:21  alpeng
fixed supported size of DIMM on O2

Revision 1.4  2012/09/26 06:11:13  alpeng
display DIMM size on CLI discovery

Revision 1.3  2012/05/31 21:21:02  palin2
