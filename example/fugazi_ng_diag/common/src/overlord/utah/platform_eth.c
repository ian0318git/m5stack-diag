/* $Id: platform_eth.c,v 1.19 2016/10/16 12:28:22 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/utah/platform_eth.c,v $
 *------------------------------------------------------------------
...
static void utah_phy_reg_access(void);
static int utah_phy_select(int);
static void dump_phy_reg(int, const mrvl_phy_regs_t *);
static void sgmii_phy_reg_dump(void);
static boolean is_item_o2_juno(void);
...
static submenu_xtable_t eth_util_submenu_table[] = {
	...
	F_GRP_E, (type_t(*)())is_us_machines, 0, 
        (type_t(*)())0,   0},
	...
	F_GRP_E, (type_t(*)())is_us_machines, 0, 
        (type_t(*)())0,   0},
    {"PHY (1548) external loopback test",
        (type_t(*)()) dagger_phy_ext_lpbk_test,   0,
	F_GRP_E, (type_t(*)())is_dg_machines, 0, 
        (type_t(*)())0,   0},
    {"PHY (1548) internal loopback test",
        (type_t(*)()) dagger_phy_int_lpbk_test,   0,
	F_GRP_E, (type_t(*)())is_dg_machines, 0,
        (type_t(*)())0,   0},
    {"Media PHY (1548) test mode util",
        ...
        0,       (type_t(*)())0,         0, 
        (type_t(*)())0,   0},
    {"SGMII/SFP PHY loopback util",
        (type_t(*)()) ovld_phy_lpbk_util,   0,
        0, (type_t(*)())is_us_machines,  0,
        (type_t(*)())0,   0},
    {"SGMII/SFP PHY loopback util",
        (type_t(*)()) dagger_phy_lpbk_util, 0,
        0, (type_t(*)())is_dg_machines,       0,
        (type_t(*)())0,   0},
    {"Send packet Util",
	...
	F_GRP, (type_t(*)())0,           0,
        (type_t(*)())0,   0},
    {"1548&1340 PHY eye enlarge", 
        (type_t(*)())utah_marvell_phy_eye_enlarge,   0,
	F_GRP, (type_t(*)())0,           0, 
        (type_t(*)())0,   0},
    {"MACsec port test",
        (PFT)macsec_test_main,              1,
        F_ALL_E, (type_t(*)())is_item_o2_juno, 0,
        (type_t(*)())macsec_test_main,   0},
...
static submenu_xtable_t eth_port_tests_submenu_table[] = {
   ...
   {"SGMII PHY Ext/Internal loopback test",
	...
	F_ALL_E, (type_t(*)())is_us_machines, 0, (type_t(*)())0,   0},
   {"Dagger SGMII PHY Ext/Internal loopback test",
        (type_t(*)()) dagger_phy_int_ext_lpbk_test,   0,
	F_ALL_E, (type_t(*)())is_dg_machines, 0, (type_t(*)())0,   0},
   {"SFP External loopback test", 
        (type_t(*)()) sfp_phy_ext_lpbk_test,   0,
        F_ALL_E, (type_t(*)())is_us_machines , 0, (type_t(*)())0,   0},
   {"Dagger SFP External loopback test", 
        (type_t(*)()) dagger_sfp_phy_ext_lpbk_test,   0,
        F_ALL_E, (type_t(*)())is_dg_machines, 0, (type_t(*)())0,   0},
   /* ---------- Below are utilities -----------*/
    {"Ethernet port utility menu",
...
/*------------------------------------------------------------------
 *
 * Function: is_item_o2_juno
 *         item is available on o2 or juno
 *
 ...
 */
boolean
is_item_o2_juno (void)
{
    return (is_overlord() || is_juno());
}
 ...
 */
int
utah_phy_reg_wr(uint portnum, ushort regnum, ushort regval)
{
 ...
 */
int
utah_phy_reg_rd(uint portnum, ushort regnum, ushort *buf)
{
    struct mii_ioctl_data *miip;
    int sk;
    struct ifreq ethreq;
 ...
 */
static int 
utah_phy_select (int mdio_bus_num)
{
    struct mii_ioctl_data *miip;
    int sk, port;
    struct ifreq ethreq;
...
void utah_marvell_1340_init (void)
{
    ...
    for (port=0; port <= SGMII2; port++) {
        ...
        utah_phy_select(phy_addr);

        /* Init sequence #1
         */
        utah_phy_reg_wr(port, OVLD_PHY_PAGE22, 0x00ff);
        utah_phy_reg_wr(port, PHY_REG(24), 0x2800);
        utah_phy_reg_wr(port, PHY_REG(23), 0x2001);
        utah_phy_reg_wr(port, OVLD_PHY_PAGE22, 0x0000);

        /* Check init sequence #1
         */
        utah_phy_reg_wr(port, OVLD_PHY_PAGE22, 0x00ff);
        utah_phy_reg_wr(port, PHY_REG(23), 0x1001);
        utah_phy_reg_rd(port, PHY_REG(25), &rdval);
        utah_phy_reg_wr(port, OVLD_PHY_PAGE22, 0x0000);

        /* Init sequence #2 for the BGA package
         */
        utah_phy_reg_wr(port, OVLD_PHY_PAGE22, 0x0000);
        utah_phy_reg_wr(port, PHY_REG(29), 0x0003);
        utah_phy_reg_wr(port, PHY_REG(30), 0x0002);
        utah_phy_reg_wr(port, PHY_REG(29), 0x0000);

        /* Check init sequence #2
         */
        utah_phy_reg_wr(port, PHY_REG(29), 0x0003);
        utah_phy_reg_rd(port, PHY_REG(30), &rdval);
        utah_phy_reg_wr(port, PHY_REG(29), 0x0000);
        ...
        rdval &= ~0x0800;
        utah_phy_reg_wr(port, PHY_REG(0), rdval);
        utah_phy_reg_rd(port, PHY_REG(0), &rdval);
...
{
    int port, phy_addr, phynum, busoff, max_phy_nu;
    ushort rdval;

    /* dagger has only on PHY 1548 */
    if (is_dagger()) {
         max_phy_nu = 1;
    } else {  /* Utah, sword */
         max_phy_nu = 2;
    ...
    /* we have 2 PHY */
    for (phynum = 0; phynum < max_phy_nu; phynum++) {
           
            if (phynum == 0) {
                /* 1548 PHY doesn't need offset */
                busoff = 0;
            } else {
                /* 1340 PHY mdio bus offset */
                busoff = ADDR_BRIDGE_PHY;
            }

        for (port = 0; port <= SGMII3; port++) {
            phy_addr = port + busoff;

            /* select mdio bus to bridge or media PHY */
            utah_phy_select(phy_addr);

            utah_phy_reg_wr(port, OVLD_PHY_PAGE22, 0x0001);
            utah_phy_reg_rd(port, PHY_REG(26), &rdval);
            rdval &= ~0x7;
            rdval |= 0x5;
            utah_phy_reg_wr(port, PHY_REG(26), rdval);
            utah_phy_reg_wr(port, OVLD_PHY_PAGE22, 0x0000);
...
void dagger_sgmii_setting (void) {
    ...
    for (port = SGMII0; port <= SGMII3; port++) {
       ...
       if (port == SGMII0) {
           ...
           utah_phy_reg_wr(port, PHY_REG(22), 0x0004);
           utah_phy_reg_rd(port, PHY_REG(27), &rdval);
           rdval |= 0x4003;

           utah_phy_reg_wr(port, PHY_REG(27), rdval);
       }
       /* end of 1st part */

       /* 2nd part: mode selection */
       /* page 18, reg 20 port0 and port2 for SGMII to QSGMII media */
       utah_phy_reg_wr(port, PHY_REG(22), 0x0012);
       ...
       utah_phy_reg_wr(port, PHY_REG(20), wrval);
       ...
       wrval = 0x1;
       utah_phy_reg_wr(port, PHY_REG(22), wrval);
       wrval = 0x1140;
       utah_phy_reg_wr(port, PHY_REG(0), wrval);
       wrval = 0x0;
       utah_phy_reg_wr(port, PHY_REG(22), wrval);
       /* end of 2nd part */
    
#if 0  /* old setting, it is done on above 2nd part */
       /* 2nd part: mode selection */
       /* page 18, reg 20 port0 and port2 for SGMII to QSGMII media */
       utah_phy_reg_wr(port, PHY_REG(22), 0x0012);
       utah_phy_reg_rd(port, PHY_REG(20), &rdval);

       /* clean up mode bits: [0:2] */
       rdval &= ~0x7;

       /* port0 and port2 set to SGMII to QSGMII */
       if (port == SGMII0 || port == SGMII2) {
           rdval |= 0x5; 
       }

       /* port1 and port3 set to QSGMII to automedia */
       if (port == SGMII1 || port == SGMII3) {
           rdval |= 0x7; 
       }
       utah_phy_reg_wr(port, PHY_REG(20), rdval);
