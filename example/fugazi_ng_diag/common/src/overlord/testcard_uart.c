/* $Id: testcard_uart.c,v 1.10 2020/05/22 02:28:35 qingcwan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/testcard_uart.c,v $
 *------------------------------------------------------------------
...
static void tc_uart_console(void); 

extern int do_all_menu_items(struct menuinfo *);

int __attribute__((weak)) adapter_uart_lpbk_txrx(int port, char* test_str,
...
static submenu_xtable_t tc_uart_diag_tbl[] = {
    ...
    {"UART External loopback test", (PFT)tc_uart_lpbk_test,     TC_EXT_LPBK,
     ...
     (PFT)0,                        0},
    {"UART console switch ",        (PFT)tc_uart_console,       0, 
     0,                             (PFT)0,                     0,
     (PFT)0,                        0},
};
...
int tc_uart_lpbk_test (int test_type)
{
    ...
    for (ctr = 0; ctr < test_total; ctr++) {
        ...
        if (tc_ngio_p->mod_type == DAUGHTER_CARD) {
            result = adapter_uart_lpbk_txrx(port, tx_str, tx_len, rx_str, &rx_sz,
                                            atoi(tc_uart_test_baud[ctr].name),
                                            TRUE);
        } else {
            result = uart_lpbk_txrx(port, tx_str, tx_len, rx_str, &rx_sz,
                                     atoi(tc_uart_test_baud[ctr].name),
                                     TRUE);
        }
        if (result == FAILED) {
...
static void tc_uart_console (void) 
{
     ...
     printf("testcard uart controller on host fpga is %d\n", 
             testcard_if_p->uart_ctrl); 
     printf("Please connect console line to testcard RJ45 console port\n"); 
     printf("The uart data path is : \n"); 
     printf("PC uart console (baud rate 9600 -> uart cable -> \n"); 
     printf("testcard uart (RJ45) -> inter connection to host fpga uart controller ->\n"); 
     printf("platform console\n"); 
     printf("Press any key to monitor what you input                  \n"); 
     printf("Press Ctrl-AX to leave go back to menu \n"); 

     sprintf(buf, "picocom /dev/ttyDASH%d", testcard_if_p->uart_ctrl); 
     system(buf);    
...
Revision 1.10  2020/05/22 02:28:35  qingcwan
Merge switzer-carrier code into main chunk.

Revision 1.9  2019/08/06 06:56:11  alpeng
merge curie, switzer and nightwatch to trunk

Revision 1.8.2.1  2018/12/05 01:48:52  alpeng
add new uart util for testcard

Revision 1.8  2018/05/18 09:24:52  alpeng
 Neptune merge to trunk with tag <neptune-branch-0518>

Revision 1.7.48.1  2016/12/15 03:42:04  alpeng
 fix old testcard uart issue, using api to get uart_ctrl num

Revision 1.7  2014/02/22 05:06:07  mcharon
add uart test that bypassse tty driver

Revision 1.6  2014/01/27 21:36:01  mcharon
break uart_lpbk_test into 3 separate functions: init/send/receive

Revision 1.5  2013/12/16 02:10:06  hroni
uart_lpbk_test() support custom compare string

Revision 1.4  2013/10/16 04:31:50  hroni
...
Revision 1.3  2013/08/14 17:55:35  hroni
...
Revision 1.7  2012/09/24 17:37:43  palin2
1. Use "Internal loopback test" as default test for TestCard.
2. Unify all tests print out format for TestCard.

Revision 1.6  2012/08/22 16:31:34  palin2
Add one more argument based on the update of function "uart_intf_test".

Revision 1.5  2012/08/20 13:22:58  palin2
Add NGSM TestCard support from Host side(Overlord) DiagMenu.

Revision 1.4  2012/08/14 11:30:55  palin2
Removed "ovld_" from TestCard related filename because TestCard is not Overlord's unique.

Revision 1.3  2012/08/08 22:19:41  palin2
