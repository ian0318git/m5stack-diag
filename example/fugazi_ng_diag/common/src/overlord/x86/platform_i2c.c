/* $Id: platform_i2c.c,v 1.53 2019/08/26 03:36:11 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/x86/platform_i2c.c,v $
 *-----------------------------------------------------------------------------
...
static n2g_i2c_if_t fpga_i2c_dev[] = {
    ...
    {
        .dev_name = "System EEPROM",
        .offset = 0,
        .i2c_bus_type = IOFPGA_I2C,
        .i2c_dev = MB_I2C_ADDR_SYS_EEPROM,
        .i2c_ctrl = I2C_CTRL_ZERO,
        .sub_addr_len = 0,  /* dont' use sub address slave register */
    ...
    {
        .dev_name = "Voltage Monitor",
        .offset = 0,
        .i2c_bus_type = IOFPGA_I2C,
        .i2c_dev = MB_I2C_ADDR_VTG_MNTR,
        .i2c_ctrl = I2C_CTRL_TWO,
        .sub_addr_len = 0,
        .size = sizeof(uint16_t),
        .mux = I2C_MUX_ZERO,
...
static submenu_xtable_t i2c_menu_table[] = {
    ...
    {"Show Barometer Info",            (PFT)show_barometer_info,        TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
    {"Environment MCU",	               (PFT)build_env_menu,             TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
    ...
    {"PCIe Clock",                     (PFT)build_pcie_clk_test_menu,   TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
    {"256 byte EEPROM",                (PFT)build_eeprom_menu,          TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
    ...
    {"Power Sequencer",                (PFT)build_pwr_seq_menu,         TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
    {"Voltage Monitor",	               (PFT)build_vtg_mntr_menu,        TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
    {"PSU1/PEM PSU1",                  (PFT)build_pem_psu_menu,         OVLD_PSU1_TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
    {"PSU2/PEM PSU2",                  (PFT)build_pem_psu_menu,         OVLD_PSU2_TRUE,
        0,                             (PFT)0,                          0,
      (PFT)0,                          0},
...
int ovld_x86_i2c_scan_test (int option)
{
    ...
    for (now_test = 0, test_num = 1; now_test < test_end; now_test++) {
        ...
        /* PSU modules used in Overlord and Juno are different.
         */
        /* o2 skips juno i2c structures */
        if (is_overlord() &&
            ( (strcmp(fpga_i2c_dev[now_test].dev_name,"PEM0 EEPROM") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PEM0 Microcontroller") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PEM1 EEPROM") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PEM1 Microcontroller") == 0 ) ) )
            continue;

        /* juno skips o2 i2c structures 
         * PoE PSU module can not be used in Juno
         */
        if (!is_overlord() &&
            ( (strcmp(fpga_i2c_dev[now_test].dev_name,"PSU1 EEPROM") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PSU1 Microcontroller") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PSU2 EEPROM") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PSU2 Microcontroller") == 0) || 
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PoE PSU1 EEPROM") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PoE PSU1 Microcontroller") == 0) || 
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PoE PSU2 EEPROM") == 0) ||
              (strcmp(fpga_i2c_dev[now_test].dev_name,"PoE PSU2 Microcontroller") == 0) ) ) 
            continue;

        /* juno-plx skips pcie switch; the others skip plx swtich */
        if (is_juno_plx() &&
            ( (strcmp(fpga_i2c_dev[now_test].dev_name,"PCIe Switch") == 0) ) )
            continue;
        if (!is_juno_plx() &&
            ( (strcmp(fpga_i2c_dev[now_test].dev_name,"PLX PCIe Switch") == 0) ) )
            continue;
           ...
           if (psu_skip == TRUE) {
               if ((fpga_i2c_dev[now_test].mux == I2C_MUX_ZERO) ||
                   ...
                   if (psu_pre != TRUE) {
                       ...
                       /* Juno psu  micro controller readable address on i2c 
                        *  is not start from 0x0 */
                       if (is_juno()) {
        ...
        if ((i2c_if.i2c_dev == MB_I2C_ADDR_POE_30W_CTRLER) ||
            ...
            if (force_skip_30wpoe()) {
                if ((NVRAM)->diagflag & D_VERBOSE) {
                    printf("\n30wpoe scan is skipped by user \n");
                }
                continue;
            }
    ...
    /* DIMM */
    for (test_ctr = 0; test_ctr < OVLD_X86_DIMM_NUM; test_ctr++) {
        if ((NVRAM)->diagflag & D_VERBOSE) {
            printf("CaveCreek I2C device: DIMM%d I2C scan test.\n", test_ctr);
        } else {
            prpass(testpass, "[%2d] CaveCreek I2C device: DIMM%d, ",
                             test_num, test_ctr);
        }
        test_num++;
    ...
    /* PCIe Clock */
    if ((NVRAM)->diagflag & D_VERBOSE) {
	printf("CaveCreek I2C device: PCIe Clock I2C scan test.\n");
    } else {
        prpass(testpass, "[%2d] CaveCreek I2C device: PCIe Clock, ", test_num);
    }
    test_num++;
    ...
    /* System Clock */
    if ((NVRAM)->diagflag & D_VERBOSE) {
	printf("CaveCreek I2C device: System Clock I2C scan test.\n");
    } else {
        prpass(testpass, "[%2d] CaveCreek I2C device: System Clock, ",
                         test_num);
    }
    test_num++;
 
    if (ovld_sys_clk_i2c_scan_test(errbuf) != PASSED) {        
        fail_ctr++;
        err_code = i2c_err_no(&status);
...
Revision 1.53  2019/08/26 03:36:11  alpeng
CSCvq64781 - dimm1 is optional, provide dimm1 option arg for MFG

Revision 1.52  2018/05/18 09:25:01  alpeng
 Neptune merge to trunk with tag <neptune-branch-0518>

Revision 1.50.20.2  2018/05/17 10:50:23  alpeng
 sync with trunk <trunk-051618>

Revision 1.51  2017/07/10 02:27:50  leschen
