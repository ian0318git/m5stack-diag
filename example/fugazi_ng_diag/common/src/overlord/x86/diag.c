/* $Id: diag.c,v 1.48 2019/08/06 06:56:16 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/x86/diag.c,v $
 *------------------------------------------------------------------
 *
...
extern void build_env_mcu_menu(void);
...
static struct mitem cookie_items[] = {
    ...
    {"program cavecreek MAC",    	          0,	0,
     (type_t(*)())program_mac,	  &zero,	0, (type_t(*)())0, 0},
    {"program cavecreek eeprom ",    	          0,	0,
     (type_t(*)())program_cavecreek_eeprom,	  &zero,0, (type_t(*)())0, 0},
...
static struct mitem utilmenuitems[] = {
    ...
    {"FAN utilities",
     0,                 0,
     (PFT)build_fan_utils_menu, (type_t *)&zero,                0,
     (type_t(*)())0,            0},
    {"Mother LED utility",
     0,                      0,
     (PFT)menu,              (type_t *)&led_menup,      0,
     (type_t(*)())0,         0},
    {"ENV mcu download utility",
     0,			0,
     (PFT)build_env_mcu_menu,(type_t *)&one,		0,
     (type_t(*)())0,		0},
    {"UART utility",          0, 0,
...
submenu_xtable_t main_menu_table[] = {
    ...
    {"test SM Slot 1",
     ...
     (type_t(*)())is_overlord, 0,	(PFT)sm_test, FIRST_SLOT + MAX_SM},
    {"test SM Slot 2",
     (PFT)sm_test,		FIRST_SLOT+1,		MM_2,
     (type_t(*)())is_overlord, 0,	(PFT)sm_test, FIRST_SLOT+1 + MAX_SM},
    {"test VM",
     (PFT)vm_test,		FIRST_SLOT,		MM_2,
     (type_t(*)())0, 0,		(PFT)vm_test, FIRST_SLOT+ MAX_VM},
 ...
 */
static
int program_mac(int user)
{
    uchar buf[15] = { 0,0,0,0,0,0 }; /* has to be at least 6 */
    int32_t size = 0;
    unsigned int port = 0;

    if (user) {
        printf("please enter 6-byte MAC address (ie, port_num aa bb cc dd ee ff)\n");
        fscanf(stdin, "%d %c %c %c %c %c %c\n", &port,  &buf[0], &buf[1], &buf[2], &buf[3],
               &buf[4], &buf[5]);
 ...
 *
 * Description: Verify Cavecreeek ports' MAC addresses are correct
 * with respect to the chassis MAC base address in cookie.
 *
 * Input : void
 *                     
...
static int verify_mac_program_result(void)
{
    ...
    char cmd[128];
    FILE *fp;
    char *check_mac_file = "./check_mac.txt";
    char rd_mac[128];
    int rc = FAILED;
    char cookie_mac[128];

    printf("Verify programmed MAC address\n");

    size = get_mac_blk_size()-1;
    if ((size+1) != 144) {
        cterr('f',0,"MAC block size = %d. Should be 144.\n", size);
	printf("Please run 'alter mb cpu cookie' option at least once.\n");
	return(FAILED);
    }
	...
	system(cmd);
        ...
        if (rc == PASSED) {
            printf("eth%d MAC %.17s is correct\n", port, rd_mac);
        } else {
	    cterr('f',0,"eth%d MAC %.17s is not correct. expected - %s\n", port, rd_mac, cookie_mac);
	    return(rc);
        }
    }
    return(rc);
...
/*---------------------------------------------------------------
$Log: diag.c,v $
Revision 1.48  2019/08/06 06:56:16  alpeng
merge curie, switzer and nightwatch to trunk

Revision 1.47.28.1  2018/10/15 10:45:10  alpeng
fixed compile error; add PLUG_FPGA define for i2c, naming confliction for tam lib and act2 lib

Revision 1.47  2017/11/11 01:31:57  ptong
Added O2 cookie util verify_mac_program_result()

Revision 1.46  2017/07/28 07:49:43  harrchan
