/* $Id: platform_macsec.c,v 1.1 2013/01/25 10:47:02 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/cavium/platform_macsec.c,v $
 *------------------------------------------------------------------
 * 
 * platform_macsec.c  
...
static void macsec_reg_wr (uint phy_addr, ushort addr, ushort offset, uint data) {
    ...
    dummy = macsec_reg_rd(phy_addr, addr, offset);
...
static void clr_macsec_cnt (uint port, uint phy_addr) {
    ...
    for (ia = 0; ia < MACSEC_CNT_OFFSET; ia++) {
        dummy = macsec_reg_rd(phy_addr, (ia+IGR_HIT), offset);
    }
}
