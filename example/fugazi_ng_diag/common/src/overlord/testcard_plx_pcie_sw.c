/* $Id: testcard_plx_pcie_sw.c,v 1.15 2020/05/22 02:28:35 qingcwan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/testcard_plx_pcie_sw.c,v $
 *------------------------------------------------------------------
...
static void get_host_pcie_bus_num(void);
static void get_testcard_pcie_bus_num(int, int);
static int is_pcie_port_odd(int, int);
void host_pcie_master_mode(void);
int host_pcie_send_test_patt(int);
...
int curie_plx_pcie_link_up_test(void);
int testcard_pcie_link_up_test_on_adapter(int slot);
...
/*******************************************************************************
 *
 * Function   : get_host_pcie_bus_num
 * Description: get host pcie bus number 
 * Inputs     : None - store into global variable: host_pcie_bus_num
 * Outputs    : None
 *
 *******************************************************************************
 */
static void get_host_pcie_bus_num (void) {
    ...
    if (is_utah_plx() || is_juno_plx()) {
        host_pcie_bus_num = get_pcie_bus_num (PLX_PCIE_SW_VID, PLX_PCIE_SW_DID_8618);
    } else if (is_sword()) {
        host_pcie_bus_num = get_pcie_bus_num (PLX_PCIE_SW_VID, PLX_PCIE_SW_DID_8617);
    } else if (is_dagger()) {
        host_pcie_bus_num = get_pcie_bus_num (PLX_PCIE_SW_VID, PLX_PCIE_SW_DID_8604);
    }
...
static void get_testcard_pcie_bus_num (int mod, int slot) {
    ...
    if (mod == TC_NGWIC) {
        ...
        testcard_pcie_bus_num = get_ngio_pcie_dev_bus_num(SM_DAUGHTER_CARD, slot); 
    }
...
{
    char pcie_info[80];
    char buffer[80], buf[8];
    FILE  *fp;

    char wic_pcie_bus_num[][8] =  {":01.0", ":02.0", ":03.0", ":05.0",
                                   ":0b.0", ":0d.0", ":08.0", ":0a.0",
                                   ":0c.0", ":0e.0", ":07.0", ":09.0", ":03.1" };

    char slot_pcie_bus_num[2][12];

    int nep_p1c_pcie_down_port_bus[6] = {0,  1, 10, 2, 11, 6 }; /* sm1-3 and nim1-3 */
    int nep_p1b_pcie_down_port_bus[6] = {10, 6, 11, 0, 1,  2 }; /* sm1-3 and nim1-3 */
    int *nep_pcie_bus_ptr; 

    char *pcie_lanes =  "x1";
    char *pcie_lanes_by_two =  "x2";
    ...
    /* Legacy plx pcie link check is not flexiable, let's use new method */
    if (is_curie_1ru() || is_curie_2ru()) {
        return (curie_plx_pcie_link_up_test());
    } 
    ...
    if (!is_goldbeach()) { 
        ...
        if ((slot == 4) && (is_ntpn_machines() || is_vg450())) {
            /*
             *  Neptune SM4 slot PCI path is connected to CPU directly
             *  so it upstream port is 00:03.1
             */
            sprintf(plx_bus_num, "0%x", 0x0);
        }
    
        /* Assign pcie bus number for corresponding platforms and wic slot */
        if (is_utah_plx()) {
            if (mod == TC_NGSM) {
            /* for the case that NIM testcard is plugged in Thule SM module */
                if (slot == 1) {
                    /* utah sm slot1  */
                    bus_index = 0;
                    disable_port_reg = 0x234;
                    disable_port_reg_value = 1;
                } else if (slot == 2) {
                    /* utah sm slot2  */
                    bus_index = 1;
                    disable_port_reg = 0x230;
		...
		if (plat_bd_rev < 2) {
            ...
            /* neptune is not plx pcie switch, don't care about 
             * disable_port_reg and disable_port_reg_value 
             */
            if (mod == TC_NGSM) {
            /* for the case that NIM testcard is plugged in Thule SM module */
                if (slot == 1) {
                    bus_index = nep_pcie_bus_ptr[0];
                    disable_port_reg = 0x234;
                    disable_port_reg_value = 0x8;
                } else if (slot == 2) {
                    bus_index = nep_pcie_bus_ptr[1];
                    disable_port_reg = 0x230;
                    disable_port_reg_value = 0x10;
                } else if (slot == 3) {
                    bus_index = nep_pcie_bus_ptr[2];
                    disable_port_reg = 0x234;
                    disable_port_reg_value = 0x10;
                } else {
                    /* sm4 */
                ...
                if (slot == 1) {
                    bus_index = nep_pcie_bus_ptr[3];
                    disable_port_reg = 0x234;
                    disable_port_reg_value = 0x1;
                } else if (slot == 2) {
                    bus_index = nep_pcie_bus_ptr[4];
                    disable_port_reg = 0x230;
                    disable_port_reg_value = 0x2;
                } else {
                    bus_index = nep_pcie_bus_ptr[5];
                    disable_port_reg = 0x234;
                    disable_port_reg_value = 0x2;
                }
            }
        ...
        if ((slot == 4) && (is_ntpn_machines() || is_vg450())) {
            /* using rescan will also rescan to the other parallel ports;
             * for neptune, sm4 is connected to CPU root complex, 
             * doing rescan will also rescan GESW pcie interface.
             */
        } else {
            system(rescan_cmd);
        }
        ...
        if (is_ntpn_machines() || is_vg450()) {
            system("sh /diag_utils/overlord/bin/generic_pcie_lane.sh");
        } else {
    ...
    /* start from secondary bus which is connected to NIM PCIe */
    bus_index = 1;

    while ((fscanf(fp, "%s", pcie_info) != EOF) && (scan_done != 1)) {
       if (strcmp(pcie_info,slot_pcie_bus_num[bus_index]) == 0) {
            /* get into secondary bus, search PLX for testcard pcie switch */
            while ( (fscanf(fp, "%s", pcie_info) != EOF) ) {
                if (strcmp(pcie_info, "PLX") == 0) {
                    plx_exist = 1;
                }
                /* get 'width' to process string 'lane num' which is next string of 'width' */
                if (strcmp(pcie_info, "Width") != 0) {
                    continue; 
                }
                ...
                if (testcard_if_p->is_10gkr == TC_10GKR_SM_ON_SM) {
                    strcpy(buf, pcie_lanes_by_two);  /* NEW SM TC, x2 */  
                } else {
                    strcpy(buf, pcie_lanes);  /* x1 */
                }
                if (strcmp(pcie_info, buf) != 0) {
                    sprintf(buffer, "For bus number %s; detected %s lanes, expected %s lanes.",
                    slot_pcie_bus_num[bus_index], pcie_info, buf);
                    printf("\n%s",buffer);
                    return (FAILED);
                }
                break; 
...
int testcard_pcie_linkup_test_wrapper (int slot)
{
    ...
    if (tc_ngio_p->mod_type == DAUGHTER_CARD) {
        return testcard_pcie_link_up_test_on_adapter(testcard_if_p->slot);
    }
    ...
    if (testcard_pcie_linkup_test() == FAILED) {
        ...
        if (is_plx() || is_ntpn_machines() || is_vg450()) {
...
*/
    ...
    sprintf(lspci_cmd,"lspci -nn | grep -i 10b5:8617 | cut -c 0-7 > %s",fname1);
    system(lspci_cmd);
    fp = fopen(fname1, "r");
    if (fp == NULL) {
    ...
    sprintf(cat_cmd,"cat %s | grep -i /sys/devices/pci0000:00/0000:00 | cut -c 0-36 > %s",fname2,fname3);
    system(cat_cmd);
    fp = fopen(fname3, "r");
    if (fp == NULL) {
...
/*******************************************************************************
 *
 * Function   : curie_plx_pcie_link_up_test
...
int curie_plx_pcie_link_up_test (void) 
{
    ...
    /* check width */ 
    /* SM_ON_SM is 10G-KR SM TC, which is x2 */
    if (testcard_if_p->is_10gkr == TC_10GKR_SM_ON_SM) { 
        if (sta_w == PCI_EXP_LINK_STA_WID_2) {
            prpass(testpass, "Link width is x2 "); 
        } else { 
            cterr('f',0, "Link width should be x2, please check the width"); 
            result = FAILED;  /* fail through */
        }
    /* NIM_ON_SM or NIM_ON_NIM are x1 */
    } else if ((testcard_if_p->is_10gkr == TC_10GKR_NIM_ON_SM) ||
               (testcard_if_p->is_10gkr == TC_10GKR_NIM_ON_NIM)){ 
...
/*******************************************************************************
 *
 * Function   : testcard_pcie_link_up_test_on_adapter
 * Description: switzer_carrier adapter is a ngsm card,
                the pcie link up test is different from the host
 * Inputs     : slot number on switzer_carrier
 * Outputs    : PASSED/FAILED
 *
 *******************************************************************************
 */
int testcard_pcie_link_up_test_on_adapter (int slot)
{
    ...
    /* Check PLX pcie switch bus number */
    bus = get_ngio_pcie_dev_bus_num(tc_ngio_p->mod_type, slot);
    if (bus == UNKNOWN_PCI_BUS_NUM) {
        cterr('f',0, "Unknown PCI bus number");
        return (FAILED);
    }
