/* $Id: dash_mem.c,v 1.7 2012/11/06 20:39:50 mcharon Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/dash_mem.c,v $
 *------------------------------------------------------------------
 *
 * Filename: dash_mem.c
 ...
 */
static void
dash_filbyte(unsigned char *addr, int length, unsigned char val)
{
 ...
 */
static void
dash_filword(unsigned short *addr, int length, unsigned short val)
{
 ...
 */
void
dash_fillword(unsigned int *addr, int length, unsigned int val)
{
 ...
 */
static void    
query_user_ex(QUERYFLAG queryflag, unsigned int *tmp0, unsigned int *tmp1, 
 ...
 */
static void
dash_dismem(unsigned char *addr, unsigned int length,
            unsigned long disaddr, unsigned int fldsize)
{
     ...
     */
    int
        dash_dis_mem(int argc)
        {
            ulong val = 0;

            //    set_map(1);
            //    printf("Note: FPGA offset should be entered in the query.\n");
            //    printf("%#x %#x %#x %d\n", MEMOP_START, MEMOP_LENGTH, MEMOP_SIZE, __LINE__);
    ...
    int
        dash_alt_mem (int argc)
        {
            ...
            //    printf("Note: offset should be entered in the query.\n");
            if(argc == 1) {
            ...
            //    printf("here  %#x %#x %d\n", MEMOP_START, MEMOP_SIZE , __LINE__);
            //    addr =  (unsigned long)map;
            dispaddr = (unsigned int)MEMOP_START;
            addr = (union location *)(map + dispaddr);
            opsiz = MEMOP_SIZE;
            //    printf("here dispaddr : %#x; map addr: %p; size= %#x; total addr: %p; line %d\n", dispaddr, addr, opsiz, map , __LINE__);
            while(1) {
    ...
    /*------------------------------------------------------------------
$Log: dash_mem.c,v $
Revision 1.7  2012/11/06 20:39:50  mcharon
add headers/cleanup/remove unneeded functions/files

Revision 1.6  2012/11/02 16:06:29  alpeng
support HDD test on overdirve, add some diag item for manufacturing using

Revision 1.5  2012/08/11 00:33:09  mcharon
support mmap to cpld address

Revision 1.4  2012/06/07 00:09:39  palin2
Clean up compiler warnings.

Revision 1.3  2012/06/05 11:44:30  palin2
Clean up compiler warnings.
