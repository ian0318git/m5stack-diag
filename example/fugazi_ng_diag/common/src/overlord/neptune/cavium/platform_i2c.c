/* $Id: platform_i2c.c,v 1.2 2018/05/18 09:24:57 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/neptune/cavium/platform_i2c.c,v $
 *-----------------------------------------------------------------------------
        ...
        if (ret_val != PASSED) {
            ...
            if(i2c_dev.dev_addr == OVLD_CAVIUM_MUX_I2C_ADDR){
                if ((NVRAM)->diagflag & D_EXT_LOOPBACK) {
                    printf("\nExternal loopback flag is off, Skipping SFP module I2C read\n");
                } else {
                    prpass(testpass, "I2C MUX is Pass, testing SFP cookie "); 
...
Revision 1.1.2.4  2018/05/04 06:38:10  alpeng
using D_EXT_LOOPBACK flag for skipping i2c scan SFP module

Revision 1.1.2.3  2018/05/03 10:06:13  alpeng
