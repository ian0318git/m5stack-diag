/* $Id: platform_dimm.c,v 1.2 2018/05/18 09:24:56 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/neptune/cavium/platform_dimm.c,v $
 *------------------------------------------------------------------
 * platform_dimm.c - Neptune DIMM main function/menu.
 *		     Leverage from Overlord Cavium.
 *
 * June 2011, Paul Lin
 * 
...
static void toggle_cvmx_ecc(void); 
...
static submenu_xtable_t dimm_util_table[] = {
    ...
    {"Show Dimm data in RAW",   (PFT)dump_dimm_raw_data_wrap,  TRUE,
     0,                         (type_t(*)())0,                0,
     (type_t(*)())0,             0},
    {"Toggle ECC ",             (PFT)toggle_cvmx_ecc,          TRUE,
     0,                         (type_t(*)())0,                0,
     (type_t(*)())0,             0},
#if 0  /* fixme */
...
/******************************************************************************
 *
 * Function   : toggle_cvmx_ecc
 * Description: toggle cvmx ecc enable / disable 
 * Inputs     : None
 * Outputs    : None 
 *
 ******************************************************************************/
static void toggle_cvmx_ecc (void) 
{
    ...
    if (1)  /* for Neptune */ 
    {
        lmc = cvmx_read_csr(CVMX_LMCX_CONFIG(0));
        if (lmc & 0x2) {
            printf("ECC was on. Turning ECC off.\n");
            cvmx_write_csr(CVMX_LMCX_CONFIG(0), lmc ^ 2);
        }
        else {
            printf("ECC is off. Turning ECC on. \n");
            cvmx_write_csr(CVMX_LMCX_CONFIG(0), lmc ^ 2);
        }
    }
    else  /* for CN6XXX and CN70XX, left for future reference */
    {
        lmc = cvmx_read_csr(CVMX_LMC_MEM_CFG0);
        if (lmc & 0x2) {
            printf("ECC was on. Turning ECC off.\n");
            cvmx_write_csr(CVMX_LMC_MEM_CFG0, lmc ^ 2);
        }
        else {
            printf("ECC is off. Turning ECC on. \n");
            cvmx_write_csr(CVMX_LMC_MEM_CFG0, lmc ^ 2);
...
Revision 1.1.2.3  2016/12/28 07:39:20  alpeng
fix issue on toggle ecc

Revision 1.1.2.2  2016/08/12 10:07:19  alpeng
update dimm utility for toggle ECC
