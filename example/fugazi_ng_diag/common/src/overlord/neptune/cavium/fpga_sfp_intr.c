/* $Id: fpga_sfp_intr.c,v 1.2 2018/05/18 09:24:56 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/neptune/cavium/fpga_sfp_intr.c,v $
 *------------------------------------------------------------------
 *
...
en_sfp_intr_type(int sfp_num, int intr_type, int flag)
{
	...
	break;
    case SFP2: /* sfp plus 0 */
        cfg_p = (unsigned int *) &sfp_stat_ctrl->sfp_p0_conf;
	break;
    case SFP3: /* sfp plug 1 */
        cfg_p = (unsigned int *) &sfp_stat_ctrl->sfp_p1_conf;
	break;
    default:
...
en_sfp_intr(int sfp_num, int flag)
{
    ...
    unsigned long sfpien;

    sfpien = (1 << sfp_num);
    /* bit definition is changed on Neptune */
    if (sfp_num == SFP2) {
        sfpien = BIT_8;
    } 
    if (sfp_num == SFP3) {
...
show_sfp_stscfg(int sfp_num, char *msg_p)
{
	...
	break;
    case SFP2:
        sts_p = (unsigned int *) &sfp_stat_ctrl->sfp_p0_intr;
	break;
    case SFP3:
        sts_p = (unsigned int *) &sfp_stat_ctrl->sfp_p1_intr;
	break;
    default:
        printf("SFP port number %d is invalid\n", sfp_num);
	break;
...
{
    uint32_t fpga_intr_bit, sfp_rlt;
...
Revision 1.1.2.3  2017/07/11 06:45:57  meho
Fixed PRRQ commnet.

Revision 1.1.2.2  2016/11/08 10:00:32  alpeng
