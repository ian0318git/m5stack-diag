// $Id: fpga_smartfan.c,v 1.2 2020/08/11 04:16:43 jiajliu Exp $
// $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/neptune/curie_2RU/fpga_smartfan.c,v $
//------------------------------------------------------------------------------
...
void fpga_smartfan_display (unsigned fan)
{
    ...
    unsigned int retry, retry_max = 3;

    /* Merge 1RU's retry workaround for read failure */
    for (retry = 1; retry <= retry_max; retry++) {
        ...
        if (ret == DATA_IS_VALID) {
            ...
            printf("mfg sn : %s\n", parsed_ptr->mfg_sn);
            if (retry > 1)
                printf("fan%u cookie retry = %d\n", fan, retry);
            break;
        } else {
            if (retry == retry_max) {
                printf("\nSmartfan %d : Data unable to read, retry = %d\n", fan+1, retry);
            }
        }
    }
...
/*
 *-----------------------------------------------------------------------------
$Log: fpga_smartfan.c,v $
Revision 1.2  2020/08/11 04:16:43  jiajliu
Add retry workaround for smartfan cookie read failure.
The smartfan vendor will fix it finally.
