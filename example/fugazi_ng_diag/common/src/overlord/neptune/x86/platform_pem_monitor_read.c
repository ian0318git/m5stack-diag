/* $Id: platform_pem_monitor_read.c,v 1.2 2018/05/18 09:25:00 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/neptune/x86/platform_pem_monitor_read.c,v $
 *------------------------------------------------------------------
 *
...
//    int size;
    ...
    for (ps_num = 0; ps_num < 2; ps_num++) {

//	if  ( rp1ruve_pem_check_present( ps_num, &size ) == TEST_PASSED ) {
 ...
 */
int pem_check( void ) {
    //uint32_t irq5status = 0;
    int rv = TEST_PASSED;
//    int size;

    //irq5status = cp_cpld_rd_quiet(RP1RUVE_FPGA_REG_IRQ_5_STATUS);
    
//    if ( rp1ruve_pem_check_present( 0, &size ))
	rv |=  pem_check_core( 0, ERR_CHECKER_LEAVE_ERRORS );

//    if ( rp1ruve_pem_check_present( 1, &size ))
	rv |=  pem_check_core( 1, ERR_CHECKER_LEAVE_ERRORS );
...
//    if ( rp1ruve_pem_check_present( 0, &size ))
	rv |=  pem_check_core( 0, ERR_CHECKER_CLEAR_ERRORS );

//    if ( rp1ruve_pem_check_present( 1, &size ))
	rv |=  pem_check_core( 1, ERR_CHECKER_CLEAR_ERRORS );
