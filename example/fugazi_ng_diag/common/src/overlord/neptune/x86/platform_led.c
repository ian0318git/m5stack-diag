/* $Id: platform_led.c,v 1.2 2018/05/18 09:25:00 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/neptune/x86/platform_led.c,v $
 *------------------------------------------------------------------
...
static struct mitem led_items[] = {
    ...
    {"POE daughter card LED util",          0,0, (type_t(*)())led_poe_dag_crd_util, 
                             (type_t *)&zero, 0, (type_t(*)())0,       0},
    {"SSD Drive LED util",                  0,0, (type_t(*)())led_hdd_util, 
                             (type_t *)&zero, 0, (type_t(*)())0,       0},
    ...
    {"SFP1 LED util",                       0,0, (type_t(*)())led_sfp_util, 
                             (type_t *)&one,  0, (type_t(*)())0,       0},
    {"TE0 LED util",                       0,0, (type_t(*)())led_sfp_plus_util,
                             (type_t *)&zero, 0, (type_t(*)())0,       0},
    {"TE1 LED util",                       0,0, (type_t(*)())led_sfp_plus_util,
                             (type_t *)&one, 0, (type_t(*)())0,       0},
...
led_poe_pwr_unit_util (int unit_num)
{
	...
	printf("  1. Turn %s to green\n", unit_name);
	if (unit_num == 2) {
	    printf("  2. (Not applicable) Turn %s to yellow\n", unit_name);
	}
	else {
	    printf("  2. Turn %s to yellow\n", unit_name);
	}
...
led_rj45_eth_util (int port_num)
{
	...
	printf("  1. Toggle port %d green link LED\n", port_num);
        if ((port_num == two) || (port_num == three)) {
	    printf("  2. (Not supported)Toggle port %d amber LED\n", port_num);
        } else {
	    printf("  2. Toggle port %d amber LED\n", port_num);
        }
 ...
 *              trun on LED without blink.
 *
 * Input : sfp_plus_num - SFP+ number 0, 1
 * 
 * Output: Always PASSED (even though there is nothing to be returned)
 *         to be called from submenu.
 *-----------------------------------------------------------------------------
 */
static int 
led_sfp_plus_util (int sfp_plus_num)
{
    uint32_t choice, led_sfp_plus_reg, act_link_mask, g_link_mask, y_link_mask, g_speed_mask;
    uint32_t act_mask;

    led_sfp_plus_reg = act_link_mask = g_link_mask = y_link_mask = g_speed_mask = act_mask = 0;
    while (1) {
        printf("\n LED TE port %d utility:\n", sfp_plus_num);
        printf("  0. Toggle port %d green activity LED\n", sfp_plus_num);
        printf("  1. Toggle port %d green link LED\n", sfp_plus_num);
        printf("  2. Toggle port %d ambert LED\n", sfp_plus_num);
        printf("  3. Show LED SFP/TE Ethernet On/Off Register\n");
        printf("  4. exit\n");
        ...
        led_sfp_plus_reg = get_led_status(LED_CTRL_SFP_ONOFF);
        ...
        switch (sfp_plus_num) {
        case 0:
            act_link_mask = SFP_PLUS_0_LED_ACT_EN;
            g_link_mask = SFP_PLUS_0_LED_GREEN;
            y_link_mask = SFP_PLUS_0_LED_YELLOW;
            g_speed_mask = SFP_PLUS_0_LED_BLINK;
            break;
        case 1:
            act_link_mask = SFP_PLUS_1_LED_ACT_EN;
            g_link_mask = SFP_PLUS_1_LED_GREEN;
            y_link_mask = SFP_PLUS_1_LED_YELLOW;
            g_speed_mask = SFP_PLUS_1_LED_BLINK;
            break;
        default:
            printf("Not support this item. \n");
        break;
        ...
        if (led_sfp_plus_reg & act_mask) {
            led_sfp_plus_reg &= ~act_mask;
        } else {
            led_sfp_plus_reg |= act_mask;
        }
        set_led_reg(LED_CTRL_SFP_ONOFF, led_sfp_plus_reg);
...
led_eth_util (void)
{
    ...
    while (!exit_flag) {
        printf("\n%s LED utility:\n", eth_name);
	printf("  0. Toggle %s port green speed\n", eth_name);
	printf("  1. Toggle %s port green link\n", eth_name);
...
led_hdd_util (void) 
{
    ...
    while (!exit_flag) {
        printf("\n%s LED utility:\n", hdd_name);
	printf("  1. Toogle SSD Activity LED to Green (bit 12) (item 8 must be set once)\n");
	printf("  2. Toggle SSD Activity LED to Yellow (bit 13)\n");
	printf("  3. Set SSD Signal polarity Inversion\n");
	printf("  4. Show SSD Activity signal status\n");
        printf("  5. Set SSD Activity LED blink with timer by Ext. ACT signal assertion\n");
        printf("  6. Set SSD Activity LED blink with timer by ACT bit(bit 12) is set and Ext. ACT signal assertion\n");
        printf("  7. Set SSD Activity LED ON only Ext. ACT signal is asserted (this is power on default)\n");
        printf("  8. Set SSD Activity LED is driven directly by ACT bit(bit 12)\n");
	printf("  9. Set SSD Activity Timer\n");
	printf(" 10. Show SSD Drive LED Register\n");
	printf(" 11. Exit\n");

	choice = getdec_answer("Enter selection:", 11, 1, 11);

        led_hdd_reg = get_led_status(LED_CTRL_HD_DRIVER);
   
	switch(choice) {
	case 1:
          field_mask = HDD_STAT_LED_MSK;
          if ((led_hdd_reg & HDD_STAT_LED_MSK) == HDD_LED_GREEN) {
              act_mask = HDD_LED_OFF;
          } else {
              act_mask = HDD_LED_GREEN;
          }
	break;
	case 2:
          field_mask = HDD_STAT_LED_MSK;
          if ((led_hdd_reg & HDD_STAT_LED_MSK) == HDD_LED_YELLOW) {
              act_mask = HDD_LED_OFF;
          } else {
              act_mask = HDD_LED_YELLOW;
          }
	break;
	case 3:
