/* $Id: platform_pem_eeprom_modify.c,v 1.2 2019/08/06 06:56:13 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/overlord/neptune/curie_1RU/platform_pem_eeprom_modify.c,v $
 *------------------------------------------------------------------
 *
...
void build_pem_menu (uint32_t option)
{
    ...
    if (psu_mux_mask == 0) {
        printf("%s:%d Failed to get Mux mask).", __FUNCTION__, __LINE__);
        return;
    }
    ...
    if (rc != PASSED) {
        printf("%s:%d Failed to set Mux channel (rc = %#x).",
 ...
 *
 * Revision 1.1.2.3  2018/11/14 01:40:29  alpeng
 * remove redundant and useless utilities from power entry module
 *
 * Revision 1.1.2.2  2018/10/31 12:27:37  alpeng
