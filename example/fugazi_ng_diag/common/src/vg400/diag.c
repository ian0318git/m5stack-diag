/* $Id: diag.c,v 1.3 2019/08/06 06:56:18 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/vg400/diag.c,v $
 *------------------------------------------------------------------
 *
 * diag.c - Informers diagmon main menu and supporting wrappers.
 *
 * Sept 2017, Sam Hsu ported from Xformers, updated for Vg400. 
 *
...
static struct mitem utilmenuitems[] = {
    ...
    {"UART utility",          0, 0,
     ...
     (type_t(*)())0,0},
    {"Show CPU ERROR utility", 0, 0,
     (PFT)cpu_err_show, (type_t *)&zero, 0, (type_t(*)())0,   0},
    {"Show CPU RESET RESON utility", 0, 0,
     (PFT)cpu_reset_reason_show, (type_t *)&zero, 0, (type_t(*)())0,   0},
    {"Show IRQ0 Status utility", 0, 0,
     (PFT)irq0_status_show, (type_t *)&zero, 0, (type_t(*)())0,   0},
...
void diag_menu (int argc, char *argv[]) 
{
    ...
    sprintf(mac,"ifconfig br0 hw ether %x:%x:%x:%x:%x:%x",(uint8_t)src_mac[0],(uint8_t)src_mac[1],(uint8_t)src_mac[2],(uint8_t)src_mac[3],(uint8_t)src_mac[4],(uint8_t)src_mac[5]);
 ...
 **********************************************************************
 */
int cli_main_menu_table_size (void)
{
...
static int display_brd_info (int val)
{
    ...
    /* VG400 Display Secure JTAG status and register */
 ...
 * Function: reset_dev
 *
 * Description: reset and unreset dev,
 *              
 *
 * Input : val -- not used
 *
 ...
 **********************************************************************
 */
static int set_uart_lpbk (int val)
{
    uart_lpbk(val);   
    return (PASSED);
}
 ...
 **********************************************************************
 */
static int program_mac (int management_port)
{
     ...
     */
    if (!management_port) {
        for (port = PORT_NUM0 ; port <= PORT_NUM3 ; port++) {
           if (port >= PORT_NUM2) {
    ...
    if (management_port) {
        port = GOLDBEACH_MGMT; /* VG400  eth1 is management port  */
        get_mac_from_block(port, buf);
...
int reset_sys_by_watchdog (void)
{
    ...
    fd_wd = open(WATCHDOG_PATH, O_RDWR);
    if (fd_wd == -1) {
        cterr('f',0,"open %s failed. \n", WATCHDOG_PATH);
        return (FAILED);
    } else {
 ...
 **********************************************************************
 */
static int display_regs (int d)
{
    /* Input d select display mode */
    display_uart_regs(d);
    return (PASSED);
}
 ...
 **********************************************************************
 */
static int enable_uart (int d)
{
    enable_platform_uart_intr(EN_UART);
    uio_enable_intr();
...
/*---------------------------------------------------------------
$Log: diag.c,v $
Revision 1.3  2019/08/06 06:56:18  alpeng
merge curie, switzer and nightwatch to trunk

Revision 1.2.50.2  2019/07/24 08:32:57  alpeng
...
Revision 1.1.2.3  2018/05/14 02:26:21  haohsu
Add board type VG400
