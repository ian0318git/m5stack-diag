 /* $Id: diag_i2c_lib.c,v 1.2 2019/12/11 10:10:30 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nanook/diag_i2c_lib.c,v $
 *-----------------------------------------------------------------------------
...
int read_i2c_reg_util (int yes_offset)
{
    ...
    if (i2c_if.i2c_bus_type == IOFPGA_I2C) {
        ctrl = getdec_answer("\nEnter i2c control number of Glory FPGA(0-28)", 2, 0, 28);
    } else {
        ctrl = bus;
    }
    ...
    size = gethex_answer("Enter length you want to read(in bytes)", 0x0, 1, 256);
    i2c_if.size = size;
