 /* $Id: diag_eth_pkt_txrx.c,v 1.3 2020/04/20 02:28:24 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nanook/diag_eth_pkt_txrx.c,v $
 *-----------------------------------------------------------------------------
...
int eth_pkt_txrx_sfp (char *, char *, int, int);
/* default mac address */
...
static void *pkt_rx_verify (eth_rx_pkt_t *rxpkt_s_ptr)
{
    unsigned char pkt[2][ETH_PKT_BUF_LEN];
    unsigned int plen[2];//, pkt_len;
    unsigned char *bufp;    
...
int pkt_trans_util (char *if_name, char *if_name2, int pkt_cnt)
{
    pthread_t threads;
    int rc, rtn_val;
    int ii, ix, retry = 5;
    ...
    /* 1. prepare the rx data structure
     */
    if (setup_eth_dev(if_name2, &rx_skt) == FAIL) {
        return (FAILED);
    }
    
    if((!strcmp(if_name2, "eth1_mac1"))||(!strcmp(if_name2, "eth1"))) {
        /* 1.5 add socket filter based on mac address 
         */
        if (set_src_mac_filt(&rx_skt, if_name2) != PASSED) {
            return (FAILED);
        }
    }
    ...
    for (ii=0; ii < pkt_cnt; ii++) {
        ...
        for (ix = 0; ix < retry; ix++) {
        	rc = pthread_create(&threads, NULL, (void *)pkt_rx_verify, (eth_rx_pkt_t *) &rxpkt_s);
	        if (rc == PASSED) {
                break;
            } else if (rc != EAGAIN) {
	            printf("%s pthread_create failed \n", __FUNCTION__);
    ...
    cleanup_eth_dev(if_name, tx_skt);
    cleanup_eth_dev(if_name2, rx_skt);
...
int eth_pkt_txrx_sfp (char *if_name1, char *if_name2, int pkt_cnt, int debug_flag) { 
    ...
    printf("%s send %d packets to %s", if_name1, pkt_cnt, if_name2);
    ret = pkt_trans_util(if_name1, if_name2, pkt_cnt);
...
{
    return NANOOK_ETH_BP_LAN0PORT0;
}
...
int sgmii_lpbk_util(int port, int pkt_cnt)
{
    ...
    if (port == NANOOK_ETH_BP_LAN0PORT0) {
        sprintf(iface_name, inface_lan0p0);
    } else {
        printf("\n%s: Unsupported port %d for sgmii_lpbk_util\n",
                      __FUNCTION__, port);
        return (FAILED);
    }
	
    main_result = eth_pkt_txrx(iface_name, pkt_cnt, FALSE);
