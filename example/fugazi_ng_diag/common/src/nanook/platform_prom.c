/* $Id: platform_prom.c,v 1.2 2019/12/11 10:10:34 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nanook/platform_prom.c,v $
 *-----------------------------------------------------------------------------
...
int write_spi_prom_status_reg(prom_t *spi_prom, uchar wr_byte);
...
static reggio_spi_eprom_info_t spi_eprom_info[2] = {
    {
        /* per fpga spec */
        .start_g = 0,
        //    .end_g = 0x2FFFFF,
    ...
    },
    {
        ...
        .start_sector_u = 60,
        .end_sector_u = 120,
        .start_header =  0,  /* not used for now */
        .end_header = 0, /* not used for now */
    },
};
...
is_read_fifo_empty (prom_t *spi_prom)
{
    ...
    do {
        ...
        if (diagflag_xram & D_TRACE) {
            printf("\t %s(%d) read status @%#x = %#x\n",
                __func__,
                __LINE__,
                &spi_prom->status,
                val);
        }
        if (val & DSWAP4(PROM_RD_FIFO_EMPTY)) {
            return(TRUE);
        }
        ...
        val = spi_prom->data;
        if (diagflag_xram & D_TRACE) {
            printf("\t %s(%d) dummy read data @%#x = %#x\n",
                __func__,
                __LINE__,
                &spi_prom->data,
                val);
...
is_rd_wr_op_done(prom_t *spi_prom)
{
    ...
    do {
        ...
        if (val & DSWAP4(PROM_WRITE_ERROR | PROM_WRITE_FIFO_OVERRUN)) {
            printf("SPI status register error : %x\n", val);
            /* Clear error bit */
            spi_prom->status |= DSWAP4(PROM_WRITE_ERROR | PROM_WRITE_FIFO_OVERRUN);
            if (diagflag_xram & D_TRACE) {
                printf("\t %s(%d) write status @%#x = %#x\n",
                    __func__,
                    __LINE__,
                    &spi_prom->status,
                    (val | DSWAP4(PROM_WRITE_ERROR | PROM_WRITE_ERROR)));
            }
            return (FALSE);
        }

        if (val & DSWAP4(PROM_OPER_DONE)) {
            /* Clear the Done bit */
            ...
            if (diagflag_xram & D_TRACE) {
                printf("\t %s(%d) write status @%#x = %#x\n",
                    ...
                    __LINE__,
                    &spi_prom->status,
                    (val | DSWAP4(PROM_OPER_DONE)));
            }
            return(TRUE);
        }
        msleep(PROM_SLEEP_1_MS); /* Sleep 1ms */
    } while (count++ < SPI_PROM_MAX_WAIT);
    return(FALSE);
}
 ...
 */
int
is_rd_fifo_full(prom_t *spi_prom)
{
    ulong count = 0;
    unsigned int val;
    ...
    do {
        ...
        if (val & DSWAP4(PROM_RD_FIFO_FULL)) {
            return(TRUE);
        }
        msleep(PROM_SLEEP_1_MS); /* Sleep 1ms */
    } while (count++ < SPI_PROM_MAX_WAIT);
 ...
 */
int
is_rd_fifo_empty(prom_t *spi_prom)
{
    ulong count = 0;
    unsigned int val;
...
read_spi_prom_status_reg (uchar *rdsr)
{
    ...
    if (diagflag_xram & D_TRACE) {
        printf("\t %s(%d) size @%#x = %#x, opcode_addr @%#x = %#x, control @%#x = %#x\n",
            __func__,
            __LINE__,
            &spi_prom->size,
            DSWAP4(PROM_RD_1_BYTE & PROM_RD_SIZE_MASK),
            &spi_prom->opcode_addr,
            DSWAP4(PROM_RDSR_OP),
            &spi_prom->control,
            DSWAP4(PROM_DFLT_BAUD));
    }
...
prom_sector_erase (prom_t *spi_prom, ushort sector)
{
    ...
    spi_prom->control     = DSWAP4(PROM_DFLT_BAUD | PROM_DATA_WRITE |
                   PROM_USE_ADDR);
    if (diagflag_xram & D_TRACE) {
        printf("\t %s(%d) opcode_addr @%#x = %#x, control @%#x = %#x\n",
            __func__,
            __LINE__,
            &spi_prom->opcode_addr,
            DSWAP4(opcode),
            &spi_prom->control,
            DSWAP4(PROM_DFLT_BAUD | PROM_DATA_WRITE | PROM_USE_ADDR));
    }
...
prom_bulk_erase (prom_t *spi_prom)
{
    ...
    if (diagflag_xram & D_TRACE) {
        printf("\t %s(%d) opcode_addr @%#x = %#x, control @%#x = %#x\n",
            ...
            __LINE__,
            &spi_prom->opcode_addr,
            DSWAP4(PROM_BULK_ERASE_OP),
            &spi_prom->control,
            DSWAP4(PROM_DFLT_BAUD | PROM_DATA_WRITE));
    }
...
bulk_erase_and_verify (prom_t *spi_prom)
{
    ...
    for (wait_sec = 0; wait_sec < max_wait; wait_sec++) {
        ...
        msleep(PROM_SLEEP_1000_MS); /* Sleep 1s */
    }
...
is_spi_prom_rdy (unsigned int max_wait)
{
    ...
    do {
        ...
        msleep(PROM_SLEEP_1_MS); /* Wait 1ms */
...
write_spi_prom_status_reg (prom_t *spi_prom, uchar wr_byte)
{
    ...
    spi_prom->control     = DSWAP4(PROM_DFLT_BAUD | PROM_DATA_WRITE);
    if (diagflag_xram & D_TRACE) {
        printf("\t %s(%d) size @%#x = %#x, opcode_addr @%#x = %#x, data @%#x = %#x, control @%#x = %#x\n",
            __func__,
            __LINE__,
            &spi_prom->size,
            DSWAP4(PROM_RD_1_BYTE & PROM_RD_SIZE_MASK),
...
/*-------------------------------------------------------------------
 *
 * Function: init_crocus_spi_prom_ds
 *
 * This function will initialize the spi_prom_reg data stricture for
 * use in this file.
 *
 * Input: mb - TRUE if it is main board
 *
 * Output: Pointer to the spi prom register file
 *
 *-------------------------------------------------------------------
 */
prom_t *
init_crocus_spi_prom_ds (void)
{
    spi_prom = (prom_t *)(scc_base + CROCUS_SPI_REGISTER_OFFSET);
    p_spi_prom_info = &spi_eprom_info[1];
...
verify_sector_erase (prom_t *spi_prom, ushort sector)
{
    ...
    while (start_addr < end_addr) {
        ...
        if (diagflag_xram & D_TRACE) {
            printf("\t %s(%d) opcode_addr @%#x = %#x, size @%#x = %#x, control @%#x = %#x\n",
                __func__,
                __LINE__,
                &spi_prom->opcode_addr,
                DSWAP4(PROM_READ_OP | start_addr),
                &spi_prom->size,
                DSWAP4(PROM_RD_256_BYTE),
                &spi_prom->control,
        ...
        if (!is_rd_fifo_full(spi_prom)) {
            cterr('f',0,"%s: read fifo not full", __FUNCTION__);
            return(FAILED);
        }
        ...
        if (!is_rd_fifo_empty(spi_prom)) {
            cterr('f',0,"%s: read fifo not empty", __FUNCTION__);
            return(FAILED);
        }
               ...
               ulong end_addr, uchar *src_img, boolean bit_reverse)
{
    ...
    while (start_addr < end_addr) {
        ...
        if ((count == PROM_RD_256_BYTE) && (!is_rd_fifo_full(spi_prom))) {
            cterr('f',0,"%s: read fifo not full", __FUNCTION__);
            return(FAILED);
        }
 ...
 */
static int
crocus_fpga_erase (uchar type)
{

    ushort sector;
    prom_t *spi_prom;

    testname("FPGA erasing");


    fpga_fw = dash_fpga_fw_array;
    fpga_fw_size = dash_fpga_fw_size;
    fflush(stdout);

    spi_prom = init_crocus_spi_prom_ds();
 ...
 */
static int
crocus_fpga_update (uchar type)
{
    prom_t *spi_prom;

    if (crocus_fpga_erase(type) != PASSED) {
        return (FAILED);
    }
    ...
    printf("\nVerifying ...\n");
    if (verify_download_image(spi_prom, fpga_info.start_addr,
                  fpga_info.end_addr, fpga_info.fpga_fw, FALSE) == FAILED) {
        cterr('f',0,"Verify of FPGA image failed.");
        return(FAILED);
    }
...
/*-------------------------------------------------------------------
 *
 * Function: diag_fpga_bin2array()
 *
 * This function will open binary file, read its contents, swap bits,
 * and stored in array.
 *
 * Input:  char * file. fpag file.
 *         uint size, size of fpga.
 ...
 *-------------------------------------------------------------------
 */
 static int diag_fpga_bin2array (char *file, uint *read_size)
{
    FILE *fp;
    uint size;
    uint val;
    ...
    if (!fp) {
        ...
        dash_fpga_fw_array = NULL;
        return (FAILED);
    }

    val = fread(dash_fpga_fw_array, 1, size, fp);
    if (val != size) {
        printf("%s: expected to read %d bytes, but only got %d bytes\n", __func__, size, val);
        fclose(fp);
        *read_size = 0;
        free(dash_fpga_fw_array);
        dash_fpga_fw_array = NULL;
 ...
 *********************************************************************
 */
int getbinaryfirmware (char *file)
{
    ...
    printf("Converting binary to array...");
    fflush(stdout);
    if (diag_fpga_bin2array(file, &read_size) == FAILED) {
        printf("FAILED! Exiting...\n");
        return (FAILED);
    } else {
        printf("OK! (Read size = %d)\n", read_size);
...
/*-------------------------------------------------------------------
 *
 * Function: program_reggio_spi_prom_async()
 *
 * This function is a wrapper function to program the Crocus FPGA image to
 * the FPGA SPI PROM. It will be done in interactive mode.
 *
 * Output: PASSED/FAILED
 *
 ...
 */
int
program_reggio_spi_prom_async (int dummy)
{
    int image_type = query();
    char   ans;
    char fw_file[128];

    sprintf(fw_file, "%s", FPGA_CROCURS_FILE_PATH);
    if (getbinaryfirmware(fw_file)==FAILED) {
        return(FAILED);
    }
    ...
    /* Program in interactive mode */
    if (crocus_fpga_update(image_type)==FAILED) {
        return FAILED;
    }
...
/*-------------------------------------------------------------------
 *
 * Function: program_reggio_spi_prom_async_erase()
 *
 * This function is a wrapper function to erase the Crocus FPGA. It
 * will be done in interactive mode.
 *
 * Output: PASSED/FAILED
 *
 *-------------------------------------------------------------------
 */
int
program_reggio_spi_prom_async_erase (int mb)
{
    int image_type = query();
    char   ans;
	char fw_file[128];
    ...
    /* Show warning and get User confirmation. */
    printf("This process will ERASE FPGA.\n");
    printf("Do you really want to do it ?\n");
    printf("(Press 'y/Y' to continue or any other key to Quit) ");
    ...
    if (!((ans == 'y') || (ans == 'Y'))) {
        printf("\nErase SPI PROM is Aborted by User !!!\n");
        return (PASSED);
    }

    if (crocus_fpga_erase(image_type)==FAILED) {
        return FAILED;
    }
           ...
           ulong end_addr, uchar *src_img, boolean bit_reverse)
{
    ...
    while (start_addr < end_addr) {
        ...
        if (data_check != PROM_BLANK_DATA) {
            ...
            if (diagflag_xram & D_TRACE) {
                printf("\t %s(%d) opcode_addr @%#x = %#x, size @%#x = %#x\n",
                    __func__,
                    __LINE__,
                    &spi_prom->opcode_addr,
