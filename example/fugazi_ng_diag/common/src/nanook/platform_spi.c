/* $Id: platform_spi.c,v 1.2 2019/12/11 10:10:35 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nanook/platform_spi.c,v $
 *-----------------------------------------------------------------------------
...
extern prom_t *init_crocus_spi_prom_ds (void);
...
void aikido_spi_read_util (void) 
{
    ...
    size = getdec_answer("Enter size", 1, 1, 0xffff);
...
void aikido_spi_write_util (void) 
{
    ...
    address = gethex_answer("Enter address", 0, 0, 0xffff);
    size = getdec_answer("Enter size", 1, 1, 0xf);
    for (ix = 0; ix < size; ix++) {
...
/*-------------------------------------------------------------------
 *
 * Function: crocus_spi_read_util
 * 
 * This function is spi read util for read crocus via spi
 *
 * Input: none.
 *
 * Output: none.
 ...
 *-------------------------------------------------------------------
 */
void crocus_spi_read_util (void) 
{
    unsigned int size, address, op, is_addr, ix; 
    unsigned char buf[259]; 

    printf("Make sure address is 4-bytes aligned \n"); 
    address = gethex_answer("Enter address", 0, 0, 0x800000);
    size = getdec_answer("Enter size", 1, 1, 0xffff);
	...
	general_fpga_spi_read(init_crocus_spi_prom_ds(), size, address, 0x03, 0x11, buf); 
    for (ix = 0; ix <= size; ix++) { 
        if (ix % 16 == 0) {
            printf("\n"); 
        }
...
/*-------------------------------------------------------------------
 *
 * Function: crocus_spi_write_util
 * 
 * This function is spi write util for write crocus via spi
 *
 * Input: none.
 *
 * Output: none.
 ...
 *-------------------------------------------------------------------
 */
void crocus_spi_write_util (void) 
{
    unsigned int size, address, op, is_addr, ix; 
    unsigned char buf[259]; 
	...
	general_fpga_spi_write(init_crocus_spi_prom_ds(), size, address, 0x02, 0x13, buf);
}

/*-------------------------------------------------------------------
 *
 * Function: crocus_spi_erase_util
 * 
 * This function is spi erase util for crocus
 *
 * Input: none.
 *
 * Output: none.
 ...
 *-------------------------------------------------------------------
 */
void crocus_spi_erase_util (void) 
{
    unsigned int sector;

    sector = gethex_answer("Enter sector", 0, 0, 127);
	prom_sector_erase(init_crocus_spi_prom_ds(), sector);
}
