/* $Id: grimlock_fpga.c,v 1.3 2020/04/14 11:13:49 letsai Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/grimlock/grimlock_fpga.c,v $
 *------------------------------------------------------------------
 * grimlock_fpga.c
 *
...
static int tdm_rd (ulong addr, int size, ulong *buf, void *param)
{
	...
	*data_buf = fpga_reg->tdmsw_data[size - 1 - ix];
	data_buf++;
    }
...
/**********************************************************************
 *
 * Function: grimlock_led_test()
 *
 * This function will test the leds on the Grimlock NIM
 *
 * Input : None
 *
 ...
 **********************************************************************
 */
int grimlock_led_test ()
{
    fpga_reg_t *fpga_reg;
    uint8_t val;
    int i, j, shift, reg_num, port_num, led_num, led_start;

    prpass(testpass, "Grimlock LED test");
...
void fpga_get_rev ()
{
    ...
    printf("Grimlock FPGA was built at %x/%x/%x at %x o'clock\n", 
	   fpga_reg->fpga_rev[2], fpga_reg->fpga_rev[1],
	   fpga_reg->fpga_rev[3], fpga_reg->fpga_rev[0]);
    printf("Grimlock FPGA Image Revision: %x.%x.%x (%s)\n", 
 ...
 **********************************************************************
 */
void fpga_set_framer_txhiz (int ena)
{
    fpga_reg_t *fpga_reg;
 ...
 **********************************************************************
 */
void fpga_set_ctclk_src (ctclk_src clk)
{
    fpga_reg_t *fpga_reg;
 ...
 **********************************************************************
 */
void fpga_config_tdm_pll (frmr_clk_mode clk_mode)
{
    fpga_reg_t *fpga_reg;
 ...
 *******************************************************************
 */
static int get_tdmsw_stream_rate (int stream)
{
    ulong rate_addr;
    ulong reg_data;
    int str;
 ...
 **********************************************************************
 */
int set_ngvm_tdm_clock (uchar tdm_format)
{
    ulong ngvm_tdm_addr;
 ...
 *****************************************************************
 */
static int rd_verify_wr_conn_mem (int size, ulong rd_mask, ulong wrval, ulong expval,
 ...
 *	The equations are specified in the Grimlock FPGA
 ...
 *******************************************************************
 */
static int tdm_get_mem_offset (uint stream, uint tslot, stream_rate rate)
{
    int cm_offset;
 ...
 *******************************************************************
 */
static int tdm_get_input_cid (uint stream, uint tslot, stream_rate rate)
{
    int cid;
 ...
 *******************************************************************
 */
static int tdm_stream_connect (int istrm, int itslot, int ostrm, int otslot, int connect_sel)
{
 ...
 *******************************************************************
 */
static int tdm_stream_disconnect (int ostrm, int otslot)
{
    int offset, orate;
    ulong rd_val;
 ...
 *******************************************************************
 */
static int tdm_disconnect (int stream_from, int num_streams,
 ...
 **********************************************************************
 */
int set_tdmsw_lpbk_test(int port, int mode, int lpbk)
{
 ...
 *
 *****************************************************************/
static void generate_nclock (unsigned int *gpvr_ptr, int n)
{
    int ix, i;
...
/*****************************************************************
 *
 * Function: grimlock_fpga_download()
 *
 * Description: This function downloads Grimlock PFGA image through
  *             NPU GPIO pins.  
 *
 * Input: None
 *
 ...
 *
 *****************************************************************/
int grimlock_fpga_download () 
{
    uint ctr,cunt, port;
    int ix, iz;
    unsigned long gpdr_addr, gpvr_addr;
    unsigned int *gpdr_ptr, *gpvr_ptr, *pad_ptr;
    unsigned char *grimlock_image_fw;
    int grimlock_image_fw_size;
    ...
    *gpdr_ptr = 0x11c0;
    ...
    port = get_num_ports();
    if (port == 8) {
	grimlock_image_fw = (unsigned char *)fortitude_image_lx25_fw;
	grimlock_image_fw_size = fortitude_image_lx25_fw_size;
    } else if (port == 4) {
	grimlock_image_fw = (unsigned char *)fortitude_image_lx9_fw;
	grimlock_image_fw_size = fortitude_image_lx9_fw_size;
    } else {
	/* for 1 or 2 ports SKUs */
	grimlock_image_fw = (unsigned char *)fortitude_image_lx4_fw;
	grimlock_image_fw_size = fortitude_image_lx4_fw_size;
    }

    printf("\ngpdr_addr @%#x = %#x, gpvr_addr @%#x = %#x, pad_ptr @%#x = %#x\n", 
	   gpdr_addr, *gpdr_ptr, gpvr_addr, *gpvr_ptr, (ulong)pad_ptr, *pad_ptr);

    printf("grimlock_image_fw @ %#x = %#x, grimlock_image_fw_size = %d\n", 
	   (ulong )grimlock_image_fw, *grimlock_image_fw, grimlock_image_fw_size);
...
/*****************************************************************
 *
 * Function: grimlock_fpga_upgrade_secondary()
 *
 * Description: This function upgrades secondary Grimlock PFGA image in
  *             the SPI flash.  
 *
 * Input: None
 *
 ...
 *
 *****************************************************************/
int grimlock_fpga_upgrade_secondary () 
{
    uint port, i, write_size;
    unsigned char *grimlock_image_fw, *fpga_data;
    int grimlock_image_fw_size;
    ...
    port = get_num_ports();
    if (port == 8) {
	grimlock_image_fw = (unsigned char *)fortitude_8p_fpga_ugd_fw;
	grimlock_image_fw_size = fortitude_8p_fpga_ugd_fw_size;
    } else if (port == 4) {
	grimlock_image_fw = (unsigned char *)fortitude_4p_fpga_ugd_fw;
	grimlock_image_fw_size = fortitude_4p_fpga_ugd_fw_size;
    } else {
	/* for 1 or 2 ports SKUs */
	grimlock_image_fw = (unsigned char *)fortitude_2p_fpga_ugd_fw;
	grimlock_image_fw_size = fortitude_2p_fpga_ugd_fw_size;
    }
    ...
    fpga_data = grimlock_image_fw;
    for (i = grimlock_image_fw_size; i > 0; ) {
...
/*****************************************************************
 *
 * Function: grimlock_fpga_upgrade_golden()
 *
 * Description: This function upgrades golden Grimlock PFGA image in
  *             the SPI flash.  
 *
 * Input: None
 *
 ...
 *
 *****************************************************************/
int grimlock_fpga_upgrade_golden () 
{
    uint port, i, write_size;
    unsigned char *grimlock_image_fw, *fpga_data;
    int grimlock_image_fw_size;
    ...
    port = get_num_ports();
    if (port == 8) {
	grimlock_image_fw = (unsigned char *)fortitude_8p_fpga_gld_fw;
	grimlock_image_fw_size = fortitude_8p_fpga_gld_fw_size;
    } else if (port == 4) {
	grimlock_image_fw = (unsigned char *)fortitude_4p_fpga_gld_fw;
	grimlock_image_fw_size = fortitude_4p_fpga_gld_fw_size;
    } else {
	/* for 1 or 2 ports SKUs */
	grimlock_image_fw = (unsigned char *)fortitude_2p_fpga_gld_fw;
	grimlock_image_fw_size = fortitude_2p_fpga_gld_fw_size;
    }
 ...
 *
 * Description: This function will display the contents of the
 *              Grimlock FPGA Multiboot registers
 *
 * Input  : None
 *
 * Output : Always return PASSED to avoid compilation warning
 ...
 *
 *****************************************************************/
int is_port_clk_master_mode (int port_num) 
{
    fpga_reg_t *fpga_reg;
...
/******** History ********
$Log: grimlock_fpga.c,v $
Revision 1.3  2020/04/14 11:13:49  letsai
Modify typo og parameter
