/* $Id: common_utils.c,v 1.2 2015/05/25 03:59:15 steja Exp $ 
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/skye_linux/src/common_utils.c,v $ 
 *------------------------------------------------------------------
 ...
 */
static uint
new_register_read (reg_info_t *reg_ptr, int reg_addr, volatile uint *buf)
{
    if (reg_ptr->type & REG_ACCESS) {
	/* User provided read function */
	return((*reg_ptr->size.ext->rd_ptr)(reg_addr, (uint *)buf));
    } else {
	/* Direct memory read */
 ...
 */
static uint
new_register_write (reg_info_t *reg_ptr, int reg_addr, int value)
{
    if (reg_ptr->type & REG_ACCESS) {
	/* User provided write function */
	return((*reg_ptr->size.ext->wr_ptr)(reg_addr, value));
    } else {
	/* Direct memory write */
...
register_display (ulong base_addr, reg_info_t *reg_ptr)
{
	...
	printf(" , %#.8x = %#.8x, (mask:%#.8x, size:%ld)",
		(unsigned int)reg_addr, readval, reg_ptr->mask, reg_ptr->size.size);
...
{
    unsigned int  cntr, rc;
    int reg_addr;
    uint size;
    uint readval, original_data, data, temp;
    ...
    while (reg_ptr->size.size != 0) {
        ...
        if ((reg_ptr->type & (READ_ONLY | WRITE_ONLY)) == READ_WRITE) {
            ...
            */
            ...
            for (cntr = 0; cntr < (size * 8); cntr++) {
                temp = ((1 << cntr) & reg_ptr->mask);
                if (diagflag_xram & D_DEBUG_OPTIONS) {
                    printf("Cnt for loop = %d\n", cntr);
                    printf("temp = %#x\n",temp);
                    printf("!temp = %#x\n",!temp);
                }
                if (!temp)
        ...
        if (temp == 0xBB) {
            ...
            for (cntr = 0; cntr < (size * 8); cntr++) {
                ...
                if ((readval&reg_ptr->mask) != temp) {
                    cterr ('f',0,"Ripple zero test failed when accessing "
                        ...
                        "%#x.", reg_ptr->name, reg_addr, temp, readval);
                    return(FAIL);
                }
            }
            ...
            */
            ...
            for (cntr=0;cntr<2;cntr++) {
                ...
                data = (uint)~PATTERN; /* complement data pattern */
            }
 ...
 */
int
mem_specific_test (mem_test_info_t *test_info_p)
{
 ...
 */
int
mem_all_zeros_ones_test (ulong start, ulong end, ulong test_freq)
{
 ...
 *
 *------------------------------------------------------------------
 * Revision 1.2  2014/02/27 15:01:43  palin2
 * Initial check-in ShrinkRay SM side Diag code.
 *
 * Revision 1.1.4.7  2014/01/28 07:39:40  steja
 * Code clean up
 *
 * Revision 1.1.4.6  2014/01/06 13:00:33  steja
 * 1. clean up code
 * 2. Add header TLK code
 *
 * Revision 1.1.4.5  2013/11/05 09:17:53  steja
