/* $Id: prince_scc_diag.c,v 1.4 2017/07/18 08:48:41 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/prince/prince_scc_diag.c,v $
 *------------------------------------------------------------------
 *
 * prince_scc_diag.c - Prince Serial Channel Controller diag function.
 *
 ...
 */
prince_baud_t prince_sync_baud[] = {
 ...
 */
prince_baud_t prince_async_baud[] = {
    { PRINCE_AS_BPS_NONE,     300,    2400,      651, PRINCE_MODEM_BRG_1M },
    { PRINCE_AS_BPS_300,      300,    2400,      651, PRINCE_MODEM_BRG_1M },
    { PRINCE_AS_BPS_600,      600,    4793,      326, PRINCE_MODEM_BRG_1M },
    { PRINCE_AS_BPS_1200,    1200,    9586,      163, PRINCE_MODEM_BRG_1M },
    { PRINCE_AS_BPS_2400,    2400,   19290,       81, PRINCE_MODEM_BRG_1M },
    { PRINCE_AS_BPS_4800,    4800,   38110,       41, PRINCE_MODEM_BRG_1M },
    { PRINCE_AS_BPS_9600,    9600,   76805,      651, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_14400,  14400,  115207,      434, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_19200,  19200,  153374,      326, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_28800,  28800,  230415,      217, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_32K,    32000,  256410,      195, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_38400,  38400,  306748,      163, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_56K,    56000,  446429,      112, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_57600,  57600,  458716,      109, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_64K,    64000,  510204,       98, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_72K,    72000,  574713,       87, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_115200,115200,  925926,       54, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_128K,  128000, 1020408,       49, PRINCE_MODEM_BRG_32M },
    { PRINCE_AS_BPS_230400,230400, 1851852,       27, PRINCE_MODEM_BRG_32M }
};

/* 
 *  3.3.1 Pin muxing between USRT and UARTS
 *  Table to convert 8A/16A channels. 
 *  Using this scheme, 8AS can use two connectors, 8A can use one connector,
 *  and 16A can use two connectors.
 */
static uchar prince_chan_adjust[] = {
    0,  8, 1,  9, 2, 10, 3, 11,
    4, 12, 5, 13, 6, 14, 7, 15
};
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_display_cntrl_1_regs 
 *
 * This function will display control 1 registers. 
 *
 * Input:  scc_base_addr  
 *
 * Output: void 
 */
void 
prince_display_cntrl_1_regs (ulong scc_base_addr)
{
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_display_proto_regs 
 *
 * This function will display protocol registers. 
 *
 * Input:  scc_base_addr
 *
 * Output: void 
 */
void 
prince_display_proto_regs (ulong scc_base_addr, uchar disp_opt,
                         uchar chan)
{
    prince_proto_regs_t *proto_regs; 
    prince_scc_regs_t * base_addr;
    int i, num_chan, start_chan, end_chan;
    ...
    for (i = start_chan; i < end_chan; i++) {  
        proto_regs = &base_addr-> proto_regs_hp1[i];
        printf("\nPRINCE PROTOCOL REGISTERS CHAN %d:................\n",i);
        printf("mode register          @%#x = %#x\n", 
               &proto_regs->mode_reg,
               proto_regs->mode_reg);
/*        printf("mode register low      @%#x = %#x\n", 
               &proto_regs->mode_reg_lo,
               proto_regs->mode_reg_lo);*/
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_display_dma_regs 
 *
 * This function will display DMA receive/transmit registers. 
 *
 * Input:  scc_base_addr
 *
 * Output: void 
 */
void 
prince_display_dma_regs (ulong scc_base_addr, uchar option, uchar 
    ...
    for (i = start_chan; i < end_chan; i++) {  
        if (option == PRINCE_DMA_TX) {
            ...
            case PRINCE_SCC_HIGH_PRIORITY_1:
                dma_regs = &base_addr-> dma_tx_regs_hp1[i];
                printf("\nPRINCE DMA-TX REGISTERS CHAN %d HP1 @%#x:................\n", dma_regs, i);
                break;
            case PRINCE_SCC_HIGH_PRIORITY_2:
                dma_regs = &base_addr-> dma_tx_regs_hp2[i];
                printf("\nPRINCE DMA-TX REGISTERS CHAN %d HP2 @%#x:................\n", dma_regs, i);
                break;
            case PRINCE_SCC_LOW_PRIORITY:
                dma_regs = &base_addr-> dma_tx_regs_lp[i];
                printf("\nPRINCE DMA-TX REGISTERS CHAN %d LP @%#x:................\n", dma_regs, i);
                break;
            default:
            ...
            dma_regs = &base_addr-> dma_rx_regs[i];
            printf("\nPRINCE DMA-RX REGISTERS CHAN %d:................\n",i);
        }
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_display_bcc_regs 
 *
 * This function will display bcc registers. 
 *
 * Input:  scc_base_addr
 *
 * Output: void 
 */
static void 
prince_display_bcc_regs (ulong scc_base_addr, uchar disp_opt,
                       uchar chan)
{
    prince_scc_regs_t * base_addr;
    int i,  num_chan, start_chan, end_chan;
    ...
    for (i = start_chan; i < end_chan; i++) {  
        printf("\nPRINCE BCC REGISTERS CHAN %d:................\n",i);
        printf("TX BCC reg       @%#x = %#x\n", &base_addr->bcc_regs[i*8],
               base_addr->bcc_regs[i*8]);
        printf("PAD char reg     @%#x = %#x\n", 
               &base_addr->bcc_regs[i*8+1],
               base_addr->bcc_regs[i*8+1]);
    }
}
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_display_ppp_regs 
 *
 * This function will display ppp transmit/receive registers. 
 *
 * Input:  scc_base_addr
 *         option     - transmit/receive
 *
 * Output: void 
 */
static void 
prince_display_ppp_regs (ulong scc_base_addr, uchar option, 
                       uchar disp_opt, uchar chan )
{
    prince_ppp_regs_t *ppp_regs; 
    prince_scc_regs_t * base_addr;
    int i, num_chan, start_chan, end_chan;
    ...
    for (i = start_chan; i < end_chan; i++) {  
        if (option == PRINCE_PPP_TX) {
            ppp_regs = &base_addr-> ppp_tx_regs[i];
            printf("\nPRINCE PPP-TX REGISTERS CHAN %d:................\n",i);
        } else {
            ppp_regs = &base_addr-> ppp_rx_regs[i];
            printf("\nPRINCE PPP-RX REGISTERS CHAN %d:................\n",i);
        }
        printf("control char map     @%#x = %#x\n", &ppp_regs->
               cntrl_char_map,
               ppp_regs->cntrl_char_map);
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_display_serial_itf_regs 
 *
 * This function will display serial interface registers. 
 *
 * Input:  scc_base_addr
 *
 * Output: void 
 */
void 
prince_display_serial_itf_regs (ulong scc_base_addr, 
                              uchar disp_opt, uchar chan)
{
    ...
    for (i = start_chan; i < end_chan; i++) {  
        serial_regs = &base_addr-> serial_itf[i];
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_display_cntrl_2_regs 
 *
 * This function will display control 2 registers. 
 *
 * Input:  scc_base_addr
 *
 * Output: void 
 */
void 
prince_display_cntrl_2_regs (ulong scc_base_addr)
{
...
/*-----------------------------------------------------------------------------
 *
 * Function: ngwic_prince_chan_lpbk_test 
 *
 * This function will run datapath channel loopback test 
 *
 * Input: prince_iface - Pointer to prince interface data structure.
 *
 * Output: PASSED FAILED
 */

int 
ngwic_prince_chan_lpbk_test (int channel)
{
    ulong scc_base_addr;
...
int
prince_lpbk_test (ulong scc_base_addr, prince_serial_ds_t *s_ds)
{
    ...
    for (index = PRINCE_SYNC_BPS_300; index <= PRINCE_SYNC_BPS_8064K;
        ...
        if (prince_chan_lpbk_test(run_mode, scc_base_addr, index, clk_src,
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_chan_lpbk_test 
 *
 * This function will run one-shot loopback test for a single channel 
 *
 ...
 *      : protocol - HDLC/Bisync/Transparent.
 *      : s_ds - Pointer to the serial data structure.
 *
 * Output: PASSED FAILED
 */
...
int 
prince_chan_lpbk_test (uchar run_mode, ulong scc_base_addr, ushort speed_idx,
                     ...
                     prince_serial_ds_t *s_ds, int priority)
{
    ...
    prince_form_name_buf(tmp_name_buf, s_ds, priority);
    ...
    prince_config_serial_ds(run_mode, scc_base_addr, speed_idx, clk_src,
    ...
    if (s_ds->protocol == PRINCE_BISYNC) {
        ...
        for (i=0; i<3; i++) {
            s_ds->bcc_tx_regs_addr->unused[i] = 0;
            s_ds->bcc_rx_regs_addr->unused[i] = 0;
        }
    }
    if (diagflag_xram & D_TRACE) {
     ...
     */
    prince_init_tx_dma_ring(run_mode, scc_base_addr, s_ds);
    prince_init_rx_dma_ring(run_mode, scc_base_addr, s_ds);
    ...
    if (protocol == PRINCE_BISYNC) {
        prince_init_tx_dma_buffers(s_ds,  0x12, PRINCE_DATA_PAT);
    } else if (protocol == PRINCE_TRANSPARENT) {
        prince_init_tx_dma_buffers(s_ds, 0, PRINCE_INC_PAT);
    } else {
        prince_init_tx_dma_buffers(s_ds, 1, PRINCE_INC_PAT);
    }
    prince_init_rx_dma_buffers(s_ds);

    /*
     * Start the Tx.
     */
    if (run_mode != PRINCE_NO_CHECK_MODE) {
        if (prince_start_dma_tx(s_ds, priority)) {
            prince_cleanup_serial(s_ds, priority);
            prince_cleanup_dma_ring(scc_base_addr, s_ds); 
            return (FAILED);
        }
        /*
         * Check that we received the Data OK.
         */
        if (prince_check_dma_received_data(s_ds)) {
            prince_trap(s_ds, priority);
            prince_cleanup_serial(s_ds, priority); 
            prince_cleanup_dma_ring(scc_base_addr, s_ds); 
            return (FAILED);
        }
...
void 
prince_config_serial_ds (uchar run_mode, ulong scc_base_addr, ushort speed_idx,
                         uchar clk_src, uchar lpbk_mode, uchar protocol,
                         prince_serial_ds_t *s_ds, int priority,
                         ulong *des_baud, ushort *clk_src_bits)
{
    ...
    *des_baud = 0;
    ...
    case PRINCE_CLK_SRC_SYNC:
        divider = prince_sync_baud[speed_idx].divider;
        baudrate = prince_sync_baud[speed_idx].baudrate;
        *des_baud = prince_sync_baud[speed_idx].desired_baud;
        *clk_src_bits = prince_sync_baud[speed_idx].clk_src;
        break;

    case PRINCE_CLK_SRC_ASYNC:
        divider = prince_async_baud[speed_idx].divider;
        baudrate = prince_async_baud[speed_idx].baudrate;
        *des_baud = prince_async_baud[speed_idx].desired_baud;
        *clk_src_bits = prince_async_baud[speed_idx].clk_src;
        break;
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_setup_dma_reg
 *
 * This function set up dma register for FPGA to pick the ring buffer 
 *
...
void 
prince_setup_dma_reg (prince_serial_ds_t *s_ds)
{
    ...
    for (i = 0; i < 6; i++) {
        rx_dma_regs->reserved[i] = 0;
        tx_dma_regs->reserved[i] = 0;
    }
 ...
 *
 * This function sets up the DMA engine for transmission of data. It checks
 * to make sure that the data has been Tx'ed, and Rx'ed OK.
 *
 * Input: s_ds      - pointer to the serial data structure.
...
ushort
prince_start_dma_tx (prince_serial_ds_t *s_ds, int priority)
{
    prince_dma_bd_t *tx_desc;
    prince_dma_bd_t *rx_desc;
    prince_dma_bd_t *last_rx_desc;
    ushort i, j, k, rx_err_mask, wait_level;
    prince_dma_regs_t * tx_dma_regs;
    prince_dma_regs_t * rx_dma_regs;
     ...
     */
    for (j = 0; j < PRINCE_MAX_DMA_PASS; j++) {
    ...
    if (j >= PRINCE_MAX_DMA_PASS) {
        for (wait_level = 1; wait_level <= 3; wait_level++) {
            ...
            for (k = 0; k < PRINCE_DMA_RETRY_PASS; k++) {
                if (s_ds->speed_idx < PRINCE_LOW_SPEED_INDEX) {
                    msleep(PRINCE_LOW_SPEED_WAIT);
                } else {
                    msleep(PRINCE_HIGH_SPEED_WAIT); 
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_check_dma_received_data
 *
 * This function compares the received data to the data that was 
 * transmitted.
 *
 ...
 */
ushort
prince_check_dma_received_data (prince_serial_ds_t *s_ds)
{
    ushort size, i;
    ...
    for (i = 0; i < size; i++) {
        if (*rx_data != *tx_data) {
            cterr('f',0," Byte %x Data Mismatch. tx = 0x%x rx = 0x%x "
                  "tx_status = 0x%x rx_status = 0x%x", i,
...
/*----------------------------------------------------------------------------
 *
 * Function: prince_init_tx_dma_buffers
 *
 * This function will initializes the tx buffers of the channel.
...
void 
prince_init_tx_dma_buffers (prince_serial_ds_t *s_ds, uchar data, uchar pattern) 
{
    ushort i, size;
    uchar *data_ptr;
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_init_rx_dma_buffers
 *
 * This function will initializes the rx buffers of the channel with
 * zeroes.
 *
 * Input: s_ds - Pointer to the serial data structure.
 *
 ...
 */
void 
prince_init_rx_dma_buffers (prince_serial_ds_t *s_ds) 
{
    ushort i, size;
    uchar *data_ptr;
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_init_tx_dma_ring 
 *
 * This function will initializes the tx DMA ring
 *
 * Input: s_ds - Pointer to the serial data structure.
 *      : scc_base_addr 
 ...
 */
int
prince_init_tx_dma_ring (uchar run_mode, ulong scc_base_addr,
                       prince_serial_ds_t *s_ds)
{
    ushort i;
    prince_dma_bd_t *tx_desc;
    ...
    for ( i = 0; i < s_ds->num_buff; i++ ) {
         ...
         */
        if ( i == (s_ds->num_buff-1) && s_ds->crc_rx_buf_opt) {
        ...
        if (diagflag_xram & D_TRACE) {
            printf("\n -Tx Descriptor number%d at 0x%08x --", i, tx_desc );
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_init_rx_dma_ring 
 *
 * This function will initializes the rx DMA ring
 *
 * Input: s_ds - Pointer to the serial data structure.
 *      : scc_base_addr
 *      : run_mode - poll or interrupt
 ...
 */
int
prince_init_rx_dma_ring (uchar run_mode, ulong scc_base_addr,
                       prince_serial_ds_t *s_ds)
{
    ushort i;
    prince_dma_bd_t *rx_desc;
    ...
    for ( i = 0; i < s_ds->num_buff; i++ ) {
        ...
        if (diagflag_xram & D_TRACE) {
            printf("\n -Rx Descriptor number%d at 0x%08x --", i, rx_desc );
            printf("\n  command= 0x%04x len= 0x%04x ptr= 0x%08x ", 
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_cleanup_dma_ring 
 *
 * This function will free up memory allocated for DMA ring 
 *
 * Input: s_ds - pointer to serial data structure
 *        scc_base_addr
 *
...
void
prince_cleanup_dma_ring (ulong scc_base_addr, prince_serial_ds_t *s_ds)
{
 ...
 *
 * This function will clean up the setup for the loopback test. 
 *
 * Input:  - s_ds       - pointer to serial data structure 
 *           priority   - TX priority
 *
...
void 
prince_cleanup_serial (prince_serial_ds_t *s_ds, int priority)
{
    int i;
 ...
 *
 * This function cleans up mode in DMA mode register. 
 *
 * Input: s_ds - pointer to serial data structure
 *
 * Output: void 
...
void 
prince_cleanup_mode_bits (prince_serial_ds_t *s_ds)
{
    s_ds->proto_regs_addr->mode_reg &= ~PRINCE_MODE_MASK;
}
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_init_serial_ds
 *
 * This function initialized the serial data struct. 
 *
 * Input: s_ds - pointer to serial data structure
 *
 * Output: void 
...
void 
prince_init_serial_ds (prince_serial_ds_t *s_ds)
{
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_trap
 *
 * This function trap some helpful information for users and 
 ...
 */
void 
prince_trap (prince_serial_ds_t *s_ds, int priority) 
{
    ...
    s_ptr = (volatile ushort *)((char *)s_ds->cntrl_2_regs_addr + 0x000a);
    *s_ptr = 0x8000;   /* logical analyzer trigger bit */
    *s_ptr = 0;

    scc_base_addr = s_ds->base_addr;
    prince_display_dma_regs(scc_base_addr, PRINCE_DMA_TX, 2,
                          s_ds->port_num, priority);
    prince_display_dma_regs(scc_base_addr, PRINCE_DMA_RX, 2,
                          s_ds->port_num, priority);
    prince_display_cntrl_1_regs(scc_base_addr);
    prince_display_cntrl_2_regs(scc_base_addr);
    prince_display_proto_regs(scc_base_addr,  2,
                            s_ds->port_num);
    prince_display_serial_itf_regs(scc_base_addr, 2, s_ds->port_num);

    prince_form_name_buf(tmp_name_buf, s_ds, priority);
    if (s_ds->clk_src == PRINCE_CLK_SRC_SYNC )
        des_baud = prince_sync_baud[s_ds->speed_idx].desired_baud;
    else if (s_ds->clk_src == PRINCE_CLK_SRC_ASYNC) 
        des_baud = prince_async_baud[s_ds->speed_idx].desired_baud;
...
/*-----------------------------------------------------------------------------
 *
 * Function: prince_prince_form_name_buf
 *
 * This function form a name buffer to print 
 *
...
int 
prince_form_name_buf (char *tmp_name_buf, prince_serial_ds_t *s_ds, int priority)
{
...
Revision 1.2  2013/06/25 08:20:05  xiaoyizh
