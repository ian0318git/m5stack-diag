/* $Id: tam_act2_api_drv_support.c,v 1.5 2020/02/04 08:49:43 alicehua Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nutella/tam_act2_api_drv_support.c,v $
 *------------------------------------------------------------------
 *
...
int act2_drv_read (void *module, char *receive_buf, unsigned int length)
{
    ...
    if (aikido_act2_flag == TRUE) {
        n2g_i2c_act = (n2g_i2c_if_t *) get_n2g_i2c_if(I2C_CTRL_ZERO, 
                                       I2C_MUX_ZERO, MB_I2C_ADDR_AIKIDO_ACT2);
    } else {
        n2g_i2c_act = (n2g_i2c_if_t *) get_n2g_i2c_if(I2C_CTRL_ZERO, 
                                       I2C_MUX_ZERO, MB_I2C_ADDR_ACT2);
    }
...
int tam_act2_i2c_initialize (void)
{
    return (diagact2_lib_initialize(MB_I2C_ADDR_AIKIDO_ACT2));
}
...
int diagact2_lib_initialize (int addr)
{
    ...
    if (addr == MB_I2C_ADDR_AIKIDO_ACT2) {
...
{   
    ...
    return(bytes_to_send); /* pass, return length */
}
 ...
 * This function does the read from the ACT2 device. The FPGA is the master.
 * Calls are made to this function from within the act2 tam libary located at.
 * /auto/sp-engops/diags/pld/act2lite/x86/WNBU/Katar_WLC/Aikido_v3_4_4
 *
 * Inputs: ptr to module(platform_opaque_handle) -
 *         min_time - min time to wait (for future use)
 *         max_time - max time to wait (for future use)
...
{
    /* If it is Aikido, don't reset FPGA_ACT2_RST_L */
    if ((is_tam_aikido_on() == TRUE) && (mask == FPGA_RST_ACT2)) {
        return;
    } else {
        /* Act2 initialization reset sequence */
        if (fpga_version_is_more_than_2p0() == FALSE) {
            fpga_reset_api(FPGA_EXTER_DEV_RST_REG, ACT2_RESET, TRUE,
                           WAITTIME_5_MS);
        } else {
            fpga_reset_api(CEDGE_LPC_DEV_RST_CONTROL_REG, CEDGE_FPGA_ACT2_RST_L,
                           TRUE, WAITTIME_5_MS);
        }
...
void unreset_plat_dev(unsigned int mask)
{
    ...
    /* If it is Aikido, don't unreset FPGA_ACT2_RST_L */
    if ((is_tam_aikido_on() == TRUE) && (mask == FPGA_RST_ACT2)) {
        ...
        if (fpga_version_is_more_than_2p0() == FALSE) {
            fpga_reset_api(FPGA_EXTER_DEV_RST_REG, ACT2_RESET, FALSE, SLEEP_5S);
        } else {
            fpga_reset_api(CEDGE_LPC_DEV_RST_CONTROL_REG, CEDGE_FPGA_ACT2_RST_L,
                           FALSE, SLEEP_5S);
        }
