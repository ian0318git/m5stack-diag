/* $Id: diag_rtc_test.c,v 1.4 2019/07/11 12:31:29 alicehua Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nutella/diag_rtc_test.c,v $
 *------------------------------------------------------------------
 *
...
int diag_rtc_exec (int test, unsigned int *param1, unsigned int *param2,
                   unsigned int *param3, char *param4)
{
    FILE *fp;
...
int rtc_test_regs (void)
{
    ...
    rtc_init();
    /* Abort update cycle */
    rtc_write_byte(RTC_UPDATE_CYCLE_BYTE_B, 0x82);
    ...
    printf(" Set day: %.2d\n", tv.date);
    BIN_TO_BCD(tv.date);
    rtc_write_byte(RTC_DAY_OF_MONTH_BYTE_7, tv.date);
    tv.date = rtc_read_byte(RTC_DAY_OF_MONTH_BYTE_7);
    BCD_TO_BIN(tv.date);
    printf(" New day: %.2d\n", tv.date);
    ...
    printf(" Set month: %.2d\n", tv.month);
    BIN_TO_BCD(tv.month);
    rtc_write_byte(RTC_MONTH_BYTE_8, tv.month);
    tv.month = rtc_read_byte(RTC_MONTH_BYTE_8);
    BCD_TO_BIN(tv.month);
    printf(" New month: %.2d\n", tv.month);
    ...
    BIN_TO_BCD(tv.year);
    rtc_write_byte(RTC_YEAR_BYTE_9, tv.year);
    tv.year = rtc_read_byte(RTC_YEAR_BYTE_9);
    BCD_TO_BIN(tv.year);
    ...
    printf(" Set hour: %.2d\n", tv.hour);
    BIN_TO_BCD(tv.hour);
    rtc_write_byte(RTC_HOUR_BYTE_4, tv.hour);
    tv.hour = rtc_read_byte(RTC_HOUR_BYTE_4);
    BCD_TO_BIN(tv.hour);
    printf(" New hour: %.2d\n", tv.hour);
    ...
    rtc_write_byte(RTC_MINUTE_BYTE_2, tv.minute);
    tv.minute = rtc_read_byte(RTC_MINUTE_BYTE_2);
    ...
    rtc_write_byte(RTC_SECOND_BYTE_0, tv.second);
    tv.second = rtc_read_byte(RTC_SECOND_BYTE_0);
    ...
    rtc_write_byte(RTC_UPDATE_CYCLE_BYTE_B, 0x0);
    rtc_write_byte(RTC_UPDATE_CYCLE_BYTE_B, 0x02);
    rtc_finish();
...
static submenu_xtable_t rtc_main_menu_table[] = {
    { "Register Test",      (PFT)rtc_test_regs,  0,
        RTC_MM_3,               (type_t(*)())0,     0,
        (type_t(*)())0, 0 },
    { "Feature Test",      (PFT)rtc_test_feature,  0,
        RTC_MM_3,               (type_t(*)())0,     0,
        (type_t(*)())0, 0 },
};
