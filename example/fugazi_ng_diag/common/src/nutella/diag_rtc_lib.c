/* $Id: diag_rtc_lib.c,v 1.4 2019/07/11 12:31:29 alicehua Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nutella/diag_rtc_lib.c,v $
 *------------------------------------------------------------------
 *
 ...
 *******************************************************************************
 */
unsigned char rtc_read_byte(char offset)
{
	unsigned char a;
 ...
 *******************************************************************************
 */
void rtc_write_byte(char offset, char data)
{
    if (rtc_inited == 0) {
		rtc_init();
    }
...
{
	unsigned char a,b;
	a = rtc_read_byte(offs);
	b = rtc_read_byte(offs+1);
...
void rtc_set_time(rtc_time_t *tv)
{
	...
	rtc_write_byte(RTC_CENTURY, CENTURY_DEFAULT);	/* CSCvn21696 : Based on IOS requested, set offset 14 to 0 */
}
