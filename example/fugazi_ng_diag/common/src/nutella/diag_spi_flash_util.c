/* $Id: diag_spi_flash_util.c,v 1.5 2019/07/12 09:13:42 alicehua Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nutella/diag_spi_flash_util.c,v $
 * 
...
int nutella_spi_flash_utils(int opt);
int nutella_spi_flash_get_prod_id(int, uint*);
int nutella_spi_flash_get_prod_name(int, char*);
int nutella_spi_flash_get_rdsr(int, uchar*);
int nutella_spi_flash_set_wrsr(int, uchar); 
int nutella_spi_flash_show_info(int); 
int nutella_spi_flash_write_protect_enable(int);
...
static submenu_xtable_t spi_flash_utils_tbl[] = {
    {"Show Golden SPI Flash Info", (type_t(*)())nutella_spi_flash_show_info, GOLDEN_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Show Upgrade SPI Flash Info", (type_t(*)())nutella_spi_flash_show_info, UPGRADE_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Show FPGA SPI Flash Info", (type_t(*)())nutella_spi_flash_show_info, FPGA_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Enable Golden SPI Flash Write Protect", (type_t(*)())nutella_spi_flash_write_protect_enable, GOLDEN_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Enable Upgrade SPI Flash Write Protect", (type_t(*)())nutella_spi_flash_write_protect_enable, UPGRADE_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Enable FPGA SPI Flash Write Protect", (type_t(*)())nutella_spi_flash_write_protect_enable, FPGA_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Golden SPI Flash Write Protect Test", (type_t(*)())diag_spi_flash_write_protect_test, GOLDEN_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Upgrade SPI Flash Write Protect Test", (type_t(*)())diag_spi_flash_write_protect_test, UPGRADE_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"FPGA SPI Flash Write Protect Test", (type_t(*)())diag_spi_flash_write_protect_test, FPGA_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
     /* for Debug*/
#ifdef SPI_FLASH_DEBUG
    {"FPGA SPI Flash Read Reg", (type_t(*)())fpga_read_spi_prom_reg, 0, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"FPGA SPI Flash Write Reg", (type_t(*)())fpga_write_spi_prom_reg, 0, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"RDSR - Golden", (type_t(*)())denverton_spi_read_status_reg, GOLDEN_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"WRSR - Golden", (type_t(*)())denverton_spi_write_status_reg, GOLDEN_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"RDSR - Upgrade", (type_t(*)())denverton_spi_read_status_reg, UPGRADE_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"WRSR - Upgrade", (type_t(*)())denverton_spi_write_status_reg, UPGRADE_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
#endif
};
 ...
 * Description : Based on opcode to read and write Denverton SPI Controller memory.
...
int denverton_spi_hw_cycle(uchar opcode, int len)
{
    ...
    /* According to CPU datasheet "Hardware Sequencing Flash Status and Control
     * (BIOS_HSFSTS_CTL) - Offset 4h" register, the Flash Cycle Go (FGO) bit is
     * cleared by hardware when the operation is complete. */
    for (ix = 0; ix < HW_CYCLE_POLLING_COUNT_MAX; ix++) {
        ret = denverton_spi_ctrl_mm_read32(HSFSTS_CTL, &status);
        if (ret != PASSED) {
            printf("%s: Failed to read Denverton SPI Controller memory.\n", __FUNCTION__);
            return (FAILED);
        }
        /* Check if flash cycle is completed. */
        if (!(status & HSFSTS_CTL_FGO)) {
            break;
        }
        msleep(HW_CYCLE_POLLING_LOOP_WAIT);
    }
...
int denverton_spi_write_status_reg (int select_flash)
{
    ...
    if (select_flash == GOLDEN_SPI_FLASH) {
        wr_data = ENABLE_FIRST_BOOTFLASH;
    } else if (select_flash == UPGRADE_SPI_FLASH) { 
        wr_data = ENABLE_SECONDARY_BOOTFLASH;
    }
    
    /* nutella uses fpga to mux the bootflash */
    ...
    if (select_flash == GOLDEN_SPI_FLASH || select_flash == UPGRADE_SPI_FLASH) {
        /* Set FPGA bootflash mux back to default value.*/
        fpga_write_reg(reg_addr, 0);
    }
...
int denverton_spi_read_status_reg (int select_flash)
{
    ...
    if (denverton_spi_read_reg(SPINOR_OP_RDSR, &rdsr, 1) == FAILED) {
        cterr('f', 0, "Unable to read Nutella SPI status register (RDSR)\n");
        return(FAILED);
    }
...
/*******************************************************************************
 *
 * Function    : nutella_spi_flash_utils
 * Description : Function to show NUTELLA SPI Flash utilities submenu.
 * Inputs      : opt - reserved for future use.
 * Outputs     : PASSED / FAILED
 *
 *******************************************************************************
...
/*******************************************************************************
 *
 * Function    : nutella_spi_flash_get_prod_id
 ...
 *******************************************************************************
 */
int nutella_spi_flash_get_prod_id (int select_flash, uint *spi_id) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
...
/*******************************************************************************
 *
 * Function    : nutella_spi_flash_get_prod_name
 ...
 *******************************************************************************
 */
int nutella_spi_flash_get_prod_name (int select_flash, char *buf) 
{
 
    uint spi_prod_id = 0;
    int ret = 0;

    ret = nutella_spi_flash_get_prod_id(select_flash, &spi_prod_id);
    if (ret != PASSED) {
        return (FAILED);
    } 
...
/*******************************************************************************
 *
 * Function    : nutella_spi_flash_get_rdsr
 ...
 *******************************************************************************
 */
int nutella_spi_flash_get_rdsr (int select_flash, uchar * rdsr_val) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
    ...
    if (select_flash == GOLDEN_SPI_FLASH) {
        ...
        ret = fpga_read_spi_prom_status_reg(&rdsr);
        if (ret != PASSED) {
...
/*******************************************************************************
 *
 * Function    : nutella_spi_flash_set_wrsr
 ...
 *******************************************************************************
 */
int nutella_spi_flash_set_wrsr (int select_flash, uchar wrsr_val) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
    ...
    if (select_flash == GOLDEN_SPI_FLASH) {
        ...
        ret = fpga_write_spi_prom_status_reg(wrsr_val);
        if (ret != PASSED) {
...
/*******************************************************************************
 *
 * Function    : nutella_spi_flash_show_info
 ...
 *******************************************************************************
 */
int nutella_spi_flash_show_info (int select_flash) 
{
    int ret = 0;
    ...
    if ( select_flash == GOLDEN_SPI_FLASH || select_flash == UPGRADE_SPI_FLASH 
        ...
        switch (select_flash) {
        case GOLDEN_SPI_FLASH:
            sprintf(flash_name, GOLDEN_SPI_FLASH_NAME);
            break;
        case UPGRADE_SPI_FLASH:
            sprintf(flash_name, UPGRADE_SPI_FLASH_NAME);
            break;
        case FPGA_SPI_FLASH:
    ...
    ret = nutella_spi_flash_get_prod_name(select_flash, prod_name);
    if (ret != PASSED) {
        printf("%s: Failed to get SPI Flash product name.\n", __FUNCTION__);
        return (FAILED);
    } 

    ret = nutella_spi_flash_get_rdsr(select_flash, &rdsr);
    if (ret != PASSED) {
...
/*******************************************************************************
 *
 * Function    : nutella_spi_flash_write_protect_enable
 ...
 *******************************************************************************
 */
int nutella_spi_flash_write_protect_enable (int select_flash) 
{
    uint spi_prod_id = 0;
    int ret = 0;
    uchar wrsr = 0;
    ...
    case SPI_GD25Q16C_ID:
        ...
        break;
    case SPI_W25Q16JV_ID:
        wrsr = SPI_W25Q16JV_PROTECTED_1M_BP_CONF;
        break;
    default:
        printf("%s: Unsupported seleceted SPI Flash ID 0x%x.\n", __FUNCTION__, spi_prod_id);
        return (FAILED);  
    ...
    ret = nutella_spi_flash_set_wrsr(select_flash, wrsr);    
    if (ret != PASSED) {
...
int diag_spi_flash_write_protect_test(int select_flash) {
    ...
    char *tname[] = {GOLDEN_SPI_FLASH_NAME, UPGRADE_SPI_FLASH_NAME, FPGA_SPI_FLASH_NAME};
    uint reg_addr;
    uint wr_data;
    uchar rdsr = 0;
    uchar rdsr_bk = 0;
    ...
    if (select_flash == GOLDEN_SPI_FLASH) {	
        wr_data = ENABLE_FIRST_BOOTFLASH;
        testname("%s write protect", tname[GOLDEN_SPI_FLASH]); 
    } else if (select_flash == UPGRADE_SPI_FLASH){
        wr_data = ENABLE_SECONDARY_BOOTFLASH;
        testname("%s write protect", tname[UPGRADE_SPI_FLASH]);	
    } else if (select_flash == FPGA_SPI_FLASH){
    ...
    /* Check if FPGA flash is under busy status. */
    if (fpga_spi_flash_busy_status() == FAILED) {
        return (FAILED);
    }
    ...
    /* Assign register value to clear the protect enable bit. */
    wrsr = (~(SPI_FLASH_STATUS_REG_PROTECT_EN) & rdsr);
    ...
    /* Check if Status register protect enable bit is clear or not.
     * If it's cleared which means it's still writable, 
     * not under protect status, needs to return fail, otherwise,
     * it's under protect status. */
    if (!(rdsr & SPI_FLASH_STATUS_REG_PROTECT_EN)) {
        wrsr = rdsr_bk;

        cterr('f', 0, "SPI Flash write protect test failed. Status Register is "
              "writable");
...
int spi_flash_write_protect_test (int select_flash)
{
    ...
    if(select_flash == GOLDEN_SPI_FLASH || select_flash == UPGRADE_SPI_FLASH) {
        retval = mtd_spi_flash_write_protect_test(BOOT_FLASH_DEV_PATH);
    } else if (select_flash == FPGA_SPI_FLASH) {
...
int mtd_spi_flash_write_protect_test (char *src)
{
    ...
    /* From SPI flash write protect from 0xFC0000 - 0xFFFFFF*/
...
int fpga_spi_flash_write_protect_test (void)
{
    ...
    ret = nutella_spi_flash_get_prod_id(FPGA_SPI_FLASH, &spi_prod_id);
    if (ret != PASSED) {
        return (FAILED);
    } 	
