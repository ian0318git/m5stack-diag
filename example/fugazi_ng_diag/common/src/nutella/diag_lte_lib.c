/* $Id: diag_lte_lib.c,v 1.6 2020/02/19 03:11:29 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nutella/diag_lte_lib.c,v $
 *------------------------------------------------------------------
 *
...
int diag_lte_dport_enable(int);
int diag_lte_chk_modem_carrier_is_match(void);
...
int diag_lte_swi_dev_create (dev_lte_swi_object_t *lte_swi_obj)
{
    ...
    if (strstr(lte_swi_obj->model, "WP7601") || strstr(lte_swi_obj->model, "WP7603")
              || strstr(lte_swi_obj->model, "WP7607") || strstr(lte_swi_obj->model,
              "WP7610")) {
        if (strstr(lte_swi_obj->model, "WP7607")) {
            lte_swi_obj->modem_type = SWI_WP7607_08_09;
        } else if (strstr(lte_swi_obj->model, "WP7610")) {
            lte_swi_obj->modem_type = SWI_WP7610;
        } else { 
            lte_swi_obj->modem_type = SWI_WP7601_03;
        }
    } else {
        printf("Unknow Modem");
        return (FAILED);
...
int diag_lte_pwr_on (int turn_on)
{
    ...
    if (turn_on == TRUE) {
        /* 1. Enable WDISABLE# pin floating, modem operational */
        ...
        /* 3. Power On modem (POWER_ON_N set to 1)*/
        if (fpga_read_reg(FPGA_LTE_CTL_REG, &read_data) != PASSED) {
        ...
        /* 1. Power down modem */
        if (diag_lte_modem_power_down() == FAILED) {
            printf("Fail to power down the LTE modem.\n");
            return (FAILED);
        }
...
static void diag_lte_atcmd_assign_ttydev (void)
{
    ...
    sprintf(lte_atcmd_tty_dev, "%s", LTE_USB_UDEV_AT_TTY_DEV);
}
...
int diag_lte_dport_host_usb_detect (char *vid, int speed)
{
    ...
    if ((strcmp(vid, usb[2].mfg[0]) == 0) && usb[2].spd == speed) {
        return (PASSED);
    } else if ((usb[3].lev == 1) && (usb[3].prnt == 1) && (usb[3].port == 1)) {
        /* If Lev = 01 and Prnt = 01 and Port = 01 then LTE USB is on CPU side not on 
         * debug port side. */
        /* LTE device still on CPU */
        return (LTE_USB_MUX_NOT_ON_DEBUG_PORT);
    } else {
        return (FAILED);
    }
}
 ...
 *
 * Input : r_opt (TRUE; Nutella CPU --> LTE --> External Micro USB) 
...
/**********************************************************************
 *
 * Function: diag_lte_dport_enable 
 *
 * Description: Enable/Disable LTE USB debug port
 *
 ...
 **********************************************************************
*/
int diag_lte_dport_enable(int enable)
{
    ...
    if (enable == TRUE){
        reg_val = (LTE_MODEM_POWER_CONTROL | EXT_PRI_LTE_WDIS_1_RESET |
                   LTE_USB_MUX_SEL_CTL | LTE_PRI_MODEM_EN_CTL);
    ...
    if (fpga_write_reg(reg_offset, reg_val) != PASSED) {
        cterr('f', 0, "Fail to write 0x%08X to FPGA register(0x%04X).\n",
              reg_val, reg_offset);
        return (FAILED);
    }
...
/*******************************************************************************
 *
 * Function    : diag_lte_chk_modem_carrier_is_match
 * Description : Function to check modem carrier is matched
 * Inputs      : None
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
...
int diag_lte_chk_modem_carrier_is_match (void)
{
    ...
    printf("\nCheck whether LTE modem carrier is matched or not.\n");

    /* Max time: 1 mins */
    for (ix = 0; ix < MAX_IMG_CARRIER_MATCH_TIME; ix++) {
        if (lte_obj_p->callin_fvt->modem_carrier_is_match((dev_object_t *)&lte_obj) == PASSED) {
            break;
        }
