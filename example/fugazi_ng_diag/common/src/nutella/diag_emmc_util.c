/* $Id: diag_emmc_util.c,v 1.5 2019/11/21 00:27:40 alicehua Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nutella/diag_emmc_util.c,v $
 *------------------------------------------------------------------
 *
...
int emmc_pslc_fully_enable (int opt)
{
    ...
    gp_length_kib = GPP_1P5_GB_LEN_KB;
    ...
    value = (enh_multiplier >> 16) & ENH_MULT_MASK;
    ...
    value = (enh_multiplier >> 8) & ENH_MULT_MASK;
    ...
    value = enh_multiplier & ENH_MULT_MASK;
    ...
    /* No GPP needed on nutella */
    value = ext_csd[EXT_CSD_PARTITIONS_ATTRIBUTE];
    value |= (1 << partition);

    value |= SET_ENHANCED_FOR_UDA; /* Set Enhanced for UDA */
...
static int get_emmc_cid_info (emmc_info_t *emmc_info)
{
    ...
    } else if (strstr(buf, EMMC_SANDISK_MANFID_STR) != NULL) {
        sprintf(emmc_info->manf_name, "SanDisk(0x45)");
    } else {
