/* $Id: diag_fpga_upgrade.c,v 1.5 2020/05/07 01:34:56 alicehua Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/nutella/diag_fpga_upgrade.c,v $
 *-----------------------------------------------------------------------------
 * diag_fpga_upgrade.c - 
 * 
 *
...
static int program_secure_boot_header(uchar type, boolean);
...
static int program_spi_prom_image_header(uchar type, boolean);
static int diag_fpga_ttf2array(char *file, uint *read_size);
int program_image_update_type(int);
...
int fpga_read_spi_prom_reg(int);
int fpga_write_spi_prom_reg(int);
...
unsigned int fpga_fw_size = 0;
unsigned char *fpga_fw_array = NULL;
...
static reggio_spi_eprom_info_t spi_eprom_info[1] = {
    {
        ...
        .start_sector_u = 16,
        .end_sector_u = 30,
        .start_header =  0x1F0000,  /* not used for now */
        .end_header = 0x1F000F, /* not used for now */
    },
};
...
{
    assert(fpga_ptr);
    return ((unsigned long)fpga_ptr + FPGA_SPI_PROM_OFFSET);
}
...
{   
    if (!fpga_fw_array) {
        assert(!"nutellalnx: missing fpga file name in its command line argument");
    }
    return (unsigned char *)(((unsigned long)fpga_fw_array));
...
/*-------------------------------------------------------------------
 *
 * Function: fpga_spi_flash_busy_status()
 * 
 * This function check if the FPGA flash busy bit is clear or not.  
 *
 * Input: None 
 *
 * Output: FAILED means flash is still under busy status, PASSED otherwise
 *
 *-------------------------------------------------------------------
 */
boolean fpga_spi_flash_busy_status(void)
{
    int ix;
    uchar rdsr = 0;

    /* Polling to check if flash is under busy status or not */
    for (ix = 0; ix < MAX_POLLING_FPGA_FLASH_BUSY_TIMES; ix++) {
        if (nutella_spi_flash_get_rdsr(FPGA_SPI_FLASH, &rdsr) != PASSED) {
            cterr('f', 0, "SPI Flash read rdsr failed.");
            return (FAILED);
        }
        /* FPGA flash is not under the busy status. */
        if (!(rdsr & FLASH_BUSY_STATUS)) {
            break;
        }
        msleep(EACH_POLLING_LOOP_DELAY_TIME);
    }

    /* Timeout for checking FPGA flash busy status */
    if (ix == MAX_POLLING_FPGA_FLASH_BUSY_TIMES) {
        cterr('f', 0, "SPI Flash is under busy status for 10 seconds.");
        return (FAILED);
    }
...
static int diag_fpga_ttf2array (char *file, uint *read_size)
{
    ...
    fpga_fw_array = malloc(fpga_fw_size + 1000);
    printf("FPGA_FW = %s\n", fpga_fw_array);
    if (fpga_fw_array == NULL) {
...
int program_reggio_spi_prom_old (int header)
{
    ...
    if ((image_type == FPGA_UPGRADE_IMAGE)) {
        printf("After Re-Program FPGA, it will do Reconfiguration FSM reset.\n");
    }
...
{ 
    ...
    //    fpga_header.flags[0] = 0xA0 | update_flag;
...
/*-------------------------------------------------------------------
 *
 * Function: fpga_read_spi_prom_reg
 * 
 * This function read the register of the SPI PROM
 * device.
 *
 * Input: None 
 *
 ...
 *-------------------------------------------------------------------
 */
int fpga_read_spi_prom_reg (int dummy)
{
    prom_t *spi_prom;
    uint opcode;
    int reg_size;
    ...
    reg_size = (uchar)gethex_answer("Read size: ", 0, 0, 0xFF);
    opcode = gethex_answer("Enter opcode: ", 0, 0, 0xFFFFFFFF);
    
    spi_prom = init_spi_prom_ds();

    /* Read the SPI PROM Read Register */
    spi_prom->size        = DSWAP4(reg_size & PROM_RD_SIZE_MASK);
    spi_prom->opcode_addr = DSWAP4(opcode);
    spi_prom->control     = DSWAP4(PROM_DFLT_BAUD); /* Read */
...
/*-------------------------------------------------------------------
 *
 * Function: fpga_write_spi_prom_reg
 * 
 * This function writes data into the SPI PROM register.
 *
 * Input: None 
 *
 * Output: PASSED if operation succeeded, FAILED otherwise
 ...
 *-------------------------------------------------------------------
 */
int fpga_write_spi_prom_reg (int dummy)
{
    prom_t *spi_prom;
    uint opcode, reg_size, w_data;

    spi_prom = init_spi_prom_ds();

    reg_size = (uchar)gethex_answer("Write size: ", 0, 0, 0xFF);
    opcode = gethex_answer("Enter opcode: ", 0, 0, 0xFFFFFFFF);
    w_data = gethex_answer("Enter write data: ", 0, 0, 0xFFFFFFFF);
...
/*-------------------------------------------------------------------
 *
 * Function: fpga_corrupt_header_spi_prom()
 * 
 * This function will set update flag into 0xA3, and corrupt the image header.
 *
 * Input: dummy-- not used.
 * Output: PASSED/FAILED
 *
 *-------------------------------------------------------------------
 */
int fpga_corrupt_header_spi_prom (int dummy)
{ 
    uint start_addr, end_addr;
    uint pgm_size = REGGIO_FPGA_HDR_SIZE;
    reggio_spi_prom_image_header_t fpga_header;
    ...
    /* Start building wrong image header to corrupt header.
     * magic_number[3] = 0x7e -> 0x7f, update_flag = 1 -> 3. */
    fpga_header.magic_number[0] = 0x06;
    fpga_header.magic_number[1] = 0x5d;
    fpga_header.magic_number[2] = 0x4f;
    fpga_header.magic_number[3] = 0x7f;
