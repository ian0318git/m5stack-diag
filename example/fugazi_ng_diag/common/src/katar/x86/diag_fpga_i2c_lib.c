/* $Id: diag_fpga_i2c_lib.c,v 1.2 2019/06/14 05:24:48 mikech2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/katar/x86/diag_fpga_i2c_lib.c,v $
 *------------------------------------------------------------------
 *
...
int is_fpga_i2c_scanned_aikido_addr(int);
...
unsigned long get_fpgai2c_reg_base (void);


#define MB_I2C_ADDR_AIKIDO_ACT2   	 0x77

#define FPGA_I2C_CTL_REG             0x000
#define FPGA_I2C_SCRATCH_PAD         0x004
#define FPGA_I2C_STAT_REG            0x008
#define FPGA_I2C_STAT_MASK_REG       0x00C
#define FPGA_I2C_SLA_ADDR_REG        0x010
#define FPGA_I2C_SLA_SUBADDR_REG     0x014
#define FPGA_I2C_BIT_BANG_REG        0x018
#define FPGA_I2C_BYTE_COUNT_REG      0x01C
#define FPGA_I2C_DATA_FIFO_REG       0x040
#define FPGA_I2C_DATA_RW_PTR_REG     0x044
...
{
    unsigned long addr = get_fpgai2c_reg_base();

    *buf = *((unsigned int *)((long)addr + reg_offset));

    if (diagflag_xram & D_DEBUG_OPTIONS) {
        printf("===== %s reg_offset=0x%x | *buf=0x%x =====\n", __FUNCTION__, reg_offset, *buf);
    }
...
int fpga_write_reg (uint reg_offset, uint wr_data)
{
    ...
    *((unsigned int *)((long)addr + reg_offset)) = wr_data;

    if (diagflag_xram & D_DEBUG_OPTIONS) {
        printf("===== %s reg_offset=0x%x | wr_data=0x%x =====\n", __FUNCTION__, reg_offset, wr_data);
    }
...
/*******************************************************************************
 *
 * Function   : is_fpga_i2c_scanned_aikido_addr (int option)
 * Description: scan aikido on fpga i2c bus
 *
 ...
 *******************************************************************************
 */
int is_fpga_i2c_scanned_aikido_addr (int option)
{   
    n2g_i2c_if_t  i2c_if;
    ...
    /* Now just have ACT2 on FPGA I2C bus */
...
{   
    n2g_i2c_if_t  i2c_if;
    uint32_t      ret_val = FAILED, now_addr = 0, ctr = 0;
    uchar         d32[80];
		                   ...
		                   uchar *data_p)
{
    ...
    for (i=0; i < word_count; i++) {
        ...
        if ((NVRAM)->diagflag & D_VERBOSE) {
            print_offset("reading from data fifo @", dash_fpga, 
...
void fpga_i2c_reset (void)
{
    ...
    if ((NVRAM)->diagflag & D_VERBOSE) {
        print_offset_val("", dash_fpga, (ulong)i2c.bb, __LINE__, 0);
    }
                 ...
                 uint32_t data_len, uchar *data_buf)
{
    ...
    if (fpga_chk_i2c_idle(i2c) == FALSE) {
        return(RC_I2C_BUSY);
    }
    if ((NVRAM)->diagflag & D_VERBOSE) {
        printf("READING %d:\n", data_len);
...
Revision 1.1.2.8  2019/06/10 01:34:01  mikech2
Remove diag_i2c_addr.h base on PRRQ#4685780 Comment#3

Revision 1.1.2.7  2019/06/06 02:44:25  mikech2
clean out the unnecessary definitions and TSN string

Revision 1.1.2.6  2019/03/13 03:34:15  peteteng
Code cleanup

Revision 1.1.2.5  2019/02/12 08:06:28  mikech2
rename katar_*.h files

Revision 1.1.2.4  2019/01/29 01:54:19  mikech2
rename katar_* files

Revision 1.1.2.3  2018/12/27 00:42:24  peteteng
Support Aikido thru UserLogic FPGA I2C

Revision 1.1.2.2  2018/12/21 07:28:45  peteteng
