/* $Id: leb_mod_mic_reg.c,v 1.2 2014/06/03 10:53:30 erwu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/lebowski_python/module/leb_mod_mic_reg.c,v $
 *------------------------------------------------------------------
 * Description: MIC register sample with pass case to show
...
main(int argc,char *argv[])
{
    ...
    if (test_mic_register_fn(argc,argv) != PASSED) {
        return FAILED;
    } else {
        return PASSED;
    }
 ...
 *****************************************************************************/
int
test_mic_register_fn(int argc,char *argv[])
{
    /* flag usage example */
    if (optional_output == 1) {
        printf("\n Optional output flag ON \n");
        printf("\n optional_output in leb_mod_mic_reg \n");
    } else {
        printf("\n Optional output flag OFF \n");
        printf("\n non optional output in leb_mod_mic_reg \n");
    }

    testname("MIC REG");
    prpass(testpass, "mic register test, ");
    prcomplete(testpass, errcount, (char *)0);
    return PASSED;
}

/******** History ********
$Log: leb_mod_mic_reg.c,v $
