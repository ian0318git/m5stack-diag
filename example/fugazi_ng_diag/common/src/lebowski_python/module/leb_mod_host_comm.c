/* $Id: leb_mod_host_comm.c,v 1.2 2014/06/03 10:53:29 erwu2 Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/lebowski_python/module/leb_mod_host_comm.c,v $
 *------------------------------------------------------------------
 * Description: lebowski host communication sample with pass case to show
...
main(int argc,char *argv[])
{
    ...
    if (test_leb_host_comm_fn(argc,argv) != PASSED) {
        return FAILED;
    } else {
        return PASSED;
    }
...
/*****************************************************************************
 *
 * Function   : test_leb_host_comm_fn
 ...
 *****************************************************************************/
int
test_leb_host_comm_fn(int argc,char *argv[])
{
    /* flag usage example */
    if (exec_authentication == 1) {
        printf("\n eXec authentication flag ON \n");
        printf("\n exec_authentication in leb_mod_host_comm \n");
    } else {
        printf("\n eXec authentication flag OFF \n");
        printf("\n non eXec authentication in leb_mod_host_comm \n");
    }

    testname("leb host comm");
    prpass(testpass, "host communication test, ");
    prcomplete(testpass, errcount, (char *)0);
    return PASSED;
}

/******** History ********
$Log: leb_mod_host_comm.c,v $
