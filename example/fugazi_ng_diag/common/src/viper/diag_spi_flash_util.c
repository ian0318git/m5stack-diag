/* $Id: diag_spi_flash_util.c,v 1.4 2019/01/09 07:37:26 chieyang Exp $
 *------------------------------------------------------------------
 * 
...
int viper_spi_flash_utils(int opt);
int viper_spi_flash_get_prod_id(int, uint*);
int viper_spi_flash_get_prod_name(int, char*);
int viper_spi_flash_get_rdsr(int, uchar*);
int viper_spi_flash_set_wrsr(int, uchar); 
int viper_spi_flash_show_info(int); 
int viper_spi_flash_write_protect_enable(int);
...
static submenu_xtable_t spi_flash_utils_tbl[] = {
    {"Show Rommon Golden SPI Flash Info", (type_t(*)())viper_spi_flash_show_info, GOLDEN_ROMMON_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Show Rommon Upgrade SPI Flash Info", (type_t(*)())viper_spi_flash_show_info, UPGRADE_ROMMON_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Show FPGA SPI Flash Info", (type_t(*)())viper_spi_flash_show_info, FPGA_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Enable Rommon Golden SPI Flash Write Protect", (type_t(*)())viper_spi_flash_write_protect_enable, GOLDEN_ROMMON_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Enable Rommon Upgrade SPI Flash Write Protect", (type_t(*)())viper_spi_flash_write_protect_enable, UPGRADE_ROMMON_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    {"Enable FPGA SPI Flash Write Protect", (type_t(*)())viper_spi_flash_write_protect_enable, FPGA_SPI_FLASH, 0,
     (type_t(*)())0, 0, (type_t(*)())0,   0},
    ...
    {"FPGA SPI Flash Write Protect Test", (type_t(*)())diag_spi_flash_write_protect_test, FPGA_SPI_FLASH, 0,
     ...
     (type_t(*)())this_is_viper_j, 0, (type_t(*)())0,   0},
...
int denverton_spi_ctrl_mm_read32 (uint offset, uint *buf)
{
    ...
    target = (off_t)(VIPER_DENVERTON_SPI_CTRL_MM_ADDR + offset);
...
int denverton_spi_write_status_reg (int select_flash)
{
    ...
    if (select_flash == GOLDEN_ROMMON_SPI_FLASH) {
        ...
        /* viper uses fpga to mux the bootflash */
...
int denverton_spi_read_status_reg (int select_flash)
{
    ...
    if (denverton_spi_read_reg(SPINOR_OP_RDSR, &rdsr, 1) == FAILED) {
        cterr('f', 0, "Unable to read Viper SPI status register (RDSR)\n");
	return(FAILED);
    }
...
/*******************************************************************************
 *
 * Function    : viper_spi_flash_utils
 * Description : Function to show VIPER SPI Flash utilities submenu.
 * Inputs      : opt - reserved for future use.
 * Outputs     : PASSED / FAILED
 *
 *******************************************************************************
...
/*******************************************************************************
 *
 * Function    : viper_spi_flash_get_prod_id
 ...
 *******************************************************************************
 */
int viper_spi_flash_get_prod_id (int select_flash, uint *spi_id) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
...
/*******************************************************************************
 *
 * Function    : viper_spi_flash_get_prod_name
 ...
 *******************************************************************************
 */
int viper_spi_flash_get_prod_name (int select_flash, char *buf) 
{
 
    uint spi_prod_id = 0;
    int ret = 0;

    ret = viper_spi_flash_get_prod_id(select_flash, &spi_prod_id);
    if (ret != PASSED) {
        return (FAILED);
    } 
...
/*******************************************************************************
 *
 * Function    : viper_spi_flash_get_rdsr
 ...
 *******************************************************************************
 */
int viper_spi_flash_get_rdsr (int select_flash, uchar * rdsr_val) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
...
/*******************************************************************************
 *
 * Function    : viper_spi_flash_set_wrsr
 ...
 *******************************************************************************
 */
int viper_spi_flash_set_wrsr (int select_flash, uchar wrsr_val) 
{
    int ret = 0;
    uint reg_addr;
    uint wr_data;
...
/*******************************************************************************
 *
 * Function    : viper_spi_flash_show_info
 ...
 *******************************************************************************
 */
int viper_spi_flash_show_info (int select_flash) 
{
    int ret = 0;
    ...
    ret = viper_spi_flash_get_prod_name(select_flash, prod_name);
    if (ret != PASSED) {
        printf("%s: Failed to get SPI Flash product name.\n", __FUNCTION__);
        return (FAILED);
    } 

    ret = viper_spi_flash_get_rdsr(select_flash, &rdsr);
    if (ret != PASSED) {
...
/*******************************************************************************
 *
 * Function    : viper_spi_flash_write_protect_enable
 ...
 *******************************************************************************
 */
int viper_spi_flash_write_protect_enable (int select_flash) 
{
    uint spi_prod_id = 0;
    int ret = 0;
    uchar wrsr = 0;
    ...
    ret = viper_spi_flash_set_wrsr(select_flash, wrsr);    
    if (ret != PASSED) {
...
int fpga_spi_flash_write_protect_test (void)
{
    ...
    ret = viper_spi_flash_get_prod_id(FPGA_SPI_FLASH, &spi_prod_id);
    if (ret != PASSED) {
        return (FAILED);
    } 	
...
Revision 1.4  2019/01/09 07:37:26  chieyang
Add SR test in Boot flash/FPGA flash HW protect test for Viper-J/Viper-Intel platform.

Revision 1.3  2018/09/05 01:20:44  chieyang
Fix Rommon SPI flash write protect range to correct range for Viper-J and Viper-Intel.

Revision 1.2  2018/08/31 03:59:30  chieyang
