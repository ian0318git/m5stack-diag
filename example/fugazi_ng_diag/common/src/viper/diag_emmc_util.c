 /* $Id: diag_emmc_util.c,v 1.2 2018/08/06 02:31:50 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/viper/diag_emmc_util.c,v $
 *------------------------------------------------------------------
 *
...
int emmc_pslc_fully_enable (int opt)
{
    ...
    gp_length_kib = GPP_8MB_LEN_KB;
    partition = GPP_PARTITION_NUM;
    ...
    align = SECTOR_SIZE * ext_csd[EXT_CSD_HC_WP_GRP_SIZE] * ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE];
    gp_size_mult = (gp_length_kib + align/2l) / align;
    ...
    value = gp_size_mult & GPP_MULT_MASK;
    ...
    /* Reduce Enhance User Data Area by GPP 1 size */
    enh_multiplier -= gp_size_mult;

    value = (enh_multiplier >> 16) & GPP_MULT_MASK;
    ...
    value = (enh_multiplier >> 8) & GPP_MULT_MASK;
    ...
    value = enh_multiplier & GPP_MULT_MASK;
    ...
    /* Set Enhanced attribute for GPP 1 and UDA */
...
int show_emmc_info (void)
{
    ...
    printf("pSLC mode     : %s.\n",
           ...
           "Enabled" : "NOT enabled");
    printf("GPP partition : %s.\n",
           (access( EMMC_GPP_BLK, F_OK ) != -1) ?
...
Revision 1.1.2.3  2018/06/14 01:23:10  harrchan
Support sumsang manfid(0x15)

Revision 1.1.2.2  2018/06/07 02:46:22  lucywang
Modified EOL Conversion

Revision 1.1.2.1  2018/06/06 11:38:06  lucywang
