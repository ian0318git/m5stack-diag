 /* $Id: diag_fpga_upgrade.h,v 1.3 2018/08/31 03:59:30 chieyang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/viper/diag_fpga_upgrade.h,v $
 *--------------------------------------------------------------------
 * diag_fpga_upgrade.h
 *
...
typedef struct prom_t_ {  //typedef struct _reggio_spi_prom_reg_t {
    volatile unsigned int control; //    volatile uint   control;		 /* 0x0031800 */
    volatile unsigned int status;  //   volatile uint   status;	         /* 0x0031804 */
    volatile unsigned int size; //    volatile uint   size;              /* 0x0031808 */
    volatile unsigned int data;    //    volatile uint   data;           /* 0x003180C */
    volatile unsigned int opcode_addr;    // volatile uint   opcode_addr;    /* 0x0031810 */
} prom_t ;
...
typedef struct sys_lvl_t_ {
    ...
    volatile unsigned char pad4[0x18];  /* 0xA8 - 0x8C - 4 */
    volatile unsigned char jtag_sts;    /* 0xA8 Goldbeach used*/
} sys_lvl_t;
#endif
