 /* $Id: diag_fpga_upgrade.c,v 1.3 2018/08/31 03:59:30 chieyang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/viper/diag_fpga_upgrade.c,v $
 *-----------------------------------------------------------------------------
 * diag_fpga_upgrade.c - 
 * 
 *
...
{
    assert(dash_fpga);
    return ((unsigned long)dash_fpga + FPGA_SPI_PROM_OFFSET);
}
 ...
 *-------------------------------------------------------------------
 */
static int is_read_fifo_empty (prom_t *spi_prom)
{
    ulong count = 0;
 ...
 *-------------------------------------------------------------------
 */
int program_reggio_spi_prom (int header)
{
    int status;
...
{ 
    ...
    fpga_header.flags[0] = update_flag | NEWER_UPGRADE_A1; //always upgrade;
 ...
 *
 * Revision 1.1.2.4  2018/05/24 08:43:49  lucywang
 * Modified FPGA upgrade utility name to be clearer
 *
 * Revision 1.1.2.3  2018/04/13 11:19:12  lucywang
 * Modified to use Cisco FPGA : 1) Upgrade 2) LED 3) FPGA register 4) FPGA I2C reset
 *
 * Revision 1.1.2.2  2018/03/23 06:41:40  lucywang
 * FPGA upgrade utility
 *
