/* $Id: switzer_common.c,v 1.2 2019/08/06 06:56:17 alpeng Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/switzer/switzer_common.c,v $
 *------------------------------------------------------------------
 *
 * switzer_common.c - Switzer common interfaces
 *
...
#define LOG_LEVEL_CHECK(lvl)                                \
    (lvl < 0 ? 0 : (lvl >= MAX_NR_SWITZER_LOG_LEVEL ?       \
                    MAX_NR_SWITZER_LOG_LEVEL - 1 : lvl))

static const char *log_prefix[] = {
    [SWITZER_LOG_LEVEL_FATAL] = COLOR("F ", RED_BLINK),
    [SWITZER_LOG_LEVEL_ERR]   = COLOR("E ", RED_BACKGROUND),
    [SWITZER_LOG_LEVEL_WARN]  = COLOR("W ", RED),
    [SWITZER_LOG_LEVEL_INFO]  = COLOR("I ", BLUE),
    [SWITZER_LOG_LEVEL_DBG]   = COLOR("D ", GREEN),
    [SWITZER_LOG_LEVEL_VDBG]  = COLOR("V ", BLACK),
};

int switzer_log_level = SWITZER_LOG_LEVEL_INFO;
                        ...
                        char *buf, size_t size, const char *fmt, va_list ap)
{
    ...
    do {
        ...
        if (lvl) {
            ...
            if ((len = snprintf(p, size, "%lu ", switzer_ticks)) > size)
                len = size;
            p += len;
            if (!(size -= len))
                break;
                       ...
                       const char *fmt, va_list ap)
{
    ...
    lvl = LOG_LEVEL_CHECK(lvl);
    if (lvl <= switzer_log_level) {
        len = switzer_log_vformat(t, lvl, buf, sizeof(buf), fmt, ap);
                      ...
                      int lvl, const char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    len = switzer_log_vtrace(t, lvl, fmt, ap);
    va_end(ap);
    return len;
}
...
int switzer_file_mmap(const char *path,
                      struct switzer_mmap *map, unsigned long mflags)
{
    int fd;
    int flags;
    off_t size;
...
#define PCI_LINK_TIMEOUT    3000 /* ms */

struct switzer_pci_dev_get_data {
    uint16_t domain;
    uint8_t bus;
    uint8_t secondary;
    uint8_t subordinate;
...
static int switzer_pci_dev_get_match(struct pci_dev *dev, void *_data)
{
    struct switzer_pci_dev_get_data *data = _data;
...
{
    struct switzer_pci_dev_get_data data = {
        .domain = domain,
        .bus = bus,
        .secondary = 0,
        .subordinate = 0xff,
    ...
    if ((pci = pci_dev_get_by_path(domain, bus, dev, func)))
        return pci;
    pci_dev_find(&data, switzer_pci_dev_get_match);
    if (!(pci = data.pci))
        return NULL;
...
static int switzer_hex_dump_width(size_t len)
{
    int width;
...
static void __switzer_hex_dump(const void *buf, size_t len, int rowsize,
...
void switzer_hex_dump(const void *buf, size_t size, off_t offset)
{
    ...
    snprintf(fmt, sizeof(fmt), "%%.%dlx(%%.%dlx): %%s\n",
             switzer_hex_dump_width(offset + size),
             switzer_hex_dump_width(size));
    ...
    for (i = 0; i < size; i += rowsize) {
        ...
        if (linelen > remaining)
            linelen = remaining;
        remaining -= rowsize;
        __switzer_hex_dump(buf + i, linelen, rowsize, linebuf, sizeof(linebuf));
        prt(fmt, offset + i, i, linebuf);
    }
}

static int __switzer_hex_to_bin(char ch)
{
    if ((ch >= '0') && (ch <= '9'))
        return ch - '0';
    ch = tolower(ch);
...
int switzer_hex_to_bin(const char *hex, void *buf, size_t size)
{
    ...
    while (size--) {
        if ((hi = __switzer_hex_to_bin(*hex++)) < 0)
            return -1;
        if ((lo = __switzer_hex_to_bin(*hex++)) < 0)
            return -1;
        *(uint8_t *)buf++ = (hi << 4) | lo;
    }
    return 0;
