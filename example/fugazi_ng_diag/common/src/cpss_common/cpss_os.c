/* $Id: cpss_os.c,v 1.2 2021/09/24 01:21:39 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/cpss_common/cpss_os.c,v $
 *------------------------------------------------------------------
 *
 * Filename:	cpss_os.c
 *
...
)
{
    ...
    if (dma_mem == NULL) {
        ERR("Fail to malloc memory for dma_mem_info_t.");
    ...
    if (ioctl(sub_fd, DM_SUB_IOC_ALLOC_DMA_MEM,
        ...
        ERR("Fail to get dma memory from kernel driver.");
    ...
    if (MAP_FAILED == dma_mem->user_addr) {
        ERR("Fail to map dma memory to user space.");
...
)
{
    ...
    if (g_dma_mem->user_addr == ptr) {
        ...
        if (dma_mem == g_dma_mem) {
            ERR("Fail to find the dma memory info.");
            pthread_mutex_unlock(&dma_mutex);
            return (GT_FAIL);
        }
    }
    ...
    if (ioctl(sub_fd, DM_SUB_IOC_FREE_DMA_MEM,
              (unsigned long)&dma_mem->mem) < 0) {
        ERR("Fail to free dma memory in kernel driver.");
...
)
{
    ...
    ERR("Fail to do convert the physical address(%lu) to virtual address.", (unsigned long)phyAddr);
...
)
{
    ...
    ERR("Fail to do convert the virtual address(%lu) to physical address.", (unsigned long)virtAddr);
...
)
{
    ...
    if (smid == NULL) {
        ERR("Bad param (%p).", smid);

        return (GT_BAD_PARAM);
    }
    csem = malloc(sizeof(count_sem_t));
    if (csem == NULL) {
        ERR("Fail to malloc memory to create a counting semaphore.");
...
)
{
    ...
    if (smidPtr == NULL) {
        ERR("Bad param (%p).", smidPtr);

        return (GT_BAD_PARAM);
    }
    csem = malloc(sizeof(count_sem_t));
    if (csem == NULL) {
        ERR("Fail to malloc memory to create a binary semaphore.");
...
)
{
    ...
    if (ret) {
        ERR("Fail to delete the mutex of a semaphore, ERROR = %d", ret);
        return (GT_FAIL);
    };

    ret = pthread_cond_destroy(&csem->cond);
    if (ret) {
        ERR("Fail to delete the cond of a semaphore, ERROR = %d", ret);
        return (GT_FAIL);
    };
...
)
{
    ...
    if (timeOut == CPSS_OS_SEM_NO_WAIT_CNS) {
        /* no wait */
        ERR("Fail to wait a semaphore without wait.");
        pthread_mutex_unlock(&csem->mtx);
        return (GT_FAIL);
    }
    ...
    if (timeOut == CPSS_OS_SEM_WAIT_FOREVER_CNS) {
        ...
        if(ret == ETIMEDOUT) {
            /* wait failed (timeout) */
            ERR("Fail to wait a semaphore with a timeout.");
            pthread_mutex_unlock(&csem->mtx);
            return (GT_TIMEOUT);
        }
    }
    ...
    if (csem->count > 0 && ret == 0) {
        ...
        ERR("Fail to wait a semaphore with error (%d).", ret);
        pthread_mutex_unlock(&csem->mtx);
        return (GT_FAIL);
    }
}
...
)
{
    ...
    if (mtx == NULL) {
        ERR("Fail to malloc a memory buffer for mutex.");
        return (GT_FAIL);
    }
...
)
{
    ...
    if (ret) {
        ERR("Fail to delete the mutex, ERROR = %d", ret);
        return (GT_FAIL);
    };
...
)
{
    ...
    if (ret) {
        ERR("Fail to create a thread(%d).", ret);
...
)
{
    ...
    if (ret) {
        ERR("Fail to wait a thread(%d, %d).", ret, errno);
...
)
{
    ...
    if (msgqId == NULL) {
        ERR("Bad param (%p).", msgqId);

        return (GT_BAD_PARAM);
    }
    qid = msgget(IPC_PRIVATE, 0);
    if (qid < 0) {
        ERR("Fail to create a MsgQ.");
        return (GT_FAIL);
    }

    if (msgctl(qid, IPC_STAT, &ds) < 0) {
        ERR("Fail to get the info of a MsgS(%d).", qid);
    ...
    if (ds.msg_qbytes < bytes) {
        ...
        if (msgctl(qid, IPC_SET, &ds) < 0) {
            ERR("Fail to set the max bytes(%u) of MsgQ(%d).",
                 (unsigned int)bytes, qid);
...
)
{
    if (msgctl((int)msgqId, IPC_RMID, NULL) < 0) {
        ERR("Fail to delete a MsgQ(%d).", (int)msgqId);
...
)
{
    ...
    if (buf == NULL) {
        ERR("Fail to malloc a message buffer for sending.");
    ...
    while (msgsnd((int)msgqId, buf, messageSize, IPC_NOWAIT) < 0) {
        if (errno != EAGAIN) {
            ERR("Fail to send a message to a MsgQ(%d).", (int)msgqId);
            free(buf);
            return (GT_FAIL);
        }

        if (timeOut == CPSS_OS_MSGQ_NO_WAIT) {
            ERR("Fail to send a message to a MsgQ(%d) without wait.", (int)msgqId);
            free(buf);
            return (GT_FAIL);
            ...
            if( timeOut-- == 0) {
                ERR("Fail to send a message to a MsgQ(%d) with timeout(%u).",
...
)
{
    ...
    if (buf == NULL) {
        ERR("Fail to malloc a message buffer for receiving.");
    ...
    while (msgrcv((int)msgqId, buf, *messageSize, 0, IPC_NOWAIT) < 0) {
        if (errno != ENOMSG) {
            ERR("Fail to receive a message from a MsgQ(%d).", (int)msgqId);
            free(buf);
            return (GT_FAIL);
        }

        if (timeOut == CPSS_OS_MSGQ_NO_WAIT) {
            ERR("Fail to receive a message from a MsgQ(%d) without wait.", (int)msgqId);
            free(buf);
            return (GT_FAIL);
        } else if(timeOut == CPSS_OS_MSGQ_WAIT_FOREVER) {
            continue;
            ...
            if (timeOut-- == 0) {
                ERR("Fail to receive a message from a MsgQ(%d) with timeout(%u).",
                     (int)msgqId, (unsigned int)timeOut);
                free(buf);
                return (GT_TIMEOUT);
            }
...
)
{
    ...
    if (numMessages == NULL) {
        ERR("Bad param(%p).", numMessages);

        return (GT_BAD_PARAM);
    }

    if (msgctl((int)msgqId, IPC_STAT, &ds) < 0) {
        ERR("Fail to get the info of a msg queue(%d).", (int)msgqId);
 ...
 *
 * Revision 1.1.2.2  2021/04/23 02:47:12  illiu
 * Clean up code
 *
 * Revision 1.1.2.1  2021/04/12 08:38:41  illiu
 * Add file: cpss common code
 *
 * Revision 1.1.2.2  2021/02/22 02:41:51  illiu
 * Clean up code
 *
 * Revision 1.1.2.1  2021/01/26 02:55:35  illiu
 * Rename nim_dm prefix file to cpss prefix
 *
 * Revision 1.1.2.2  2020/10/07 09:13:09  illiu
 * Clean up code
 *
