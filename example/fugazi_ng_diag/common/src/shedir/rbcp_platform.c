/* $Id: rbcp_platform.c,v 1.2 2015/05/25 03:58:21 steja Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/shedir/rbcp_platform.c,v $
 *------------------------------------------------------------------
...
int platform_rbcp_recv (uint8_t *buf, int *length)
{
    ...
    if (status) {
        if (diagflag_xram & D_SET_OPTIONS) {
            printf("\nShedir RBCP receive packet fail. status = %d.\n",status);
        }
...
int platform_rbcp_clear_recv (void)
{
    ...
    if (diagflag_xram & D_SET_OPTIONS) {
        printf("\nShedir RBCP clear receive packet. status = %d,"
               "loopcount = %d.\n",status, loopcount);
    }
 ...
 *
 * Revision 1.1  2015/05/14 05:32:14  hondwang
