/* $Id: diag_moka_fpga_lib.c,v 1.2 2021/09/24 01:21:06 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/elixir/diag_moka_fpga_lib.c,v $
 *------------------------------------------------------------------
 * 
...
static submenu_xtable_t fpga_led_ctrl_tbl[] = {
    ...
    {"WLAN LED ON in GREEN", (type_t(*)())wifi_led_control, WIFI_LED_GREEN, 0,
     (type_t(*)())0, 0, (type_t(*)())0, 0},
    {"WLAN LED ON in BLUE",  (type_t(*)())wifi_led_control, WIFI_LED_BLUE, 0,
     (type_t(*)())0, 0, (type_t(*)())0, 0},
 ...
 *              This is by checking register  of
 *              FPGA SFP status and Control Reg(0x1134).
 * Inputs     : none
 * Outputs    : PASSED/FAILED
 *******************************************************************************
 */
int is_sfp_present (int current_sfp_select)
{
    ...
    if (current_sfp_select == GE0) {

        if ((reg_val & (uint)SFP0_SC_MODULE_DEF) == (uint)SFP0_SC_MODULE_DEF) {
            return (PASSED);
        } else {
            return (FAILED);
        }
    } else {
        if ((reg_val & (uint)SFP1_SC_MODULE_DEF) == (uint)SFP1_SC_MODULE_DEF) {
            return (PASSED);
        } else {
            return (FAILED);
        }
...
/*******************************************************************************
 *
 * Function   : sfp_mux_select
 * Description: Function to select mux to switch sfp0 or sfp1.
 *              This is by  access register  of
 *              FPGA SFP status and Control Reg(0x1134).
 * Inputs     : none
 * Outputs    : PASSED/FAILED
 *
 *******************************************************************************
 */
int sfp_mux_select (int current_sfp_select)
{
    uint reg_offset = (uint)FPGA_LTE_CTL_REG;
    uint reg_val = 0;
    ...
    if (current_sfp_select == GE0) {
        reg_val = (reg_val | (uint)SFP_MUX_SEL);
        printf("switch to sfp 0\n");
        if (fpga_write_32_reg(reg_offset, reg_val) != PASSED) {
            printf("%s: Failed to write FPGA SFP status and Control Reg(0x%04X).\n",
                   __FUNCTION__, reg_offset);
            return (FAILED);
        }
        return (PASSED);
    } else if (current_sfp_select == GE1) {
        reg_val = reg_val & (~(uint)SFP_MUX_SEL);
        printf("switch to sfp 1\n");
        if (fpga_write_32_reg(reg_offset, reg_val) != PASSED) {
            printf("%s: Failed to write FPGA SFP status and Control Reg(0x%04X).\n",
                   __FUNCTION__, reg_offset);
            return (FAILED);
 ...
 *******************************************************************************
 */
int sfp_tx_enable_switch (int opt, int ge_num)
{
    uint reg_offset = (uint)FPGA_SFP_AND_CTRL_REG;
    uint reg_val = 0;
    ...
    if (opt == ENABLE) {
        if (ge_num == GE0) {
            reg_val |= (uint)SFP0_SC_TX_DIS;
        } else {
            reg_val |= (uint)SFP1_SC_TX_DIS;
        }
    } else {
        if (ge_num == GE0) {
            reg_val &= (uint)(~SFP0_SC_TX_DIS);
    ...
    if (opt == ENABLE) {
        if (ge_num == GE0) {
            if ((reg_val & SFP0_SC_TX_DIS) != SFP0_SC_TX_DIS) {
                printf("%s: Failed to enable SFP0 TX.\n", __FUNCTION__);
                return (FAILED);
            }
        } else {
            if ((reg_val & SFP1_SC_TX_DIS) != SFP1_SC_TX_DIS) {
                printf("%s: Failed to enable SFP1 TX.\n", __FUNCTION__);
                return (FAILED);
            }
    ...
    if (opt == DISABLE) {
        if (ge_num == GE0) {
            if ((reg_val & SFP0_SC_TX_DIS) == SFP0_SC_TX_DIS) {
                printf("%s: Failed to disable SFP0 TX.\n", __FUNCTION__);
                return (FAILED);
            }
        } else {
            if ((reg_val & SFP1_SC_TX_DIS) == SFP1_SC_TX_DIS) {
                printf("%s: Failed to disable SFP1 TX.\n", __FUNCTION__);
                return (FAILED);
            }
...
/*******************************************************************************
 *
 * Function    : diag_moka_fpga_get_ext_intr_pending
 * Description : Function to check External Interrupt Pending Register(0x1128)
 *               is getting to "Interrupt Pending" state
 * Inputs      : register_bit - which bit of the register
 * Outputs     : TRUE / FALSE
 *
 *******************************************************************************
 */
int diag_moka_fpga_get_ext_intr_pending (int register_bit)
{
    ...
    /* set FPGA EIPR GE interrupt pending bit as 1(in order to read interrupt pin)*/
    fpga_wr_data = fpga_rd_data | register_bit;
    if (fpga_write_32_reg(fpga_offset, fpga_wr_data) != PASSED) {
        printf("%s:%d: Failed to write FPGA reg:0x%x with data:0x%x\n",
                __FUNCTION__, __LINE__, fpga_offset, fpga_wr_data);
        return (FALSE);
    ...
    /* checking pending bit is low */
    if ((fpga_rd_data & register_bit) != 0) {
        if ((NVRAM)->diagflag & D_VERBOSE) {
            printf("%s:%d: The EIPR(0x1128) pending bit is still high!!\n", __FUNCTION__, __LINE__);
            printf("%s:%d: The EIPR(0x1128) data = 0x%x\n", __FUNCTION__, __LINE__, fpga_rd_data);
            return (FALSE);
 ...
 *
 * Revision 1.1.2.11  2021/05/05 06:15:37  illiu
 * Change Wifi Led color keyword: AMBER to BLUE
 *
 * Revision 1.1.2.10  2021/04/23 02:38:29  illiu
 * Add VERBOSE print
 *
 * Revision 1.1.2.9  2021/03/15 10:18:23  illiu
 * Rename diag_check_esw_ext_intr_pending() to diag_moka_fpga_get_ext_intr_pending()
 * Add one input int parameter to diag_moka_fpga_get_ext_intr_pending()
 * Modify diag_moka_fpga_get_ext_intr_pending()'s return value from PASSED/FAILED to TRUE/FALSE
 *
 * Revision 1.1.2.8  2020/11/12 08:56:54  harrchan
 * Add CTS/RTS DTR/DSR pin test
 *
 * Revision 1.1.2.7  2020/11/05 06:40:37  harrchan
 ...
 *
 * Revision 1.1.2.6  2020/11/05 03:02:16  illiu
