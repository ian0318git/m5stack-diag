/* $Id: diag_ge_phy_lib.c,v 1.2 2021/09/24 01:21:06 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/elixir/diag_ge_phy_lib.c,v $
 *------------------------------------------------------------------
 * 
 * diag_ge_phy_lib.c
 *
...
static int diag_88e1112_cpu_to_ge1_mac_check_linkup (void);
...
static int diag_88e1112_sfp0_tx_disable(void);
static int diag_88e1112_sfp1_tx_disable(void);
static int diag_88e1112_sfp0_tx_enable(void);
static int diag_88e1112_sfp1_tx_enable(void);
...
dev_object_t *diag_get_88e11112_obj (int ge_num)
{
    ...
    mrvl_obj_p->callout_fvt->sfp_tx_disable            = (ge_num == GE0)? diag_88e1112_sfp0_tx_disable:diag_88e1112_sfp1_tx_disable;
    mrvl_obj_p->callout_fvt->sfp_tx_enable             = (ge_num == GE0)? diag_88e1112_sfp0_tx_enable:diag_88e1112_sfp1_tx_enable;
    ...
    mrvl_obj_p->callout_fvt->cpu_phy_mac_check_linkup  = (ge_num == GE0)? diag_88e1112_cpu_to_ge0_mac_check_linkup:diag_88e1112_cpu_to_ge1_mac_check_linkup;
...
static int diag_88e1112_cpu_phy_mac_setup (int ge_num, int test_spd)
{
    ...
    switch (ge_num) {
        ...
        case GE1:
            cpu_reg_addr = (uint)CPU_PORT_AN_CONF_REG(2);
        break;
...
static int diag_88e1112_cpu_to_ge0_mac_check_linkup (void)
{
    return (plat_cpu_mac_check_linkstat(3, CPUMAC_LINKUP));
}

/**********************************************************************
 * Function: diag_88e1112_cpu_to_ge1_mac_check_linkup
 * Description: Confirm CPU MAC link to GE PHY MAC
 * Inputs     : 
 * Outputs    : PASSED/FAILED
 **********************************************************************
...
static int diag_88e1112_cpu_to_ge1_mac_check_linkup (void)
{
    return (plat_cpu_mac_check_linkstat(2, CPUMAC_LINKUP));
}
...
int diag_88e1112_cpu_ext_lpbk_init (int ge_num)
{
    ...
    cpu_reg_offset = (ge_num == GE0)? (uint)CPU_PORT_AN_CONF_REG(3):
                                          (uint)CPU_PORT_AN_CONF_REG(2);
    ...
    cpu_reg_offset = (ge_num == GE0)? (uint)CPU_PORT_STATUS_REG0(3):
                                      (uint)CPU_PORT_STATUS_REG0(2);
...
{
    printf("current_sfp_select = %d", current_sfp_select);
    /* Check if SFP is present */
    if (is_sfp_present(current_sfp_select) == PASSED) {
        printf("SFP%d module is detected\n", current_sfp_select);
    } else {
        printf("SFP module is not detected\n.");
        return (FAILED);
    }

    /* Need to access FPGA register to switch SFP MUX  */
    if (sfp_mux_select(current_sfp_select) != PASSED) {
...
/**********************************************************************
 * Function: diag_88e1112_sfp0_tx_disable
 * Description: Disable SFP0 TX by FPGA
 * Inputs     : NONE
 * Outputs    : PASSED/FAILED
 **********************************************************************
 */
int diag_88e1112_sfp0_tx_disable (void)
{
    return (sfp_tx_enable_switch(DISABLE, GE0));
}

/**********************************************************************
 * Function: diag_88e1112_sfp1_tx_disable
 * Description: Disable SFP1 TX by FPGA
 * Inputs     : NONE
 * Outputs    : PASSED/FAILED
 **********************************************************************
 */
int diag_88e1112_sfp1_tx_disable (void)
{
    return (sfp_tx_enable_switch(DISABLE, GE1));
}

/**********************************************************************
 * Function: diag_88e1112_sfp0_tx_enable
 * Description: Disable SFP TX by FPGA
 * Inputs     : NONE
 * Outputs    : PASSED/FAILED
 **********************************************************************
 */
int diag_88e1112_sfp0_tx_enable (void)
{
    return (sfp_tx_enable_switch(ENABLE, GE0));
}

/**********************************************************************
 * Function: diag_88e1112_sfp1_tx_enable
 * Description: Disable SFP TX by FPGA
 * Inputs     : NONE
 * Outputs    : PASSED/FAILED
 **********************************************************************
 */
int diag_88e1112_sfp1_tx_enable (void)
{
    return (sfp_tx_enable_switch(ENABLE, GE1));
}
...
static int diag_88e1112_ge0_phy_tx_rx_test (void)
{
    return (eth_pkt_txrx("eth2", 3, FALSE));
}
...
static int diag_88e1112_ge1_phy_tx_rx_test (void)
{
    return (eth_pkt_txrx("eth1", 3, FALSE));
}
 ...
 * Outputs    : PASSED/FAILED
 *******************************************************************************/
int diag_ge_phy_config_gephy_fiber (int ge_num)
{
    smi_t set_data = 0;

    /* Set SIGDET Polarity(88E1112, 16_1.9) to 1 based on platform HW design.*/
    if (diag_ge_phy_setting_reg(ge_num, 
                                (smi_t)REG_PAGE(1), 
                                (uchar)REG_ADDR(16),
...
static int diag_set_sfp_glc_ge_100fx (int i2c_bus)
{
    ...
    if (rc != PASSED) {
        snprintf(err_buf, sizeof(err_buf), "%s %s () Unable to write FX Enable Register "
        ...
        if (rc != PASSED) {
            ...
            rc = n2g_i2c_write(&i2c_if);
            if (rc != PASSED) {
            	snprintf(err_buf, sizeof(err_buf), "%s() Unable to write GLC-GE-FX100 "
...
static int diag_set_sfp_I2C_op (uint8_t reg_high, uint8_t reg_low, n2g_i2c_if_t *i2c_if)
{
    ...
    rc = n2g_i2c_write(i2c_if);
    if (rc != PASSED) {
        snprintf(err_buf, sizeof(err_buf), "%s %s () Unable to write GLC-T SFP.\nrc = %#x",
			     __FILE__, __FUNCTION__, rc);
        rc = FAILED;
    } else {
        /* Ready to write the low byte */
 ...
 *
 * Revision 1.1.2.6  2021/08/18 10:58:44  illiu
 * Add function description
 *
 * Revision 1.1.2.5  2021/04/23 02:37:44  illiu
 * Replace sprintf with snprintf
 *
 * Revision 1.1.2.4  2020/11/13 11:48:24  illiu
 * Fix GE0/1 Led
 *
