/* $Id: diag_mcu_util.c,v 1.2 2021/09/24 01:21:06 harrchan Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/elixir/diag_mcu_util.c,v $
 *------------------------------------------------------------------
 * 
 * diag_mcu_util.c
 *
 ...
 *******************************************************************************
 */
int        plat_mcu_utils(int);
int        plat_mcu_reg_rd(uint32_t, uint16_t *);
int        plat_mcu_reg_wr(uint32_t, uint16_t);
...
static int mcu_reg_dump_util(void);


static plat_mcu_table mcu_reg_info[] = {
 ...
 *******************************************************************************
 */
#define PLAT_MCU_ACCESS_TIME   100   /* 100ms */
...
static submenu_xtable_t mcu_utils_tbl[] = {
    ...
    {"MCU version",             (type_t(*)())mcu_fw_show,    0,
     0,                             (type_t(*)())0,                  0,
     (type_t(*)())0,                0},
    {"MCU register dump",       (type_t(*)())mcu_reg_dump_util,    0,
     0,                             (type_t(*)())0,                  0,
     (type_t(*)())0,                0},
...
/*******************************************************************************
 *
 * Function    : plat_mcu_utils
 * Description : Function to show MCU utilities submenu.
 * Inputs      : opt - reserved for future use.
 * Outputs     : PASSED / FAILED
 *
 *******************************************************************************
 */
int plat_mcu_utils (int opt)
{
 ...
 * Description : Utility to read MCU register.
 * Inputs      : opt - reserved for future use
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
...
static int mcu_reg_rd_util (int opt)
{
    ...
    reg_offset = (uint32_t)gethex_answer("Enter Reg. address: ",
                                         PLAT_MCU_VERSION_REG, 0, 0xFF);

    if (plat_mcu_reg_rd(reg_offset, &reg_val) != PASSED) {
        return (FAILED);
    }
 ...
 * Description : Utility to write MCU register.
 * Inputs      : opt - reserved for future use
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
...
static int mcu_reg_wr_util (int opt)
{
    ...
    reg_offset = (uint32_t)gethex_answer("Enter Reg. address: ",
                                         PLAT_MCU_PS_MARGIN_REG, 0, 0xFF);

    if (plat_mcu_reg_rd(reg_offset, &orig_val) != PASSED) {
        return (FAILED);
    }
    ...
    if (plat_mcu_reg_wr(reg_offset, wr_val) != PASSED) {
        return (FAILED);
    }
...
/*******************************************************************************
 *
 * Function    : plat_mcu_reg_rd
 * Description : Function to read MCU register.
 ...
 *******************************************************************************
 */
int plat_mcu_reg_rd (uint32_t reg_off, uint16_t *rd_buf)
{
    n2g_i2c_if_t *i2c_if;
    int          ret_val = 0;
    uint16_t     reg_val = 0;
...
/*******************************************************************************
 *
 * Function    : plat_mcu_reg_wr
 * Description : Function to write MCU register.
 * Inputs      : reg_off - register offset
 *               wr_data - write data
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
 */
int plat_mcu_reg_wr (uint32_t reg_off, uint16_t wr_data)
{
    n2g_i2c_if_t *i2c_if;
    int           ret_val = 0;
...
/*******************************************************************************
 *
 * Function    : diag_volts_margin_util
 * Description : Utility to do voltages(1.8/2.5/3.3V) margin.
 * Inputs      : opt - reserved for future use
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
 */
int diag_volts_margin_util (int opt)
{
    uint32_t reg_offset = (uint32_t)PLAT_MCU_PS_MARGIN_REG;
    uint16_t m_mode = 0, c_mode = 3;
    
    if (plat_mcu_reg_rd(reg_offset, &c_mode) != PASSED) {
        printf("Failed to get current voltage margin setup.\n");
        return (FAILED);
    }
    ...
    if ((c_mode & (uint16_t)PLAT_MCU_R17_PS_MSK) != PLAT_MCU_R17_PS_NORMAL) {

        if (plat_mcu_reg_wr(reg_offset,
                           (uint16_t)PLAT_MCU_R17_PS_NORMAL) != PASSED) {
            return (FAILED);
        }

        msleep(PLAT_MCU_ACCESS_TIME);

        if (plat_mcu_reg_rd(reg_offset, &c_mode) != PASSED) {
            return (FAILED);
        }

        if (c_mode != (uint16_t)PLAT_MCU_R17_PS_NORMAL) {
            printf("\nFailed to set voltage margin to Normal.\n");
            return (FAILED);
        }
    }

    if (m_mode == (uint16_t)PLAT_MCU_R17_PS_NORMAL) {
        printf("\ncurrent Voltage Margin: Normal.\n");
        return (PASSED);
    }

    /* 2. Set Voltage Margin to High/Low based on user requeset. */
    if (plat_mcu_reg_wr(reg_offset, m_mode) != PASSED) {
        return (FAILED);
    }
    ...
    if ((c_mode & (uint16_t)PLAT_MCU_R17_PS_MSK) !=
        (m_mode & (uint16_t)PLAT_MCU_R17_PS_MSK)) {
            printf("\nFailed to set voltage margin to %s.\n",
               ((m_mode & (uint16_t)PLAT_MCU_R17_PS_MSK) == PLAT_MCU_R17_PS_ML) ?
               "Low" : "High");
        return (FAILED);
    }
        printf("\ncurrent Voltage Margin: %s.\n",
           ((c_mode & (uint16_t)PLAT_MCU_R17_PS_MSK) == PLAT_MCU_R17_PS_ML) ?
           "Low" : "High");
...
/*******************************************************************************
 *
 * Function    : plat_display_voltage
 * Description : Display voltage margin information.
 * Inputs      : None
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
 */
int plat_display_voltage (void)
{
    uint32_t reg_offset = (uint32_t)PLAT_MCU_PS_MARGIN_REG;
    uint16_t c_mode = 3;
    ...
    if (plat_mcu_reg_rd(reg_offset, &c_mode) != PASSED) {
        ...
        if (c_mode == 0) { 
            printf("current Voltage Margin Normal\n");
        } else if (c_mode == 1) {
            printf("current Voltage Margin Low\n");
        } else if (c_mode == 2) {
            printf("current Voltage Margin High\n");
        }
    return (PASSED);
}
...
static int mcu_reg_dump (void)
{
    ...
    for (reg_offset = 0; reg_offset < 0xFF; reg_offset++) {
        ...
        plat_mcu_reg_rd(reg_offset, &reg_val);
    ...
    while (mcup->p_regs != NULL) {
        plat_mcu_reg_rd(mcup->reg_addr, &reg_val);
...
int alter_reg (void)
{
    ...
    if ((rc = plat_mcu_reg_rd(offset,  &data)) != PASSED) {
        printf("%s:%d:Failed to read Power Sequencer Reg %#x\n", 
    ...
    if ((rc = pwr_seq_write_reg(offset, data)) != PASSED) {
        printf("%s:%d:Failed to wrote Power Sequencer Reg %#x.\n",
               __FUNCTION__, __LINE__, offset);
    }
 ...
 *
 * Revision 1.1.2.3  2021/08/18 11:16:38  illiu
 * Remove redundant function
 *
 * Revision 1.1.2.2  2021/08/18 11:07:53  illiu
 * Change function type to static
 *
