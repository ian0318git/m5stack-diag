/* $Id: platform_psu.c,v 1.2 2021/06/02 08:22:36 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/fugazi/platform_psu.c,v $
 *------------------------------------------------------------------
...
void build_psu_menu (uint32_t option)
{
    ...
    /* Get the PSU number */
    psu_no_now = (option >> FUGAZI_PSU_OFF);
    if ((NVRAM)->diagflag & D_VERBOSE) {
    ...
    /* Setup Mux channel */
    rc = set_mux_channel(NULL, psu_mux_mask, FUGAZI_PSU_I2C_MUX);
    if (rc != PASSED) {
    ...
    menu_opt = (option & FUGAZI_MENU_OPT_MSK);
    if (menu_opt) {
 ...
 *********************************************************************
 */
static void dev_obj_create(dev_at_object_t *pe, n2g_i2c_if_t *i2c_if, int psu_num)
{
...
int psu_i2c_test_warp (void)
{
    ...
    for (ia = FUGAZI_MUX_PSU0; ia < FUGAZI_MUX_PSU_INVALID; ia++) {
        /* check PSU and POE PSU is available or not. */
        switch (ia){
           case FUGAZI_MUX_PSU0:
               psu_valid = check_psu_stat(FUGAZI_PSU1);
           ...
           case FUGAZI_MUX_PSU2:
               ...
               size = sizeof(uint8_t);
               break;
           case FUGAZI_MUX_PSU1:
           case FUGAZI_MUX_PSU3:
