/* $Id: platform_prom.c,v 1.2 2021/06/02 08:22:35 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/fugazi/platform_prom.c,v $
 *-----------------------------------------------------------------------------
 ...
 *-------------------------------------------------------------------
 */
static int get_spi_prom_info (prom_t *spi_prom, boolean verbose)
{ 
    return(PASSED);
} 
 ...
 *-------------------------------------------------------------------
 */
int debug_reggio_erase_spi_prom (void)
{ 
    ushort sector;
    ulong start_addr;
    ulong end_addr;
 ...
 *-------------------------------------------------------------------
 */
int program_reggio_spi_prom_old (int header) /* naming conflict with tam_aikido_upgrade.c */
{
    int status;
 ...
 *-------------------------------------------------------------------
 */
static int erase_header_spi_prom_image (uchar type, boolean verbose)
{ 
    uchar sector;
 ...
 *-------------------------------------------------------------------
 */
int display_prom_sector (int dummy)
{
    volatile uchar data[PROM_RD_MAX_BYTE];
    ushort i;
 ...
 *-------------------------------------------------------------------
 */
static int get_nios_sector (int sector, int bytes,
                 unsigned char *data)
{
    ushort i;
 ...
 *-------------------------------------------------------------------
 */
int nios_test_spi_prom (int type)
{
 ...
 *-------------------------------------------------------------------
 */
int erase_config_header (int type)
{
 ...
 *-------------------------------------------------------------------
 */
int program_boot_upgrade_flag (int type)
{
 ...
 *
 * Revision 1.1.8.3  2020/10/22 01:55:28  iachang
