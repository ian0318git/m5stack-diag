/* $Id: platform_pwr_seq.c,v 1.2 2021/06/02 08:22:36 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/fugazi/platform_pwr_seq.c,v $
 *------------------------------------------------------------------
...
int mcu_pin_shorten_check(void);
...
int pwr_seq_read_reg(int, uint16_t *);
...
int fugazi_show_mb_watts(void);


/* Fugazi - scratchpad register table */
...
{
    /* 
     * update register table for fugazi (EDCS-1042044)
...
static submenu_xtable_t pwr_menu_table[] = {
    ...
    {"Alter & Show Black Box Cmd register",  (PFT)alter_dump_blk_cmd_reg, 0,
        0,     (type_t(*)())0, 0, (PFT)0, 0},
    {"Show MB watts ",            (PFT)fugazi_show_mb_watts,     0,
        0,     (type_t(*)())0, 0, (PFT)0, 0},
...
int mcu_pin_shorten_check (void)
{
    ...
    if ((atof(ver) >= atof(v1p04)) && (val < shorten_value)) {
        /* Based on HW suggestoin don't abort the upgrade if it's not */
 ...
 **********************************************************************
 */
int init_pwr_seq (int err_log)
{
    n2g_i2c_if_t i2c_if;
    uint32_t rc = FAILED;
    ren_t data;
    char err_buf[FUGAZI_BUF_SIZE *2];
    char *reg_data;
 ...
 *******************************************************************************
 */
int pwr_seq_read_reg (int offset, uint16_t *data)
{
    n2g_i2c_if_t i2c_if;
    char         err_buf[FUGAZI_BUF_SIZE];
    int          result = FAILED;
...
static int pwr_vtg_check(int submenu)
{
    ...
    char         err_buf[FUGAZI_BUF_SIZE *2], rd_buf[FUGAZI_BUF_SIZE];
 ...
 *********************************************************************
 */
static int pwr_stat_check(char *err_buf)
{
    n2g_i2c_if_t i2c_if;
    uint32_t rc = FAILED;
 ...
 ******************************************************************************
 */
int write_pwr_seq_fw(int dummy)
{
    n2g_i2c_if_t i2c_if;
    int rc;
    uint16_t data;
    ...
    printf("        Please plug-in 'jumper' on pin 1-2 of P2_PSQ\n");
    ...
    /* Based on HW's request to check version and debug pin */
    if (mcu_pin_shorten_check() == FAILED) {
        return (FAILED);
    }
    ...
    if ((*is_active == APP_IS_ACTIVE) && (*is_valid == APP_IS_VALID)) {
        printf("- Using Bootloadable application 'app_2.h' firmware image\n");
        ...
        if ((*is_active == APP_IS_ACTIVE) && (*is_valid == APP_IS_VALID)) {
            printf("- Using Bootloadable application 'app_1.h' firmware image\n");
    ...
    if (lineCntr == total_row) {
        ...
        printf("\n*** Fatal error: sent 0x%x, expected sent 0x%x", lineCntr, total_row);
        fclose(fp);
        return (FAILED);
    }
...
int fugazi_show_mb_watts (void)
{
    ...
    if ((NVRAM)->diagflag & D_VERBOSE) {
        printf("CONST_P = %d. iadc = %d, vadc = %d.\n",
               FUGAZI_PWR_CONSUMP_CONST, iadc, vadc);
    }

    printf("\nMotherboard, NGWIC, NGVM, and FanTray Power consumption = %d mW\n",
           FUGAZI_PWR_CONSUMP_CONST * vadc * iadc / 1000);
 ...
 *
 * Revision 1.1.8.3  2021/04/29 01:33:57  pdoong
 ...
 *
 * Revision 1.1.6.7  2020/07/27 08:51:36  iachang
 * Add mcu_pin_shorten_check() funciton.
 *
 * Revision 1.1.6.6  2019/12/24 03:26:25  iachang
 * HW request add this message when MCU upgrade : Please plug-in 'jumper' on pin 1-2 of P2_PSQ
 *
 * Revision 1.1.6.5  2019/12/18 01:41:33  iachang
 * Modify the Power Sequencer FW upgrade error information.
 *
 * Revision 1.1.6.4  2019/12/17 08:28:05  iachang
 * Check Power Sequencer FW upgrade completed
 *
 * Revision 1.1.6.3  2019/04/18 01:21:30  letsai
 * 1. Clean up code
