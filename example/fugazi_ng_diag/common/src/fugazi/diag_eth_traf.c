/* $Id: diag_eth_traf.c,v 1.2 2021/06/02 08:22:34 iachang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/fugazi/diag_eth_traf.c,v $ 
 *------------------------------------------------------------------
 *
 * diag_eth_raf.c
...
static void gen_socket_pkt (struct packet_setting *pkt, const char *iface, int pkt_len)
{
    int i;
    int data_len;
...
static int eth_pkt_tx (struct eth_task *tx_task)
{
    ...
    if (tx_task->tx_mode == ETH_TRAF_TX_MODE_FIXED){
        gen_socket_pkt(tx_pkt, tx_task->iface, pkt_len);
...
int eth_traf_tx_pkt(struct eth_traf *traf, const char *pkt, size_t len)
{
    ...
    if (len > PKT_MAX_LEN) {
        printf("the len of input pkt err\n");
        return (FAILED);
    }
                    ...
                    time_t timeout)
{
    ...
    if (tx_info.tx_cnt != rx_info.rx_cnt) {
        cterr('f', 0, "Tx:(%d) Rx:(%d) mismatched ",tx_info.tx_cnt, rx_info.rx_cnt);
        return (FAILED);
    } else if (rx_info.rx_errcnt) {
        cterr('f', 0, "RxErr : %d ", rx_info.rx_errcnt);
        return (FAILED);
    } else if (!rx_info.rx_cnt) {
        cterr('f', 0, "Didn't receive packets ");
        return (FAILED);
    }    
    return (PASSED); 
}
 ...
 *
 * Revision 1.1.2.4  2020/07/31 09:52:08  iachang
 * Code clean up.
 *
 * Revision 1.1.2.3  2019/07/19 02:29:36  iachang
 ...
 *
 * Revision 1.1.2.2  2019/06/17 10:28:56  iachang
 * Fixed compile issue.
 *
 * Revision 1.1.2.1  2019/06/17 06:02:19  iachang
