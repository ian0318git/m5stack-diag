/* $Id: nim_dm_cpss_extdrv.c,v 1.2 2019/12/11 10:10:26 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/dreamliner_cpss41/nim_dm_cpss_extdrv.c,v $
 *------------------------------------------------------------------
...
)
{
    ...
    if(pciBaseAddr == NULL || pciSize == NULL ||
       ...
       exPciBaseAddr == NULL || exPciSize == NULL ) {
        ERR("The dm instance of slot/bay(%u/%u) get called with bad param"
            "(%p, %p, %p, %p %p %p )", dm_cpss_slot, dm_cpss_bay,
            pciBaseAddr, pciSize, internalPciBase, internalSize, exPciBaseAddr, exPciSize);
        return GT_BAD_PARAM;
    }
    ...
    exbaseAddr = mmap(0,
                    ...
                    MAP_SHARED,
                    sub_fd,
                    size.config_size + size.ppregs_size);
    ...
    *pciSize = size.ppregs_size;
    ...
    /*
     * ex register space base address and size
     */
    *exPciBaseAddr = (GT_UINTPTR)exbaseAddr;
    *exPciSize = size.exregs_size;
...
)
{
    ...
    if(pciBaseAddr == 0 || pciSize == 0 ||
       ...
       exPciBaseAddr == 0 || exPciSize == 0 ) {
        ERR("The dm instance of slot/bay(%u/%u) get called with bad param"
            "(%lu, %lu, %lu, %lu, %lu, %lu)", dm_cpss_slot, dm_cpss_bay,
            (unsigned long)pciBaseAddr, (unsigned long)pciSize,
            (unsigned long)internalPciBase, (unsigned long)internalSize,
            (unsigned long)exPciBaseAddr, (unsigned long)exPciSize);
        return GT_BAD_PARAM;
    }
    ...
    /*
     * unmap ex register space
     */
    ret = munmap((void*)exPciBaseAddr, (size_t)exPciSize);
    if (ret) {
        ERR("The dm instance of slot/bay(%u/%u) fail to unmap the ex register "
            "memory space.", dm_cpss_slot, dm_cpss_bay);
        return GT_FAIL;
    }
