/* $Id: ngwic_dreamliner.c,v 1.2 2019/12/11 10:10:26 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/dreamliner_cpss41/ngwic_dreamliner.c,v $
 *------------------------------------------------------------------
 *
...
{ 
    ...
    if (is_nanook() || is_nanook_plus()) {
        system("rmmod ixgbe.ko");
        msleep(1000);
        system("insmod /lib/modules/4.14.3/ixgbe_LAN0AN.ko");
        msleep(1000);
        dynamic_get_inface(1, dname);
        sprintf(cmd, "ifconfig %s up > /dev/null", dname);
        system(cmd);
    }
    ...
    if (is_juno_plx() || is_utah_plx() || is_sword() || is_dagger() || is_goldbeach() || is_ntpn_machines() || is_vg450()) {
        skip_setting = TRUE;
    } else {
        skip_setting = FALSE;
    }
    ...
    for (retry = 0; retry < 4 ; retry++) { 
	   ...
	   if (is_nanook() || is_nanook_plus()) {
              dynamic_get_inface(1, dname);
              sprintf(cmd, "ifconfig %s down > /dev/null", dname);
              system(cmd);
          }
...
set_gen_speed (int slot)
{
    ...
    if (is_goldbeach()) {
        printf("\nGoldbeach Didn't Support PCIe SW \n");
        return; /* Goldbeach didn't have PLX */
    } else if (is_nanook() || is_nanook_plus()) {
        printf("\nNanook/Nanook+ Didn't Support PCIe SW \n");
        return; /* Nanook didn't have PLX */
    }
