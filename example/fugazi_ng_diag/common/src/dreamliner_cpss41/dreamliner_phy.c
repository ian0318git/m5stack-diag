/* $Id: dreamliner_phy.c,v 1.2 2019/12/11 10:10:25 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/dreamliner_cpss41/dreamliner_phy.c,v $
 *------------------------------------------------------------------
 *
...
phy_internal_lpbk_test_ge1 (void)
{
    ...
    if (is_goldbeach() || is_nanook() || is_nanook_plus()) {
        if(is_goldbeach()) {
            printf("\nGoldbeach Didn't Support GE1\n");
        }
        if(is_nanook() || is_nanook_plus()) {
            printf("\nNanook/Nanook+ Didn't Support GE1\n");
        }
        return (PASSED);
    }
    cterr_setup();
