/* $Id: plug_host_fpga_prog.c,v 1.2 2018/11/23 08:49:53 hondwang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tsn/plug_host_fpga_prog.c,v $
 *------------------------------------------------------------------
 *
 * Filesname: plug_host_fpga_prog.c
 * Description: Host plug FPGA SPI Program Utility
 *
...
static int PLUG_FPGA_DSWAP4(int);
...
int plug_fpga_display_sector (int dummy)
{
    ...
    while (start_addr < end_addr) {
        ...
        fpga_write_32_reg(PLUG_FPGA_SPI_PROM_OP_ADDR_REG, 
                          PLUG_FPGA_DSWAP4(PROM_READ_OP | start_addr));
        fpga_write_32_reg(PLUG_FPGA_SPI_PROM_RD_SIZE_REG, PLUG_FPGA_DSWAP4(count));
        fpga_write_32_reg(PLUG_FPGA_SPI_PROM_CTRL_REG, PLUG_FPGA_DSWAP4(ctrl_flag));
        ...
        for (ix = 0; ix <= count; ix++) {
            ...
            data[ix] = (uchar)(PLUG_FPGA_DSWAP4(temp_data));
...
static int PLUG_FPGA_DSWAP4 (int org)
{
    return (org);
}
                                         ...
                                         uchar *src_img, boolean bit_reverse)
{
    ...
    while (start_addr < end_addr) {
        ...
        if (data_check != PROM_BLANK_DATA) {
            ...
            fpga_write_32_reg(PLUG_FPGA_SPI_PROM_OP_ADDR_REG, 
                              PLUG_FPGA_DSWAP4(PROM_PAGE_PROG_OP | start_addr));
            fpga_write_32_reg(PLUG_FPGA_SPI_PROM_RD_SIZE_REG, PLUG_FPGA_DSWAP4(pgm_size));

            /* Fill data FIFO 256 bytes max at a time */
            for (ix = 0; ix <= pgm_size; ix++) {
                fpga_write_32_reg(PLUG_FPGA_SPI_PROM_RW_DATA_REG, PLUG_FPGA_DSWAP4(src_img[img_idx]));
                img_idx++;
            }
...
static int plug_fpga_verify_sector_erase (uint sector)
{
    ...
    while (start_addr < end_addr) {
        fpga_write_32_reg(PLUG_FPGA_SPI_PROM_OP_ADDR_REG, 
                          PLUG_FPGA_DSWAP4(PROM_READ_OP | start_addr));
        fpga_write_32_reg(PLUG_FPGA_SPI_PROM_RD_SIZE_REG, PLUG_FPGA_DSWAP4(PROM_RD_256_BYTE));
        fpga_write_32_reg(PLUG_FPGA_SPI_PROM_CTRL_REG, 
                          PLUG_FPGA_DSWAP4(PROM_DFLT_BAUD | PROM_USE_ADDR));
        ...
        for (ix = 0; ix < PROM_RD_MAX_BYTE; ix++) {
            ...
            data[ix] = (uchar)PLUG_FPGA_DSWAP4(data_in);
...
static int plug_fpga_is_read_fifo_empty (void)
{
    ...
    do {
        ...
        if (data_in & PLUG_FPGA_DSWAP4(PROM_RD_FIFO_EMPTY)) {
            return (TRUE);
        }
...
static int plug_fpga_is_write_fifo_empty (void)
{
    ...
    do {
        ...
        fpga_read_32_reg(PLUG_FPGA_SPI_PROM_STS_REG, (uint *)&data_in);
        if (data_in & PLUG_FPGA_DSWAP4(PROM_WR_FIFO_EMPTY)) { 
            return (TRUE);
        }
...
static int plug_fpga_prom_sect_erase (uint sector)
{
    ...
    opcode = (((sector * SPI_PROM_SECTOR_SIZE) & PROM_SPI_ADDR_MASK) | 
                PROM_SECT_ERASE_OP);
    fpga_write_32_reg(PLUG_FPGA_SPI_PROM_OP_ADDR_REG, PLUG_FPGA_DSWAP4(opcode));
    fpga_write_32_reg(PLUG_FPGA_SPI_PROM_CTRL_REG, PLUG_FPGA_DSWAP4(PROM_DFLT_BAUD | 
...
static int plug_fpga_device_write_enable (int enable)
{
    ...
    if (fpga_write_32_reg(PLUG_FPGA_SPI_PROM_OP_ADDR_REG, PLUG_FPGA_DSWAP4(opcode))
                          == FAILED) {
    ...
    if (fpga_write_32_reg(PLUG_FPGA_SPI_PROM_CTRL_REG, 
                          PLUG_FPGA_DSWAP4(PROM_DFLT_BAUD | PROM_DATA_WRITE)) == FAILED) {
...
static int plug_fpga_is_wr_op_done (void)
{
    ...
    do {
        ...
        if (rd_status & PLUG_FPGA_DSWAP4(PROM_OPER_DONE)) {
            /* Clear the done bit */
            fpga_write_32_reg(PLUG_FPGA_SPI_PROM_STS_REG, 
                              rd_status | PLUG_FPGA_DSWAP4(PROM_OPER_DONE));
            return (TRUE);
        }
        usleep(1000);
    } while (count++ < SPI_PROM_MAX_WAIT);
...
static int plug_fpga_read_spi_prom_status_reg (int *data_in)
{
    ...
    fpga_write_32_reg(PLUG_FPGA_SPI_PROM_RD_SIZE_REG, 
                      PLUG_FPGA_DSWAP4(PROM_RD_1_BYTE & PROM_RD_SIZE_MASK));
    fpga_write_32_reg(PLUG_FPGA_SPI_PROM_OP_ADDR_REG, PLUG_FPGA_DSWAP4(PROM_RDSR_OP));
    fpga_write_32_reg(PLUG_FPGA_SPI_PROM_CTRL_REG, PLUG_FPGA_DSWAP4(PROM_DFLT_BAUD)); /* Read */
    ...
    *data_in = PLUG_FPGA_DSWAP4(rd_data);
    return (PASSED);
}
