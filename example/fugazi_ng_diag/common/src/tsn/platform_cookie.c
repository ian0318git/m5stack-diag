/* $Id: platform_cookie.c,v 1.9 2019/03/07 09:51:32 lucywang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tsn/platform_cookie.c,v $
 *------------------------------------------------------------------
 *
...
extern int aikido_read_32_reg(uint, uint *);
...
static ushort g_control_type = 0x0; 
...
static uchar default_mb_c1109_2p_cookie[COOKIE_SIZE_512] = {
,
...
#if 0   /* TSN_DIAG */
    /* Get SKU ID */
    get_sku_id(&sku_id_val);
#else
    printf("%s: FIX ME !! TSN currently skip SKU ID check.\n", __FUNCTION__);
    sku_id_val = DSL_SKU_ISR967_K9;
#endif

    switch (board_type) {
    case MOTHER_BOARD:
        printf("\nLoading default cookie format for TSN-HIGH Motherboard.\n");
        switch (sku_id_val) {
        case DSL_SKU_ISR967_K9:
            if (this_is_star_c1109_2p() || this_is_supernova_c959_2p()) {
			    movbyte(default_mb_c1109_2p_cookie, contents, cookie_size);
            } else {
...
void init_cookie_default(int board_type, uint8_t * contents)
{
    ...
    case DSL_SKU_ISR967_K9:
        if (this_is_star_c1109_2p() || this_is_supernova_c959_2p())
		    movbyte(default_mb_c1109_2p_cookie, contents, EEPROM_RD_WR_LENGTH);
	    else
...
static int alter_cookie(int board_type)
{
    ...
    if (aikido_mailbox_flag) {
        printf("\nDBG :Select AIKIDO ACT2: tam_lib_device_open_mailbox()\n");
        ...
        printf("\nDBG :Select AIKIDO ACT2: tam_lib_device_open()\n");
...
int ilp_eeprom_read (uint offset, uchar *dest, int byte_count)
{
    ...
    if (rc != PASSED) {
        cterr('f', 0, "%s: Unable to open I2C device", __FUNCTION__);
        return (FAILED);
    }
    ...
    if (rc != PASSED) {
        cterr('f', 0, "%s: Unable to read I2C, statux = %x", __FUNCTION__, rc);
        return (FAILED);
    }

    *dest = (uchar)data;

    return (n2g_i2c_close(&i2c_if));
}
...
int alter_poe_cookie(void)
{
    if (tsn_has_poe(0) == FALSE) {
        printf("POE DC not present\n");
        return (PASSED); 
    }
    
    /* Do the POE reset initialization sequence. */
    if (ilp_poe_reset() == FAILED) {
        return (FAILED);
    }
...
int alter_plug_cookie(void)
{
    ...
    if (star_plug_is_present(slot) != TRUE) {
        printf("\n Plug module not present \n");
        return (PASSED);
    }
...
int alter_mb_cookie(void)
{
    ...
    aikido_mailbox_flag = FALSE;
#ifdef SUPPORT_DISCRETE_AIKIDO_ACT2
    act2_chip = getdec_answer("\n(0-Discrete-ACT2, 1-Aikido-ACT2):", 0, 0, 2);
#else
    act2_chip = getdec_answer("\n(1-Aikido-ACT2):", 1, 0, 1);
#endif
    if (act2_chip == 1) {
 ...
 *
 * This functions read control type on Mother Board cookie.
 *
 * Input : boolean printinfo - TRUE/FALSE
 *
 * Output: none.
 *
 *------------------------------------------------------------------------
 */
ushort get_control_type (void)
{
    return g_control_type;
}
...
Revision 1.9  2019/03/07 09:51:32  lucywang
...
Revision 1.6.44.2  2018/11/13 07:10:29  hondwang
Base on PRRQ comment add gephy_set_test_mode back

Revision 1.6.44.1  2018/10/15 06:53:07  hondwang
...
Revision 1.5.2.2  2018/03/09 05:55:36  shjung
Supported WP7608/7609

Revision 1.5.2.1  2018/03/07 03:31:42  hondwang
Fix TSN wifi ACT2 program utility issue

Revision 1.5  2018/02/09 09:56:54  hondwang
Merge Star branch star-branch-c9xx to main trunk

Revision 1.4.16.5  2018/02/07 10:23:01  lucywang
Followed coding rule

Revision 1.4.16.4  2018/02/06 10:13:55  lucywang
Removed and Skipped Discrete ACT2 in I2C scan test

Revision 1.4.16.3  2018/02/02 02:31:51  hondwang
remove mark for TAM lib approved by Ian
...
Revision 1.4  2017/09/07 09:37:52  steja
Remove Discrete ACT2 (CSCvf81035)
...
Revision 1.2.4.4  2017/11/22 09:45:46  hondwang
Fix demo SKU and menu show

Revision 1.2.4.3  2017/11/20 07:54:31  lucywang
Changed PID to C1101/C1109-2P/C1109-4P

Revision 1.2.4.2  2017/09/20 06:02:56  lucywang
modified default cookies for C949-2P
...
Revision 1.1.4.6  2016/07/14 12:57:00  steja
Add POE cookie eeprom programming

Revision 1.1.4.5  2016/07/11 01:59:02  steja
Fix Wifi ACT2 programming

Revision 1.1.4.4  2016/07/01 08:33:30  steja
...
Revision 1.1.2.4  2016/04/14 13:47:38  steja
