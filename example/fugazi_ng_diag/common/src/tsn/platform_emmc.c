/* $Id: platform_emmc.c,v 1.4 2020/08/28 06:42:59 hondwang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tsn/platform_emmc.c,v $
 *-----------------------------------------------------------------------------
 * 
 * Filename   : platform_emmc.c
 * Description: TSN eMMC Library.
 *
...
static int   get_emmc_cid_info(tsn_emmc_info_t *);
 ...
 * Description: Function to enable TSN eMMC fully pSLC mode.
 ...
 * Description : Function to show current TSN eMMC info 
 * Inputs      : None
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
 ...
 * Description : Function to get current TSN eMMC CID info 
 * Inputs      : *emmc_info - buffer to put eMMC info
 * Outputs     : PASSED/FAILED
 *
 *******************************************************************************
 */
static int get_emmc_cid_info (tsn_emmc_info_t *emmc_info)
{
    FILE  *fd = NULL;
    char  filename[TSN_EMMC_BUF_SIZE];
    char  buf[TSN_EMMC_BUF_SIZE];

    /* Get Manufacturer ID */ 
    memset(buf, 0, sizeof(buf));
    memset(filename, 0, sizeof(filename));
    sprintf(filename, TSN_EMMC_SYSFS_MANFID);
    fd = fopen(filename, "r");
    if (fd == NULL) {
    ...
    memset(buf, 0, sizeof(buf));
    memset(filename, 0, sizeof(filename));
    sprintf(filename, TSN_EMMC_SYSFS_NAME);
    fd = fopen(filename, "r");
    if (fd == NULL) {
        printf("%s(%d) Failed to fopen %s: %s(%d)\n",
               __func__, __LINE__, filename, strerror(errno), errno);
 ...
 * Revision 1.4  2020/08/28 06:42:59  hondwang
 * CSCvv50894:C1109-4P deviation D554288 support
 *
