/* $Id: diag_fpga.c,v 1.3 2018/02/09 09:56:53 hondwang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tsn/diag_fpga.c,v $ 
 *-----------------------------------------------------------------------------
 *
 * Filename   : diag_fpga.c
 * Description: FPGA Diag tests and utilities.
 *
...
static submenu_xtable_t fpga_diag_tbl[] = {
   ...
   {"FPGA Register Test",          (type_t(*)())diag_fpga_reg_test,   0,
    ...
    (type_t(*)())0,                0,
    (type_t(*)())0,                0},
   {"Aikido Register Test",          (type_t(*)())aikido_reg_test,   0,
    (MF_CONTINUOUS | MF_DOGRP | MF_DOALL | MF_SHOW_ERRCOUNT),
    (type_t(*)())0,                0,
    (type_t(*)())0,                0},
...
static reg_info_t fpga_reg_test_tbl[] = {
    ...
    {"Force External Interrupt Mask",      FPGA_EXT_INTR_MASK_REG,   FPGA_RW,
     {(unsigned long)&tsn_fpga_reg_ext},   0x000FFBFF,               0x0},
    {"END",                                0x00,                     0,
     {0},                                  0x0,                      0x0},
};

static tsn_reg_bit_t fpga_f_intr_test_tbl[] = {
...
int diag_fpga_f_intr_test (int opt)
{
    ...
    total_f_intr_num = (sizeof(fpga_f_intr_test_tbl) / sizeof(tsn_reg_bit_t));

    /* Confirmed that CP_MPP8 is configured as GPIO_IN */
    if (tsn_mem_read32((uint)CP_MPP_CTRL_REG(1), &cpu_mpp) != PASSED) {
    ...
    if ((cpu_mpp & 0xf) != 0) {
        cpu_mpp &= (uint)(~0xf);
        if (tsn_mem_write32((uint)CP_MPP_CTRL_REG(1), cpu_mpp) != PASSED) {
    ...
    if (tsn_mem_read32((uint)CP_GPIO_DATA_OUT_EN_REG(0),
                       &gpio_dout_en) != PASSED) {
    ...
    if ((gpio_dout_en & (uint)(1 << 8)) != 1) {
        ...
        if (tsn_mem_write32((uint)CP_GPIO_DATA_OUT_EN_REG(0),
                            gpio_dout_en) != PASSED) {
    ...
    if (tsn_mem_read32((uint)CP_GPIO_DATA_IN_POLAR_REG(0),
    ...
    if ((gpio_din_polar & (uint)(1 << 8)) != 1) {
        ...
        if (tsn_mem_write32((uint)CP_GPIO_DATA_IN_POLAR_REG(0),
                            gpio_din_polar) != PASSED) {
    ...
    /* Check CPU side */
    if (tsn_mem_read32((uint)CP_GPIO_DATA_IN_REG(0),
                       &gpio_din) != PASSED) {
    ...
    for (ctr = 0; ctr < total_f_intr_num; ctr++, reg_p++) {
        ...
        if (tsn_mem_read32((uint)CP_GPIO_DATA_IN_REG(0), &cpu_intr) != PASSED) {
        ...
        if ((cpu_intr & TSN_FPGA_CPU_INTR) != TSN_FPGA_CPU_INTR) {
 ...
 *
 * Revision 1.2.4.1  2017/09/28 21:46:11  hondwang
 ...
 *
 * Revision 1.1.2.3  2016/06/17 12:30:19  steja
 * Add prpass
 *
 * Revision 1.1.2.2  2016/04/24 12:42:56  palin2
