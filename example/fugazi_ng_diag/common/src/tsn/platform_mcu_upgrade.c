/* $Id: platform_mcu_upgrade.c,v 1.2 2018/02/09 09:56:56 hondwang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tsn/platform_mcu_upgrade.c,v $
 *-----------------------------------------------------------------------------
 *
 * Filename   : platform_mcu_upgrade.c
...
int mcu_volcur_check(void)
{
    ...
    if (tsn_mcu_reg_rd(TSN_MCU_VP12P0_VOLTAGE_REG, &reg_val) != PASSED) {
    ...
    if (tsn_mcu_reg_rd(TSN_MCU_MB12V_CUR_REG, &reg_val) != PASSED) {
...
int mcu_fw_verno(void)
{
    ...
    if (tsn_mcu_reg_rd(TSN_MCU_VERSION_REG, &reg_val) != PASSED) {
...
static uint32_t mcu_read_mem_set_bl_f1 (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_SET_READ_F1_S_TIMES;
    ...
    (e2prom_data.msgs[0]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[0]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[0]).flags = STAR_MCU_FLAG_WRITE;
    (e2prom_data.msgs[0]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[0]).buf[0] = RM_COMMAND; 
    
    /* 2. <S><SlaveID-W><A><Neg-RM_Command><A><P>              */
    (e2prom_data.msgs[1]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[1]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[1]).flags = STAR_MCU_FLAG_WRITE;
    (e2prom_data.msgs[1]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[1]).buf[0] = ENV_RM_COMMAND; 
  
    /* 3. <S><SlaveID-R><A><ACK/NACK><A><P>// Check Ack(0x79)  */ 
    (e2prom_data.msgs[2]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[2]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[2]).flags = STAR_MCU_FLAG_READ;
    (e2prom_data.msgs[2]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
...
static uint32_t mcu_read_mem_set_bl_f2 (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_SET_READ_F2_S_TIMES;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
    ...
    (e2prom_data.msgs[2]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[2]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[2]).flags = STAR_MCU_FLAG_WRITE;
    (e2prom_data.msgs[2]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[2]).buf[0] = *(maddr+1); 
  
    /* 4. <S><SlaveID-W><A><ADDR0><A><P>                       */
    (e2prom_data.msgs[3]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[3]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[3]).flags = STAR_MCU_FLAG_WRITE;
    (e2prom_data.msgs[3]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[3]).buf[0] = *(maddr); 
 
    /* 5. <S><SlaveID-W><A>< ADDR CheckSum ><A><P>             */
    maddrsum = (*(maddr+3)) ^ (*(maddr+2)) ^ (*(maddr+1)) ^ (*(maddr));
    
    (e2prom_data.msgs[4]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[4]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[4]).flags = STAR_MCU_FLAG_WRITE;
    (e2prom_data.msgs[4]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[4]).buf[0] = maddrsum; 
   
    /* 6. <S><SlaveID-R><A><ACK/NACK><A><P> // Check Ack(0x79) */
    (e2prom_data.msgs[5]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[5]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[5]).flags = STAR_MCU_FLAG_READ;
    (e2prom_data.msgs[5]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
...
static uint32_t mcu_read_mem_set_bl_f3 (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_SET_READ_F3_S_TIMES;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
...
static uint32_t mcu_read_mem_cmd_bl (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    /* Read command have two I2C star single */
    ...
    for (ix = 0; ix < mlen; ix++) {
        (e2prom_data.msgs[ix]).len = STAR_MCU_COMMAND_SIZE_1;
        (e2prom_data.msgs[ix]).addr = STAR_MCU_BLMODE_ADD;
        (e2prom_data.msgs[ix]).flags = STAR_MCU_FLAG_READ;
        (e2prom_data.msgs[ix]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
...
static uint32_t mcu_write_mem_set_bl_f1 (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_SET_WRITE_F1_S_TIMES;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
...
static uint32_t mcu_write_mem_set_bl_f2 (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_SET_WRITE_F2_S_TIMES;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
...
static uint32_t mcu_write_mem_set_bl_f3 (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_SET_WRITE_F3_S_TIMES;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
...
static uint32_t mcu_write_mem_cmd_bl (int fd, uint8_t *maddr, uint16_t mlen, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_WRITE_S_TIMES + mlen;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
    ...
    for (ix = 0; ix < mlen; ix++){
        (e2prom_data.msgs[ix]).len = STAR_MCU_COMMAND_SIZE_1;
        (e2prom_data.msgs[ix]).addr = STAR_MCU_BLMODE_ADD;
        (e2prom_data.msgs[ix]).flags = STAR_MCU_FLAG_WRITE;
        (e2prom_data.msgs[ix]).buf = (unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    ...
    (e2prom_data.msgs[mlen]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[mlen]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[mlen]).flags = STAR_MCU_FLAG_WRITE;
    (e2prom_data.msgs[mlen]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[mlen]).buf[0] = mdatasum; 
    
    /* 4. <S><SlaveID-R><A><ACK/NACK><A><P>  // Check Ack(0x79) */
    (e2prom_data.msgs[mlen + 1]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[mlen + 1]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[mlen + 1]).flags = STAR_MCU_FLAG_READ;
    (e2prom_data.msgs[mlen + 1]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
...
static uint32_t mcu_go_addr_set_bl (int fd, uint8_t *maddr)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_SET_GO_S_TIMES;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
    ...
    /* Fill stable matrix */
    for (ix = 0; ix < STAR_MCU_SET_GO_S_TIMES; ix++) {
        (e2prom_data.msgs[ix]).len = STAR_MCU_COMMAND_SIZE_1;
        (e2prom_data.msgs[ix]).addr = STAR_MCU_BLMODE_ADD;
        (e2prom_data.msgs[ix]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    }
...
static uint32_t mcu_go_addr_cmd_bl (int fd, uint8_t *maddr)
{
    ...
    /* Write command only one I2C star single */
    e2prom_data.nmsgs = STAR_MCU_GO_S_TIMES;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
    ...
    /* Fill stable matrix */
    for (ix = 0; ix < STAR_MCU_GO_S_TIMES; ix++) {
        (e2prom_data.msgs[ix]).len = STAR_MCU_COMMAND_SIZE_1;
        (e2prom_data.msgs[ix]).addr = STAR_MCU_BLMODE_ADD;
        (e2prom_data.msgs[ix]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    }
...
static uint32_t mcu_check_ack_bl (int fd, uint8_t *mdata)
{
    ...
    e2prom_data.nmsgs = STAR_MCU_COMMAND_SIZE_1;
    e2prom_data.msgs = (struct i2c_msg*)malloc(e2prom_data.nmsgs*sizeof(struct i2c_msg));
    if (!e2prom_data.msgs) {
        printf("Fail to malloc mcu e2prom.\n");
        return (FAILED);
    ...
    (e2prom_data.msgs[0]).len = STAR_MCU_COMMAND_SIZE_1;
    (e2prom_data.msgs[0]).addr = STAR_MCU_BLMODE_ADD;
    (e2prom_data.msgs[0]).flags = STAR_MCU_FLAG_READ;
    (e2prom_data.msgs[0]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[0]).buf[0] = STAR_MCU_CLEAN_BUF; 
...
static uint32_t mcu_en_upgrade_bl (int fd)
{
    ...
    (e2prom_data.msgs[0]).flags = STAR_MCU_FLAG_WRITE;
    (e2prom_data.msgs[0]).buf=(unsigned char*)malloc(STAR_MCU_COMMAND_SIZE_1);
    (e2prom_data.msgs[0]).buf[0] = STAR_MCU_UPGRADE_ENABLE; 
...
uint32_t mcu_fw_upgrade(void)
{
    ...
    uint fwsize;
    uint8_t addr[4]; 
    uint8_t data[STAR_MCU_RW_MAX_SIZE]; 
    uint8_t *fwdata = (uint8_t *)malloc(STAR_MCU_AP_SIZE * sizeof(uint8_t)); 
    uint8_t *fwdataop;
    ...
    sprintf(parms_file, STAR_MCU_FILE);
    ...
    if (tsn_mcu_reg_wr(STAR_MCU_UPGRADE_OFFSET, STAR_MCU_UPGRADE_SET) != PASSED) {
    ...
    rwloop = fwsize / STAR_MCU_RW_MAX_SIZE;
    rwloop_rm = fwsize % STAR_MCU_RW_MAX_SIZE;
    
    fulladdr = STAR_MCU_APP_MODE_ADDR;
    fwdataop = fwdata;
    /*** WRITE DATA to MCU***/
    /* Write FULL 64 Bytes loops*/
    for (ix = 0; ix < rwloop; ix++) {
        /* Set Write Address */
        fulladdrop = fulladdr + (ix * STAR_MCU_RW_MAX_SIZE);
        ...
        for (iy = 0; iy < STAR_MCU_RW_MAX_SIZE; iy++) {
            /* KEY byte write 0x00, the byte will be program later */
            if ((ix == 0) && (iy == 0) ) {
                data[iy] = STAR_MCU_UPGRADE_MAGIC;
                fwdataop++;
                continue;
            }
            data[iy] = *fwdataop;
        ...
        /* Write command set */ 
        mcu_write_mem_set_bl_f1(file_desc, addr,STAR_MCU_RW_MAX_SIZE, data);
        mcu_write_mem_set_bl_f2(file_desc, addr,STAR_MCU_RW_MAX_SIZE, data);
        mcu_write_mem_set_bl_f3(file_desc, addr,STAR_MCU_RW_MAX_SIZE, data);

        /* Write data to MCU  */
        ret = mcu_write_mem_cmd_bl(file_desc, addr,STAR_MCU_RW_MAX_SIZE, data);
        if (ret == FAILED) {
    ...
    fulladdrop = fulladdr + (rwloop * STAR_MCU_RW_MAX_SIZE);
...
NO_REMIND_WRITE:
    ...
    for (ix = 0; ix < rwloop; ix++) {
        ...
        /* READ command set */ 
        mcu_read_mem_set_bl_f1(file_desc, addr, STAR_MCU_RW_MAX_SIZE, data);
        mcu_read_mem_set_bl_f2(file_desc, addr, STAR_MCU_RW_MAX_SIZE, data);
        mcu_read_mem_set_bl_f3(file_desc, addr, STAR_MCU_RW_MAX_SIZE, data);
    
        /* READ data from MCU  */
        for (iy = 0; iy < STAR_MCU_RW_MAX_SIZE; iy++) {
            ret = mcu_read_mem_cmd_bl(file_desc, addr, STAR_MCU_COMMAND_SIZE_1, data+iy);
            if (ret == FAILED) {
    ...
    /* Clear data matrix */
    for (ix = 0;ix < STAR_MCU_RW_MAX_SIZE; ix++) {
        data[ix] = MCU_ZERO;
    }
    ...
    /* READ data from MCU  */
    for (ix = 0; ix < rwloop_rm; ix++) {
        ret = mcu_read_mem_cmd_bl(file_desc, addr, STAR_MCU_COMMAND_SIZE_1, data+ix);
        if (ret == FAILED) {
...
NO_REMIND_READ:
    ...
    /* Write command set */ 
    mcu_write_mem_set_bl_f1(file_desc, addr,STAR_MCU_COMMAND_SIZE_1, data);
    mcu_write_mem_set_bl_f2(file_desc, addr,STAR_MCU_COMMAND_SIZE_1, data);
    mcu_write_mem_set_bl_f3(file_desc, addr,STAR_MCU_COMMAND_SIZE_1, data);

    /* Write data to MCU  */
    ret = mcu_write_mem_cmd_bl(file_desc, addr,STAR_MCU_COMMAND_SIZE_1, data);
    if (ret == FAILED) {
...
/*******************************************************************************
 *
 * Function    : _pwr_seq_eeprom_update 
 ...
 *******************************************************************************
 */
static int _pwr_seq_eeprom_update (void)
{
    int rc = PASSED;
    n2g_i2c_if_t *i2c_if;
    uint16_t data;
 ...
 *******************************************************************************
 */
static int _write_pwr_seq_fw(unsigned char *fw_buf, uint16_t to_address,
 ...
 ******************************************************************************
 */
uint32_t write_pwr_seq_fw(int dummy)
{
    n2g_i2c_if_t *i2c_if;
...
int get_pwr_seq_fw_rev (int option)
{
    ...
    rc = tsn_mcu_reg_rd(PWR_SEQ_REV, &rev); 
    if (rc != PASSED) {
        if (option) {
...
uint32_t mcu_fw_upg (void)
{
    ...
    if (this_is_star_c1109_4p()){
        printf("Cisco MCU Upgrade.\n");
        rc = write_pwr_seq_fw(0);
    } else {
        printf("MCU Upgrade.\n");
        rc = mcu_fw_upgrade();
    }
...
Revision 1.1.4.6  2017/12/13 21:10:13  steja
Update MCU to support both CISCO MCU and others MCU

Revision 1.1.4.5  2017/12/09 04:14:19  steja
Support CISCO MCU upgrade

Revision 1.1.4.4  2017/10/20 04:41:53  hondwang
Add C949-4P MCU voltage and currently display by HW request.

Revision 1.1.4.3  2017/10/20 03:30:43  hondwang
Add MCU upgrade Warning message by HW request

Revision 1.1.4.2  2017/08/15 14:18:39  hondwang
star branch c9xx initial check in

Revision 1.1.2.2  2017/07/10 07:01:01  hondwang
remove MCU upgrade fail injection
