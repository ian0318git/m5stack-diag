/* $Id: rbcp_util.c,v 1.2 2018/05/09 06:53:12 letsai Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tsn/rbcp_util.c,v $
 *------------------------------------------------------------------
 * Filename: rbcp_util.c
 *
...
int tsn_rbcp_send(uint8_t *, int);
int tsn_rbcp_recv(uint8_t *, int *);
int tsn_rbcp_clear_recv(void);
int tsn_setup_rbcp_ge_env(void);
int tsn_cleanup_rbcp_ge_env(void);
int tsn_wait_for_ge_packet(uchar *);
...
int tsn_rbcp_send (uint8_t *buf, int length)
{
...
int tsn_rbcp_recv (uint8_t *buf, int *length)
{
    ...
    if (status) {
        if (diagflag_xram & D_SET_OPTIONS) {
            printf("\nTSN RBCP receive packet fail. status = %d.\n",status);
        }
 ...
 **********************************************************************
 *
 *  Function: tsn_wait_for_ge_packet
 *
 ...
 **********************************************************************
 */
int tsn_wait_for_ge_packet (uchar *buf)
{
