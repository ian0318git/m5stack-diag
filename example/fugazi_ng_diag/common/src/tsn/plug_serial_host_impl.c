/* $Id: plug_serial_host_impl.c,v 1.2 2018/11/23 08:49:53 hondwang Exp $
 * $Source: /auto/diag/ngd-linux-rep/cvs/ng_diag/common/src/tsn/plug_serial_host_impl.c,v $
 *------------------------------------------------------------------
 *
 * plug_serial_host_impl.c: Pluggable Serial Common Host Function Implementation
 *                              
 * Sep 2018 - Ian Chang
 *
...
int plug_ser_host_set_1000basex_mode (int slot) 
{
    ...
    if (slot == PLUG_SLOT_1) {
        ...
        if(gephy_set_1000basex_mode(TSN_GE1) == FAILED){
...
int plug_ser_host_set_loopback_mode (int slot, int enable)
{
    ...
    if (slot == PLUG_SLOT_1) {
	    ...
	    gephy_set_loopback_mode(TSN_GE1, enable);	
    } else {
...
int plug_ser_host_get_loopback_mode (int slot)
{
    ...
    if (slot == PLUG_SLOT_1) {
        ...
        return gephy_get_loopback_mode(TSN_GE1);
    } else {
        printf("%s: Unknown GE number(%d) for loopback mode\n", 
                __FUNCTION__, ge_num);
        return (FAILED);
